...
/**
 * AbstractClassifierTestCase.java Copyright (C) 2013 University of Waikato, Hamilton, New Zealand
 */
 ...
 * @version $Revision$
 */
public abstract class AbstractClassifierTestCase extends MoaTestCase {

    /**
     * Container for the data collected from a classifier at a specified inspection point in the
     * stream.
     *
     * @author fracpete (fracpete at waikato dot ac dot nz)
     * @version $Revision$
            ...
            for (Measurement m : measurements)
                result.append(
                        "  "
                                + m.getName()
                                + ": "
                                + MoaTestCase.doubleToString(m.getValue(), 8)
                                + "\n");

            result.append("Model measurements\n");
            for (Measurement m : modelMeasurements) {
                if (m.getName().indexOf("serialized") > -1) continue;
                result.append(
                        "  "
                                + m.getName()
                                + ": "
     ...
     */
    protected InspectionData[] inspect(
            Instances data,
            int[] inspectionPoints,
     ...
     * @return the setups
     */
    protected abstract LearningPerformanceEvaluator<Example<Instance>>[]
            getRegressionEvaluatorSetups();
    ...
    public void testRegression() {
        ...
        assertEquals(
                "Number of classifier setups and evaluator setups differ!",
                setups.length,
                evals.length);
        assertEquals(
                "Number of classifier setups and inspection points differ!",
                setups.length,
                points.length);
        ...
        for (i = 0; i < input.length; i++) {
            ...
            assertNotNull(
                    "Failed to create copy of algorithm: " + MOAUtils.toCommandLine(setups[i]),
                    current);
