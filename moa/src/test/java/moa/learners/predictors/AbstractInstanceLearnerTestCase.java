...
public abstract class AbstractInstanceLearnerTestCase<MLTask extends InstanceLearner> extends MoaTestCase {
	...
	/**
	 * Container for the data collected from a classifier at a specified inspection
	 * point in the stream.
	 *
	 * @author fracpete (fracpete at waikato dot ac dot nz)
	 * @version $Revision$
	 ...
	 */
	public AbstractInstanceLearnerTestCase(String name) {
		super(name);
	}
	...
	protected AbstractTestHelper newTestHelper() {
		return new TestHelper(this, "moa/learners/predictors/data");
	}
	 ...
	 */
	protected InspectionData[] inspect(InstancesHeader data, int[] inspectionPoints,
			LearningPerformanceEvaluator<Example<Instance>> evaluator, MLTask scheme) {
		...
		for (i = 0; i < data.numInstances(); i++) {
			...
			if (i > 0) {
				...
				evaluator.addResult(new InstanceExample(inst), votes.asDoubleArray());
				...
				if (point < inspectionPoints.length) {
					if (i == inspectionPoints[point] - 1) {
						...
						result[point].votes = votes.asDoubleArray();
	...
	/**
	 * Returns the filenames (without path) of the input data files to use in the
	 * regression test.
	 *
	 ...
	 * @return the setups
	 */
	protected abstract MLTask[] getLearnerSetups();
	...
	public abstract MLTask copySetup(MLTask mlTask);
