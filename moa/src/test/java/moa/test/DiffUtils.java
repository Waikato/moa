...
/** DiffUtils.java Copyright (C) 2012 University of Waikato, Hamilton, New Zealand */
package moa.test;
...
public class DiffUtils {
     ...
     * @version $Revision: 6358 $
     */
    public static class SideBySideDiff implements Serializable, Cloneable {
        ...
        public int lineToDelta(int line) {
            ...
            if (!hasDifferences()) return result;
        ...
        public int deltaToLine(int delta, boolean start) {
            ...
            if ((delta >= 0) && (delta < m_Diff[SIDEBYSIDE_STARTPOS].size())) {
                if (start) result = (Integer) m_Diff[SIDEBYSIDE_STARTPOS].get(delta);
                else result = (Integer) m_Diff[SIDEBYSIDE_ENDPOS].get(delta);
            }
        ...
        public int getNextDelta(int line) {
            ...
            delta++;
            if (delta < m_Diff[SIDEBYSIDE_STARTPOS].size()) result = delta;
     ...
     * @version $Revision: 6358 $
     */
    public static class Filler implements Serializable {
        ...
        public boolean equals(Object obj) {
            ...
            else if (obj instanceof Filler) return true;
            else return false;
        }
    ...
    protected static List<String> loadFromFile(File file) {
        if (file.isDirectory()) return new ArrayList<String>();
        else return FileUtils.loadFromFile(file);
    }
    ...
    protected static String createRange(Chunk chunk) {
        if (chunk.size() == 1) return (chunk.getPosition() + 1) + "";
        else return (chunk.getPosition() + 1) + "," + (chunk.getPosition() + chunk.size());
    }
    ...
    protected static void addToList(int from, int to, List<String> source, List dest) {
        for (int i = from; i <= to; i++) dest.add(source.get(i));
    }
    ...
    protected static void addToList(int from, int to, Object obj, List dest) {
        for (int i = from; i <= to; i++) dest.add(obj);
    }
        ...
        for (Delta delta : patch.getDeltas()) {
            ...
            if (delta instanceof InsertDelta) {
                ...
                addToList(
                        ...
                        INDICATOR_ADDED,
                        result[SIDEBYSIDE_INDICATOR]);
                to = delta.getOriginal().getPosition() + delta.getOriginal().size();
            } else if (delta instanceof DeleteDelta) {
                // deleted content
                result[SIDEBYSIDE_FIRST].addAll(delta.getOriginal().getLines());
                addToList(
                        0, delta.getOriginal().size() - 1, new Filler(), result[SIDEBYSIDE_SECOND]);
                addToList(
                        0,
                        delta.getOriginal().size() - 1,
                        INDICATOR_DELETED,
                        result[SIDEBYSIDE_INDICATOR]);
                to = delta.getOriginal().getPosition() + delta.getOriginal().size();
            } else {
                ...
                if (sizeDiff > 0) addToList(1, sizeDiff, new Filler(), result[SIDEBYSIDE_FIRST]);
                if (sizeDiff < 0) addToList(1, -sizeDiff, new Filler(), result[SIDEBYSIDE_SECOND]);
                to = delta.getOriginal().getPosition() + delta.getOriginal().size();
            }
