...
public class DiffUtils {
		...
		public SideBySideDiff(List[] diff) {
			if (diff.length != SIDEBYSIDE_SIZE)
				throw new IllegalArgumentException(
						"Expected array with length " + SIDEBYSIDE_SIZE + " but got: " + diff.length);
			m_Diff = diff.clone();
		}
	...
	/**
	 * Loads the file. If file points to a directory, an empty vector is returned
	 * instead.
	 *
	 ...
	 * @param file1 the first text file
	 * @param file2 the second text file
	 * @return the side-by-side diff (first file: index 0, second file: index 1,
	 *         indicator: )
	 */
	...
	public static SideBySideDiff sideBySide(List<String> list1, List<String> list2) {
		...
		for (Delta delta : patch.getDeltas()) {
			...
			if (delta instanceof InsertDelta) {
				...
				result[SIDEBYSIDE_FIRST].addAll(delta.getOriginal().getLines());
				result[SIDEBYSIDE_SECOND].addAll(delta.getRevised().getLines());
				addToList(1, Math.max(delta.getOriginal().size(), delta.getRevised().size()), INDICATOR_CHANGED,
	...
	/**
	 * Usage: DiffUtils &lt;unified|side-by-side|brief&gt; &lt;file1&gt;
	 * &lt;file2&gt;
	 *
	...
	public static void main(String[] args) throws Exception {
		if (args.length == 3) {
			...
			if (args[0].equals(OPTION_UNIFIED)) {
				...
				System.err.println("Only '" + OPTION_UNIFIED + "', '" + OPTION_SIDEBYSIDE + "' and '" + OPTION_BRIEF
						+ "' are available as options!");
				return;
			}
		} else {
			System.err.println("\nUsage: " + DiffUtils.class.getName() + " <" + OPTION_UNIFIED + "|" + OPTION_SIDEBYSIDE
					+ "|" + OPTION_BRIEF + "> <file1> <file2>\n");
			return;
		}
	}
}
