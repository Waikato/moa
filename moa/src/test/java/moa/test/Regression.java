...
public class Regression {
    ...
    public static final String REFERENCES = "src/test/resources";
     ...
     *
     * <p>Normally, the reference file is determined based on the classname. This method should only
     * be called if a file different from the class name must be used, for instance for an
    ...
    /**
     * Compares the content generated by the specified class with the stored regression data.
     * Creates a new reference file if not present yet.
     *
    ...
    public static List<String> trim(List<String> list, int[] ignore) {
        ...
        if (ignore.length == 0) return list;
    ...
    /**
     * Compares the content generated by the specified class with the stored regression data.
     * Creates a new reference file if not present yet. Uses "\n" for splitting the string content
     * into lines.
     *
     * @param content the content to check
    ...
    /**
     * Compares the file content generated by the specified class with the stored regression data.
     * Creates a new reference file if not present yet.
     *
    ...
    public String compare(File[] files, int[] ignore) {
        ...
        for (i = 0; i < files.length; i++) {
            ...
            if (!files[i].exists()) content.add("file is missing");
            else content.addAll(trim(FileUtils.loadFromFile(files[i]), ignore));
            content.add("");
        }
    ...
    public static File createReferenceFile(Class regressionClass, String suffix, String extension) {
        ...
        return new File(
                REFERENCES
                        + "/"
                        + regressionClass.getName().replace(".", "/")
                        + suffix
                        + extension);
    }
}
