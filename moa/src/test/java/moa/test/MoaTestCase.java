...
/** MoaTestCase.java Copyright (C) 2010-2013 University of Waikato, Hamilton, New Zealand */
package moa.test;
 ...
 *
 * <p>Any regression test can be skipped as follows: <br>
...
public class MoaTestCase extends TestCase {
    ...
    public static final String PROPERTY_HEADLESS = "moa.test.headless";

    /** property indicating whether regression tests should not be executed. */
    public static final String PROPERTY_NOREGRESSION = "moa.test.noregression";
    ...
    protected Class getTestedClass() throws ClassNotFoundException {
        if (!getClass().getName().endsWith("Test")) {
            throw new IllegalStateException(
                    "Class name must end with 'Test': " + getClass().getName());
        }
        return Class.forName(getClass().getName().replaceAll("Test$", ""));
    }

    /**
     * Returns whether the test can be executed in a headless environment. If not then the test gets
     * skipped.
     *
    ...
    /**
     * Override to run the test and assert its state. Checks whether the test or test-method is
     * platform-specific.
     *
    ...
    protected void runTest() throws Throwable {
        ...
        if (m_Headless && !canHandleHeadless()) proceed = false;

        if (proceed) super.runTest();
    ...
    protected Object getInstance(Class cls, Class intf, boolean fail) {
        ...
        if (!intf.isAssignableFrom(cls)) return null;
        ...
        try {
            ...
            if (fail) fail("No default constructor, requires custom test method: " + cls.getName());
            return null;
        }
    ...
    /**
     * Creates a deep copy of the given object (must be serializable!). Returns null in case of an
     * error.
     *
    ...
    public static String doubleToString(double value, int afterDecimalPoint) {
        ...
        if (Math.abs(temp) < Long.MAX_VALUE) {
            ...
            if (precisionValue == 0) builder = new StringBuilder(String.valueOf(0));
            else builder = new StringBuilder(String.valueOf(precisionValue));

            if (afterDecimalPoint == 0) return builder.toString();

            separator = '.';
            dotPosition = builder.length() - afterDecimalPoint;
            while (((precisionValue < 0) && (dotPosition < 1)) || (dotPosition < 0)) {
                if (precisionValue < 0) builder.insert(1, '0');
            ...
            if ((precisionValue < 0) && (builder.charAt(1) == separator)) builder.insert(1, '0');
            else if (builder.charAt(0) == separator) builder.insert(0, '0');

            currentPos = builder.length() - 1;
            while ((currentPos > dotPosition) && (builder.charAt(currentPos) == '0'))
                builder.setCharAt(currentPos--, ' ');

            if (builder.charAt(currentPos) == separator) builder.setCharAt(currentPos, ' ');
    ...
    /**
     * For classes (with default constructor) that are serializable, are tested whether they are
     * truly serializable.
     */
    public void testSerializable() {
        if (m_Regression != null) performSerializableTest(m_Regression.getRegressionClass());
    }
