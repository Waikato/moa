...
public class SimpleClusterTest extends TestCase {
	...
	final static String[] Clusterers = new String[] { "ClusterGenerator", "CobWeb", "KMeans", "clustream.Clustream",
			"clustree.ClusTree", "denstream.WithDBSCAN -i 1000", "streamkm.StreamKM" };

	@Test
	public void testClusterGenerator() {
...
//	public void testCobWeb(){testClusterer(Clusterers[1]);}
	...
	@Test
	public void testClusTree() {
		testClusterer(Clusterers[4]);
	}

	@Test
	public void testDenStream() {
		testClusterer(Clusterers[5]);
	}

	@Test
	public void testStreamKM() {
	...
	public void doTask(String[] args) throws Exception {
		if (args.length < 1) {
			...
			FlagOption suppressStatusOutputOption = new FlagOption("suppressStatusOutput", 'S',
					"Suppress the task status output that is normally send to stderr.");
			FlagOption suppressResultOutputOption = new FlagOption("suppressResultOutput", 'R',
					"Suppress the task result output that is normally send to stdout.");
			IntOption statusUpdateFrequencyOption = new IntOption("statusUpdateFrequency", 'F',
					"How many milliseconds to wait between status updates.", 1000, 0, Integer.MAX_VALUE);
			Option[] extraOptions = new Option[] { suppressStatusOutputOption, suppressResultOutputOption,
			...
			if (suppressStatusOutputOption.isSet()) {
				...
				if (!(result instanceof FailedTaskReport)) {
					System.out.print(
							"Task completed in " + StringUtils.secondsToDHMSString(taskThread.getCPUSecondsElapsed()));
					if (preciseTiming) {
