...
public class RankingGraph extends JFrame {
	...
	final static BasicStroke dashed = new BasicStroke(1.5f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f, dash1,
			0.0f);
	 ...
	 * @param pvalue
	 */
	public RankingGraph(ArrayList<RankPerAlgorithm> algRank, ArrayList<PValuePerTwoAlgorithm> PValues, String imgPath,
			double pvalue) {
	...
	private void initComponents() {
		...
		controlPanel.setPreferredSize(new Dimension(60 * width / 100, 20 * getSize().height / 100));
		zoomPanel.setPreferredSize(new Dimension(30 * width / 100, 20 * getSize().height / 100));
		...
		this.availableStrokeSamples[0] = new StrokeSample(
				new BasicStroke(1.5f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f, dash1, 0.0f));
	...
	private void strokeSelection(StrokeSample str) {
		...
		int result = JOptionPane.showConfirmDialog(this, panel, "Stroke Selection", JOptionPane.OK_CANCEL_OPTION,
	...
	public void fontSelection() {
		...
		int result = JOptionPane.showConfirmDialog(this, panel, "Font Selection", JOptionPane.OK_CANCEL_OPTION,
				JOptionPane.PLAIN_MESSAGE);
...
//                }
	...
	class Graph extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener {
		...
		void Graficar(Graphics ap, int xg, int yg) {
			...
			for (int i = 0; i < algRank.size(); i++) {
				g.draw(new Line2D.Double(xdiv * algRank.get(i).rank + xin, yg + ya, xdiv * algRank.get(i).rank + xin,
						yg));
				gb.draw(new Line2D.Double(xdiv * algRank.get(i).rank + xin, yg + ya, xdiv * algRank.get(i).rank + xin,
						yg));
				int k;
				...
				if (i == 0 && k - 1 != 0) {
					g.draw(new Line2D.Double(xdiv * algRank.get(i).rank + xin, (yg + ya + yg) / 2,
							xdiv * algRank.get(k - 1).rank + xin, (yg + ya + yg) / 2));
					gb.draw(new Line2D.Double(xdiv * algRank.get(i).rank + xin, (yg + ya + yg) / 2,
							xdiv * algRank.get(k - 1).rank + xin, (yg + ya + yg) / 2));
				} else if (i != 0) {
					// If the last visited by the current algorithm is not in the visited of the
					// previous one then draw a line
				...
				if (i < algRank.size() / 2) {
					...
					g.draw(new Line2D.Double(xdiv * algRank.get(i).rank + xin, yg + ya, xinr - 3, yg + ya));
					g.drawString(algRank.get(i).algName, xinr - 3 - lenght - 10, yg + ya);
					gb.draw(new Line2D.Double(xdiv * algRank.get(i).rank + xin, yg + ya, xinr - 3, yg + ya));
					gb.drawString(algRank.get(i).algName, xinr - 3 - lenght - 10, yg + ya);
				} else {
					g.draw(new Line2D.Double(xdiv * algRank.get(i).rank + xin, yg + ya, xfin + 3, yg + ya));
					g.drawString(algRank.get(i).algName, xfin + 3 + 10, yg + ya);
					gb.draw(new Line2D.Double(xdiv * algRank.get(i).rank + xin, yg + ya, xfin + 3, yg + ya));
	...
	public class SliderPanel extends JPanel {
		...
		public SliderPanel() {
			...
			xSlider = new JSlider(SwingConstants.VERTICAL, -400, 400, 50);
			xSlider.addChangeListener((ChangeEvent e) -> {
				xScale = xSlider.getValue();
				graphPanel.repaint();
			});

			add(xSlider);

			ySlider = new JSlider(SwingConstants.VERTICAL, 1, 400, 20);
			ySlider.addChangeListener((ChangeEvent e) -> {
				yScale = ySlider.getValue();
				graphPanel.repaint();
			});
			add(ySlider);
