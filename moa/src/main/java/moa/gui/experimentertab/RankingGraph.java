...
/**
 * Shows the comparison of several online learning algorithms on multiple datasets by performing
    ...
    class Graph extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener {
        ...
        void Graficar(Graphics ap, int xg, int yg) {
            ...
            for (int i = 0; i < algRank.size(); i++) {
                g.draw(
                        new Line2D.Double(
                                (double) (xdiv * algRank.get(i).rank + xin),
                                yg + ya,
                                (double) (xdiv * algRank.get(i).rank + xin),
                                yg));
                gb.draw(
                ...
                for (k = i + 1; k <= algRank.size() - 1; k++) {
                    int index =
                            PValuePerTwoAlgorithm.getIndex(
                                    PValues, algRank.get(i).algName, algRank.get(k).algName);
                ...
                if (i == 0 && k - 1 != 0) {
                    g.draw(
                            new Line2D.Double(
                                    (double) (xdiv * algRank.get(i).rank + xin),
                                    (yg + ya + yg) / 2,
                                    (double) (xdiv * algRank.get(k - 1).rank + xin),
                                    (yg + ya + yg) / 2));
                    gb.draw(
                            new Line2D.Double(
                                    (double) (xdiv * algRank.get(i).rank + xin),
