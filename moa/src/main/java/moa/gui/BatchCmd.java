...
import moa.learners.clusterers.clustream.WithKmeans;
...
public class BatchCmd implements ClusterEventListener {
	...
	public BatchCmd(AbstractClusterer clusterer, ClusteringStream stream, MeasureCollection[] measures,
			int totalInstances) {
	...
	public static void runBatch(ClusteringStream stream, AbstractClusterer clusterer, boolean[] measureCollection,
			int amountInstances, String outputFile) {
	...
	public void run() {
		ArrayList<DataPoint> pointBuffer0 = new ArrayList<>();
		int m_timestamp = 0;
				...
				if (clusterer.implementsMicroClusterer()) {
					...
					if (clusterer.evaluateMicroClusteringOption.isSet()) {
						...
						if (clustering0 == null && microC != null)
							clustering0 = moa.learners.clusterers.KMeans.gaussianMeans(gtClustering0, microC);
					}
				}
	...
	public static void exportCSV(String filepath, ArrayList<ClusterEvent> clusterEvents, MeasureCollection[] measures,
			int horizon) {
