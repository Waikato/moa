...
public class BatchCmd implements ClusterEventListener {
    ...
    public BatchCmd(
        ...
        this.stream = stream;
        if (totalInstances == -1) this.totalInstances = Integer.MAX_VALUE;
        else this.totalInstances = totalInstances;
    ...
    private static ArrayList<Class> getMeasureSelection(boolean[] selection) {
        ...
        if (selection[0]) mclasses.add(General.class);
        if (selection[1]) mclasses.add(F1.class);
        if (selection[2]) mclasses.add(EntropyCollection.class);
        if (selection[3]) mclasses.add(CMM.class);
        if (selection[4]) mclasses.add(SSQ.class);
        if (selection[5]) mclasses.add(Separation.class);
        if (selection[6]) mclasses.add(SilhouetteCoefficient.class);
        if (selection[7]) mclasses.add(StatisticalCollection.class);
    ...
    public void run() {
        ...
        while (m_timestamp < totalInstances && stream.hasMoreInstances()) {
            ...
            if (clusterer instanceof ClusterGenerator) traininst0.setDataset(point0.dataset());
            ...
            if (counter <= 0) {
                ...
                if (clusterer.implementsMicroClusterer()) {
                    if (clusterer instanceof ClusterGenerator
                        ...
                        ((ClusterGenerator) clusterer)
                                .setSourceClustering(
                                        ((RandomRBFGeneratorEvents) stream).getMicroClustering());
                    }
                    ...
                    if (clusterer.evaluateMicroClusteringOption.isSet()) {
                        ...
                        if (clustering0 == null && microC != null)
                            clustering0 =
                                    moa.clusterers.KMeans.gaussianMeans(gtClustering0, microC);
                    }
                }
                ...
                for (int i = 0; i < measures.length; i++) {
                    try {
                        /*double sec =*/ measures[i].evaluateClusteringPerformance(
                                clustering0, gtClustering0, pointBuffer0);
                        // System.out.println("Eval of "+measures[i].getClass().getSimpleName()+" at
                        // "+m_timestamp+" took "+sec);
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                }
    ...
    private static MeasureCollection[] getMeasures(ArrayList<Class> measure_classes) {
        ...
        for (int i = 0; i < measure_classes.size(); i++) {
            try {
                ...
                Logger.getLogger(
                        "Couldn't create Instance for " + measure_classes.get(i).getName());
                ex.printStackTrace();
            }
        }
        return measures;
