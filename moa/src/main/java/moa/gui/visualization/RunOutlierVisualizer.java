...
import moa.learners.clusterers.outliers.MyBaseOutlierDetector.OutlierNotifier;
import moa.learners.clusterers.outliers.MyBaseOutlierDetector.PrintMsg;
...
public class RunOutlierVisualizer implements Runnable, ActionListener, ClusterEventListener {
	...
	private class MyOutlierNotifier extends OutlierNotifier {
		...
		public void OnOutlier(Outlier outlier) {
			...
			// System.out.println("OnOutlier outlier.id " + outlier.id + " timestamp " +
			// timestamp);
		}
		...
		public void OnInlier(Outlier outlier) {
			...
			// System.out.println("OnInlier outlier.id " + outlier.id + " timestamp " +
			// timestamp);
		}
	}
	...
	public RunOutlierVisualizer(OutlierVisualTab visualPanel, OutlierSetupTab outlierSetupTab) {
		...
		eventBuffer[ALGORITHM_1] = new TreeSet<>();
		eventBuffer[ALGORITHM_2] = new TreeSet<>();
	...
	private void processData() {
		if (m_stream0.hasMoreInstances()) {
			...
			/*
			 * if (nProcessed % m_drawOutliersInterval == 0) drawOutliers();
			 */
	...
	private void UpdateTimePerObj(int idxAlgorithm, double t) {
		...
		m_logPanel
				.addText("Algorithm " + idxAlgorithm + ", process time per object (ms): " + String.format("%.3f", ms));
	}
	...
	private void simulateVisualSpeed() {
		...
		// System.out.println("iSleepMS="+iSleepMS + ",
