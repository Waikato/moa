...
public class StreamPanel extends JPanel implements ComponentListener {
    ...
    public void setPointVisibility(boolean visibility) {
        ...
        if (!visibility) layerPointCanvas.setVisible(false);
    }

    void drawPointPanels(
            ArrayList<DataPoint> points, int timestamp, double decay_rate, double decay_threshold) {
        for (int p = 0; p < points.size(); p++) {
            PointPanel pointPanel =
                    new PointPanel(points.get(p), this, decay_rate, decay_threshold);
    ...
    private void drawClustering(
            JPanel layer, Clustering clustering, List<DataPoint> points, Color color) {
    ...
    private void drawNonConvexClustering(
            JPanel layer, Clustering clustering, List<DataPoint> points, Color color) {
    ...
    public void screenshot(String filename, boolean svg, boolean png) {
        if (layerPoints.getComponentCount() == 0
                && layerMacro.getComponentCount() == 0
                && layerMicro.getComponentCount() == 0) return;
