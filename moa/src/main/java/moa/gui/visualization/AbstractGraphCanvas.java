...
public abstract class AbstractGraphCanvas extends JPanel {
	 ...
	 * unnecessary scrolling after reducing the x_resolution, the baseWidth is reset
	 * to its initial value of 500 on x_resolution = 1.
	 */
	...
	/**
	 * Updates the base height, which is used to determine the canvas size. It is
	 * defined as the current height divided by the y_resolution. To prevent
	...
	/**
	 * Updates the canvas: if values have changed or it is forced, the canvas and
	 * preferred sizes are updated and the canvas is repainted. Size and preferred
	 * size are determined by the subclass extending AbstractGraphCanvas.
	 *
	...
	/**
	 * Computes the minimum and maximum values for the x and y-axis and updates the
	...
	private boolean updateMinMaxValues() {
		...
		// resizing needed?
		if (min_x_value_new != this.min_x_value || max_x_value_new != this.max_x_value
	...
	private void updateChildren() {
		...
		plotPanel.setSize(getWidth() - X_OFFSET_LEFT - X_OFFSET_RIGHT, getHeight() - Y_OFFSET_BOTTOM - Y_OFFSET_TOP);
	}
