...
import moa.learners.clusterers.ClusterGenerator;
...
public class RunVisualizer implements Runnable, ActionListener, ClusterEventListener {
	...
	/**
	 * after how many instances do we repaint the streampanel? the GUI becomes very
	 * slow with small values
	 */
	...
	/*
	 * amount of relevant instances; older instances will be dropped; creates the
	 * 'sliding window' over the stream; is strongly connected to the decay rate and
	 * decay threshold
	 */
	...
	/*
	 * the decay rate of the stream, often reffered to as lambda; is being
	 * calculated from the horizion and the threshold as these are more intuitive to
	 * define
	 */
	...
	// not pretty to have all the clusterings, but otherwise we can't just redraw
	// clusterings
	...
	public void runVisual() {
		...
		int speedCounter = 0;
		LinkedList<DataPoint> pointBuffer0 = new LinkedList<>();
		LinkedList<DataPoint> pointBuffer1 = new LinkedList<>();
	...
	private void processClusterings(ArrayList<DataPoint> points0, ArrayList<DataPoint> points1) {
		...
		// TODO: should we check if micro/macro is being drawn or needed for evaluation
		// and skip otherwise to speed things up?
		if (m_clusterer0.implementsMicroClusterer()) {
			micro0 = m_clusterer0.getMicroClusteringResult();
			if (macro0 == null && micro0 != null) {
				// TODO: we need a Macro Clusterer Interface and the option for kmeans to use
				// the non optimal centers
				macro0 = moa.learners.clusterers.KMeans.gaussianMeans(gtClustering0, micro0);
			}
		...
		if (m_clusterer1 != null) {
			...
			if (m_clusterer1.implementsMicroClusterer()) {
				...
				if (macro1 == null && micro1 != null) {
					macro1 = moa.learners.clusterers.KMeans.gaussianMeans(gtClustering1, micro1);
				}
	...
	private void evaluateClustering(Clustering found_clustering, Clustering trueClustering, ArrayList<DataPoint> points,
			boolean algorithm0) {
		...
		for (int i = 0; i < m_measures0.length; i++) {
			if (algorithm0) {
				if (found_clustering != null && found_clustering.size() > 0) {
					try {
						double msec = m_measures0[i].evaluateClusteringPerformance(found_clustering, trueClustering,
								points);
						sb.append(m_measures0[i].getClass().getSimpleName() + " took " + msec + "ms (Mean:"
								+ m_measures0[i].getMeanRunningTime() + ")");
						sb.append("\n");
				...
				if (m_clusterer1 != null && found_clustering != null && found_clustering.size() > 0) {
					try {
						double msec = m_measures1[i].evaluateClusteringPerformance(found_clustering, trueClustering,
								points);
						sb.append(m_measures1[i].getClass().getSimpleName() + " took " + msec + "ms (Mean:"
								+ m_measures1[i].getMeanRunningTime() + ")");
						sb.append("\n");
					} catch (Exception ex) {
						ex.printStackTrace();
					}
