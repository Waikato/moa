...
public class RunVisualizer implements Runnable, ActionListener, ClusterEventListener {
    ...
    public void runVisual() {
            ...
            if (m_stream0.hasMoreInstances()) {
                ...
                if (m_visualPanel.isEnabledDrawPoints()) {
                    ...
                    if (processCounter % m_redrawInterval == 0) {
                        m_streampanel0.applyDrawDecay(
                                m_stream0_decayHorizon / (float) (m_redrawInterval));
                        if (m_clusterer1 != null)
                            m_streampanel1.applyDrawDecay(
                                    m_stream0_decayHorizon / (float) (m_redrawInterval));
                    }
                }
        ...
        if (!stop) {
            m_streampanel0.drawPointPanels(
                    pointarray0, timestamp, m_stream0_decay_rate, m_stream0_decay_threshold);
            if (m_clusterer1 != null)
                m_streampanel1.drawPointPanels(
                        pointarray1, timestamp, m_stream0_decay_rate, m_stream0_decay_threshold);
            work_pause();
        }
    }
    ...
    private void processClusterings(ArrayList<DataPoint> points0, ArrayList<DataPoint> points1) {
        ...
        if (m_clusterer0.implementsMicroClusterer()) {
            ...
            if (macro0 == null && micro0 != null) {
                // TODO: we need a Macro Clusterer Interface and the option for kmeans to use the
                // non optimal centers
                macro0 = moa.clusterers.KMeans.gaussianMeans(gtClustering0, micro0);
            }
            if (m_clusterer0.evaluateMicroClusteringOption.isSet()) evalClustering0 = micro0;
            else evalClustering0 = macro0;
        }
        ...
        if (m_clusterer1 != null) {
            ...
            if (m_clusterer1.implementsMicroClusterer()) {
                ...
                if (m_clusterer1.evaluateMicroClusteringOption.isSet()) evalClustering1 = micro1;
                else evalClustering1 = macro1;
            }
        }
    ...
    private void evaluateClustering(
            Clustering found_clustering,
        ...
        for (int i = 0; i < m_measures0.length; i++) {
            if (algorithm0) {
                if (found_clustering != null && found_clustering.size() > 0) {
                    try {
                        double msec =
                                m_measures0[i].evaluateClusteringPerformance(
                                        found_clustering, trueClustering, points);
                        sb.append(
                                m_measures0[i].getClass().getSimpleName()
                                        + " took "
                                        + msec
                                        + "ms (Mean:"
                ...
                if (m_clusterer1 != null
                        && found_clustering != null
                        && found_clustering.size() > 0) {
                    try {
                        double msec =
                                m_measures1[i].evaluateClusteringPerformance(
                                        found_clustering, trueClustering, points);
                        sb.append(
                                m_measures1[i].getClass().getSimpleName()
                                        + " took "
                                        + msec
                                        + "ms (Mean:"
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        m_graphcanvas.setGraph(
                m_measures0[m_select], m_measures1[m_select], m_select_offset, m_processFrequency);
    }
                    ...
                    for (int i = 0; i < measurecol[m].length; i++) {
                        for (int j = 0; j < measurecol[m][i].getNumMeasures(); j++) {
                            if (measurecol[m][i].isEnabled(j)) {
                                ...
                                if (Double.isNaN(value)) out.write(del);
                                else out.write(value + del);
                            }
                        }
                    }
                }
    ...
    public void weka() {
        ...
        if (m_clusterer0.implementsMicroClusterer()
                && m_clusterer0.evaluateMicroClusteringOption.isSet()) wekaClustering = micro0;
        else wekaClustering = macro0;
