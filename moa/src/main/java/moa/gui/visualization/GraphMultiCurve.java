...
/**
 * GraphMultiCurve is an an implementation of AbstractGraphPlot that draws several curves on a
 * Canvas.
 *
 * @author Tim Sabsch (tim.sabsch@ovgu.de)
 * @version $Revision: 1 $
...
public class GraphMultiCurve extends AbstractGraphPlot {
    ...
    /**
     * Sets the minimum process frequency, which may be used to stretch or compress curves. NOTE
    ...
    private void paintFullCurve(Graphics g, int i) {
        ...
        for (int j = 0; j < n; j++) {
            ...
            y[j] =
                    (int)
                            (height
                                    - (this.measures[i].getValue(this.measureSelected, j)
                                                    / this.upper_y_value)
                                            * height);
            ...
            if (this.isStandardDeviationPainted) {
                int len =
                        (int)
                                ((this.measureStds[i].getValue(this.measureSelected, j)
                                                / this.upper_y_value)
                                        * height);
