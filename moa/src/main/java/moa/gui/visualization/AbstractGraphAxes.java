...
public abstract class AbstractGraphAxes extends JPanel {
	...
	/**
	 * Draws the y axis, containing og the axis line, the horizontal helping line
	 * and the labels.
	 * 
	 * @param g the Graphics context in which to paint
	 */
	private void yAxis(Graphics g) {
		...
		/*
		 * technically, this is numLabels-1, but as we're iterating 0 <= i <= numLabels,
		 * we need the extra label. Also don't draw more than 32 labels.
		 */
		...
		for (int i = 0; i <= numLabels; i++) {
			...
			g.drawLine(X_OFFSET_LEFT - 5, (int) ((1 - fraction) * height) + Y_OFFSET_TOP, X_OFFSET_LEFT,
