...
/**
 * This class generates colors in the HSV space. The space where the colors are
 * sampled from can be configured by specifying the range of hue, saturation and
 * brightness.
 *
...
public class HSVColorGenerator implements ColorGenerator {
	...
	/**
	 * constructor which sets the range to: hue - [0.0, 1.0) saturation - [1.0, 1.0]
	 * brightness - [1.0, 1.0]
	 */
	...
	/**
	 * constructor which sets the range of the hue to [0,1) and sets the ranges for
	 * saturation and brightness to the parameter
	 */
	...
	/**
	 * constructor which sets the ranges for saturation and brightness to the
	 * parameter
	 */
	public HSVColorGenerator(float hueMin, float hueMax, float saturationMin, float saturationMax, float brightnessMin,
			float brightnessMax) {
	...
	public Color[] generateColors(int numColors) {
		...
		// fix the seed to always get the same colors for the same numColors parameter
		// and ranges for hue, saturation and brightness
	 ...
	 *
	 * @param min   the minimal value which can be reached by setting the factor to
	 *              0.0
	 * @param max   the maximal value which can be reached by setting the factor to
	 *              1.0
	 * @param ratio a float between 0.0 and 1.0 which describes the ratio between
	 *              min and max to incorporate
