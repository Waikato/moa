...
/**
 * Manages setting the look and feel. Uses the {@link #KEY_LOOKANDFEEL} property
 * from {@link GUIDefaults} to determine what Look'n'Feel to use. See examples
 * in the GUI.props file for more details.
 *
...
public class LookAndFeel {
	...
	protected static boolean attemptInstallJavaLookAndFeel(String lnf) {
		...
		if (!result) {
			...
			if (!result)
				LOGGER.severe("Failed to set cross-platform Look'n'Feel (" + CROSSPLATFORM_LNF
	 ...
	 * cross-platform Java Look'n'Feel and JIDE style
	 * {@link LookAndFeelFactory#VSNET_STYLE_WITHOUT_MENU} if it fails.
	 *
	 * @param style the style to install, uses default strategy if -1
	 * @return true if successful
	 * @see LookAndFeelFactory
	...
	protected static boolean attemptInstallJideLookAndFeel(int style) {
		...
		if (!result) {
			LOGGER.info("Falling back on cross-platform Look'n'Feel and JIDE style "
					+ LookAndFeelFactory.VSNET_STYLE_WITHOUT_MENU + "...");
			result = installJavaLookAndFeel(CROSSPLATFORM_LNF)
					&& installJideLookAndFeel(LookAndFeelFactory.VSNET_STYLE_WITHOUT_MENU);
			if (!result)
				LOGGER.severe("Failed to set cross-platform Look'n'Feel (" + CROSSPLATFORM_LNF + ") and JIDE style "
						+ LookAndFeelFactory.VSNET_STYLE_WITHOUT_MENU + ", which should always succeed!");
		}
		return result;
	}
