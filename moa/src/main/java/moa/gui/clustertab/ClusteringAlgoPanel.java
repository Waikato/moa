...
import moa.learners.clusterers.Clusterer;
...
public class ClusteringAlgoPanel extends javax.swing.JPanel implements ActionListener {
	...
	private ClassOption algorithmOption0 = new ClassOption("Algorithm0", 'a', "Algorithm to use.", Clusterer.class,
			"ClusterGenerator");

	private ClassOption algorithmOption1 = new ClassOption("Algorithm1", 'c', "Comparison algorithm", Clusterer.class,
			"clustream.WithKmeans", "None");
	...
	public AbstractClusterer getClusterer0() {
		...
		applyChanges();
		try {
			c = (AbstractClusterer) ClassOption.cliStringToObject(algorithmOption0.getValueAsCLIString(),
					Clusterer.class, null);
		} catch (Exception ex) {
	...
	public AbstractClusterer getClusterer1() {
		...
		if (!algorithmOption1.getValueAsCLIString().equals("None")) {
			try {
				c = (AbstractClusterer) ClassOption.cliStringToObject(algorithmOption1.getValueAsCLIString(),
						Clusterer.class, null);
			} catch (Exception ex) {
				Logger.getLogger(ClusteringAlgoPanel.class.getName()).log(Level.SEVERE, null, ex);
			}
