...
public class OutlierAlgoPanel extends javax.swing.JPanel implements ActionListener {
	...
	private ClassOption streamOption = new ClassOption("Stream", 's', "", ClusteringStream.class,
			"RandomRBFGeneratorEvents");

	private ClassOption algorithmOption0 = new ClassOption("Algorithm0", 'a', "Algorithm to use.",
			MyBaseOutlierDetector.class, "MCOD.MCOD");

	private ClassOption algorithmOption1 = new ClassOption("Algorithm1", 'a', "Algorithm to use.",
			MyBaseOutlierDetector.class, "Angiulli.ExactSTORM", "None");
	...
	public MyBaseOutlierDetector getClusterer0() {
		...
		applyChanges();
		try {
			c = (MyBaseOutlierDetector) ClassOption.cliStringToObject(algorithmOption0.getValueAsCLIString(),
					MyBaseOutlierDetector.class, null);
		} catch (Exception ex) {
	...
	public MyBaseOutlierDetector getClusterer1() {
		...
		if (algorithmOption1.getValueAsCLIString().equalsIgnoreCase("None") == false) {
			applyChanges();
			try {
				c = (MyBaseOutlierDetector) ClassOption.cliStringToObject(algorithmOption1.getValueAsCLIString(),
						MyBaseOutlierDetector.class, null);
			} catch (Exception ex) {
				Logger.getLogger(OutlierAlgoPanel.class.getName()).log(Level.SEVERE, null, ex);
			}
	...
	public ClusteringStream getStream() {
		...
		applyChanges();
		try {
			s = (ClusteringStream) ClassOption.cliStringToObject(streamOption.getValueAsCLIString(),
	...
	/*
	 * We need to fetch the right item from editComponents list, index needs to
	 * match GUI order
	 */
	...
	private void initComponents() {
		...
		setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Cluster Algorithm Setup",
				javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION,
