...
/**
 * This is VisualizeFeatures tab main panel which loads data stream and shows other sub panels. This
 * panel refers to weka.gui.explorer.PreprocessPanel.
 *
...
public class VisualizeFeaturesPanel extends AbstractPerspective {
    ...
    /**
     * Set feature range shown in a popup window In default, Nine plots is shown in every popup
     * window at most.
     */
    protected JComboBox m_featureRangeBox;

    /**
     * For line graph or scatter diagram, the default max instance index is 500. In other words, the
     * default x axis is from 1 to 500 if the total instance number is bigger than 500, or the
     * default x axis is from 1 to numInstance if the total instance number is no more than 500.
    ...
    public VisualizeFeaturesPanel() {
        ...
        m_OpenFileBut.addActionListener(
                new ActionListener() {
                ...
                new ActionListener() {
                    public void actionPerformed(ActionEvent e) {
                        try {
                            ...
                            /**
                             * SamoaToWekaInstanceConverter.wekaInstances() method does not support
                             * class attribute*
                             */
                            for (int i = 0; i < selected.length; i++) {
                                if (selected[i] == m_Instances.classIndex()) {
                                    JOptionPane.showMessageDialog(
                                            VisualizeFeaturesPanel.this,
        ...
        colorBox.setToolTipText(
                "The chosen attribute will also be used as the "
        ...
        m_startInstanceInput.addFocusListener(
                new FocusAdapter() {
                    @Override
                        ...
                        try {
                            ...
                            if (o == null) {
                                ...
                                if (i < 1 || i >= m_Instances.numInstances()) {
                                    ...
                                    JOptionPane.showMessageDialog(
                                            m_startInstanceInput.getParent(),
                                            "The instance start index must be positive integer, \n"
                                                    + "more than 0 and less than total instance"
                                                    + " number!",
                                            "Instance startIndex input error prompt!",
        ...
        m_endInstanceInput.addFocusListener(
                new FocusAdapter() {
                    @Override
                    public void focusLost(FocusEvent e) {
                        super.focusLost(e);
                        if (m_Instances != null) {
                            ...
                            try {
                                ...
                                if (o == null) {
                                    m_endInstanceInput.setValue(
                                            numInst); // Default is set so this could not happen.
                                } else {
                                    ...
                                    if (inputInt < 2 || inputInt > numInst) {
                                        ...
                                        if (numInst <= m_defaultMaxInstanceIndex) {
                                            if (numInst <= m_defaultMaxInstanceIndex) {
                                                ...
                                                m_endInstanceInput.setValue(
                                                        m_defaultMaxInstanceIndex);
                                            }
                                        }
                                        // warning
                                        Toolkit.getDefaultToolkit().beep();
                                        JOptionPane.showMessageDialog(
                                                m_endInstanceInput.getParent(),
                                                "The instance end index must be positive integer,"
                                                    + " \n"
                                                    + "more than 1 and not more than total instance"
                                                    + " number \""
                                                        + numInst
                                                        + "\"!",
                                                "Instance startIndex input error prompt!",
         ...
         */
        m_startInstanceInput.setToolTipText(
                "Start index must be positive integer, bigger than 0 and less than instance"
                        + " number!");

        /** numInstance=>endIndex>1 */
        m_endInstanceInput.setToolTipText(
                "End index must be positive integer, bigger than 1 and no than instance number!");

        m_plotTypeBox.setEditable(false);
        m_plotTypeBox.setToolTipText("The chosen item will determine the feature plot type.");
        m_plotTypeBox.addItemListener(
                new ItemListener() {
                    @Override
                    public void itemStateChanged(ItemEvent e) {
                        if (e.getStateChange() == ItemEvent.SELECTED) {
                            if (colorBox.getSelectedItem() != null) {
                                if (m_Instances != null) {
                                    Object objectStartInstanceInput =
                                            m_startInstanceInput.getValue();
                                    Object objectEndInstanceInput = m_endInstanceInput.getValue();
                                    m_intStartIndex =
                                            Integer.parseInt(objectStartInstanceInput.toString());
                                    m_intEndIndex =
                                            Integer.parseInt(objectEndInstanceInput.toString());
                                    ...
                                    if (m_intEndIndex > m_intStartIndex) {
                                        ...
                                        String selectedPlotItem =
                                                m_plotTypeBox.getSelectedItem().toString();
                                        ...
                                        if (attributeIndex == 0) { //
                                            /**
                                             * if the class is "no class",set the first class whose
                                             * index is 0 as default class index.
                                             */
                                            m_graphPanel.setSelectedPlotInfo(
                                                    m_selectedPlotTypeIndex,
                                                    selectedPlotItem,
                                                    0,
                                                    colorBox.getItemAt(1).toString());
                                        } else {
                                            m_graphPanel.setSelectedPlotInfo(
                                                    m_selectedPlotTypeIndex,
                                                    selectedPlotItem,
                                        ...
                                        Toolkit.getDefaultToolkit().beep();
                                        JOptionPane.showMessageDialog(
                                                m_plotTypeBox.getParent(),
                                                "EndIndex must be bigger than StartIndex",
                                                "Instance index range error prompt!",
                                                JOptionPane.WARNING_MESSAGE);
                                    }
                                }
                            }
                    ...
                    public void actionPerformed(ActionEvent ae) {
                        if (m_Instances != null) {
                            try {
                                ...
                                as.setInstances(
                                        m_samoaToWekaInstanceConverter.wekaInstances(m_Instances));

                                final JFrame jf =
                                        Utils.getWekaJFrame(
                                                "All attributes", VisualizeFeaturesPanel.this);
        ...
        m_plotAmplify.addActionListener(
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        if (m_Instances != null) {
        ...
        m_visAllGraphBut.addActionListener(
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
                        if (m_Instances != null) {
        ...
        m_featureRangeBox.addItemListener(
                new ItemListener() {
                    @Override
                    public void itemStateChanged(ItemEvent e) {
                        if (e.getStateChange() == ItemEvent.SELECTED) {
    ...
    public boolean startIndexValidation(int startIndex) {
        if (startIndex >= 1) {
            ...
            JOptionPane.showMessageDialog(
                    this,
                    "The instance start index must be integer and not less than 1:\n",
                    "Instance Index",
                    JOptionPane.ERROR_MESSAGE);
            return false;
        }
    ...
    public boolean endIndexValidation(int endIndex) {
        if (endIndex <= m_Instances.numInstances()) {
            ...
            JOptionPane.showMessageDialog(
                    this,
                    "The instance end index must be integer and not bigger than \""
            ...
            Runnable r =
                    new Runnable() {
                        public void run() {
                            ...
                            /**
                             * When new data is loaded, set default value for m_intStartIndex and
                             * m_intEndIndex
                             */
                            if (m_Instances != null) {
                            ...
                            /**
                             * The suggested best set for maxPlotsNumberInOneScreen is 9 for 3 rows
                             * * 3 columns plots.
                             */
                            int maxPlotsNumberInOneScreen = 9; //
                            if (m_Instances != null && m_Instances.numAttributes() > 0) {
                                m_featureRangeBox
                                        .removeAllItems(); // when user changes dataset, this code
                                // will be used.
                                featureRangeBoxSet(
                                        m_Instances.numAttributes(), maxPlotsNumberInOneScreen);
                            }
                                        ...
                                        new TableModelListener() {
                                            public void tableChanged(TableModelEvent e) {
                                                if (m_AttPanel.getSelectedAttributes() != null
                                                        && m_AttPanel.getSelectedAttributes().length
                                                                > 0) {
                            ...
                            m_AttVisualizePanel.setInstances(
                                    m_samoaToWekaInstanceConverter.wekaInstances(m_Instances));
                            ...
                            /**
                             * The default selected item of component colorBox in this class is the
                             * class attribute, so the default attribute shown in scatter diagram is
                             * the class attribute too. In other words, the initial attribute shown
                             * in histogram and scatter diagram both is the class attribute which is
                             * always the last attribute in a dataset.
                             */
                            m_graphPanel.setAttributeIndex(m_Instances.classIndex());

                            m_Log.logMessage(
                                    "Base relation is now "
                                            + m_Instances.getRelationName()
                                            + " ("
                                            + m_Instances.numInstances()
                                            + " instances)");
    ...
    public void featureRangeBoxSet(int numAttributes, int maxFeaturesInOneScreen) {
        if (numAttributes >= maxFeaturesInOneScreen) {
            ...
            for (int i = 0; i < itemNum; i++) {
                m_featureRangeBox.addItem(
                        "feature range: "
                                + (i * maxFeaturesInOneScreen + 1)
                                + " -- "
                                + (i + 1) * maxFeaturesInOneScreen);
            }
            if (remainder > 0 && remainder == 1) {
                m_featureRangeBox.addItem(
                        "feature range: " + (itemNum * maxFeaturesInOneScreen + 1));
            } else if (remainder > 1) {
                m_featureRangeBox.addItem(
                        "feature range: "
                                + (itemNum * maxFeaturesInOneScreen + 1)
                                + " -- "
                                + (itemNum * maxFeaturesInOneScreen + remainder));
            }
        } else if (numAttributes < maxFeaturesInOneScreen && numAttributes > 1) {
            /**
             * Then number of attributes is more than 1 but less than max features whose plots are
             * shown in "visualizeAll" screen.
             */
                        ...
                        public void run() {
                            try {
                                ...
                                if (filter != null) {
                                    ...
                                    weka.core.Instances copyToWeka =
                                            m_samoaToWekaInstanceConverter.wekaInstances(copy);
                                    filterCopy.setInputFormat(copyToWeka);
                                    weka.core.Instances newInstances =
                                            Filter.useFilter(copyToWeka, filterCopy);
                                    ...
                                    // if class was not set before, reset it again after use of
                                    // filter
                                    /* if (m_Instances.classIndex() < 0)
                                    newInstances.setClassIndex(-1); */
                                    m_Instances =
                                            m_wekaToSamoaInstanceConverter.samoaInstances(
                                                    newInstances);
    ...
    /**
     * Queries the user for a file to load instances from, then loads the instances in a background
     * process. This is done in the IO thread, and an error message is popped up if the IO thread is
     * busy.
     */
                    ...
                    new Thread() {
                        ...
                        public void run() {
                            try {
                                ...
                                Instances insts =
                                        new Instances(reader, 1, -1); // parameter size is not used.
    ...
    /**
     * Queries the user for a file to save instances as, then saves the instances in a background
     * process. This is done in the IO thread, and an error message is popped up if the IO thread is
     * busy.
     */
    ...
    public void instantiationComplete() {
        ...
        if (perspectivesThatAcceptInstances.size() > 0) {
            ...
            sendToAllItem.addActionListener(
                    new ActionListener() {
                        @Override
                        public void actionPerformed(ActionEvent e) {
                            for (Perspective p : perspectivesThatAcceptInstances) {
                                if (getInstances() != null && p.acceptsInstances()) {
                                    p.setInstances(
                                            m_samoaToWekaInstanceConverter.wekaInstances(
                                                    getInstances()));
                                    getMainApplication()
    ...
    public static class PreprocessDefaults extends Defaults {
        ...
        public static final Settings.SettingKey INITIAL_DIR_KEY =
                new Settings.SettingKey(
                        ID + ".initialDir", "Initial directory for opening datasets", "");
        public static final File INITIAL_DIR = new File("${user.dir}");

        public static final Settings.SettingKey UNDO_DIR_KEY =
                new Settings.SettingKey(ID + ".undoDir", "Directory for storing undo files", "");
        ...
        public static final Settings.SettingKey ALWAYS_SEND_INSTANCES_TO_ALL_KEY =
                new Settings.SettingKey(
                        ID + ".alwaysSendInstancesToAllPerspectives",
        ...
        public PreprocessDefaults() {
            ...
            INITIAL_DIR_KEY.setMetadataElement(
                    "java.io.File.fileSelectionMode", "" + JFileChooser.DIRECTORIES_ONLY);
            INITIAL_DIR_KEY.setMetadataElement(
                    "java.io.File.dialogType", "" + JFileChooser.OPEN_DIALOG);
            UNDO_DIR_KEY.setMetadataElement(
                    "java.io.File.fileSelectionMode", "" + JFileChooser.DIRECTORIES_ONLY);
            UNDO_DIR_KEY.setMetadataElement(
                    "java.io.File.dialogType", "" + JFileChooser.DIRECTORIES_ONLY);
            ...
            m_defaults.put(ALWAYS_SEND_INSTANCES_TO_ALL_KEY, ALWAYS_SEND_INSTANCES_TO_ALL);
        }
    }
