 ...
 * a data stream as a table. The user can chooses one or multiple features' scores to show as line
 * graphs. This panel refers to weka.gui.AttributeSelectionPanel.
 *
...
public class FeatureImportanceDataModelPanel extends JPanel {
    ...
    /**
     * A table model that looks at the names of attributes and maintains a list of attributes that
     * have been "selected".
     */
    class FeatureImportanceDataModel extends AbstractTableModel {
                ...
                default:
                    if (Double.isNaN(getFeatureImportance()[column - 3][row])) {
                        ...
                        if (getFeatureImportance()[column - 3][row] >= 1.0) {
                            return "<html><strong color='red'>"
                                    + Double.toString(getFeatureImportance()[column - 3][row])
                                    + "</strong></html>";
                        } else {
        ...
        /**
         * applies the perl regular expression pattern to select the attribute names (expects a
         * valid reg expression!)
         *
        ...
        public void setSelectedAttributes(boolean[] selected) throws Exception {
            if (selected.length != m_Selected.length) {
                throw new Exception(
                        "Supplied array does not have the same number "
     ...
     */
    public FeatureImportanceDataModelPanel(
            boolean include, boolean remove, boolean invert, boolean pattern) {

        m_IncludeAll.setToolTipText("Selects all attributes");
        m_IncludeAll.setEnabled(false);
        m_IncludeAll.addActionListener(
                new ActionListener() {
                    @Override
        ...
        m_Table.setPreferredScrollableViewportSize(
                new Dimension(super.getWidth(), super.getHeight()));

        m_Table.addMouseListener(
                new MouseAdapter() {
                    @Override
    ...
    protected void popupCopyRangeMenu(int x, int y) {
        ...
        copyRangeItem.addActionListener(
                new ActionListener() {
                    @Override
                    public void actionPerformed(ActionEvent e) {
    ...
    public void setFeatureImportanceScores(double[][] scores) {
        if (m_Model == null) {
            ...
            /**
             * These are the crucial codes to update table show when data set "m_Instances" or
             * scores is changed.
             */
            m_Table.removeAll();
            m_Model = new FeatureImportanceDataModel(m_Instances);
        ...
        /** show horizontal scroll bar; auto resize column width. */
    ...
    /**
     * Set the selected attributes in the widget. Note that setInstances() must have been called
     * first.
     *
     * @param selected an array of boolean indicating which attributes are to have their check boxes
     *     selected.
     * @throws Exception if the supplied array of booleans does not have the same number of elements
     *     as there are attributes.
     */
    public void setSelectedAttributes(boolean[] selected) throws Exception {
        if (m_Model != null) {
