...
/**
 * This panel is the FeatureImportance tab which provides config GUI for feature importance
 * algorithm, run button to trigger the execution of the algorithm, table line graphs to display
 * scores of the the execution result.
 */
public class FeatureImportancePanel extends AbstractPerspective {
    ...
    /**
     * Configure GUI so that user can set parameters for feature importance algorithm and trigger
     * task execution to compute scores of feature importance.
     */
    protected MainTask currentTask = new FeatureImportanceConfig();

    /**
     * Tasks are encapsulated in task thread to execute. This List Stores tasks threads which
     * include the execution results of the threads.
     */
    ...
    /**
     * Feature importance data model includes two parts, the dataset and scores which will be shown
     * in table so user can view data and choose which feature importance scores to be shown in line
     * graph.
     */
    protected FeatureImportanceDataModelPanel featureImportanceDataModelPanel =
            new FeatureImportanceDataModelPanel();
    ...
    private FeatureImportancePanel() {
        ...
        configPanel.setBorder(
                BorderFactory.createTitledBorder(
                        "Feature importance algorithm parameters config:"));
                    ...
                    public void actionPerformed(ActionEvent arg0) {
                        String newTaskString =
                                ClassOptionSelectionPanel.showSelectClassDialog(
                                        FeatureImportancePanel.this,
                                        "Configure task",
                                        ClassificationMainTask.class,
                                        FeatureImportancePanel.this.currentTask
                                                .getCLICreationString(ClassificationMainTask.class),
                                        null);
                        ...
                        if (m_Instances != null) {
                            ...
                            if (numInstances <= windowSize) {
                                ...
                                JOptionPane.showMessageDialog(
                                        runTaskButton.getParent(),
                                        "Parameter windowSize of feature importance algorithm must"
                                                + " be smaller than numInstances.\n"
                                                + "The current set: windowSize is \""
                                                + windowSize
                                                + "\" and numInstances is \""
                                                + numInstances
                                                + "\".",
                                ...
                                // runTask((Task)
                                // FeatureImportancePanel.this.currentTask.copy());//Can not be used
                                // in this case.
                                runTask((Task) FeatureImportancePanel.this.currentTask);

                                TaskThread thread = taskList.get(0);
                                thread.addTaskCompletionListener(
                                        new TaskCompletionListener() {
                                            ...
                                            public void taskCompleted(TaskThread task) {
                                                ...
                                                /**
                                                 * Feature importance score values may be like
                                                 * 0.053471126481504741, 0.0, 1.0, NAN. If the value
                                                 * is like 0.053471126481504741, keep just 4
                                                 * rounding numbers after point.
                                                 */
                                                for (int row = 0; row < scores.length; row++) {
                                                    for (int col = 0;
                                                            col < scores[row].length;
                                                            col++) {
                                                        if (!Double.isNaN(scores[row][col])
                                                                && Double.toString(scores[row][col])
                                                                                .length()
                                                                        >= 6) {
                                                            scores[row][col] =
                                                                    new BigDecimal(scores[row][col])
                                                                            .setScale(
                                                                                    4,
                                                                                    BigDecimal
                                                                                            .ROUND_HALF_UP)
                                                ...
                                                /**
                                                 * pass copied feature importance data to table
                                                 * panel
                                                 */
                                                double[][] scoresCopyForDataModel = scores.clone();
                                                featureImportanceDataModelPanel
                                                        .setFeatureImportanceScores(
                                                                scoresCopyForDataModel);

                                                /**
                                                 * pass attributes names and feature importance data
                                                 * to line graph panel
                                                 */
                                                featureImportanceGraph.setAttributeNames(
                                                        attributeNames);

                                                int rows = scores.length;
                                                int columns = scores[0].length;

                                                double[][] scoresCopyForGraph =
                                                        new double[rows][columns];
                                                for (int k = 0; k < scores.length; k++) {
                                                    /**
                                                     * This is deep value copy so that the object
                                                     * scoresCopyForGraph and scoresCopyForDataModel
                                                     * don't interact each other. For
                                                     * FeatureImportanceGraph, if feature importance
                                                     * is NaN, the line graph shows 0.0; For
                                                     * FeatureImportanceDataModelPanel, if feature
                                                     * importance is NaN, the table shows NaN;
                                                     */
                                                    scoresCopyForGraph[k] = scores[k].clone();
                                                }

                                                double naNSubstitute =
                                                        ((FeatureImportanceConfig) currentTask)
                                                                .getNaNSubstitute();
                                                for (int i = 0; i < rows; i++) {
                                                    for (int j = 0; j < columns; j++) {
                                                        if (Double.isNaN(
                                                                scoresCopyForGraph[i][j])) {
                                                            /**
                                                             * Plot2DPanel object can only show
                                                             * double data values, so if feature
                                                             * importance value is "NaN", the value
                                                             * "NaN" is replaced by a user-set
                                                             * double value which will be show in
                                                             * line graph.
                                                             */
                                                            scoresCopyForGraph[i][j] =
                                                                    naNSubstitute;
                                                ...
                                                featureImportanceGraph.setFeatureImportance(
                                                        scoresCopyForGraph);
                                            }
                                        });

                                JProgressBar sourceProgressBar =
                                        ((FeatureImportanceConfig) currentTask).getProgressBar();

                                sourceProgressBar.addChangeListener(
                                        new ChangeListener() {
                                            @Override
                            ...
                            JOptionPane.showMessageDialog(
                                    runTaskButton.getParent(),
                                    "Please open data stream file from \n"
                                            + "VisualizerFeatures tab first!",
                                    "Not open file error prompt!",
        ...
        featureImportanceTablePanel.setBorder(
                BorderFactory.createTitledBorder("Feature importance table"));
                ...
                .addListSelectionListener(
                        new ListSelectionListener() {
                            ...
                            public void valueChanged(ListSelectionEvent e) {
                                if (!e.getValueIsAdjusting()) { // True means that user finish one
                                    // row selection with mouse down
                                    // and up

                                    m_selectedAttributeIndices =
                                            featureImportanceDataModelPanel.m_Model
                                                    .getSelectedAttributes(); // probably produce
                                    // empty int array
                                    // like "[]"
                                    featureImportanceGraph.setSelectedAttributeIndices(
                                            m_selectedAttributeIndices);
                                }
                            }
                        });
        ...
        featureImportanceLineGraphPanel.setBorder(
                BorderFactory.createTitledBorder("Feature importance line graph"));
    ...
    /**
     * Parse parameter windowSize from user's configuration or preference。 The parameter windowSize
     * is used to check whether the total instance number is bigger than windowSize after user click
     * the Run button and before the feature importance task being executed.
     *
                ...
                try {
                    ...
                    setWindowSize(
                            Integer.parseInt(
                                    subString.substring(
                                            0,
                                            subStringLength - 1))); // Deal with sting like "505)".
                }
            }
        }
    }
    ...
    public void setTaskString(String cliString, boolean storePreference) {
        try {
            this.currentTask =
                    (MainTask) ClassOption.cliStringToObject(cliString, MainTask.class, null);
            String taskText = this.currentTask.getCLICreationString(MainTask.class);
    ...
    public static void main(String[] args) {
        WekaToSamoaInstanceConverter m_wekaToSamoaInstanceConverter =
                new WekaToSamoaInstanceConverter();
        try {
            if (args.length == 0) {
