...
/**
 * This is a sub panel in VisualizeFeatures tab. It is used to show a feature as a line graph or a
 * scatter diagram, or to show an amplified line graph or scatter diagram, or to show all features'
 * line graphs or scatter diagrams.
 */
public class LineAndScatterPanel extends JPanel {
     ...
     *
     * <p>m_selectedPlotTyeIndex means the selected plot index
     */
    ...
    /**
     * The feature range start index. For example, if the feature range is from 1 to 9, then the
     * start index is 1 and the end index is 9.
     *
     * <p>The default plot number in a popup screen is 9 at most.
     */
    protected int m_featureRangeStartIndex;

    /**
     * The feature range end index. For example, if the feature range is from 1 to 9, then the start
     * index is 1 and the end index is 9.
     *
     * <p>The default plot number in a popup screen is 9 at most.
     */
    ...
    /**
     * Parse String to number. For example, featureRange is "feature range: 1 -- 9", then is parsed
     * into 0 and 8.
     */
    public void setFeatureRange(String featureRange) {
        ...
        /**
         * The string is separated by one single space character so it is split by one single space
         * character. featureRange example: "feature range: 1 -- 9", or "feature range: 5".
         */
        ...
        /**
         * The number shown to user in droplist is natural number,but the index of data in array is
         * from 0, so the following number should minus one.
         */
    ...
    public void setInstances(Instances inst) {
        ...
        if (m_data != null) {
            ...
            for (int i = 0; i < numAttributes; i++) {
                // Future<double[]> model = service.submit(new
                // ExtractFeatureVectorCallable(Integer.valueOf(i)));
                futures[i] = service.submit(new ExtractFeatureVectorCallable(Integer.valueOf(i)));

                /**
                 * In particular, "model.get()" is a synchronized operation, which can not be run
                 * with the above service.submit(), otherwise the multiple threads can not exert
                 * their effect.
                 */
    ...
    public void setSelectedPlotInfo(
            int selectedPlotTyeIndex,
        ...
        /**
         * remove the last plot in which maybe there is empty graph or there is a graph, so that the
         * next plot is new and updated by new data
         */
    ...
    /**
     * This override method is used to paint embedded line graph or scatter diagram in
     * VisualizeFeature Tab.
     */
    @Override
    public void paintComponent(Graphics g) {
        ...
        if (m_data != null) {
            ...
            /**
             * when Plot2DPanel plots, it's plot range is [startIndex,endIndex) For example,range
             * [1,201) means that Plot2DPanel will plot the first point but not plot the 201th
             * point.
             */
            double[] y_axis_segment =
                    Arrays.copyOfRange(y_axis, m_intStartIndex - 1, m_intEndIndex);
    ...
    /**
     * This method is used to paint line graph or scatter diagram in popup window from
     * VisualizeFeature Tab.
     */
    public void paintAmplifiedPlot() {
        /**
         * The plot object here must be different from the plot object in method
         * paintComponent(Graphics g), otherwise, the diagram here will overlapped on the diagram
         * plotted in method paintComponent(Graphics g).
         */
        ...
        if (m_data != null) {
            ...
            if (getSelectedPlotItem().equalsIgnoreCase("plot type: Line graph")) {
                ...
                if (attrName.endsWith("(nominal)")) {
                    // multiple legend
                    /**
                     * 1.filter different y axis values, count for each different value 2.according
                     * to y axis values, get new sub pair x_axis array and y axis array 3.plot graph
                     *
                     * <p>For example,the original x axis array and y axis array are the following
                     * x_axis: [50.0, 51.0, 52.0, 53.0, 54.0, 55.0, 56.0, 57.0, 58.0, 59.0, 60.0]
                     * y_axis: [0.0, 1.0, 0.0, 1.0, 2.0, 1.0, 0.0, 1.0, 2.0, 0.0, 2.0]
                     *
                     * <p>After step 1,get y axis values: 0.0,1.0,2.0. The count of 0.0 is 4,the
                     * count of 1.0 is 4, and the count of 2.0 is 3
                     *
                     * <p>After step 2,get the following three sub pair arrays sub array 1: the y
                     * axis value is 0.0 x_axis: [50.0, 52.0, 56.0, 59.0] y_axis: [0.0, 0.0, 0.0,
                     * 0.0]
                     *
                     * <p>sub array 1:the y axis value is 1.0 x_axis: [51.0, 53.0, 55.0, 57.0]
                     * y_axis: [1.0, 1.0, 1.0, 1.0]
                     *
                     * <p>sub array 1:the y axis value is 2.0 x_axis: [54.0, 58.0, 60.0] y_axis:
                     * [2.0, 2.0, 2.0]
                     */
                    ...
                    for (Object key : hm.keySet()) {
                        // System.out.println("y value: "+(double)key + ", y value count: "+
                        // hm.get(key));
                        ...
                        /**
                         * Plot2DPanel.addScatterPlot(String name, double[] X, double[] Y) method
                         * has a bug. When it plot two points, it wrongly swap a pair of x axis and
                         * y axis. But when it plots one point or more than two point, it behaves
                         * well.
                         *
                         * <p>Bug example: the original data is as the following two points x_axis:
                         * [6.0, 58.0] y_axis: [96.0, 96.0] but actually, it plots the following two
                         * wrong points x_axis: [6.0, 96.0] y_axis: [58.0, 96.0]
                         *
                         * <p>A compromise solution: a little bit strange, but it works. 1. swap
                         * x_axis[1] for y_axis[0] in advance; before swap(right data): x_axis:
                         * [6.0, 58.0] y_axis: [96.0, 96.0] after swap(wrong data): x_axis: [6.0,
                         * 96.0] y_axis: [58.0, 96.0]
                         *
                         * <p>2. take advantage of the bug, it swap x_axis[1] for y_axis[0] the 2nd
                         * time, and get the right data x_axis: [6.0, 58.0] y_axis: [96.0, 96.0]
                         */
                        if (value_count == 2) {
                            ...
                            for (int s = 0; s < len; s++) {
                                if (y_axis_segment[s] == y_value) {
                                    ...
                                    if (count == 1) {
                                        ...
                                        al.add(
                                                Double.valueOf(
                                                        y_value)); // the source of y_axis_sub[1]
                                    }
                                    continue;
                                }
                            }
                    ...
                    amplifyPlot.addScatterPlot(
                            attrName, Color.BLUE, x_axis_segment, y_axis_segment);
                }
            }
        }
    }
    ...
    /**
     * 1. For nominal feature, just display scatter diagrams in which the data range is from
     * m_intStartIndex to m_intEndIndex 2. Fro numeric feature, show line graph or scatter diagram
     * depending on user's plot type choice, in which the data range is from m_intStartIndex to
     * m_intEndIndex
     */
    public void visualizeAll() {
        if (m_data != null) {
            ...
            /** String: plot type: Line graph plot type: Scatter diagram No plot type */
            ...
            for (int p = m_featureRangeStartIndex; p <= m_featureRangeEndIndex; p++) {
                ...
                double[] y_axis_visualizeAll_segment =
                        Arrays.copyOfRange(y_axis_visualizeAll, m_intStartIndex - 1, m_intEndIndex);
                ...
                if (isNominal) {
                    plot2d.addScatterPlot(
                            name,
                            Color.BLUE,
                            x_axis_visualizeAll_segment,
                            y_axis_visualizeAll_segment);
                } else {
                    if (getSelectedPlotItem().equalsIgnoreCase("plot type: Line graph")) {
                        plot2d.addLinePlot(
                                name,
                                ...
                                x_axis_visualizeAll_segment,
                                y_axis_visualizeAll_segment);
                    } else if (getSelectedPlotItem()
                            .equalsIgnoreCase("plot type: Scatter diagram")) {
                        plot2d.addScatterPlot(
                                name,
                                Color.BLUE,
                                x_axis_visualizeAll_segment,
