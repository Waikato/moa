	...
	public static void main(String[] args) {
		try {
			if (args.length < 1) {
				...
				AbstractTask task;
				try {
					task = (AbstractTask) ClassOption.cliStringToObject(cliString.toString(), MainTask.class,
							extraOptions);
				} catch (Exception e) {
					// regular task could not be found, maybe it is a meta task
					task = (AbstractTask) ClassOption.cliStringToObject(cliString.toString(), MetaMainTask.class,
							extraOptions);
				}
				task.prepareForUse();
