...
/**
 * A simple implementation of the <code>Cluster</code> interface representing spherical clusters.
...
public class SphereCluster extends Cluster {
     ...
     * overlapRadiusDegree only calculates the overlap based on the centers and the radi, so not the
     * real overlap
     *
     * <p>TODO: should we do this by MC to get the real overlap???
     *
     * @param other
     * @return
     */
    public double overlapRadiusDegree(SphereCluster other) {
        ...
        if (dist > radiusSmall + radiusBig) return 0;
    ...
    public void combine(SphereCluster cluster) {
        ...
        for (int i = 0; i < center.length; i++) {
            newcenter[i] =
                    (center[i] * getWeight() + other_center[i] * other_weight)
                            / (getWeight() + other_weight);
        }
     ...
     */
    /**
     * When a clusters looses points the new minimal bounding sphere can be partly outside of the
     * originating cluster. If a another cluster is right next to the original cluster (without
     * overlapping), the new cluster can be overlapping with this second cluster. OverlapSave will
     * tell you if the current cluster can degenerate so much that it overlaps with cluster 'other'
     *
