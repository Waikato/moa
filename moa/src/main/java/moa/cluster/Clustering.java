 ...
 * Clustering.java
 *
 * <p>Represents a collection of clusters.
 *
...
public class Clustering extends AbstractMOAObject {
    ...
    public Clustering(List<? extends Instance> points) {
        ...
        if (!classLabel.isNominal())
            throw new IllegalStateException(
                    "Class attribute '" + classLabel.name() + "' is not nominal!");
                ...
                if (micro_points.size() > 0) {
                    ...
                    for (int c = 0; c < microByClass.size(); c++) {
                        if (((SphereCluster) microByClass.get(c)).overlapRadiusDegree(s)
                            ...
                            // System.out.println("Removing redundant cluster based on radius
                            // overlap"+c);
                        }
                    }
            ...
            while (changed) {
                ...
                for (int c = 0; c < microByClass.size(); c++) {
                    for (int c1 = c + 1; c1 < microByClass.size(); c1++) {
                        ...
                        // C"+(clustering.size()+c)+" ->C"+(clustering.size()+c1)+": "+overlap);
    ...
    public static HashMap<Integer, Integer> classValues(List<? extends Instance> points) {
        ...
        if (hasnoise) classes.put(-1, workcluster);
        return classes;
    }
