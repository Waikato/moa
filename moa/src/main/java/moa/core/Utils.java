...
public final class Utils {
    ...
    /**
     * Returns the value used to code a missing value. Note that equality tests on this value will
     * always return false, so use isMissingValue(double val) for testing..
     *
    ...
    /**
     * Casting an object without "unchecked" compile-time warnings. Use only when absolutely
     * necessary (e.g. when using clone()).
     */
    ...
    /**
     * Pads a string to a specified length, inserting spaces on the left as required. If the string
    ...
    public static Class getArrayClass(Class c) {
        if (c.getComponentType().isArray()) return getArrayClass(c.getComponentType());
        else return c.getComponentType();
    }

    /**
     * Returns the dimensions of the given array. Even though the parameter is of type "Object" one
     * can hand over primitve arrays, e.g. int[3] or double[2][4].
     *
    ...
    /**
     * Returns the given Array in a string representation. Even though the parameter is of type
     * "Object" one can hand over primitve arrays, e.g. int[3] or double[2][4].
     *
    ...
    public static String arrayToString(Object array) {
        ...
        if (dimensions == 0) {
            ...
            for (i = 0; i < Array.getLength(array); i++) {
                if (i > 0) result += ",";
                if (Array.get(array, i) == null) result += "null";
                else result += Array.get(array, i).toString();
            }
        } else {
            for (i = 0; i < Array.getLength(array); i++) {
                if (i > 0) result += ",";
    ...
    /**
     * Checks if the given array contains the flag "-String". Stops searching at the first marker
     * "--". If the flag is found, it is replaced with the empty string.
     *
    ...
    public static boolean getFlag(String flag, String[] options) throws Exception {
        ...
        if (pos > -1) options[pos] = "";

        return (pos > -1);
    }

    /**
     * Gets an option indicated by a flag "-Char" from the given array of strings. Stops searching
     * at the first marker "--". Replaces flag and option with empty strings.
     *
    ...
    /**
     * Gets an option indicated by a flag "-String" from the given array of strings. Stops searching
     * at the first marker "--". Replaces flag and option with empty strings.
     *
    ...
    /**
     * Gets the index of an option or flag indicated by a flag "-Char" from the given array of
     * strings. Stops searching at the first marker "--".
     *
     * @param flag the character indicating the option.
     * @param options the array of strings containing all the options.
    ...
    /**
     * Gets the index of an option or flag indicated by a flag "-String" from the given array of
     * strings. Stops searching at the first marker "--".
     *
     * @param flag the String indicating the option.
     * @param options the array of strings containing all the options.
     ...
     * unquoted question mark in arff files.
     *
    ...
    /**
     * unquotes are previously quoted string (but only if necessary), i.e., it removes the single
     * quotes around it. Inverse to quote(String).
     *
     ...
     * secondary set is defined to be any options after the first "--". These options are removed
     * from the original options array.
     *
    ...
    /**
     * The inverse operation of backQuoteChars(). Converts back-quoted carriage returns and new
     * lines in a string to the corresponding character ('\r' and '\n'). Also "un"-back-quotes the
     ...
     * @throws Exception in case of an unterminated string, unknown character or a parse error
     */
    ...
    /**
     * Joins all the options in an option array into a single string, as might be used on the
     * command line.
     *
     ...
     * positions of the elements of the original array in the sorted array. The sort is stable.
     * (Equal elements remain in their original order.)
     *
     ...
     * Sorts a given array of doubles in ascending order and returns an array of integers with the
     * positions of the elements of the original array in the sorted array. The sort is stable
     * (Equal elements remain in their original order.) Occurrences of Double.NaN are treated as
     * Double.MAX_VALUE
     *
     * @param array this array is not changed by the method!
     * @return an array of integers with the positions in the sorted array.
    ...
    public static /*@pure@*/ double variance(double[] vector) {
        ...
        double result =
                (sumSquared - (sum * sum / (double) vector.length)) / (double) (vector.length - 1);
    ...
    private static void quickSort(
            /*@non_null@*/ double[] array, /*@non_null@*/ int[] index, int left, int right) {
    ...
    //  assignable index;
    private static void quickSort(
            /*@non_null@*/ int[] array, /*@non_null@*/ int[] index, int left, int right) {
    ...
    private static int select(
            /*@non_null@*/ double[] array, /*@non_null@*/ int[] index, int left, int right, int k) {
    ...
    /**
     * Converts a File's absolute path to a path relative to the user (ie start) directory. Includes
     * an additional workaround for Cygwin, which doesn't like upper case drive letters.
     *
    ...
    // @ requires 0 <= first && first <= right && right < array.length;
    private static int select(
            /*@non_null@*/ int[] array, /*@non_null@*/ int[] index, int left, int right, int k) {
     ...
     * </pre></code>
     *
     * @param classType the class that the instantiated object should be assignable to -- an
     *     exception is thrown if this is not the case
     * @param className the fully qualified class name of the object
     * @param options an array of options suitable for passing to setOptions. May be null. Any
     *     options accepted by the object will be removed from the array.
