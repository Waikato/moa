 ...
 * Java libraries:
 *
 * <p>Copyright 2001 University of Waikato Copyright 1999 CERN - European Organization for Nuclear
 * Research. Permission to use, copy, modify, distribute and sell this software and its
...
public class Statistics {
    ...
    /**
     * Returns chi-squared probability for given value and degrees of freedom. (The probability that
     * the chi-squared variate will be greater than x for the given degrees of freedom.)
     *
     * @param x the value
     ...
     * <tt>y</tt> (assumes mean is zero, variance is one).
     *
     * <p>For small arguments <tt>0 {@literal <} y {@literal <} exp(-2)</tt>, the program computes
     * <tt>z = sqrt( -2.0 * log(y) )</tt>; then the approximation is <tt>x = z - log(z)/z - (1/z)
     * P(1/z) / Q(1/z)</tt>. There are two rational functions P/Q, one for <tt>0 {@literal <} y
     * {@literal <} exp(-32)</tt> and the other for <tt>y</tt> up to <tt>exp(-2)</tt>. For larger
     * arguments, <tt>w = y - 0.5</tt>, and <tt>x/sqrt(2pi) = w + w**3 R(w**2)/S(w**2))</tt>.
     *
    ...
    public static double normalInverse(double y0) {
        ...
        else x1 = z * polevl(z, P2, 8) / p1evl(z, Q2, 8);
        x = x0 - x1;
        if (code != 0) x = -x;
        return (x);
    }
     ...
     * </pre>
     *
     * <b>Implementation:</b> For <tt>0 {@literal <=} |x| {@literal <} 1, erf(x) = x *
     * P4(x**2)/Q5(x**2)</tt>; otherwise <tt>erf(x) = 1 - erfc(x)</tt>.
     *
     * <p>Code adapted from the <A HREF="http://www.sci.usq.edu.au/staff/leighb/graph/Top.html">
     * Java 2D Graph Package 2.4</A>, which in turn is a port from the <A
    ...
    /**
     * Evaluates the given polynomial of degree <tt>N</tt> at <tt>x</tt>. Evaluates polynomial when
     * coefficient of N is 1.0. Otherwise same as <tt>polevl()</tt>.
     *
     * <pre>
     *                     2          N
     * y  =  C  + C x + C x  +...+ C x
     ...
     * </pre>
     *
     * The function <tt>p1evl()</tt> assumes that <tt>coef[N] = 1.0</tt> and is omitted from the
     * array. Its calling arguments are otherwise the same as <tt>polevl()</tt>.
     *
     * <p>In the interest of speed, there are no checks for out of bounds arithmetic.
     *
    ...
    /**
     * Returns the Gamma function computed by Stirling's formula. The polynomial STIR is valid for
     * {@literal 33 <= x <= 172}.
     */
