...
/**
 * FixedLengthList is an extension of an ArrayList with a fixed maximum size. If an element is added
 * that would put the list over its maximum size then the oldest element in the list is removed.
 *
 * @author Richard Hugh Moulton
...
public class FixedLengthList<E> extends ArrayList<E> {
    ...
    /**
     * Calls super.add(entry) to append the entry to the end of the FixedLengthList. Removes the
     * oldest entry if required. Returns true at completion.
     *
    ...
    /**
     * Appends all of the elements in the argument collection in the order that they are returned by
     * the collection's iterator. Does so by calling FixedLengthList.add(E entry).
     */
