...
import moa.learners.clusterers.clustree.util.Budget;
import moa.learners.clusterers.clustree.util.SimpleBudget;
...
public class ClusTree extends AbstractClusterer {
	...
	public FlagOption breadthFirstStrategyOption = new FlagOption("breadthFirstStrategy", 'B',
	...
	/**
	 * This variable is used to keep the inverse height that is stored in every node
	 * correct.
	 */
	private int numRootSplits;
	/**
	 * The threshold for the weighting of an Entry. An Entry is irrelevant, if it is
	 * in a leaf and the weightedN of the data Cluster is smaller than this
	 * threshold.
	 * 
	 * @see Entry#data
	 */
	...
	public void resetLearningImpl() {
		...
		negLambda = (1.0 / horizonOption.getValue()) * (Math.log(weightThreshold) / Math.log(2));
	...
	/**
	 * Insert a new point in the <code>Tree</code>. The point should be represented
	 * as a cluster with a single data point(i.e. N = 1). A <code>Budget</code>
	 * class is also given, which is informed of the number of operation the tree
	 * does, and informs the tree when it does not have time left and should stop
	 * the insertion.
	 * 
	...
	/**
	 * insert newPoint into the tree using the BreadthFirst strategy, i.e.: insert
	 * into the closest entry in a leaf node.
	 * 
	 * @param newPoint
	 * @param budget
	 * @param timestamp
	...
	private Entry insertBreadthFirst(ClusKernel newPoint, Budget budget, long timestamp) {
		...
		// if there is space, add it to the node ( doesn't ever occur, since nodes are
		// created with 3 entries)
		if (numFreeEntries > 0) {
		...
		// if there is space/outdated cluster on path to top, split. Else merge without
		// split
		else {
	...
	/**
	 * This method checks if there is an outdated (or empty) entry on the path from
	 * node to root. It updates the weights of nodes on path and then checks if it
	 * is outdated.
	 * 
	 * @param node
	...
	/**
	 * This method calculates the distances between the new point and each Entry in
	 * a leaf node. It returns the node that contains the entry with the smallest
	...
	/**
	 * Calculates the best merge possible between two nodes in a node. This means
	 * that the pair with the smallest distance is found.
	 * 
	 ...
	 * will generate two new nodes. One of them will be put where the old node was,
	 * and for the other a new <code>Entry</code> will be generated and returned.
	 * 
	 ...
	 * @return An entry which points at the second node created in the split. This
	 *         entry has to be introduced later in the tree.
	 */
	...
	/**
	 * Return the number of time the tree has grown in size. If the tree grows and
	 * is then cutted from a certain depth, it also counts.
	 * 
	 ...
	 */
	// TODO: Microcluster unter dem Threshhold nich zur�ckgeben (WIe bei outdated
	// entries)
	@Override
...
//                    }
	...
	class BestMergeInNode {
		...
		/**
		 * The constructor of this return value. It will automatically make sure that
		 * the first position is the smaller one of the two.
		 * 
