...
public class Node implements Serializable {
	 ...
	 * @param fakeRoot         A parameter the says if the node is to be fake or
	 *                         not.
	 */
	...
	/**
	 * Checks if this node is a leaf. A node is a leaf when none of the entries in
	 * the node have children.
	 * 
	 ...
	 * @see Kernel
	 * @see Entry#calcDistance(moa.learners.clusterers.tree.Kernel)
	 */
	 ...
	 * <code>calcDistance(Entry)</code> function is find the one with the shortest
	 * distance in this node to the given one.
	 * 
	 * @param newEntry The entry to which the entry with the minimal distance to it
	 *                 is calculated.
	...
	/**
	 * If there exists an entry, whose relevance is under the threshold given as a
	 * parameter to the tree, this entry is returned. Otherwise <code>null</code> is
	 * returned.
	 * 
	...
	/**
	 * Return an array with references to the children of this node. These are not
	 * copies, that means side effects are possible!
	 * 
	...
	/**
	 * Return the level number in the node. This is not the real level. For the real
	 * level one has to call <code>getLevel(Tree tree)</code>.
	 * 
	 * @return The raw level of the node.
	 * @see #getLevel(moa.learners.clusterers.LiarTree.Tree)
	 */
	...
	/**
	 * Returns the level at which this node is in the tree. If a tree is passed to
	 * which the node does not belonged a value is returned, but it is gibberish.
	 * 
	 ...
	 * <code>shallowClear</code> is called upon all the entries of the node, that
	 * the split counter is set to zero and the node is set to not be a fake root.
	 * Notice that the level stays the same after calling this function.
	 * 
	 * @see Kernel#clear()
	...
	/**
	 * Merge the two entries at the given position. The entries are reordered in the
	 * <code>entries</code> array so that the non-empty entries are still at the
	 * beginning.
	 * 
	 * @param pos1 The position of the first entry to be merged. This position has
	 *             to be smaller than the the second position.
	 * @param pos2 The position of the second entry to be merged. This position has
	 *             to be greater than the the first position.
	 */
