...
public class Entry implements Serializable {
	...
	/**
	 * A reference to the next node in the tree. <code>null</code> if we are at a
	 * leaf, or this is an entry is part of a lying <code>Node</code>.
	 */
	private Node child;
	/**
	...
	/**
	 * Constructor for the entry. To be used when we want to create an empty entry.
	 * Notice that the timestamp will be set to zero, since there is no reason to
	 * know when an empty entry was generated.
	 * 
	 * @param numberDimensions The dimensionality of the data point in tree where
	 *                         this entry is used.
	 */
	...
	/**
	 * Constructor that creates an <code>Entry</code> that points to the given node.
	 * The values of <code>data</code> will be calculated for this.
	 * 
	 * @param numberDimensions The dimensionality of the node.
	 * @param node             The node to which the new <code>Entry</code> should
	 *                         point.
	 * @param currentTime      The timestamp for the moment where this Entry was was
	 *                         generated.
	 * @see Node
	 * @see #data
	 */
	...
	/**
	 * Constructuctor that creates an <code>Entry</code> with an empty buffer and
	 * the <code>data</code> given by the <code>Kernel</code>.
	 * 
	 * @param numberDimensions The dimensionality of the information in the cluster.
	 * @param cluster          The cluster from which the information is to be
	 *                         extracted.
	 * @param currentTime      The timestamp for the moment where this Entry was was
	 *                         generated.
	 * @see Kernel
	...
	/**
	 * Clear the Entry. All points in the buffer and in the data cluster are lost,
	 * the connection to the child is lost and the timestamp is set to the default
	 * value.
	 */
	...
	/**
	 * Clear the <code>data</code> and the <code>buffer Custer</code> in this entry.
	 * This function does not clear the child of this <code>Entry</code>.
	 * 
	 * @see #data
	 * @see #buffer
	 * @see Kernel
	...
	/**
	 * Calculates the distance to the data in this entry of the data in the given
	 * entry.
	 * 
	...
	/**
	 * When this entry is empty, give it it's first values. It makes sense to have
	 * this operation separated from the aggregation, because the aggregation first
	 * weights the values in <code>data</code> and <code>Kernel</code>, which makes
	 * no sense in an empty entry.
	 * 
	...
	/**
	 * Add the data cluster of another entry to the data cluster of this entry. By
	 * using this function the timestamp does not get updated, nor does this entry
	 * get older.
	 * 
	 * @param other The entry of which the data cluster should be added to the local
	 *              data cluster.
	 * @see #data
	...
	/**
	 * Aggregate the given <code>Kernel</code> to the <code>data</code> cluster of
	 * this entry.
	 * 
	...
	/**
	 * Aggregate the given <code>Kernel</code> to the <code>buffer</code> cluster of
	 * this entry.
	 * 
	...
	/**
	 * Return the reference to the child of this <code>Entry</code> to navigate in
	 * the tree.
	 * 
	 ...
	 * possible (given that the copy constructor of <code>Kernel</code> makes a deep
	 * copy).
	 * 
	...
	/**
	 * Check if this <code>Entry</code> is empty or not. An <code>Entry</code> is
	 * empty if the <code>data Kernel</code> is empty, since then the buffer has to
	 * be empty.
	 * 
	 ...
	 * weight of an entry becomes to small, and we want to forget the information of
	 * the old points.
	 * 
	 ...
	 * This is done by comparing the weighted N of this Entry to the threshold, if
	 * it is smaller, than the entry is deemed to be irrelevant.
	 * 
	...
	/**
	 * Ages this entrie's data AND buffer according to the given time and aging
	 * constant.
	 * 
