 ...
 *
 * Citation: D. Fisher (1987). Knowledge acquisition via incremental conceptual
 * clustering. Machine Learning. 2(2):139-172.
 **/
public class CobWeb extends AbstractClusterer {

	private static final long serialVersionUID = 1L;
	public FloatOption acuityOption = new FloatOption("acuity", 'a', "Acuity (minimum standard deviation)", 1.0, 0.0,
			90.0);
	public FloatOption cutoffOption = new FloatOption("cutoff", 'c', "Cutoff (minimum category utility)", 0.002, 0.0,
...
//            "Seed for random noise.", 1);	//42
	...
	private class CNode implements Serializable {
		...
		private double cuScoreForBestTwoMerged(CNode merged, CNode a, CNode b, Instance newInstance) {// throws
																										// Exception {
			...
			for (int i = 0; i < m_numAttributes; i++) {
				if (!updateInstance.isMissing(i)) {
					...
					if (m_clusterInstances.attribute(i).isNominal()) {
						...
						m_attStats[i].totalCount += (delete) ? (-1.0 * updateInstance.weight())
								: updateInstance.weight();
					} else {
						if (delete) {
		...
		protected void computeTreeClustering(int depth, Clustering clustering) {
			...
			if (m_children == null) {
				// Append Cluster
				/*
				 * text.append("\n"); for (int j = 0; j < depth; j++) { text.append("|   "); }
				 * text.append("leaf " + m_clusterNum + " [" + m_clusterInstances.numInstances()
				 * + "]"); clustering.add(SphereCluster(this.coordinates, .05,
				 * m_clusterInstances.numInstances()));
				 */
				if (depth == 0) {
