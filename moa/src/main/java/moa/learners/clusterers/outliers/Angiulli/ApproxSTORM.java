...
package moa.learners.clusterers.outliers.Angiulli;
...
import moa.learners.clusterers.outliers.Angiulli.ISBIndex.ISBNode;
import moa.learners.clusterers.outliers.Angiulli.ISBIndex.ISBSearchResult;
...
public class ApproxSTORM extends STORMBase {
	...
	void UpdateNodeType(ISBNodeAppr node) {
		...
		prec_neighs = node.fract_before * Math.abs((node.id + m_WindowSize) - GetWindowEnd());
		if (bTrace)
	...
	void UpdateNodeStatistics(ISBNodeAppr node) {
		...
		double prec_neighs = node.fract_before * Math.abs((node.id + m_WindowSize) - GetWindowEnd());
		if (succ_neighs + prec_neighs < m_k) {
