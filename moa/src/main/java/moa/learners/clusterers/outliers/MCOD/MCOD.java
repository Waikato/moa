...
package moa.learners.clusterers.outliers.MCOD;
...
import moa.learners.clusterers.outliers.MCOD.ISBIndex.ISBNode;
import moa.learners.clusterers.outliers.MCOD.ISBIndex.ISBSearchResult;
import moa.learners.clusterers.outliers.MCOD.ISBIndex.ISBNode.NodeType;
...
public class MCOD extends MCODBase {
	...
	void ProcessNewNode(ISBNode nodeNew, boolean bNewNode) {
		...
		if (bFoundMC) {
			...
			// create helper sets for micro-cluster management
			ArrayList<ISBNode> setNC = new ArrayList<>();
			ArrayList<ISBNode> setNNC = new ArrayList<>();
	...
	void AddToEventQueue(ISBNode x, ISBNode nodeMinExp) {
		...
		if (nodeMinExp != null) {
			...
			if (bWarning)
				Println("AddToEventQueue: Cannot add x.id: " + x.id
						+ " to event queue (nn_before is empty, count_after=" + x.count_after + ")");
		}
	}
