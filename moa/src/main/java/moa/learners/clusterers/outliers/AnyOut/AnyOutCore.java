...
package moa.learners.clusterers.outliers.AnyOut;
...
import moa.learners.clusterers.clustree.ClusTree;
import moa.learners.clusterers.clustree.Entry;
import moa.learners.clusterers.clustree.Node;
import moa.learners.clusterers.outliers.AnyOut.util.DataObject;
import moa.learners.clusterers.outliers.AnyOut.util.DataSet;
...
public class AnyOutCore extends ClusTree {
	...
	// the lists have then to be used by the methods from MultipleDetector (see
	/////////////////////////////////////////////// below)
	...
	public IntOption trainingSetSizeOption = new IntOption("TrainingSetSize", 't', "Training Set Size.", 1000, 0,
			10000);
	// public FlagOption UseBulkLoadingOption = new FlagOption("UseBulkLoading",
	// 'b', "Use Bulkloading or traditional learning.");
	...
	public void train(DataSet trainingSet) {
		...
		// ClusTree private variables are not updated but are mandatory for the
...
//		}
	...
	private void useAggregatedOScoreResults(int objectId) {
		if (oScoreK <= 1) {
			...
			for (int i = Math.max(0,
					previousOScoreResultList.get(objectId).size() - (oScoreK - 1)); i < previousOScoreResultList
							.get(objectId).size(); i++) {
	...
	/**
	 * Calculates the Confidence on the basis of the standard deviation of previous
	 * OScore results
	 * 
	 * @return confidence
	 */
	private double calcC1(int objectId) {
		...
		int count = 1;
		double difSum_k = Math.abs(
				lastOScoreResult.get(objectId) - previousOScoreResultList.get(objectId).get(nrOfPreviousResults - 1));

		// if previousOscoreResultList contains more than two results, this loop sums up
		// further diffs
		for (int i = Math.max(0, nrOfPreviousResults - (confK - 1)) + 1; i < nrOfPreviousResults; i++) {
			difSum_k += Math.abs(
					previousOScoreResultList.get(objectId).get(i) - previousOScoreResultList.get(objectId).get(i - 1));
			count++;
		}
		// hier msste gelten count==confK-1, d.h. wenn ich die letzten 3 Werte
		// betrachten will, bekomme ich 2 differenzen
		// XXX SW: Nicht ganz. Wenn ich die letzten 4 Werte betrachten will, aber erst 2
		// Ergebnisse zur Verf�gung stehen, bekomme ich an anstatt der 3 Differenzen nur
		// 1
