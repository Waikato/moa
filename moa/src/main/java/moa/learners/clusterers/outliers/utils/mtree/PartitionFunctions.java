...
public final class PartitionFunctions {
	...
	/**
	 * A {@linkplain PartitionFunction partition function} that tries to distribute
	 * the data objects equally between the promoted data objects, associating to
	 * each promoted data objects the nearest data objects.
	 *
	...
	public static class BalancedPartition<DATA> implements PartitionFunction<DATA> {
		 ...
		 */
		@Override
		public Pair<Set<DATA>> process(final Pair<DATA> promoted, Set<DATA> dataSet,
			...
			Pair<Set<DATA>> partitions = new Pair<>(new HashSet<DATA>(), new HashSet<DATA>());
