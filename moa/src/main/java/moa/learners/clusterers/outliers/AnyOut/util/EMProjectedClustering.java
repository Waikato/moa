...
/**
 * Implements clustering via Expectation Maximization but return a clear
 * partitioning of the data, i.e. a point belongs to only one cluster (the most
 * probable)
 * 
 * @author kranen
 *
...
public class EMProjectedClustering {
	...
	// stop criterion: deviation of expectation as minDeviation = 1 - (E_old /
	// E_new)
	...
	/**
	 * Performs an EM clustering on the provided data set !! Only the variances are
	 * calculated and used for point assignments ! !!! the number k' of returned
	 * clusters might be smaller than k !!!
	 * 
	 * @param pointArray the data set as an array[n][d] of n points with d
	 *                   dimensions
	 * @param k          the number of requested partitions (!might return less)
	 * @return a mapping int[n][k'] of the n given points to the k' resulting
	 *         clusters
	 */
	...
	private void setInitialPartitions(double[][] pointArray, int k) {
		...
		// TODO: choose more stable initialization (comment: this is run several times
		// (with different random seeds), i.e. the worst case should not occur)
		Random random = new Random(randomSeed);
		TreeSet<Integer> usedPoints = new TreeSet<>();
		int nextIndex;
		for (int i = 0; i < k; i++) {
	...
	private int[][] createProjectedClustering() {
		TreeSet<Integer> nonEmptyClusters = new TreeSet<>();
		...
		// copy the member lists to an array while removing the empty lists (i.e. empty
		// clusters)
