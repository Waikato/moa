...
/**
 * Grid Clusters are defined in Definition 3.6 of Chen and Tu 2007 as: Let G
 * =(g1, ·· · ,gm) be a grid group, if every inside grid of G is a dense grid
 * and every outside grid is either a dense grid or a transitional grid, then G
 * is a grid cluster.
 *
...
public class GridCluster extends NonConvexCluster {
	...
	public GridCluster(CFCluster cluster, List<CFCluster> microclusters, HashMap<DensityGrid, Boolean> hashMap,
			int label) {
		super(cluster, microclusters);
		this.grids = new HashMap<>();
	...
	public void absorbCluster(GridCluster gridClus) {
		...
		// System.out.println("Absorb cluster "+gridClus.getClusterLabel()+" into
		// cluster "+this.getClusterLabel()+".");
	...
	/**
	 * Inside Grids are defined in Definition 3.5 of Chen and Tu 2007 as: Consider a
	 * grid group G and a grid g ∈ G, suppose g =(j1, ··· ,jd), if g has neighboring
	 * grids in every dimension i =1, ·· · ,d, then g is an inside grid in
	 * G.Otherwise g is an outside grid in G.
	 *
	 ...
	 * Inside Grids are defined in Definition 3.5 of Chen and Tu 2007 as: Consider a
	 * grid group G and a grid g ∈ G, suppose g =(j1, ··· ,jd), if g has neighboring
	 * grids in every dimension i =1, ·· · ,d, then g is an inside grid in G.
	...
	public boolean isConnected() {
		...
		if (initIter.hasNext()) {
			...
			do {
				...
				HashMap<DensityGrid, Boolean> toAdd = new HashMap<>();
		...
		if (this.visited.size() == this.grids.size()) {
			// System.out.println("The cluster is still connected. "+this.visited.size()+"
			// of "+this.grids.size()+" reached.");
			return true;
		} else {
			// System.out.println("The cluster is no longer connected.
			// "+this.visited.size()+" of "+this.grids.size()+" reached.");
			return false;
		}
	}
