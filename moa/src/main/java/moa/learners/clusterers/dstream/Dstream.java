...
public class Dstream extends AbstractClusterer {
	...
	public FloatOption decayFactorOption = new FloatOption("decayFactor", 'd', "The decay factor, lambda, in (0,1)",
			0.998, 0.001, 0.999);

	public FloatOption cmOption = new FloatOption("Cm", 'm', "Controls the " + "threshold for dense grids, > 1", 3.0,
			1.001, Double.MAX_VALUE);

	public FloatOption clOption = new FloatOption("Cl", 'l', "Controls the " + "threshold for sparse grids, in (0,1)",
			0.8, 0.001, 0.999);

	public FloatOption betaOption = new FloatOption("Beta", 'b',
			"Adjusts the " + "window of protection for renaming previously deleted grids as " + "sporadic, > 0", 0.3,
			0.001, Double.MAX_VALUE);
	...
	/**
	 * Density threshold for dense grids; controlled by cm; given in eq 8 of Chen
	 * and Tu 2007
	 *
	 * @see cm
	 */
	...
	/**
	 * Density threshold for sparse grids; controlled by cl; given in eq 9 of Chen
	 * and Tu 2007
	 *
	 * @see cl
	 */
	...
	/**
	 * The number of dimensions in the data stream; defined in section 3.1 of Chen
	 * and Tu 2007
	 */
	private int d;
	...
	/**
	 * A list of all density grids which are being monitored; given in figure 1 of
	 * Chen and Tu 2007
	 */
	private HashMap<DensityGrid, CharacteristicVector> grid_list;

	/**
	 * A list of all density grids which have been deleted; allows the recording of
	 * tm - the last time when the grid is removed from grid list as a sporadic grid
	 * (if ever).
	 */
	private HashMap<DensityGrid, Integer> deleted_grids;

	/**
	 * A list of all Grid Clusters, which are defined in Definition 3.6 of Chen and
	 * Tu 2007
	 */
	...
	/**
	 * @see moa.learners.clusterers.Clusterer#isRandomizable() D-Stream is not randomizable.
	 */
	@Override
	...
	/**
	 * @see moa.learners.clusterers.Clusterer#getVotesForInstance(com.yahoo.labs.samoa.instances.Instance)
	...
	/**
	 * @see moa.learners.clusterers.Clusterer#getClusteringResult()
	 */
	@Override
	...
	/**
	 * @see moa.learners.clusterers.AbstractClusterer#resetLearningImpl()
	 */
	@Override
	...
	/**
	 * @see moa.learners.clusterers.AbstractClusterer#trainOnInstanceImpl(com.yahoo.labs.samoa.instances.Instance)
	 *
	 *      trainOnInstanceImpl implements the procedure given in Figure 1 of Chen
	 *      and Tu 2007
	 */
	@Override
	public void trainOnInstanceImpl(Instance inst) {
		...
		if (recalculateN) {
			...
			// Calculate the value for gap using the method defined in eq 26 of Chen and Tu
			// 2007
			double optionA = this.cl / this.cm;
			double optionB = (this.N - this.cm) / (this.N - this.cl);
			gap = (int) Math.floor(Math.log(Math.max(optionA, optionB)) / Math.log(this.getDecayFactor()));
			// Ensure that gap is not zero (i.e. if the procedure to calculate gap rounds
			// down to zero, then set gap to 1 and adjust clustering every instance)
			if (gap == 0) {
				gap = 1;
			}
		...
		if (!this.grid_list.containsKey(dg)) {
			...
			if (this.deleted_grids.containsKey(dg)) {
				...
				cv = new CharacteristicVector(this.getCurrTime(), this.deleted_grids.get(dg), 1.0, -1, false,
						this.getDL(), this.getDM());
	...
	/**
	 * @see moa.learners.clusterers.AbstractClusterer#getModelMeasurementsImpl()
	 */
	@Override
	...
	/**
	 * @see moa.learners.clusterers.AbstractClusterer#getModelDescription(java.lang.StringBuilder,
	 *      int)
	 */
	@Override
	 ...
	 * <li>If h belongs to c', label c and c' with the label of the largest
	 * cluster</li>
	...
	private boolean adjustLabels() {
		...
		while (gridClusIter.hasNext()) {
			...
			// System.out.print("Adjusting from cluster "+c.getClusterLabel()+",
			// standby...");
			...
			for (Map.Entry<DensityGrid, Boolean> grid : c.getGrids().entrySet()) {
				...
				// System.out.print(" Inspecting density grid, dg:"+dg.toString()+",
				// standby...");
						...
						if (this.grid_list.containsKey(dgprime)) {
							...
							if (class1 != class2) {
								...
								if (class2 != NO_CLASS) {
									...
									if (this.cluster_list.get(class1).getWeight() < this.cluster_list.get(class2)
											.getWeight())
	 ...
	 *
	 * @see moa.learners.clusterers.dstream.Dstream.gap
	 */
	...
	private boolean inspectChangedGrids() {
		HashMap<DensityGrid, CharacteristicVector> glNew = new HashMap<>();
		...
		// If there are grids in glNew, update the corresponding grids in grid_list and
		// clean up the cluster list
		if (!glNew.isEmpty()) {
			// System.out.println("There are "+glNew.size()+" entries to update from glNew
			// to grid_list.");
	 ...
	 */
	private HashMap<DensityGrid, CharacteristicVector> adjustForSparseGrid(DensityGrid dg, CharacteristicVector cv,
			int dgClass) {
		HashMap<DensityGrid, CharacteristicVector> glNew = new HashMap<>();
		// System.out.print("Density grid "+dg.toString()+" is adjusted as a sparse grid
		// at time "+this.getCurrTime()+". ");
		if (dgClass != NO_CLASS) {
	...
	/**
	 * Reclusters a gridcluster into two (or more) constituent clusters when it has
	...
	private HashMap<DensityGrid, CharacteristicVector> adjustNewLabels(
		...
		HashMap<DensityGrid, CharacteristicVector> glAdjusted = new HashMap<>();
	 ...
	 *         for update after this iteration
	 */
	private HashMap<DensityGrid, CharacteristicVector> adjustForDenseGrid(DensityGrid dg, CharacteristicVector cv,
			int dgClass) {
		// System.out.print("Density grid "+dg.toString()+" is adjusted as a dense grid
		// at time "+this.getCurrTime()+". ");

		// Among all neighbours of dg, find the grid h whose cluster ch has the largest
		// size
		...
		if (hChosenClass != NO_CLASS && hChosenClass != dgClass) {
			...
			// Else if h is a transitional grid
				...
				// If dg is labelled as no class and if h is an outside grid if dg is added to
				// ch
				...
				// Else if dg is in cluster c and |c| >= |ch|
					...
					if (gSize >= hChosenSize) {
						...
						// System.out.println("dgClass is "+dgClass+", hChosenClass is "+hChosenClass+",
						// gSize is "+gSize+" and hChosenSize is "+hChosenSize+" h is added to cluster
						// "+dgClass+".");
						this.cluster_list.set(hChosenClass, ch);
		...
		// If dgClass is dense and not in a cluster, and none if its neighbours are in a
		// cluster,
		// put it in its own new cluster and search the neighbourhood for transitional
		// or dense
		// grids to add
		else if (dgClass == NO_CLASS) {
			...
			// Iterate through the neighbourhood until no more transitional neighbours can
			// be added
	 ...
	 *         for update after this iteration
	 */
	private HashMap<DensityGrid, CharacteristicVector> adjustForTransitionalGrid(DensityGrid dg,
			CharacteristicVector cv, int dgClass) {
		// System.out.print("Density grid "+dg.toString()+" is adjusted as a
		// transitional grid at time "+this.getCurrTime()+". ");
	...
	private void cleanClusters() {
		...
		ArrayList<GridCluster> toRem = new ArrayList<>();
	...
	private void removeSporadic() {
		...
		Iterator<Map.Entry<DensityGrid, CharacteristicVector>> glIter = this.grid_list.entrySet().iterator();
		HashMap<DensityGrid, CharacteristicVector> newGL = new HashMap<>();
		ArrayList<DensityGrid> remGL = new ArrayList<>();
	 ...
	 * Chen and Tu 2007
	 *
	 * @param cv - the CharacteristicVector of the density grid being assessed for
	 *           sporadicity
	 */
	private boolean checkIfSporadic(CharacteristicVector cv) {
		// Check S1
		if (cv.getCurrGridDensity(this.getCurrTime(), this.getDecayFactor()) < densityThresholdFunction(
	 ...
	 *
	 * @param tg          - the update time in the density grid's characteristic
	 *                    vector
	 * @param cl          - user defined parameter which controls the threshold for
	 *                    sparse grids
	 * @param decayFactor - user defined parameter which is represented as lambda in
	 *                    Chen and Tu 2007
	 * @param N           - the number of density grids, defined after eq 2 in Chen
	 *                    and Tu 2007
	 */
	...
	/**
	 * Reassign all grids belonging in the small cluster to the big cluster Merge
	 * the GridCluster objects representing each cluster
	 *
	...
	/**
	 * @return dm - the density threshold for dense grids. It is controlled by cl
	 *         and given in eq 8 of Chen and Tu 2007
	 */
	public double getDM() {
		return this.dm;
	}
	...
	/**
	 * @return dl - the density threshold for sparse grids. It is controlled by cl
	 *         and given in eq 9 of Chen and Tu 2007
	 */
	public double getDL() {
		return this.dl;
	}
	...
	/**
	 * Prints out the values of the parameters associated with this instance of the
	 * D-Stream algorithm:
	 * <ol>
	 * <li>gap;</li>
	...
	/**
	 * Iterates through grid_list and prints out each density grid therein as a
	 * string.
	 *
	 * @see moa.learners.clusterers.dstream.Dstream.grid_list
	 * @see moa.learners.clusterers.dstream.DensityGrid.toString
	 */
	...
	/**
	 * Iterates through cluster_list and prints out each grid cluster therein as a
	 * string.
	 *
	 * @see moa.learners.clusterers.dstream.Dstream.cluster_list
	 * @see moa.learners.clusterers.dstream.GridCluster.toString
	 */
