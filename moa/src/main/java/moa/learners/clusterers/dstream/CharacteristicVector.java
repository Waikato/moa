...
/**
 * The Characteristic Vector of a density grid is defined in Definition 3.2 of
 * Chen and Tu 2007 as: The characteristic vector of a grid g is a tuple
 * (tg,tm,D, label,status), where tg is the last time when g is updated, tm is
 * the last time when g is removed from grid list as a sporadic grid (if ever),
 * D is the grid density at the last update, label is the class label of the
 * grid, and status = {SPORADIC, NORMAL} is a label used for removing sporadic
 * grids.
 *
...
public class CharacteristicVector {
	...
	/**
	 * t_m in the characteristic vector tuple; the last time when g is removed from
	 * grid_list as a sporadic grid (if ever).
	 */
	...
	/**
	 * attribute mentioned in figure 4, line 3 of Chen and Tu 2007; attribute =
	 ...
	 * and adjust clusterings)
	 */
	private int densityTimeStamp;

	/**
	 * Flag marking whether there was a change in the attribute field the last time
	 * the grid density was updated.
	 */
	...
	/**
	 * @param isSporadic TRUE if the characteristic vector is to be labelled as
	 *                   sporadic, FALSE otherwise
	 */
	...
	/**
	 * Implements the density update function given in eq 5 (Proposition 3.1) of
	 * Chen and Tu 2007.
	 *
	...
	public void densityWithNew(int currTime, double decayFactor) {
		...
		// System.out.print("["+decayFactor+"^("+currTime+" -
		// "+this.getDensityTimeStamp()+") * "+densityOfG+"] + 1.0 = ");
	...
	/**
	 * Implements the update the density of all grids step given at line 2 of both
	 * Fig 3 and Fig 4 of Chen and Tu 2007.
	 *
	 * @param currTime    the data stream's current internal time
	 * @param decayFactor the value of lambda
	 * @param dl          the threshold for sparse grids
	 * @param dm          the threshold for dense grids
	 * @param addRecord   TRUE if a record has been added to the density grid, FALSE
	 *                    otherwise
	 */
	public void updateGridDensity(int currTime, double decayFactor, double dl, double dm) {
		...
		// Evaluate whether or not the density grid attribute has changed and set the
	...
	/**
	 * Implements the test for whether a density grid is dense given in eq 8 of Chen
	 * and Tu 2007.
	 *
	 * @param dm the threshold for dense grids
	 */
	...
	/**
	 * Implements the test for whether a density grid is sparse given in eq 9 of
	 * Chen and Tu 2007.
	 *
	 * @param dl the threshold for sparse grids
	 */
	...
	/**
	 * Implements the test for whether a density grid is transitional given in eq 10
	 * of Chen and Tu 2007.
	 *
	 * @param dm the threshold for dense grids
	 * @param dl the threshold for sparse grids
