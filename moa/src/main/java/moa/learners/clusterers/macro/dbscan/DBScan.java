...
package moa.learners.clusterers.macro.dbscan;
...
import moa.learners.clusterers.macro.AbstractMacroClusterer;
...
public class DBScan extends AbstractMacroClusterer {
	...
	private ArrayList<DenseMicroCluster> expandCluster(DenseMicroCluster dmc, List<DenseMicroCluster> neighbours,
			ArrayList<DenseMicroCluster> arrayList, Vector<DenseMicroCluster> dbmc) {
	...
	private List<DenseMicroCluster> getNeighbourhood(DenseMicroCluster mc, Vector<DenseMicroCluster> dbmc) {
		List<DenseMicroCluster> res = new Vector<>();
	...
	public Clustering getClustering(Clustering microClusters) {
		if (microClusters != null && microClusters.size() != 0) {
			Vector<DenseMicroCluster> dbmc = new Vector<>();
			...
			ArrayList<ArrayList<DenseMicroCluster>> clusters = new ArrayList<>();
			...
			for (DenseMicroCluster dmc : dbmc) {
				if (!dmc.isVisited()) {
					...
					if (neighbours.size() >= mMinPts) {
						ArrayList<DenseMicroCluster> cluster = expandCluster(dmc, neighbours,
			...
			for (ArrayList<DenseMicroCluster> cluster : clusters) {
				if (cluster.size() != 0) {
					CFCluster temp = new NonConvexCluster(cluster.get(0).getCFCluster(),
	...
	private List<CFCluster> Convert2microclusterList(ArrayList<DenseMicroCluster> cluster) {
		List<CFCluster> cfCluster = new Vector<>();
