...
public class BucketManager {
	...
	/**
	 * initializes a bucketmanager for n points with bucketsize maxsize and
	 * dimension d
	 **/
	...
	void insertPoint(Point p) {
		...
		if (cursize >= this.maxBucketsize) {
			...
			if (this.buckets[nextbucket].cursize == 0) {
				...
				nextbucket++;
				/*
				 * as long as the next bucket is full output the coreset to the spillover of the
				 * next bucket
				 */
	 ...
	 * coreset of all nonempty buckets
	 * 
	 * Case 1: the last bucket is full => n is a power of 2 and we return the
	 * contents of the last bucket
	 * 
	 * Case2: the last bucket is not full => we compute a coreset of all nonempty
	 * buckets
	 * 
