 ...
 *
 */
package moa.learners.clusterers.kmeanspm;
 ...
 *
 * Citation: Hendrik Fichtenberger, Marc Gillé, Melanie Schmidt, Chris
 * Schwiegelshohn, Christian Sohler: BICO: BIRCH Meets Coresets for k-Means
 * Clustering. ESA 2013: 481-492 (2013) http://ls2-www.cs.tu-dortmund.de/bico/
 *
 */
public class BICO extends AbstractClusterer {
	...
	public IntOption numClustersOption = new IntOption("Cluster", 'k', "Number of desired centers.", 5, 1,
			Integer.MAX_VALUE);

	public IntOption numDimensionsOption = new IntOption("Dimensions", 'd',
			"Number of the dimensions of the input points.", 10, 1, Integer.MAX_VALUE);

	public IntOption maxNumClusterFeaturesOption = new IntOption("MaxClusterFeatures", 'n',
			"Maximum size of the coreset.", 5 * 200, 1, Integer.MAX_VALUE);

	public IntOption numProjectionsOption = new IntOption("Projections", 'p',
			"Number of random projections used for the nearest neighbour search.", 10, 1, Integer.MAX_VALUE);
	 ...
	 * (non-Javadoc)
	 *
	 * @see moa.learners.clusterers.Clusterer#isRandomizable()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see moa.learners.clusterers.AbstractClusterer#implementsMicroClusterer()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see moa.learners.clusterers.AbstractClusterer#getMicroClusteringResult()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 *
	 * @see moa.learners.clusterers.Clusterer#getPredictionForInstance(weka.core.Instance)
	 */
	@Override
	...
	public Clustering getClusteringResult() {
		...
		for (int i = 0; i < 5; i++) {
			...
			List<double[]> newResult = CoresetKMeans.generatekMeansPlusPlusCentroids(this.numClusters, microClustering,
		...
		AutoExpandVector<Cluster> resultClustering = new AutoExpandVector<>(result.size());
	...
	public void resetLearningImpl() {
		...
		this.root = new ClusteringTreeHeadNode(null, new ClusteringFeature(new double[0], 1),
				this.numDimensionsOption.getValue(), this.numProjectionsOption.getValue(), Math.min(hashSize, 30),
				this.clustererRandom);
		this.rootCount = 0;
	}
	 ...
	 * (non-Javadoc)
	 *
	 * @see moa.learners.clusterers.AbstractClusterer#trainOnInstanceImpl(weka.core.Instance)
	 */
	@Override
	public void trainOnInstanceImpl(Instance inst) {
		...
		if (this.numDimensions != x.length) {
			System.out.println(
					"Line skipped because line dimension is " + x.length + " instead of " + this.numDimensions);
			return;
		}
	...
	/**
	 * If the number of ClusteringTreeNodes exceeds the maximum bound, the global
	 * threshold T will be doubled and the tree will be rebuild with the new
	 * threshold.
	 *
	 */
	protected void rebuild() {
		...
		while (this.rootCount > this.maxNumClusterFeatures) {
			...
			Queue<ClusteringTreeNode> Q = new LinkedList<>();
	 ...
	 *
	 * @param x the ClusteringTreeNode
	 */
	...
	/**
	 * Calculates the squared threshold at a specific level in the ClusteringFeature
	 * tree.
	 *
	 ...
	 *
	 * @see
	 * moa.learners.clusterers.AbstractClusterer#getModelDescription(java.lang.StringBuilder,
	 * int)
	 */
	@Override
