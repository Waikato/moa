...
public class ClusteringTreeHeadNode extends ClusteringTreeNode {
	...
	/**
	 * Creates a ClusteringTreeNode with an extended nearest neighbor search in the
	 * root.
	 *
	 * @param dimension      the number of dimensions of the points
	 * @param numProjections the number of projections to use for the nearest
	 *                       neighbor search
	 * @param hashSize       size of the hash function (must be smaller than 31)
	 * @param random         instance to generate a stream of pseudorandom numbers
	 */
	public ClusteringTreeHeadNode(double[] center, ClusteringFeature cf, int dimension, int numProjections,
			int hashSize, Random random) {
		super(center, cf);
		assert (hashSize < 31);
		this.numProjections = numProjections;

		this.projections = new ArrayList<>(numProjections);
		...
		this.buckets = new ArrayList<>(numProjections);
		for (int i = 0; i < numProjections; i++) {
			this.buckets.add(new CuckooHashing<List<ClusteringTreeNode>>(hashSize, random));
		}
	}
	 ...
	 *
	 * @param pointA the point to project
	 ...
	 *
	 * @param val position of the point
	...
	public ClusteringTreeNode nearestChild(double[] pointA) {
		...
		// for (int i = -1; i <= 1; i++) {
		List<ClusteringTreeNode> currentBucket = this.buckets.get(minBucketProjection).get(bucketNumber);// + i);
