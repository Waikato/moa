 ...
 *
 * Citation: Rasmus Pagh, Flemming Friche Rodler: Cuckoo Hashing. ESA 2001:
 * 121-133
 *
 * @param <T> class of stored elements
 */
public class CuckooHashing<T> {
	 ...
	 *
	 * @param startHashSize  size of the hash function at the beginning (must be
	 *                       smaller than 31)
	 * @param maxStashSize   maximum size of the stash
	 * @param startNumTables number of tables for Cuckoo hashing
	...
	public CuckooHashing(int startHashSize, int maxStashSize, int startNumTables, Random random) {
		...
		this.hashfunctions = new ArrayList<>(startNumTables);
		this.elements = new ArrayList<>(sizeTables);
		this.numElements = 0;
		this.tables = new ArrayList<>(startNumTables);
		...
		for (int i = 0; i < startNumTables; i++) {
			...
			List<Entry<T>> table = new ArrayList<>(sizeTables);
	 ...
	 * Creates a new hash table based on Cuckoo Hashing.
	 *
	 * @param hashSize size of the hash function
	 * @param random   instance to generate a stream of pseudorandom numbers
	 */
	...
	public void put(long key, T element) {
		Entry<T> entry = new Entry<>(key, element);
		elements.add(entry);
		this.numElements++;
	 ...
	 *
	 * @param currentElement entry to add in a table
	 * @param rehash         if <code>true</code> the hash table will be rebuild
	 *                       when the stash became too big.
	 */
	 ...
	 *
	 * @param <V> class of stored elements
	 */
