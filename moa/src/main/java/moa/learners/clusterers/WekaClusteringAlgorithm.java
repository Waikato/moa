...
//			We do have the parameter option info, but not really a place to show it somewhere
			/*
			 * //System.out.println(clustererClasses[i].getSimpleName()); for (Class c :
			 * clustererClasses[i].getInterfaces()) { if
			 * (c.equals(weka.core.OptionHandler.class)) { try { Enumeration options =
			 * ((weka.core.OptionHandler)clustererClasses[i].newInstance()).listOptions();
			 * while(options.hasMoreElements()){ weka.core.Option o =
			 * (weka.core.Option)options.nextElement(); System.out.print(o.synopsis()+" ");
			 * }
			 * 
		...
		if (clustererClasses != null && clustererClasses.length > 0) {
			wekaAlgorithmOption = new MultiChoiceOption("clusterer", 'w', "Weka cluster algorithm to use.",
	...
	public void resetLearningImpl() {
		try {
			...
			clusterer = (weka.clusterers.AbstractClusterer) ClassOption.cliStringToObject(clistring,
					weka.clusterers.Clusterer.class, null);
	...
	public Clustering getClusteringResult() {
		...
		try {
			...
			List<Instance> newInstances = new ArrayList<>(); // Instances(dataset);
