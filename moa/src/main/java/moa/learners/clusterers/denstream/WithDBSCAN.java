...
import moa.learners.clusterers.macro.dbscan.DBScan;
...
public class WithDBSCAN extends AbstractClusterer {
	...
	public IntOption initPointsOption = new IntOption("initPoints", 'i', "Number of points to use for initialization.",
			1000);
	...
	public IntOption speedOption = new IntOption("processingSpeed", 's', "Number of incoming points per time unit.",
			100, 1, 1000);
...
//						/ (double) horizonOption.getValue();
		...
		for (int p = 0; p < initBuffer.size(); p++) {
			...
			if (!point.covered) {
				...
				if (neighbourhood.size() > minPoints) {
					MicroCluster mc = new MicroCluster(point, point.numAttributes(), timestamp, lambda,
							currentTimestamp);
	...
	public void trainOnInstanceImpl(Instance inst) {
		...
		if (!initialized) {
			...
			if (!merged) {
				o_micro_cluster.getClustering().add(new MicroCluster(point.toDoubleArray(),
						point.toDoubleArray().length, timestamp, lambda, currentTimestamp));
			}
