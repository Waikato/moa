...
public class NominalRulePredicate extends AbstractMOAObject implements Predicate {
	...
	public void getDescription(StringBuilder sb, int indent, InstancesHeader instHeader) {
		if (instHeader != null) {
			if (isEqual)
				StringUtils.appendIndented(sb, indent + 1, instHeader.inputAttribute(inputAttributeIndex).name()
						+ " == " + instHeader.inputAttribute(inputAttributeIndex).value((int) attributeValue));
			else
				StringUtils.appendIndented(sb, indent + 1, instHeader.inputAttribute(inputAttributeIndex).name()
						+ " <> " + instHeader.inputAttribute(inputAttributeIndex).value((int) attributeValue));
		} else
