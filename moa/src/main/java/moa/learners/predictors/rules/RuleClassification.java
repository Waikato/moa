...
public class RuleClassification extends AbstractMOAObject {
	...
	protected AutoExpandVector<AttributeClassObserver> observers = new AutoExpandVector<>(); // Statistics.

	protected AutoExpandVector<AttributeClassObserver> observersGauss = new AutoExpandVector<>(); // Statistics.
	...
	protected DoubleVector attributeMissingValues = new DoubleVector(); // for each attribute counts the number of
																		// missing values.
	...
	public RuleClassification(RuleClassification x) {
		for (int i = 0; i < x.predicateSet.size(); i++) {
			Predicates pred = new Predicates(x.predicateSet.get(i).getAttributeValue(),
					x.predicateSet.get(i).getSymbol(), x.predicateSet.get(i).getValue());
