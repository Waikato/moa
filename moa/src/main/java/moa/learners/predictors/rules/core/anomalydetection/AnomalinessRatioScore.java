...
import moa.learners.predictors.rules.core.anomalydetection.probabilityfunctions.ProbabilityFunction;
 ...
 * univariateAnomalyprobabilityThresholdOption - Univariate anomaly threshold
 * value minNumberInstancesOption - The minimum number of instances required to
 * perform anomaly detection probabilityFunctionOption - Probability function
 * selection
 */
...
public class AnomalinessRatioScore extends AbstractAnomalyDetector {
	...
	public FloatOption percentageAnomalousAttributesOption = new FloatOption("percentageAnomalousAttributes", 'm',
			"Percentage of anomalous attributes.", 0.5, 0.0, 1.0);
	public FloatOption univariateAnomalyprobabilityThresholdOption = new FloatOption(
			"univariateAnomalyprobabilityThreshold", 'u', "Univariate anomaly threshold value.", 0.90, 0.0, 1.0);

	public IntOption minNumberInstancesOption = new IntOption("minNumberInstances", 'n',
			"The minimum number of instances required to perform anomaly detection.", 30, 0, Integer.MAX_VALUE);

	public ClassOption probabilityFunctionOption = new ClassOption("probabilityFunction", 'p', "Probability function",
			ProbabilityFunction.class, "GaussInequality");
	...
	public boolean updateAndCheckAnomalyDetection(Instance instance) {
		...
		if (weightSeen > minInstances) {
			...
			for (int i = 0; i < instance.numInputAttributes(); i++) {
				...
				if (instance.attribute(i).isNumeric()) {
					...
					if (sd > MINSTD) {
						prob = probabilityFunction.getProbability(stats[0] / weightSeen,
								Utils.computeSD(stats[1], stats[0], weightSeen), val);
		...
		if (!isAnomaly) {
			if (sufficientStatistics == null)
				sufficientStatistics = new AutoExpandVector<>();
