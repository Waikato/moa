...
import moa.learners.predictors.SemiSupervisedMultiLabelClassifier;
import moa.learners.predictors.core.MultiLabelRule;
import moa.learners.predictors.core.MultiLabelRuleSet;
import moa.learners.predictors.core.ObserverMOAObject;
...
import moa.learners.predictors.core.featureranking.messages.ChangeDetectedMessage;
import moa.learners.predictors.core.inputselectors.InputAttributesSelector;
import moa.learners.predictors.core.inputselectors.SelectAllInputs;
import moa.learners.predictors.core.instancetransformers.NoInstanceTransformation;
import moa.learners.predictors.core.outputselectors.OutputAttributesSelector;
import moa.learners.predictors.core.outputselectors.SelectAllOutputs;
...
import moa.learners.predictors.rules.core.anomalydetection.OddsRatioScore;
import moa.options.ClassOption;
import weka.core.pmml.jaxbbindings.TrainingInstances;

/**
 * Semi-supervised method for online multi-target regression. This method uses
 * the AMRules as learner and it based on probabilistic method very similar to
 * anomaly detection. This method measures predicts the benefit of a unlabeled
 * example to the models(using only the input information) to the model. If this
 * benefit is higher than <scoreThreshold> then the algorithm predicts an output
 * and artificially labels the example and use it for training.
 * 
 * 
 * The algorithm is fully explained in the following publication:
 * 
 * Sousa R., Gama J. (2016) Online Semi-supervised Learning for Multi-target
 * Regression in Data Streams Using AMRules. In: Boström H., Knobbe A., Soares
 * C., Papapetrou P. (eds) Advances in Intelligent Data Analysis XV. IDA 2016.
 * Lecture Notes in Computer Science, vol 9897. Springer.
 * 
 * @author RSousa
 * @version $Revision: 2 $
 */

public abstract class AMRulesMultiLabelLearnerSemiSuper extends AbstractMultiLabelClassifier
		implements MultiLabelClassifier, SemiSupervisedMultiLabelClassifier {
	...
	public IntOption gracePeriodOption = new IntOption("gracePeriod", 'g',
			"Hoeffding Bound Parameter. The number of instances a leaf should observe between split attempts.", 200, 1,
			Integer.MAX_VALUE);
	...
	public ClassOption anomalyDetector = new ClassOption("anomalyDetector", 'A', "Anomaly Detector.",
			AnomalyDetector.class, OddsRatioScore.class.getName());
	...
	public ClassOption numericObserverOption = new ClassOption("numericObserver", 'y', "Numeric observer.",
			NumericStatisticsObserver.class, "MultiLabelBSTree");
	public ClassOption nominalObserverOption = new ClassOption("nominalObserver", 'z', "Nominal observer.",
			NominalStatisticsObserver.class, "MultiLabelNominalAttributeObserver");
	public IntOption VerbosityOption = new IntOption("verbosity", 'v',
			"Output Verbosity Control Level. 1 (Less) to 5 (More)", 1, 1, 5);
	public ClassOption outputSelectorOption = new ClassOption("outputSelector", 'O', "Output attributes selector",
			OutputAttributesSelector.class, SelectAllOutputs.class.getName());
	public ClassOption inputSelectorOption = new ClassOption("inputSelector", 'I', "Input attributes selector",
			InputAttributesSelector.class, SelectAllInputs.class.getName());
...
//			1,Integer.MIN_VALUE, Integer.MAX_VALUE);
	...
	public FloatOption scoreThreshold = new FloatOption("scoreThreshold", 'h', "Initial dataset (%)", 1);
	...
	/**
	 * getVotes extension of the instance method getVotesForInstance in
	 * moa.classifier.java returns the prediction of the instance.
	 */
	public ErrorWeightedVoteMultiLabel getVotes(Instance instance) {
		...
		for (MultiLabelRule rule : ruleSet) {
			...
			if (rule.isCovering(instance) == true) {
				...
				if (vote != null) { // should only happen for first instance
					...
					if (errors == null) // if errors==null, rule has seen no predictions since expansion: return maximum
										// error, since prediction is not reliable
						errors = defaultRuleErrors(vote);
					debug("Rule No" + rule.getRuleNumberID() + " Vote: " + vote.toString() + " Error: " + errors
		...
		if (!errorWeightedVote.coversAllOutputs()) {
			...
			if (vote == null) { // use default rule
				vote = new MultiTargetRegressionPrediction(instance.numOutputAttributes());
			}
			...
			if (defaultVote != null) {
				...
				Prediction fixVote = new MultiTargetRegressionPrediction(vote.numOutputAttributes());
	...
	/*
	 * Returns the estimate error for each output of a rule Should be used when
	 * rule.getCurrentErrors() returns null
	 */
	...
	/**
	 * AMRules Algorithm. Method for updating (training) the AMRules model using a
	 * new instance
	 */
	...
	public void trainOnInstanceImpl(Instance instance) {
		...
		debug("Train", 3);
		debug("NÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âº instance "
				+ numInstances + " - " + instance.toString(), 3);
		...
		prediction = new MultiLabelClassificationPrediction(instance.numOutputAttributes());
			...
			if (rule.isCovering(instance) == true) {
				...
				if (!rule.updateAnomalyDetection(instance)) {
					...
					if (rule.updateChangeDetection(instance)) {
						debug("I) Drift Detected. Exa. : " + numInstances + " (" + rule.getWeightSeenSinceExpansion()
								+ ") Remove Rule: " + rule.getRuleNumberID(), 1);
						...
						if (rule.getWeightSeenSinceExpansion() % gracePeriodOption.getValue() == 0.0) {
							if (rule.tryToExpand(splitConfidenceOption.getValue(), tieThresholdOption.getValue())) {
								...
								MultiLabelRule otherMultiLabelRule = rule.getNewRuleFromOtherOutputs(); // Need to be
																										// outside to
																										// make sure
																										// other rules
																										// are cleaned
		...
		if (rulesCoveringInstance == false) {
			...
			if (defaultRule.getWeightSeenSinceExpansion() % this.gracePeriodOption.getValue() == 0.0) {
				...
				if (defaultRule.tryToExpand(this.splitConfidenceOption.getValue(),
						this.tieThresholdOption.getValue()) == true) {
	...
	protected void setRuleOptions(MultiLabelRule rule) {
		rule.setSplitCriterion(
				(MultiLabelSplitCriterion) ((MultiLabelSplitCriterion) getPreparedClassOption(splitCriterionOption))
						.copy());
		rule.setChangeDetector(((ChangeDetector) getPreparedClassOption(changeDetector)).copy());
		rule.setAnomalyDetector(((AnomalyDetector) getPreparedClassOption(anomalyDetector)).copy());
		rule.setNumericObserverOption(
				(NumericStatisticsObserver) ((NumericStatisticsObserver) getPreparedClassOption(numericObserverOption))
						.copy());
		rule.setNominalObserverOption(
				(NominalStatisticsObserver) ((NominalStatisticsObserver) getPreparedClassOption(nominalObserverOption))
						.copy());
		rule.setErrorMeasurer(
				(MultiLabelErrorMeasurer) ((MultiLabelErrorMeasurer) getPreparedClassOption(errorMeasurerOption))
						.copy());
		rule.setOutputAttributesSelector(
				(OutputAttributesSelector) ((OutputAttributesSelector) getPreparedClassOption(outputSelectorOption))
						.copy());
		rule.setRandomGenerator(this.classifierRandom);
		rule.setAttributesPercentage(this.attributesPercentage);
		rule.setInputAttributesSelector(
				(InputAttributesSelector) ((InputAttributesSelector) getPreparedClassOption(inputSelectorOption))
						.copy());
	}
	...
	public ErrorWeightedVoteMultiLabel newErrorWeightedVote() {
		return (ErrorWeightedVoteMultiLabel) ((ErrorWeightedVoteMultiLabel) getPreparedClassOption(weightedVoteOption))
				.copy();
	}
	...
	@Override
	public Prediction getTrainingPrediction(Instance trainInst) {
		return prediction;
	}
