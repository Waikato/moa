...
public class Perceptron extends AbstractRegressor implements Regressor {
...
//            "Seed for random behaviour of the classifier.", 1);
	...
	public void trainOnInstanceImpl(Instance inst) {
		accumulatedError = Math.abs(this.prediction(inst) - inst.classValue()) * inst.weight()
		...
		if (this.initialisePerceptron == true) {
			...
			LinkedList<Integer> numericIndices = new LinkedList<>();
		...
		// accumulatedError= Math.abs(prediction-inst.classValue()) +
		// fadingFactor*accumulatedError;
	...
	public void updateWeights(Instance inst, double learningRatio) {
		...
		if (sumWeights > numericAttributesIndex.length) { // Lasso regression
			...
			this.weightAttribute[numericAttributesIndex.length] = this.weightAttribute[numericAttributesIndex.length]
					/ sumWeights;
		}
