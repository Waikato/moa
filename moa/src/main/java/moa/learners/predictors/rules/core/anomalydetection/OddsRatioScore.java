...
import moa.learners.predictors.rules.core.anomalydetection.probabilityfunctions.CantellisInequality;
 ...
 * minNumberInstancesOption - The minimum number of instances required to
...
public class OddsRatioScore extends AbstractAnomalyDetector {
	...
	public FloatOption thresholdOption = new FloatOption("threshold", 't',
			"The threshold value for detecting anomalies.", -0.75, -10, 0);

	public ClassOption probabilityFunctionOption = new ClassOption("probabilityFunction", 'p', "Probability function",
			ProbabilityFunction.class, CantellisInequality.class.getName());
	...
	public boolean updateAndCheckAnomalyDetection(Instance instance) {
		...
		/*
		 * else { //System.out.println("Anomaly = " + anomaly + "#instances: " +
		 * weightSeen); printAnomaly(instance, anomaly); }
		 */
		return isAnomaly;
	}
	...
	protected void printAnomaly(Instance inst, double anomaly) {
		...
		for (int i = 0; i < inst.numInputAttributes(); i++) {
			if (inst.attribute(i).isNumeric()) {
				...
				double prob = probabilityFunction.getProbability(mean, Utils.computeSD(stats[1], stats[0], weightSeen),
						val);
				// Mean
