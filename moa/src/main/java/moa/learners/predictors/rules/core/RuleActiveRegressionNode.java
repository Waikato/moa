...
import moa.learners.predictors.rules.core.splitcriteria.AMRulesSplitCriterion;
...
public class RuleActiveRegressionNode extends RuleActiveLearningNode {
	 ...
	 * 
	 * @see
	 * moa.learners.predictors.rules.RuleActiveLearningNodeInterface#getLearnerToUse(weka.
	 * core.Instance, int)
	 */
	@Override
	public int getLearnerToUse(Instance instance, int predMode) {
		...
		if (predictionMode == 0) {
			...
			debug("Rule" + this.owner.ruleNumberID + " P:"
					+ this.perceptron.getPredictionForInstance(instance).asDoubleArray()[0] + " (" + perceptronError
					+ ")" + " M:" + this.targetMean.getPredictionForInstance(instance).asDoubleArray()[0] + " ("
					+ meanTargetError + ")", 3); // Commented by JD
	...
	@Override
	public boolean isAnomaly(Instance instance, double uniVariateAnomalyProbabilityThreshold,
		...
		if (perceptronIntancesSeen >= numberOfInstanceesForAnomaly) {
			...
			for (int x = 0; x < instance.numAttributes() - 1; x++) {
				...
				int instAttIndex = AbstractAMRules.modelAttIndexToInstanceAttIndex(x, instance);
				...
				/*
				 * old implementation if (probability > 0.0) { D = D +
				 * Math.abs(Math.log(probability)); if (probability <
				 * uniVariateAnomalyProbabilityThreshold) {//0.10 N = N +
				 * Math.abs(Math.log(probability)); } } else {
			...
			/*
			 * anomaly = 0.0; //Old implementation if (D != 0.0) { anomaly = N / D; } if
			 * (anomaly >= multiVariateAnomalyProbabilityThreshold) { debuganomaly(instance,
			 * uniVariateAnomalyProbabilityThreshold,
			 * multiVariateAnomalyProbabilityThreshold, anomaly); return true; }
			 */
			 ...
			 * FileWriter("/home/jduarte/fried_anomalies.txt", true)));
			 * out.println(anomaly); out.close(); } catch (IOException e) { //exception
	...
	public boolean tryToExpand(double splitConfidence, double tieThreshold) {
		...
		// SplitCriterion splitCriterion = new SDRSplitCriterionAMRulesNode();//JD for
		// assessing only best branch
		AMRulesSplitCriterion splitCriterion = (AMRulesSplitCriterion) ((AMRulesSplitCriterion) ((AMRulesRegressorOld) this.amRules).splitCriterionOption
		...
		if (bestSplitSuggestions.length < 2) {
			...
			if (bestSuggestion.merit > 0) {
				// if ((((secondBestSuggestion.merit / bestSuggestion.merit) + hoeffdingBound) <
				// 1) //ratio
	...
	public void learnFromInstance(Instance inst) {
		...
		/*
		 * for (int i = 0; i < inst.numAttributes() - 1; i++) { int instAttIndex =
		 * AbstractAMRules.modelAttIndexToInstanceAttIndex(i, inst);
		 * 
		 * AttributeClassObserver obs = this.attributeObservers.get(i); if (obs == null)
		 * { // At this stage all nominal attributes are ignored if
		 * (inst.attribute(instAttIndex).isNumeric()) //instAttIndex { obs =
		 * newNumericClassObserver(); this.attributeObservers.set(i, obs); } } if (obs
		 * != null) { ((FIMTDDNumericAttributeClassObserver)
		 * obs).observeAttributeClass(inst.value(instAttIndex), inst.classValue(),
		 * inst.weight()); } }
		 */
		// if was of attributes was not created so far, generate one and create
		// perceptron
		if (attributesMask == null) {
			numAttributesSelected = (int) Math
					.round((inst.numAttributes() - 1) * this.amRules.getAttributesPercentage()) / 100;
		...
		for (int i = 0, ct = 0; i < attributesMask.length; i++) {
			if (attributesMask[i]) {
				...
				if (obs != null) {
					((FIMTDDNumericAttributeClassObserver) obs).observeAttributeClass(inst.value(i), inst.classValue(),
							inst.weight());
				}
				++ct;
			}
