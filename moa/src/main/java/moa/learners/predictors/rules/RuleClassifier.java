...
import moa.learners.predictors.core.attributeclassobservers.BinaryTreeNumericAttributeClassObserver;
import moa.learners.predictors.core.attributeclassobservers.GaussianNumericAttributeClassObserver;
import moa.learners.predictors.core.attributeclassobservers.NominalAttributeClassObserver;
import moa.learners.predictors.core.attributeclassobservers.BinaryTreeNumericAttributeClassObserver.Node;

/**
 * This classifier learn ordered and unordered rule set from data stream. This
 * algorithm also does the detection of anomalies.
 *
 * <p>
 * Learning Decision RuleClassifications from Data Streams, IJCAI 2011, J. Gama,
 * P. Kosina
 * </p>
 *
 *
 ...
 * <li>-g: GracePeriod, the number of instances a leaf should observe between
 * split attempts</li>
...
public class RuleClassifier extends AbstractClassifier implements Classifier {
	...
	protected DoubleVector saveBestEntropy = new DoubleVector(); // Saves the best value of entropy, cut_Point and
																	// symbol.

	protected DoubleVector saveBestEntropyNominalAttrib = new DoubleVector(); // Saves the best value of entropy and
																				// their cut_Point.
	...
	protected ArrayList<ArrayList<Double>> saveBestValGlobalEntropy = new ArrayList<>(); // For each
																											// attribute
																											// contains
																											// the best
																											// value of
	...
	protected ArrayList<Double> saveTheBest = new ArrayList<>(); // Contains the best attribute.
	...
	public FloatOption PminOption = new FloatOption("Pmin", 'p',
			"Percentage of the total number of example seen in the node.", 0.1, 0.0, 1.0);

	public FloatOption splitConfidenceOption = new FloatOption("splitConfidence", 'c',
			"The allowable error in split decision, values closer to 0 will take longer to decide.", 0.000001, 0.0,
			1.0);
	...
	public FloatOption anomalyProbabilityThresholdOption = new FloatOption("anomalyprobabilityThreshold", 'o',
			"The threshold value.", 0.99, 0.0, 1.0);

	public FloatOption probabilityThresholdOption = new FloatOption("probabilityThreshold", 'k', "The threshold value.",
			0.10, 0.0, 1.0);

	public IntOption anomalyNumInstThresholdOption = new IntOption("anomalyThreshold", 'i',
			"The threshold value to be used in the anomaly detection.", 15, 0, Integer.MAX_VALUE);

	public IntOption gracePeriodOption = new IntOption("gracePeriod", 'g',
			"The number of instances a leaf should observe between split attempts.", 200, 0, Integer.MAX_VALUE);

	public MultiChoiceOption predictionFunctionOption = new MultiChoiceOption("predictionFunctionOption", 'z',
			"The prediction function to use.", new String[] { "firstHit", "weightedSum", "weightedMax" },
			new String[] { "first Hit", "weighted Sum", "weighted Max" }, 0);
	...
	public void trainOnInstanceImpl(Instance inst) {
		...
		for (int j = 0; j < ruleSet.size(); j++) {
			if (this.ruleSet.get(j).ruleEvaluate(inst) == true) {
				...
				if (this.Supervised.isSet()) {
					anomaly = computeAnomalySupervised(this.ruleSet.get(j), j, inst); // compute anomaly (Supervised
																						// method)
				} else if (this.Unsupervised.isSet()) {
					anomaly = computeAnomalyUnsupervised(this.ruleSet.get(j), j, inst); // compute anomaly (Unsupervised
																						// method)
				}
				...
				if ((this.ruleSet.get(j).instancesSeen <= this.anomalyNumInstThresholdOption.getValue())
					...
					for (int i = 0; i < inst.numAttributes() - 1; i++) {
						...
						if (!inst.isMissing(instAttIndex)) {
							AttributeClassObserver obs = this.ruleSet.get(j).observers.get(i); // Nominal and binary
																								// tree.
		...
		if (ruleFired == false) { // If none of the rules cover the example update sufficient statistics of the
									// default rule
	...
	public void printAnomaliesUnsupervised(StringBuilder out, int indent) { // Get Model Description (Unsupervised
																			// method)
		...
		for (int k = 0; k < this.ruleSetAnomalies.size(); k++) {
			StringUtils.appendIndented(out, indent, "Case: " + this.caseAnomaly.get(k).get(0) + "   Anomaly Score: "
					+ this.caseAnomaly.get(k).get(1) + "%");
			...
			for (int i = 0; i < this.ruleSetAnomalies.get(k).predicateSet.size(); i++) {
				if (this.ruleSetAnomalies.get(k).predicateSet.size() == 1) {
					if (this.ruleSetAnomalies.get(k).predicateSet.get(i).getSymbol() == 0.0) {
						String nam = this.instance
								.attribute((int) this.ruleSetAnomalies.get(k).predicateSet.get(i).getAttributeValue())
								.name();
						String val = this.instance
								.attribute((int) this.ruleSetAnomalies.get(k).predicateSet.get(i).getAttributeValue())
								.value((int) this.ruleSetAnomalies.get(k).predicateSet.get(i).getValue());
						StringUtils.appendIndented(out, indent, nam + " = " + val + " --> " + this.instance
								.classAttribute().value((int) getRuleMajorityClassIndex(this.ruleSetAnomalies.get(k))));
						StringUtils.appendNewline(out);
						...
						StringUtils.appendIndented(out, indent,
								nam + " <= " + this.ruleSetAnomalies.get(k).predicateSet.get(i).getValue() + " --> "
										+ this.instance.classAttribute()
												.value((int) ruleClassIndex.getValue(ruleAnomaliesIndex.get(k) - 1)));
						StringUtils.appendNewline(out);
					} else {
						String nam = this.instance
								.attribute((int) this.ruleSetAnomalies.get(k).predicateSet.get(i).getAttributeValue())
						...
						StringUtils.appendIndented(out, indent,
								nam + " > " + this.ruleSetAnomalies.get(k).predicateSet.get(i).getValue() + " --> "
										+ this.instance.classAttribute().value(
												(int) this.ruleClassIndex.getValue(ruleAnomaliesIndex.get(k) - 1)));
						StringUtils.appendNewline(out);
					}
				} else {
					if (this.ruleSetAnomalies.get(k).predicateSet.get(i).getSymbol() == 0.0) {
					...
					if (i < this.ruleSetAnomalies.get(k).predicateSet.size() - 1) {
						...
						if ((this.ruleSetAnomalies.get(k).predicateSet.get(i).getSymbol() == 0.0) || (count != 0)) {
							StringUtils.appendIndented(out, indent, " --> " + this.instance.classAttribute()
									.value((int) getRuleMajorityClassIndex(this.ruleSetAnomalies.get(k))));
							StringUtils.appendNewline(out);
						} else {
							StringUtils.appendIndented(out, indent, " --> " + this.instance.classAttribute()
									.value((int) this.ruleClassIndex.getValue(ruleAnomaliesIndex.get(k) - 1)));
							StringUtils.appendNewline(out);
						}
					}
				}
			...
			for (int z = 0; z < this.ruleAttribAnomalyStatistics.get(k).size(); z++) {
				if (this.ruleAttribAnomalyStatistics.get(k).get(z).size() == 5) {
					...
					StringUtils.appendIndented(out, indent,
							instance.attribute(this.ruleAttribAnomalyStatistics.get(k).get(z).get(0).intValue()).name()
									+ "=" + round(this.ruleAttribAnomalyStatistics.get(k).get(z).get(1)) + "   ("
									+ round(this.ruleAttribAnomalyStatistics.get(k).get(z).get(2)) + " +- "
									+ round(this.ruleAttribAnomalyStatistics.get(k).get(z).get(3)) + ")   P=" + s);
					StringUtils.appendNewline(out);
				} else {
					String s = String.format("%.3e", this.ruleAttribAnomalyStatistics.get(k).get(z).get(2));
					String val = this.instance
							.attribute(this.ruleAttribAnomalyStatistics.get(k).get(z).get(0).intValue())
							.value(this.ruleAttribAnomalyStatistics.get(k).get(z).get(1).intValue());
					StringUtils.appendIndented(out, indent,
							instance.attribute(this.ruleAttribAnomalyStatistics.get(k).get(z).get(0).intValue()).name()
									+ "=" + val + "   P=" + s);
					StringUtils.appendNewline(out);
	...
	public void printAnomaliesSupervised(StringBuilder out, int indent) { // Get Model Description (Supervised method)
		...
		for (int k = 0; k < this.ruleSetAnomaliesSupervised.size(); k++) {
			StringUtils.appendIndented(out, indent, "Case: " + this.caseAnomalySupervised.get(k).get(0)
					+ "   Anomaly Score: " + this.caseAnomalySupervised.get(k).get(1) + "%");
			StringUtils.appendNewline(out); // Ver outra parte default
			...
			for (int i = 0; i < this.ruleSetAnomaliesSupervised.get(k).predicateSet.size(); i++) {
				if (this.ruleSetAnomaliesSupervised.get(k).predicateSet.size() == 1) {
					if (this.ruleSetAnomaliesSupervised.get(k).predicateSet.get(i).getSymbol() == 0.0) {
						String nam = this.instance.attribute(
								(int) this.ruleSetAnomaliesSupervised.get(k).predicateSet.get(i).getAttributeValue())
								.name();
						String val = this.instance.attribute(
								(int) this.ruleSetAnomaliesSupervised.get(k).predicateSet.get(i).getAttributeValue())
								.value((int) this.ruleSetAnomaliesSupervised.get(k).predicateSet.get(i).getValue());
						StringUtils.appendIndented(out, indent,
								nam + " = " + val + " --> " + this.instance.classAttribute().value(
										(int) getRuleMajorityClassIndex(this.ruleSetAnomaliesSupervised.get(k))));
						StringUtils.appendNewline(out);
					} else if (this.ruleSetAnomaliesSupervised.get(k).predicateSet.get(i).getSymbol() == -1.0) {
						String nam = this.instance.attribute(
								(int) this.ruleSetAnomaliesSupervised.get(k).predicateSet.get(i).getAttributeValue())
								.name();
						StringUtils.appendIndented(out, indent, nam + " <= "
								+ this.ruleSetAnomaliesSupervised.get(k).predicateSet.get(i).getValue() + " --> "
								+ this.instance.classAttribute()
										.value((int) ruleClassIndex.getValue(ruleAnomaliesIndexSupervised.get(k) - 1)));
						StringUtils.appendNewline(out);
					} else {
						String nam = this.instance.attribute(
								(int) this.ruleSetAnomaliesSupervised.get(k).predicateSet.get(i).getAttributeValue())
						...
						StringUtils.appendIndented(out, indent, nam + " > "
								+ this.ruleSetAnomaliesSupervised.get(k).predicateSet.get(i).getValue() + " --> "
								+ this.instance.classAttribute().value(
										(int) this.ruleClassIndex.getValue(ruleAnomaliesIndexSupervised.get(k) - 1)));
						StringUtils.appendNewline(out);
					}
				} else {
					if (this.ruleSetAnomaliesSupervised.get(k).predicateSet.get(i).getSymbol() == 0.0) {
					...
					if (i < this.ruleSetAnomaliesSupervised.get(k).predicateSet.size() - 1) {
						...
						if ((this.ruleSetAnomaliesSupervised.get(k).predicateSet.get(i).getSymbol() == 0.0)
								|| (count != 0)) {
							StringUtils.appendIndented(out, indent, " --> " + this.instance.classAttribute()
									.value((int) getRuleMajorityClassIndex(this.ruleSetAnomaliesSupervised.get(k))));
							StringUtils.appendNewline(out);
						} else {
							StringUtils.appendIndented(out, indent, " --> " + this.instance.classAttribute().value(
									(int) this.ruleClassIndex.getValue(ruleAnomaliesIndexSupervised.get(k) - 1)));
							StringUtils.appendNewline(out);
						}
					}
			...
			for (int z = 0; z < this.ruleAttribAnomalyStatisticsSupervised.get(k).size(); z++) {
				if (this.ruleAttribAnomalyStatisticsSupervised.get(k).get(z).size() == 5) {
					...
					StringUtils.appendIndented(out, indent,
							instance.attribute(
									this.ruleAttribAnomalyStatisticsSupervised.get(k).get(z).get(0).intValue()).name()
									+ "=" + round(this.ruleAttribAnomalyStatisticsSupervised.get(k).get(z).get(1))
									+ "   (" + round(this.ruleAttribAnomalyStatisticsSupervised.get(k).get(z).get(2))
									+ " +- " + round(this.ruleAttribAnomalyStatisticsSupervised.get(k).get(z).get(3))
									+ ")   P=" + s);
					StringUtils.appendNewline(out);
				} else {
					String s = String.format("%.3e", this.ruleAttribAnomalyStatisticsSupervised.get(k).get(z).get(2));
					String val = this.instance
							.attribute(this.ruleAttribAnomalyStatisticsSupervised.get(k).get(z).get(0).intValue())
							.value(this.ruleAttribAnomalyStatisticsSupervised.get(k).get(z).get(1).intValue());
					StringUtils.appendIndented(out, indent,
							instance.attribute(
									this.ruleAttribAnomalyStatisticsSupervised.get(k).get(z).get(0).intValue()).name()
									+ "=" + val + "   P=" + s);
	...
	public void getModelDescriptionNoAnomalyDetection(StringBuilder out, int indent) { // Get Model Description no
																						// Anomaly detection
		StringUtils.appendNewline(out);
			...
			for (int i = 0; i < this.ruleSet.get(k).predicateSet.size(); i++) {
				if (this.ruleSet.get(k).predicateSet.size() == 1) {
					if (this.ruleSet.get(k).predicateSet.get(i).getSymbol() == 0.0) {
						String nam = this.instance
								.attribute((int) this.ruleSet.get(k).predicateSet.get(i).getAttributeValue()).name();
						String val = this.instance
								.attribute((int) this.ruleSet.get(k).predicateSet.get(i).getAttributeValue())
								.value((int) this.ruleSet.get(k).predicateSet.get(i).getValue());
						StringUtils.appendIndented(out, indent, nam + " = " + val + " --> " + this.instance
								.classAttribute().value((int) getRuleMajorityClassIndex(this.ruleSet.get(k))));
						StringUtils.appendNewline(out);
					} else if (this.ruleSet.get(k).predicateSet.get(i).getSymbol() == -1.0) {
						String nam = this.instance
								.attribute((int) this.ruleSet.get(k).predicateSet.get(i).getAttributeValue()).name();
						StringUtils.appendIndented(out, indent,
								nam + " <= " + this.ruleSet.get(k).predicateSet.get(i).getValue() + " --> "
										+ this.instance.classAttribute().value((int) ruleClassIndex.getValue(k)));
						StringUtils.appendNewline(out);
					} else {
						String nam = this.instance
								.attribute((int) this.ruleSet.get(k).predicateSet.get(i).getAttributeValue()).name();
						StringUtils.appendIndented(out, indent,
								nam + " > " + this.ruleSet.get(k).predicateSet.get(i).getValue() + " --> "
										+ this.instance.classAttribute().value((int) this.ruleClassIndex.getValue(k)));
						StringUtils.appendNewline(out);
					}
				} else {
					if (this.ruleSet.get(k).predicateSet.get(i).getSymbol() == 0.0) {
					...
					if (i < this.ruleSet.get(k).predicateSet.size() - 1) {
						...
						if ((this.ruleSet.get(k).predicateSet.get(i).getSymbol() == 0.0) || (count != 0)) {
							StringUtils.appendIndented(out, indent, " --> " + this.instance.classAttribute()
									.value((int) getRuleMajorityClassIndex(this.ruleSet.get(k))));
							StringUtils.appendNewline(out);
						} else {
							StringUtils.appendIndented(out, indent, " --> "
									+ this.instance.classAttribute().value((int) this.ruleClassIndex.getValue(k)));
		...
		for (int s = 0; s < inst.numAttributes() - 1; s++) {
			...
			if (!inst.isMissing(instAttIndex)) {
				if (instance.attribute(instAttIndex).isNumeric()) {
					...
					double squaredSumvalue = rl.squaredAttributeStatisticsSupervised.get(s).get((int) inst.classValue())
							+ (inst.value(s) * inst.value(s));
	...
	public double computeAnomalyUnsupervised(RuleClassification rl, int ruleIndex, Instance inst) { // Unsupervised
		ArrayList<Integer> caseAnomalyTemp = new ArrayList<>();
		ArrayList<ArrayList<Double>> AttribAnomalyStatisticTemp2 = new ArrayList<>();
		double D = 0.0;
		double N = 0.0;
		...
		if (rl.instancesSeen > this.anomalyNumInstThresholdOption.getValue() && this.anomalyDetectionOption.isSet()) {
			for (int x = 0; x < inst.numAttributes() - 1; x++) {
				if (!inst.isMissing(x)) {
					ArrayList<Double> AttribAnomalyStatisticTemp = new ArrayList<>();
					...
					if (inst.attribute(x).isNumeric()) { // Numeric Attributes
						if ((rl.instancesSeen - rl.attributeMissingValues.getValue(x)) > 30) {
							double mean = computeMean(
									rl.attributeStatisticsSupervised.get(x).get((int) inst.classValue()),
									(int) rl.obserClassDistrib.getValue((int) inst.classValue()));
							double sd = computeSD(
									rl.squaredAttributeStatisticsSupervised.get(x).get((int) inst.classValue()),
									rl.attributeStatisticsSupervised.get(x).get((int) inst.classValue()),
									(int) rl.obserClassDistrib.getValue((int) inst.classValue()));
							double probability = computeProbability(mean, sd, inst.value(x));
							if (probability != 0.0) {
	...
	public void createRule(Instance inst) {
		...
		if (remainder == 0) {
			...
			boolean HB = checkBestAttrib(numInstanciaObservers, this.attributeObservers,
	...
	public void expandeRule(RuleClassification rl, Instance inst, int ruleIndex) {
		int remainder = (int) Double.MAX_VALUE;
		int numInstanciaObservers = (int) rl.obserClassDistrib.sumOfValues(); // Number of instances for this rule
																				// observers.
		...
		if (remainder == 0) {
			...
			boolean HB = checkBestAttrib(numInstanciaObservers, rl.observers, rl.obserClassDistrib); // Check if the
																										// best
																										// attribute
																										// value is
																										// really the
			...
			if (HB == true) {
				...
				this.pred = new Predicates(attributeValue, symbol, value);
				int countPred = 0;
				for (int i = 0; i < rl.predicateSet.size(); i++) { // Checks if the new predicate is not yet in the
																	// predicateSet.
				...
				if (countPred == rl.predicateSet.size()) {
					int countDifPred = 0;
					ArrayList<Predicates> predicSetTemp = new ArrayList<>();
					...
					for (int f = 0; f < this.ruleSet.size(); f++) {
						...
						if (this.ruleSet.get(f).predicateSet.size() == predicSetTemp.size()) {
							for (int x = 0; x < this.ruleSet.get(f).predicateSet.size(); x++) {
								if (this.ruleSet.get(f).predicateSet.get(x).getAttributeValue() == predicSetTemp.get(x)
										.getAttributeValue()
										&& this.ruleSet.get(f).predicateSet.get(x).getSymbol() == predicSetTemp.get(x)
												.getSymbol()
										&& this.ruleSet.get(f).predicateSet.get(x).getValue() == predicSetTemp.get(x)
												.getValue()) {
		...
		for (int z = 0; z < instance.numAttributes() - 1; z++) {
			if (!instance.isMissing(z)) {
				...
				if (instance.attribute(instAttIndex).isNominal()) {
					...
					findBestValEntropyNominalAtt(attribNominal, instance.attribute(z).numValues()); // The best value
																									// (lowest entropy)
																									// of a nominal
																									// attribute.
	...
	public void findBestValEntropy(Node node, DoubleVector classCountL, DoubleVector classCountR, boolean status,
			double minEntropy, DoubleVector parentCCLeft) {
		if (this.root != null) {
			if (node != null) {
				int numClass = 0;
				...
				if (node.cut_point != this.root.cut_point) {
					for (int i = 0; i < numClass; i++) {
						if (status == true) { // Left node.
							double parentss = parentCCL.getValue(i)
									- (node.classCountsLeft.getValue(i) + node.classCountsRight.getValue(i));
				...
				if ((classCountLSum > this.PminOption.getValue() * numInst)
					...
					if (((classCountLSum / numInst) * classCountLEntropy
							+ (classCountRSum / numInst) * classCountREntropy) <= minEntropy) {
				...
				findBestValEntropy(node.left, classCountLTemp, classCountRTemp, true, this.minEntropyTemp,
						parentCCLParameter);
				findBestValEntropy(node.right, classCountLTemp, classCountRTemp, false, this.minEntropyTemp,
						parentCCLParameter);
			}
		}
	}
	...
	public void findBestValEntropyNominalAtt(AutoExpandVector<DoubleVector> attrib, int attNumValues) {
		ArrayList<ArrayList<Double>> distClassValue = new ArrayList<>();
	...
	public boolean checkBestAttrib(double n, AutoExpandVector<AttributeClassObserver> observerss,
	...
	protected double[] weightedMax(Instance inst) {
		...
		double highest = 0.0;
		double[] votes = new double[this.numClass];
		ArrayList<Double> ruleSetVotes = new ArrayList<>();
		ArrayList<ArrayList<Double>> majorityProb = new ArrayList<>();
		for (int j = 0; j < this.ruleSet.size(); j++) {
			ArrayList<Double> ruleProb = new ArrayList<>();
			if (this.ruleSet.get(j).ruleEvaluate(inst) == true) {
				countFired = countFired + 1;
				for (int z = 0; z < this.numClass; z++) {
					ruleSetVotes.add(this.ruleSet.get(j).obserClassDistrib.getValue(z)
							/ this.ruleSet.get(j).obserClassDistrib.sumOfValues());
					ruleProb.add(this.ruleSet.get(j).obserClassDistrib.getValue(z)
							/ this.ruleSet.get(j).obserClassDistrib.sumOfValues());
				}
