...
import moa.learners.predictors.rules.core.conditionaltests.NumericAttributeBinaryRulePredicate;
...
public class Rule extends AbstractMOAObject {
	...
	/*
	 * private RuleActiveLearningNode newRuleActiveLearningNode(double[]
	 * initialClassObservations) { return
	 * amRules.newRuleActiveLearningNode(initialClassObservations); }
	 */
	...
	public void split() {
		...
		if (st instanceof NumericAttributeBinaryTest) {
			...
			NumericAttributeBinaryRulePredicate predicate = new NumericAttributeBinaryRulePredicate(
					splitTest.getAttsTestDependsOn()[0], splitTest.getSplitValue(), splitIndex + 1);
			...
			if (this.nodeListAdd(ruleSplitNode) == true) {
				...
				RuleActiveLearningNode newLearningNode = newRuleActiveLearningNode(
						this.getBuilder().statistics(this.learningNode.getStatisticsNewRuleActiveLearningNode()));
	...
	private boolean nodeListAdd(RuleSplitNode ruleSplitNode) {
		...
		for (RuleSplitNode node : nodeList) {
			...
			if (nodeTest.isUsingSameAttribute(ruleSplitNodeTest)) {
				...
				if (nodeTest.isIncludedInRuleNode(ruleSplitNodeTest) == true) { // remove this line to keep the most
																				// recent attribute value
					// replace the value
					nodeTest.setAttributeValue(ruleSplitNodeTest);
					isUpdated = true; // if is updated (i.e. an expansion happened) a new learning node should be
										// created
				}
			}
		}
	...
	public String printRule() {
		...
		StringUtils.appendIndented(out, indent,
				"Rule Nr." + this.ruleNumberID + " Instances seen:" + this.learningNode.getInstancesSeen() + "\n"); // AC
		for (RuleSplitNode node : nodeList) {
	...
	public boolean isAnomaly(Instance instance, double uniVariateAnomalyProbabilityThreshold,
		...
		return this.learningNode.isAnomaly(instance, uniVariateAnomalyProbabilityThreshold,
				multiVariateAnomalyProbabilityThreshold, numberOfInstanceesForAnomaly);
	}
