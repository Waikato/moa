...
/**
 * This classifier learn ordered and unordered rule set from data stream with
 ...
 * </p>
 * <ul>
 * <li>-q: The number of instances a leaf should observe before permitting Naive
 * Bayes.</li>
...
public class RuleClassifierNBayes extends RuleClassifier {
	...
	protected double[] firstHitNB(Instance inst) {
		...
		for (int j = 0; j < this.ruleSet.size(); j++) {
			if (this.ruleSet.get(j).ruleEvaluate(inst) == true) {
				...
				if (this.ruleSet.get(j).obserClassDistrib.sumOfValues() >= this.nbThresholdOption.getValue()) {
					votes = NaiveBayes.doNaiveBayesPredictionLog(inst, this.ruleSet.get(j).obserClassDistrib,
							this.ruleSet.get(j).observers, this.ruleSet.get(j).observersGauss);
		...
		if (fired == false) {
			if (super.getWeightSeen() >= this.nbThresholdOption.getValue()) {
				votes = NaiveBayes.doNaiveBayesPredictionLog(inst, this.observedClassDistribution,
						this.attributeObservers, this.attributeObserversGauss);
				votes = exponential(votes);
				votes = normalize(votes);
			} else {
	...
	protected double[] weightedMaxNB(Instance inst) {
		...
		for (int j = 0; j < this.ruleSet.size(); j++) {
			ArrayList<Double> ruleClassDistribProb = new ArrayList<>();
			if (this.ruleSet.get(j).ruleEvaluate(inst) == true) {
				countFired = countFired + 1;
				if (this.ruleSet.get(j).obserClassDistrib.sumOfValues() >= this.nbThresholdOption.getValue()) {
					votes = NaiveBayes.doNaiveBayesPredictionLog(inst, this.ruleSet.get(j).obserClassDistrib,
					...
					for (int z = 0; z < this.numClass; z++) {
						ruleSetVotes.add(this.ruleSet.get(j).obserClassDistrib.getValue(z)
								/ this.ruleSet.get(j).obserClassDistrib.sumOfValues());
						ruleClassDistribProb.add(this.ruleSet.get(j).obserClassDistrib.getValue(z)
								/ this.ruleSet.get(j).obserClassDistrib.sumOfValues());
					}
	...
	protected double[] weightedSumNB(Instance inst) {
		...
		for (int j = 0; j < this.ruleSet.size(); j++) {
			...
			if (this.ruleSet.get(j).ruleEvaluate(inst) == true) {
				...
				if (this.ruleSet.get(j).obserClassDistrib.sumOfValues() >= this.nbThresholdOption.getValue()) {
					votes = NaiveBayes.doNaiveBayesPredictionLog(inst, this.ruleSet.get(j).obserClassDistrib,
							ruleSet.get(j).observers, this.ruleSet.get(j).observersGauss);
					votes = exponential(votes);
					votes = normalize(votes);
				} else {
					count = count + 1;
