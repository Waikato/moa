...
import moa.learners.predictors.core.functions.MultiLabelPerceptronClassification;
...
/**
 * Adaptive Model Rules for MultiLabel problems (AMRulesML), the streaming rule
 * learning algorithm.
 *
 * @author J. Duarte, J. Gama (jgama@fep.up.pt)
 * @version $Revision: 2$*
 *
 *          This algorithm learns ordered and unordered rule set from data
 *          streams. Each rule detect anomalies and react to changes by pruning
 *          the rule set. This algorithm also does the detection of anomalies.
 *
 **/
...
public abstract class AMRulesMultiLabelLearner extends AbstractMultiTargetRegressor implements MultiTargetRegressor {
	...
	public ClassOption changeDetector = new ClassOption("changeDetector", 'H', "Change Detector.", ChangeDetector.class,
...
//			1,Integer.MIN_VALUE, Integer.MAX_VALUE);
	...
	/**
	 * getVotes extension of the instance method getPredictionForInstance in
	...
	public ErrorWeightedVoteMultiLabel getVotes(Instance instance) {
		...
		for (MultiLabelRule rule : ruleSet) {
			if (rule.isCovering(instance) == true) {
				...
				if (!this.unorderedRulesOption.isSet()) { // Ordered Rules Option. //TODO: Only break if all outputs
															// have values assigned.Complete Prediction only with the
															// missing values
	...
	public void trainOnInstanceImpl(Instance instance) {
		 ...
		 * //Else //Update sufficient statistics of rule //If number of examples in rule
		 * > Nmin //Expand rule //If ordered set then //break //If none of the rule
		 * covers the instance //Update sufficient statistics of default rule //If
		 * number of examples in default rule is multiple of Nmin //Expand default rule
		 * and add it to the set of rules //Reset the default rule
		 */
		if (nAttributes == 0)
		...
		while (ruleIterator.hasNext()) {
			...
			if (rule.isCovering(instance) == true) {
				rulesCoveringInstance = true; // TODO: JD use different strategies for this validation (first rule,
												// first complete rule (all out attributes covered), voted complete
												// rule, etc)
				if (!rule.updateAnomalyDetection(instance)) {
					if (rule.updateChangeDetection(instance)) {
						debug("I) Drift Detected. Exa. : " + this.numInstances + " ("
								+ rule.getWeightSeenSinceExpansion() + ") Remove Rule: " + rule.getRuleNumberID(), 1);
						ruleIterator.remove();
						...
						if (rule.getWeightSeenSinceExpansion() % this.gracePeriodOption.getValue() == 0.0) {
							if (rule.tryToExpand(this.splitConfidenceOption.getValue(),
