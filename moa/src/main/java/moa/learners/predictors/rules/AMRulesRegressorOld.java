...
import moa.learners.predictors.rules.core.RuleActiveRegressionNode;
...
public class AMRulesRegressorOld extends AbstractAMRules implements MultiTargetRegressor {
	...
	public FloatOption learningRatioOption = new FloatOption("learningRatio", 's',
			"Constante Learning Ratio to use for training the Perceptrons in the leaves.", 0.025);

	public MultiChoiceOption predictionFunctionOption = new MultiChoiceOption("predictionFunctionOption", 'P',
			"The prediction function to use.", new String[] { "Adaptative", "Perceptron", "Target Mean" },
			new String[] { "Adaptative", "Perceptron", "Target Mean" }, 0);

	public ClassOption votingTypeOption = new ClassOption("votingType", 'V', "Voting Type.", ErrorWeightedVote.class,
			"InverseErrorWeightedVote");

	public ClassOption splitCriterionOption = new ClassOption("splitCriterionOption", 'S', "Split Criterion",
			AMRulesSplitCriterion.class, "VRSplitCriterion");
		...
		if (node != null) {
			if (((RuleActiveRegressionNode) node).getPerceptron() != null) {
				((RuleActiveRegressionNode) r.getLearningNode())
						.setPerceptron(new Perceptron(((RuleActiveRegressionNode) node).getPerceptron()));
				((RuleActiveRegressionNode) r.getLearningNode()).getPerceptron()
	...
	private Rule newRule(int ID) {
		Rule r = new Rule.Builder().threshold(this.pageHinckleyThresholdOption.getValue())
				.alpha(this.pageHinckleyAlphaOption.getValue()).changeDetection(this.DriftDetectionOption.isSet())
				.predictionFunction(this.predictionFunctionOption.getChosenIndex()).statistics(new double[3]).id(ID)
				.amRules(this).build();
