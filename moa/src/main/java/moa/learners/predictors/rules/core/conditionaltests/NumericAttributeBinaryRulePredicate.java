 ...
 *
 */
package moa.learners.predictors.rules.core.conditionaltests;

import com.yahoo.labs.samoa.instances.Instance;
import com.yahoo.labs.samoa.instances.InstancesHeader;

import moa.core.StringUtils;
import moa.learners.predictors.core.conditionaltests.InstanceConditionalBinaryTest;
...
public class NumericAttributeBinaryRulePredicate extends InstanceConditionalBinaryTest implements Predicate {
	...
	public String describeConditionForBranch(int branch, InstancesHeader context) {
		if ((branch >= 0) && (branch <= 2)) {
			...
			return InstancesHeader.getAttributeNameString(context, this.attIndex) + ' ' + compareChar
	...
	public boolean isEqual(NumericAttributeBinaryRulePredicate predicate) {
		return (this.attIndex == predicate.attIndex && this.attValue == predicate.attValue
	...
	public void getDescription(StringBuilder sb, int indent, InstancesHeader header) {
		String compareChar = (operator == 0) ? "=" : (operator == 1) ? "<=" : ">";
		StringUtils.appendIndented(sb, indent,
				InstancesHeader.getAttributeNameString(header, attIndex) + compareChar + attValue);
	}
