 ...
 *
 */
package moa.learners.predictors.core.conditionaltests;
...
public class NumericAttributeBinaryTest extends InstanceConditionalBinaryTest {
	...
	public int branchForInstance(Instance inst) {
		...
		double v = inst.valueInputAttribute(instAttIndex); // if the attIndex is not calculated above this is the
															// correct method call
		if (v == this.attValue) {
	...
	public String describeConditionForBranch(int branch, InstancesHeader context) {
		if ((branch == 0) || (branch == 1)) {
			...
			return InstancesHeader.getInputAttributeNameString(context, this.attIndex) + ' ' + compareChar
					+ (branch == equalsBranch ? "= " : " ")
