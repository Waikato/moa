...
/**
 * SeqDriftChangeDetector.java. This extends Abstract Change Detector as
 * required by MOA.
 *
 * Pears, R., Sakthithasan, S., & Koh, Y. (2014). Detecting concept change in
 * dynamic data streams. Machine Learning, 97(3), 259-293.
...
public class SeqDrift2ChangeDetector extends AbstractChangeDetector {
	...
	public IntOption blockSeqDrift2Option = new IntOption("blockSeqDrift2Option", 'b',
			"Block size of SeqDrift2 change detector", 200, 100, 10000);
	...
	public void resetLearning() {
		seqdrift = new SeqDrift2(this.deltaSeqDrift2Option.getValue(), (this.blockSeqDrift2Option.getValue()));
	}
	 ...
	 * on false positive and false negative rates. This is a block based approach
	 * and checks for changes in the data values only at block boundaries as opposed
	 * to the methods on per instance basis. SeqDrift maintains a reservoir and a
	 * repository. Repository gathers the new instances and reservoir stores only
	 * the data values that are statistically not different, in other words from the
	 * same distribution. If the data values in the repository are consistent with
	 * the values in reservoir, the data values of the repository are copied to the
	 * reservoir applying reservoir algorithm.The hypothesis is that the mean values
	 * of the reservoir and right repository are not statistically different
	 * 
	 * 
	 * 
	 * Pears, R., Sakthithasan, S., & Koh, Y. (2014). Detecting concept change in
	...
	public class SeqDrift2 extends AbstractMOAObject {
		 ...
		 * size sets the interval of two consecutive hypothesis tests Block Size is a
		 * positive integer and significance level is a double value between 0 and 1
		 * 
		 ...
		 * This method requires a numerical value as an input. The return value
		 * indicates whether there is a change detected or not.
		 * 
		...
		public boolean setInput(double _inputValue) {
			...
			if ((instanceCount % blockSize) == 0) // checking for drift at block boundary
			{
				...
				/*
				 * else if (iDriftType == INTERNAL_DRIFT) { //Ignoring the changes where mean
				 * values decrease clearLeftReservoir(); moveFromRepositoryToReservoir(); return
				 * false; }
				 */
	...
	public class Block {
		...
		public void add(double _dValue) {
			if (indexOfLastValue < data.length) {
				...
				System.out.println("ERROR in adding to Block. Last Index :" + indexOfLastValue + " Total :" + total
