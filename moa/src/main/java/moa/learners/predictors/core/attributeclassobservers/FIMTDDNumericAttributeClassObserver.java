...
public class FIMTDDNumericAttributeClassObserver extends BinaryTreeNumericAttributeClassObserver
	...
	protected class Node implements Serializable {
		...
		/**
		 * Insert a new value into the tree, updating both the sum of values and sum of
		 * squared values arrays
		 */
...
//            	if ((long) attVal != attVal) attVal = roundToSignificantFigures(attVal, 4);
	 ...
	 */
	public void removeBadSplits(SplitCriterion criterion, double lastCheckRatio, double lastCheckSDR,
			double lastCheckE) {
	...
	private boolean removeBadSplitNodes(SplitCriterion criterion, Node currentNode, double lastCheckRatio,
		...
		if (isBad) {
			...
			// double[][]{{currentNode.leftStatistics.getValue(0),
			...
			// double[] preSplitDist = new double[]{(currentNode.leftStatistics.getValue(0)
			// + currentNode.rightStatistics.getValue(0)),
			// (currentNode.leftStatistics.getValue(1) +
			// currentNode.rightStatistics.getValue(1)),
			// (currentNode.leftStatistics.getValue(2) +
