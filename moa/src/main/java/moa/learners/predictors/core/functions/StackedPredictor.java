...
public class StackedPredictor extends AbstractMultiTargetRegressor implements MultiTargetRegressor, AMRulesFunction {
	...
	public FloatOption learningRatio2ndLayerOption = new FloatOption("learningRatio2ndLayer", 'n',
	...
	public FlagOption skipStackingOption = new FlagOption("skipStackingOption", 's',
...
//			"Seed for random behaviour of the classifier.", 1);
	...
	public FlagOption printWeightsOption = new FlagOption("printWeights", 'p',
	...
	public void trainOnInstanceImpl(Instance instance) {
		...
		if (!hasStarted) {
			...
			/*
			 * Initialize first layer randomly uniform between -1 and 1 Initialize second
			 * layer such that the weights between correspondent outputs are 1 and the
			 * remaining are 0
			 */
			...
			/*
			 * for (int i=0; i<numOutputs+1; i++) for (int j=0; j<numOutputs; j++)
		...
		if (!constantLearningRatioDecayOption.isSet()) {
			currentLearningRate = learningRatioOption.getValue()
	...
	public Prediction getPredictionForInstance(Instance inst) {
		...
		if (hasStarted) {
			...
			pred = new MultiLabelClassificationPrediction(numOutputs);
	...
	protected double[] getNormalizedInput(Instance instance) {
		int numInputs = numericIndices.size();
	...
	protected double[] getNormalizedOutput(Instance instance) {
		int numOutputs = instance.numOutputAttributes();
