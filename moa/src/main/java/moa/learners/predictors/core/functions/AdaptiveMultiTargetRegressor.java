...
import moa.learners.predictors.core.errormeasurers.MeanAbsoluteDeviationMT;
import moa.learners.predictors.core.errormeasurers.MultiTargetErrorMeasurer;
import moa.options.ClassOption;

/**
 * Adaptive MultiTarget Regressor uses two learner The first is used in first
 * stage when high error are produced(e.g. Target mean) The second is used in a
 * second stage when low error are produced(e.g perceptron) baseLearnerOption1-
 * Learner one ; baseLearnerOption2- learner two
 */

public class AdaptiveMultiTargetRegressor extends AbstractMultiTargetRegressor
...
//			1,Integer.MIN_VALUE, Integer.MAX_VALUE);
	...
	public AdaptiveMultiTargetRegressor() {
		...
		baseLearnerOption1 = new ClassOption("baseLearner1", 'l', "First base learner.", AMRulesFunction.class,
				MultiTargetMeanRegressor.class.getName());
		baseLearnerOption2 = new ClassOption("baseLearner2", 'm', "Second base learner.", AMRulesFunction.class,
				MultiTargetPerceptronRegressor.class.getName());
		errorMeasurerOption = new ClassOption("errorMeasurer", 'e',
				...
				MeanAbsoluteDeviationMT.class.getName());
	...
	public void trainOnInstanceImpl(Instance instance) {
		if (!this.hasStarted) {
			...
			for (int i = 0; i < NUM_LEARNERS; i++) {
				...
				baseLearner[i].setModelContext(instance.dataset());
				errorMeasurer[i] = (MultiTargetErrorMeasurer) ((MultiTargetErrorMeasurer) getPreparedClassOption(
						this.errorMeasurerOption)).copy();
