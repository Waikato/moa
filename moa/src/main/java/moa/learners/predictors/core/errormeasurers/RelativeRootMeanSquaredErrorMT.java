 ...
 *
 */
package moa.learners.predictors.core.errormeasurers;
...
public class RelativeRootMeanSquaredErrorMT extends AbstractMultiTargetErrorMeasurer {
	...
	public void addPrediction(Prediction prediction, Prediction trueClass, double weight) {
		...
		for (int i = 0; i < numOutputs; i++) {
			if (prediction.hasVotesForAttribute(i)) {
				sumY[i] = trueClass.getPrediction(i) * weight + fadingErrorFactor * sumY[i]; // sum target
				double errorOutput = prediction.getPrediction(i) - trueClass.getPrediction(i);
				double errorOutputTM = prediction.getPrediction(i) - sumY[i] / weightSeen; // error to target mean
				sumSquaredError[i] = errorOutput * errorOutput * weight + fadingErrorFactor * sumSquaredError[i];
				sumSquaredErrorToTargetMean[i] = errorOutputTM * errorOutputTM * weight
