...
public class RelativeMeanAbsoluteDeviationMT extends AbstractMultiTargetErrorMeasurer {
	...
	public void addPrediction(Prediction prediction, Prediction trueClass, double weight) {
		...
		for (int i = 0; i < numOutputs; i++) {
			if (prediction.hasVotesForAttribute(i)) {
				sumError[i] = Math.abs(prediction.getPrediction(i) - trueClass.getPrediction(i)) * weight
						+ fadingErrorFactor * sumError[i];
				sumY[i] = trueClass.getPrediction(i) * weight + fadingErrorFactor * sumY[i];
				double errorOutputTM = Math.abs(prediction.getPrediction(i) - sumY[i] / weightSeen); // error to target
																									// mean
