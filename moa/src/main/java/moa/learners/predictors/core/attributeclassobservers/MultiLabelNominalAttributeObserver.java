...
import moa.learners.predictors.rules.core.NominalRulePredicate;
...
public class MultiLabelNominalAttributeObserver extends AbstractOptionHandler implements NominalStatisticsObserver {
	...
	public void observeAttribute(double inputAttributeValue, DoubleVector[] observedStatistics) {
		if (statisticsByNominalValue == null) {
			statisticsByNominalValue = new HashMap<>();
		}
	...
	public AttributeExpansionSuggestion getBestEvaluatedSplitSuggestion(MultiLabelSplitCriterion criterion,
		...
		while (it.hasNext()) {
			...
			if (merit > bestSuggestionMerit) {
				...
				bestSuggestiong = new AttributeExpansionSuggestion(
						new NominalRulePredicate(inputAttributeIndex, splitValue, true),
						Utils.copy(resultingStatistics), merit);
			}
		}
