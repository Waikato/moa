...
public class MultiLabelRule extends ObservableMOAObject {
	...
	protected List<Literal> literalList = new LinkedList<>();
	...
	public void getDescription(StringBuilder out, int indent) {
		StringUtils.appendIndented(out, indent + 1, "Rule Nr." + this.ruleNumberID + " Instances seen:"
	...
	public void trainOnInstance(Instance instance) {
		if (this.instanceInformation == null)
			this.instanceInformation = instance.dataset();
	...
	public boolean tryToExpand(double splitConfidence, double tieThresholdOption) {
		...
		if (hasExpanded) {
			...
			// if && this.literalList.size()>0 is removed then consider including this code
			// for default rule also
			if (otherOutputsLiteral != null && this.literalList.size() > 0) { // only add "other outputs" rule if
																				// antecedent is not empty
				otherOutputsRule = new MultiLabelRule();
				otherOutputsRule.instanceInformation = instanceInformation;
				otherOutputsRule.literalList = new LinkedList<>(this.literalList);
