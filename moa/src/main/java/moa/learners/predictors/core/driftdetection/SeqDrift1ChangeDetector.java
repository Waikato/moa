...
import moa.learners.predictors.core.driftdetection.SeqDrift2ChangeDetector.Repository;
import moa.tasks.TaskMonitor;

/**
 * SeqDrift1ChangeDetector.java. This extends Abstract Change Detector as
 * required by MOA.
 *
...
public class SeqDrift1ChangeDetector extends AbstractChangeDetector {
	...
	public FloatOption deltaOption = new FloatOption("deltaSeqDrift1", 'd', "Delta of SeqDrift1 change detection", 0.01,
			0.0, 1.0);

	public FloatOption deltaWarningOption = new FloatOption("deltaWarningOption", 'w',
			"Delta of SeqDrift1 change detector to declare warning state", 0.1, 0.0, 1.0);

	public IntOption blockSeqDriftOption = new IntOption("blockSeqDrift1Option", 'b',
			"Block size of SeqDrift1 change detector", 200, 100, 10000);
	...
	public void resetLearning() {
		seqDrift1 = new SeqDrift1(this.deltaOption.getValue(), (this.blockSeqDriftOption.getValue()),
				(this.deltaWarningOption.getValue()));
	}
	...
	public class SeqDrift1 extends AbstractMOAObject {
		...
		public SeqDrift1(double _significanceLevel, int _blockSize, double _significanceWarningLevel) {
			...
			slidingWindowBlockCount = (int) (1 / _significanceLevel); // Sliding window size is automaticall set as
																		// 1/delta
		...
		public boolean setInput(double _inputValue) {
			...
			if ((instanceCount % sampleSize) == 0) // Drift point check
			{
				...
				if (iDriftType == DRIFT) {
					...
					return false;// In warning state no instance is moved from sliding window to repository. Thus
									// returning now
				} /*
					 * else if(iDriftType == INTERNAL_DRIFT) { isWarning = false; isDrift = false;
					 * sampleSize = blockSize; clearrepository(); //Due to drift clear the data that
					 * belong to the old concept in repository movefromsliding windowTorepository();
					 * return false; }
					 */
				else {
					isWarning = false;
					moveValuesFromRightToLeft(); // All instances in sliding window should be moved to repository if no
													// drift or if drift but not in warning
					return false;
				}
			}
			return false;
		...
		private void addToRightRepository(double _inputValue) {
			if ((rightRepository.getSize() < sampleSize) || isWarning) // By default sliding window should have maximum
																		// of blockSize instances except in warning
																		// state
			{
				rightRepository.add(new Double(_inputValue));
			} else {
				System.out.println("request to add to sliding window sliding window size :" + rightRepository.getSize()
						+ " Warning :" + isWarning);
			}
		}

		/**
		 * Removes excess instances in sliding window when the number of blocks is more
		 * than the sliding window size threshold
		 * <p>
		 * 
		 * @param void
		 * @return void
		...
		private void moveValuesFromRightToLeft() {
			for (int iIndex = 0; iIndex < rightRepository.getSize(); iIndex++) // Copy all instances from sliding window
																				// to repository
			{
			...
			blockCount = blockCount + rightRepository.getSize() / blockSize; // Determine the block counter value after
		 ...
		 * <p>
		 * 
		 * @param _bIsWarning Drift detection in warning significance level or actual
		 *                    significance level
