...
import moa.learners.predictors.core.functions.AMRulesFunction;
...
import moa.learners.predictors.rules.core.NumericRulePredicate;
import moa.learners.predictors.rules.core.Utils;
import moa.tasks.TaskMonitor;

/**
 * This class contains the functions for learning the literals for Multi-label
 * classification (in same way as Multi-Target regression). used by multi-label
 * AMRules.
 * 
 * @author RSousa * @version $Revision: 1 $
 */
...
public class LearningLiteralClassification extends LearningLiteral {
	...
	private double normalizeOutputValue(int outputToLearnIndex, double value) {
		double meanY = this.literalStatistics[outputToLearnIndex].getValue(1)
				/ this.literalStatistics[outputToLearnIndex].getValue(0);
		double sdY = Utils.computeSD(this.literalStatistics[outputToLearnIndex].getValue(2),
	...
	public boolean tryToExpand(double splitConfidence, double tieThreshold) {
		...
		// if merit==0 it means the split have not enough examples in the smallest
		// branch
		if (sumMerit == 0)
		...
		if (bestSplitSuggestions.length < 2) {
			...
			double hoeffdingBound = computeHoeffdingBound(splitCriterion.getRangeOfMerit(this.literalStatistics),
					splitConfidence, weightSeen);
		...
		if (shouldSplit) {
			...
			// check which branch is better and update bestSuggestion (in amrules the splits
			// are binary )
			...
			int[] newOutputs = outputSelector.getNextOutputIndices(newLiteralStatistics, literalStatistics,
					outputsToLearn);
			...
			otherBranchLearningLiteral.instanceTransformer = this.instanceTransformer;

			// keep a rule learning to the complement set of newOutputs

			// Set expanding branch
			// if is AMRulesFunction and the number of output attributes changes, start
			// learning a new predictor
			// should we do the same for input attributes (attributesMask)?. It would have
			// impact in RandomAMRules
			if (learner instanceof AMRulesFunction) { // Reset learning
				if (newOutputs.length != outputsToLearn.length) {
					...
					if (otherOutputs.length > 0) {
						...
						otherOutputsLearningLiteral.instanceTransformer = new InstanceOutputAttributesSelector(
								instanceHeader, otherOutputs);
					}
					// expanded
			...
			// just reset learning
				...
				// other outputs //TODO JD: Test for general learner (other than AMRules
				// functions
				if (newOutputs.length != outputsToLearn.length) {
					int[] otherOutputs = Utils.complementSet(outputsToLearn, newOutputs);
					if (otherOutputs.length > 0) {
			...
			expandedLearningLiteral.instanceTransformer = new InstanceOutputAttributesSelector(instanceHeader,
					newOutputs);
	...
	private AttributeExpansionSuggestion[] getBestSplitSuggestions(MultiLabelSplitCriterion criterion) {
		...
		for (int i = 0; i < this.inputsToLearn.length; i++) {
			if (attributesMask[inputsToLearn[i]]) { // Should always be true (check trainOnInstance(). Remove?
				...
				if (obs != null) {
					AttributeExpansionSuggestion bestSuggestion = obs.getBestEvaluatedSplitSuggestion(criterion,
							literalStatistics, inputsToLearn[i]);

					if (bestSuggestion == null) {
						// ALL attributes must have a best suggestion. Adding dummy suggestion with
						// minimal merit.
						bestSuggestion = new AttributeExpansionSuggestion(
	...
	public void trainOnInstance(Instance instance) {
		...
		if (!hasStarted) {
			...
			instanceHeader = instance.dataset();
			hasStarted = true;
		}
		...
		for (int i = 0; i < outputsToLearn.length; i++) {
			...
			double squaredSumShifted = weight * (target - EntropyShift[i]) * (target - EntropyShift[i]);// Not needed
																										// for now
			exampleStatistics[i] = new DoubleVector(
					new double[] { weight, sum, squaredSum, sumShifted, squaredSumShifted });
		...
		for (int i = 0; i < inputsToLearn.length; i++) {
			if (attributesMask[inputsToLearn[i]]) { // this is checked above. Remove?
				...
				if (obs == null) {
					...
					} else if (instance.attribute(inputsToLearn[i]).isNominal()) { // just to make sure its nominal (in
																					// the future there may be ordinal?
	...
	public String getStaticOutput(InstancesHeader instanceInformation) {
		...
		if (this.literalStatistics != null) {
			for (int i = 0; i < this.literalStatistics.length; i++) {
				sb.append(instanceInformation.outputAttribute(outputsToLearn[i]).name() + ": "
						+ literalStatistics[i].getValue(1) / literalStatistics[i].getValue(0) + " "); //
			}
		}
	...
	/*
	 * @Override public void resetLearning() { weightSeen=0; //hasStarted=false; }
	 */
}
