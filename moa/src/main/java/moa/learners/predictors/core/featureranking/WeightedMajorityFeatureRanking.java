...
package moa.learners.predictors.core.featureranking;
...
import moa.learners.predictors.core.ObservableMOAObject;
import moa.learners.predictors.core.featureranking.messages.ChangeDetectedMessage;
import moa.learners.predictors.core.featureranking.messages.MeritCheckMessage;
import moa.learners.predictors.core.featureranking.messages.RuleExpandedMessage;

/**
 * Weighted Majority Feature Ranking method João Duarte, João Gama,Feature
 * ranking in hoeffding algorithms for regression. SAC 2017: 836-841
 */
...
public class WeightedMajorityFeatureRanking extends AbstractFeatureRanking implements FeatureRanking {
	...
	public FloatOption meritThresholdOption = new FloatOption("meritThreshold", 'm',
	...
	public void update(ObservableMOAObject o, Object arg) {
		...
		if (arg instanceof MeritCheckMessage) {
			...
			// Merit check of an existing rule
				...
				List<Integer> updated = ri.update(msg.getMerits().getArrayRef(), attributesMask,
						meritThresholdOption.getValue());

				Iterator<Integer> it = updated.iterator();
				while (it.hasNext()) {
					int attIndex = it.next();

					this.attributeImportance[attIndex] = (attributeImportance[attIndex] * numRules - old[attIndex]
							+ ri.getAttributeImportance(attIndex)) / numRules;
				}
			}
		}
		// Rule expanded
	...
	public DoubleVector getFeatureRankings() {
		/*
		 * DoubleVector normRankings=new DoubleVector(); if(attributeImportance!=null){
		 * double total=0; for (int i=0; i<attributeImportance.numValues();i++)
		 * total+=attributeImportance.getValue(i);
		 * 
		 * // double[] aux= new double[attributeImportance.numValues()]; for (int i=0;
		 * i<attributeImportance.numValues();i++)
		 * aux[i]=attributeImportance.getValue(i)/total; normRankings.addValues(aux); }
		 * return normRankings;
		 */
	...
	public class RuleInformation {
		 ...
		 * and the attribute is being considered (attributesMask[i]==true) Returns the
		 * list of attributes whose merit had change
		 */
