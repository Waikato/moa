...
public interface AttributeClassObserver extends OptionHandler {
	...
	/**
	 * Updates statistics of this observer given an attribute value, a class and the
	 * weight of the instance observed
	 *
	 ...
	 */
	AttributeSplitSuggestion getBestEvaluatedSplitSuggestion(SplitCriterion criterion, double[] preSplitDist,
			int attIndex, boolean binaryOnly);

	void observeAttributeTarget(double attVal, double target);

	void observeAttributeValue(double attVal, double value, double weight);
