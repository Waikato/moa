...
public class EnsembleDriftDetectionMethods extends AbstractChangeDetector {
	...
	public ListOption changeDetectorsOption = new ListOption("changeDetectors", 'c', "Change Detectors to use.",
			new ClassOption("driftDetectionMethod", 'd', "Drift detection method to use.", ChangeDetector.class, "DDM"),
			new Option[0], ',');

	public MultiChoiceOption predictionOption = new MultiChoiceOption("prediction", 'l', "Prediction to use.",
			new String[] { "max", "min", "majority" }, new String[] { "Maximum", "Minimum", "Majority" }, 0);
	...
	// double[] res = {this.isChangeDetected ? 1 : 0, this.isWarningZone ? 1 : 0,
	// this.delay, this.estimation};
	// return res;
	// }
	...
	protected void prepareForUseImpl(TaskMonitor monitor, ObjectRepository repository) {
		...
		for (int i = 0; i < cds.length; i++) {
			...
			// + "...", -1.0);
			cds[i] = ((ChangeDetector) ((ClassOption) changeDetectorOptions[i]).materializeObject(monitor, repository))
					.copy();
