...
public interface StatisticalTest extends OptionHandler, Callable<Double> {
	 ...
	 * @return p-value
	 */
	double test(List<Instance> x, List<Instance> y);

	/**
	 * This method sets the instances for later use in concurrent scenarios. The
	 * test is performed by using the call() method.
	 * 
	 * @param x List of instances
	 * @param y List of instances
	 */
	void set(List<Instance> x, List<Instance> y);
}
