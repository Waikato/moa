 ...
 * @version $Revision: 7 $
 */
public class VFMLNumericAttributeClassObserver extends AbstractAttributeClassObserver
		...
		for (Bin bin : this.binList) {
			...
			if ((bestSuggestion == null) || (merit > bestSuggestion.merit)) {
				bestSuggestion = new AttributeSplitSuggestion(
						new NumericAttributeBinaryTest(attIndex, bin.upperBound, false), postSplitDists, merit);
			}
		}
