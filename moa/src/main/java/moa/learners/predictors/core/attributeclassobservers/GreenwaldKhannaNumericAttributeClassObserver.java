 ...
 * @version $Revision: 7 $
 */
public class GreenwaldKhannaNumericAttributeClassObserver extends AbstractAttributeClassObserver
	...
	public IntOption numTuplesOption = new IntOption("numTuples", 'n', "The number of tuples.", 10, 1,
			Integer.MAX_VALUE);
				...
				for (double cutpoint : cutpoints) {
					...
					if ((bestSuggestion == null) || (merit > bestSuggestion.merit)) {
						bestSuggestion = new AttributeSplitSuggestion(
								new NumericAttributeBinaryTest(attIndex, cutpoint, true), postSplitDists, merit);
					}
				}
			}
		}
