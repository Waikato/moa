 ...
 *
 */
package moa.learners.predictors.core.statisticaltests;
...
public class Cramer extends AbstractOptionHandler implements StatisticalTest {
	...
	public FloatOption confidenceLevelOption = new FloatOption("confidenceLevel", 'q',
			"The confidence level to use in the Cramer test.", 0.95, 0, 1);

	public IntOption replicatesOption = new IntOption("replicates", 'r', "Number of replications.", 1000, 1,
			Integer.MAX_VALUE);

	public MultiChoiceOption kernelOption = new MultiChoiceOption("kernel", 'f', "Kernel function to use.",
			new String[] { "CRAMER", "BAHR", "LOG", "FRAC A", "FRAC B" },
			new String[] { "CRAMER", "BAHR", "LOG", "FRAC A", "FRAC B" }, 0);

	public FloatOption maxMOption = new FloatOption("maxM", 'm', "Maximum M.", Math.pow(2, 14), 1, Float.MAX_VALUE);
	...
	private double cramerStatistic(int m, int n, double[][] lookup) {
		...
		double mm = m, nn = n;
		return mm * nn / (mm + nn) * (2 * this.sumCells(lookup, xind, yind) / (mm * nn)
				- this.sumCells(lookup, xind, xind) / (mm * mm) - this.sumCells(lookup, yind, yind) / (nn * nn));
	}
	...
	private Kritwert kritwertfft(double[] lambdasquare, double confLevel, double maxM, int k) {
		...
		double quantile = xx[xindex]
				+ (confLevel - Fx[xindex]) * (xx[xindex + 1] - xx[xindex]) / (Fx[xindex + 1] - Fx[xindex]);
		if (Fx[M / 2] < confLevel) {
	...
	public CramerTest cramerTest(List<Instance> x, List<Instance> y) {
		return this.cramerTest(x, y, this.confidenceLevelOption.getValue(), this.replicatesOption.getValue(),
				"ordinary", false, this.kernelOption.getChosenIndex(), this.maxMOption.getValue(),
				this.kOption.getValue());
	}

	public CramerTest cramerTest1(List<List<Double>> x, List<List<Double>> y) {
		return this.cramerTest1(x, y, this.confidenceLevelOption.getValue(), this.replicatesOption.getValue(),
				"ordinary", false, this.kernelOption.getChosenIndex(), this.maxMOption.getValue(),
				this.kOption.getValue());
	}

	public CramerTest cramerTest1(List<List<Double>> x, List<List<Double>> y, double confLevel, int replicates,
			String sim, boolean justStatistic, int kernel, double maxM, int k) {
	...
	private CramerTest compute(CramerTest RVAL, double[][] daten, int replicates, String sim, boolean justStatistic,
			int kernel, double maxM, int k) {
	...
	public CramerTest cramerTest(List<Instance> x, List<Instance> y, double confLevel, int replicates, String sim,
			boolean justStatistic, int kernel, double maxM, int k) {
		CramerTest RVAL = new CramerTest(0, 0, 0, 0, 0, 0, 0, confLevel, replicates, null, null, null);
		// if ((is.matrix(x))&&(is.matrix(y))) if (ncol(x)==ncol(y)) RVAL$d<-ncol(x)
	...
	public class CramerTest {
		...
		public CramerTest(int d, int m, int n, double pValue, double critValue, double statistic, double result,
				double confLevel, double replicates, double[] hypdistX, double[] hypdistFx, double[] ev) {
			super();
			this.d = d;
			this.m = m;
			this.n = n;
