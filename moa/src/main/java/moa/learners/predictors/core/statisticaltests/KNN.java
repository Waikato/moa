...
public class KNN extends AbstractOptionHandler implements StatisticalTest {
	...
	public IntOption kValueOption = new IntOption("kValue", 'k', "K value of the K nearest neighbours algorithm.", 5, 1,
			Integer.MAX_VALUE);
	...
	/**
	 * Computes, for each instance, the number of the k nearest neighbors that are
	 * from the same sample.
	 *
	 * @param points Instances of both samples put together, with the last column
	 *               having 1 for the first sample and 2 for the second sample.
	...
	private double[] do_del(double X, boolean log_p, double cum, double ccum, boolean lower, double x, double temp,
			boolean upper) {
		final int SIXTEN = 16;
