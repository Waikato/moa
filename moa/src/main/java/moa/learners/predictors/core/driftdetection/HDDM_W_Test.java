 ...
 *
 */
package moa.learners.predictors.core.driftdetection;
 ...
 * HDDM<sub><i>W</i>-test</sub> uses the EWMA statistic as estimator. It
 * receives as input a stream of real values and returns the estimated status of
 * the stream: STABLE, WARNING or DRIFT.
 * </p>
 *
 * <p>
 * I. Frias-Blanco, J. del Campo-Avila, G. Ramos-Jimenez, R. Morales-Bueno, A.
 * Ortiz-Diaz, and Y. Caballero-Mota, Online and non-parametric drift detection
 * methods based on Hoeffding's bound, IEEE Transactions on Knowledge and Data
 * Engineering, 2014. DOI 10.1109/TKDE.2014.2345382.
 * </p>
 *
 * <p>
 * Parameters:
 * </p>
 * <ul>
 * <li>-d : Confidence to the drift</li>
 * <li>-w : Confidence to the warning</li>
 * <li>-m : Controls how much weight is given to more recent data compared to
 * older data. Smaller values mean less weight given to recent data</li>
 * <li>-t : Option to monitor error increments and decrements (two-sided) or
 * only increments (one-sided)</li>
 * </ul>
 *
 * @author Isvani Frias-Blanco (ifriasb@udg.co.cu)
 *
...
public class HDDM_W_Test extends AbstractChangeDetector {
	...
	public FloatOption driftConfidenceOption = new FloatOption("driftConfidence", 'd', "Confidence to the drift", 0.001,
			0, 1);
	public FloatOption warningConfidenceOption = new FloatOption("warningConfidence", 'w', "Confidence to the warning",
			0.005, 0, 1);
	public FloatOption lambdaOption = new FloatOption("lambda", 'm',
			...
			0.050, 0, 1);
	public MultiChoiceOption oneSidedTestOption = new MultiChoiceOption("typeOfTest", 't',
			"Monitors error increments and decrements (two-sided) or only increments (one-sided)",
			new String[] { "One-sided", "Two-sided" }, new String[] { "One-sided", "Two-sided" }, 0);
	...
	private static SampleInfo sample1_IncrMonitoring, sample2_IncrMonitoring, sample1_DecrMonitoring,
			sample2_DecrMonitoring, total;
	...
	public void resetLearning() {
		super.resetLearning();
		HDDM_W_Test.total = new SampleInfo();
		HDDM_W_Test.sample1_DecrMonitoring = new SampleInfo();
		HDDM_W_Test.sample1_IncrMonitoring = new SampleInfo();
		HDDM_W_Test.sample2_DecrMonitoring = new SampleInfo();
		HDDM_W_Test.sample2_IncrMonitoring = new SampleInfo();
	...
	public void input(double value) {
		...
		if (total.EWMA_Estimator < 0) {
			...
			total.independentBoundedConditionSum = lambda * lambda
					+ auxDecayRate * auxDecayRate * total.independentBoundedConditionSum;
		}
	...
	public boolean detectMeanIncrement(SampleInfo sample1, SampleInfo sample2, double confidence) {
		...
		double bound = Math.sqrt((sample1.independentBoundedConditionSum + sample2.independentBoundedConditionSum)
				* Math.log(1 / confidence) / 2);
	...
	void updateIncrStatistics(double valor, double confidence) {
		...
		if (total.EWMA_Estimator + bound < incrCutPoint) {
			...
			if (sample2_IncrMonitoring.EWMA_Estimator < 0) {
				...
				sample2_IncrMonitoring.EWMA_Estimator = lambda * valor
						+ auxDecay * sample2_IncrMonitoring.EWMA_Estimator;
				sample2_IncrMonitoring.independentBoundedConditionSum = lambda * lambda
						+ auxDecay * auxDecay * sample2_IncrMonitoring.independentBoundedConditionSum;
			}
		}
	...
	void updateDecrStatistics(double valor, double confidence) {
		...
		if (total.EWMA_Estimator - epsilon > decrCutPoint) {
			...
			if (sample2_DecrMonitoring.EWMA_Estimator < 0) {
				...
				sample2_DecrMonitoring.EWMA_Estimator = lambda * valor
						+ auxDecay * sample2_DecrMonitoring.EWMA_Estimator;
				sample2_DecrMonitoring.independentBoundedConditionSum = lambda * lambda
						+ auxDecay * auxDecay * sample2_DecrMonitoring.independentBoundedConditionSum;
			}
		}
	}
