...
public class NaiveBayesMultinomial extends AbstractClassifier implements Classifier {
	...
	public FloatOption laplaceCorrectionOption = new FloatOption("laplaceCorrection", 'l', "Laplace correction factor.",
	 ...
	 * is in the this format: m_wordTotalForClass[wordAttribute][class]
	 */
	...
	public void getModelDescription(StringBuilder result, int indent) {
		...
		for (int c = 0; c < m_numClasses; c++) {
			result.append(m_headerInfo.classAttribute().value(c)).append("\t").append(Double.toString(m_probOfClass[c]))
					.append("\n");
		}
