...
public class AccuracyUpdatedEnsemble extends AbstractClassifier implements Classifier {
		...
		if (this.trainingWeightSeenByModel > 0.0) {
			for (int i = 0; i < this.learners.length; i++) {
				if (this.weights[i][0] > 0.0) {
					DoubleVector vote = this.learners[(int) this.weights[i][1]].getPredictionForInstance(inst)
							.asDoubleVector();
	...
	protected void processChunk() {
		...
		for (int i = 0; i < this.learners.length; i++) {
			this.weights[i][0] = 1.0 / (mse_r
					+ this.computeMse(this.learners[(int) this.weights[i][1]], this.currentChunk) + Double.MIN_VALUE);
		}
	...
	protected double computeMse(Classifier learner, InstancesHeader chunk) {
		...
		for (int i = 0; i < chunk.numInstances(); i++) {
			try {
				...
				if (voteSum > 0) {
					f_ci = learner.getPredictionForInstance(chunk.instance(i))
							.asDoubleArray()[(int) chunk.instance(i).classValue()] / voteSum;
					mse_i += (1 - f_ci) * (1 - f_ci);
				} else {
					mse_i += 1;
				}
