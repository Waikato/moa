...
// import moa.learners.predictors.MultiClassClassifier;
import moa.core.DoubleVector;
import moa.core.InstanceExample;
import moa.core.Measurement;
import moa.core.MiscUtils;
import moa.evaluation.evaluators.BasicClassificationPerformanceEvaluator;
...
import moa.learners.predictors.trees.ISOUPTreeRF;
 ...
 *
 * <p>
 * Adaptive Random Forest (ARF). The 3 most important aspects of this ensemble
 * classifier are: (1) inducing diversity through resampling; (2) inducing
 * diversity through randomly selecting subsets of features for node splits (See
 * moa.learners.predictors.trees.ARFHoeffdingTree.java); (3) drift detectors per base
 ...
 * <p>
 * See details in:<br>
 * Heitor Murilo Gomes, Albert Bifet, Jesse Read, Jean Paul Barddal, Fabricio
 * Enembreck, Bernhard Pfharinger, Geoff Holmes, Talel Abdessalem. Adaptive
 * random forests for evolving data stream classification. In Machine Learning,
 * DOI: 10.1007/s10994-017-5642-8, Springer, 2017.
 * </p>
 *
 * <p>
 * Parameters:
 ...
 * <li>-o : How the number of features is interpreted (4 options): "Specified m
 * (integer value)", "sqrt(M)+1", "M-(sqrt(M)+1)"</li>
 ...
 * <li>-u : Should use drift detection? If disabled then bkg learner is also
 * disabled</li>
 ...
 * @version $Revision: 1 $
 */
public class AdaptiveRandomForest extends AbstractMultiTargetRegressor
	...
	public ClassOption treeLearnerOption = new ClassOption("treeLearner", 'l', "Random Forest Tree.",
			MultiTargetRegressor.class, "iSOUPTreeRF");

	public IntOption ensembleSizeOption = new IntOption("ensembleSize", 's', "The number of trees.", 10, 1,
			Integer.MAX_VALUE);

	public MultiChoiceOption mFeaturesModeOption = new MultiChoiceOption("mFeaturesMode", 'o',
			"Defines how m, defined by mFeaturesPerTreeSize, is interpreted. M represents the total number of features.",
			new String[] { "Specified m (integer value)", "sqrt(M)+1", "M-(sqrt(M)+1)", "Percentage (M * (m / 100))" },
	...
	protected ARFBaseLearnerMTR[] ensemble;
	...
	public void trainOnInstanceImpl(Instance instance) {
		...
		for (int i = 0; i < this.ensemble.length; i++) {
			DoubleVector vote = this.ensemble[i].getPredictionForInstance(instance).asDoubleVector();
	...
	public Prediction getPredictionForInstance(Instance instance) {
		...
		for (int i = 0; i < this.ensemble.length; ++i) {
			DoubleVector vote = this.ensemble[i].getPredictionForInstance(testInstance).asDoubleVector();
	...
	protected void initEnsemble(Instance instance) {
		...
		this.ensemble = new ARFBaseLearnerMTR[ensembleSize];

		// TODO: this should be an option with default =
...
//        BasicClassificationPerformanceEvaluator classificationEvaluator = (BasicClassificationPerformanceEvaluator) getPreparedClassOption(this.evaluatorOption);
		...
		case AdaptiveRandomForest.FEATURES_PERCENT:
			// If subspaceSize is negative, then first find out the actual percent, i.e.,
			// 100% - m.
		...
		ISOUPTreeRF treeLearner = (ISOUPTreeRF) getPreparedClassOption(this.treeLearnerOption);
		treeLearner.resetLearning();

		for (int i = 0; i < ensembleSize; ++i) {
			treeLearner.subspaceSize = this.subspaceSize;
			this.ensemble[i] = new ARFBaseLearnerMTR(i, (ISOUPTreeRF) treeLearner.copy(),
					(BasicClassificationPerformanceEvaluator) classificationEvaluator.copy(), this.instancesSeen,
					!this.disableBackgroundLearnerOption.isSet(), !this.disableDriftDetectionOption.isSet(),
					driftDetectionMethodOption, warningDetectionMethodOption, false);
		}
	}

	/**
	 * Inner class that represents a single tree member of the forest. It contains
	 * some analysis information, such as the numberOfDriftsDetected,
	 */
	protected final class ARFBaseLearnerMTR extends AbstractMOAObject {
		public int indexOriginal;
		public long createdOn;
		public long lastDriftOn;
		public long lastWarningOn;
		public ISOUPTreeRF classifier;
		...
		// Bkg learner
		protected ARFBaseLearnerMTR bkgLearner;
		// Statistics
		...
		private void init(int indexOriginal, ISOUPTreeRF instantiatedClassifier,
		...
		public ARFBaseLearnerMTR(int indexOriginal, ISOUPTreeRF instantiatedClassifier,
				BasicClassificationPerformanceEvaluator evaluatorInstantiated, long instancesSeen,
				boolean useBkgLearner, boolean useDriftDetector, ClassOption driftOption, ClassOption warningOption,
				boolean isBackgroundLearner) {
			init(indexOriginal, instantiatedClassifier, evaluatorInstantiated, instancesSeen, useBkgLearner,
					useDriftDetector, driftOption, warningOption, isBackgroundLearner);
		}
		...
		public void trainOnInstance(Instance instance, double weight, long instancesSeen) {
			...
			// Should it use a drift detector? Also, is it a backgroundLearner? If so, then
			// do not "incept" another one.
			if (this.useDriftDetector && !this.isBackgroundLearner) {
				// boolean correctlyClassifies = this.classifier.(instance);
				// public double getAdwinError(Instance inst, int i) {
				double sum = 0.0;
				if (this.classifier instanceof ISOUPTree) {
					double[] normalizedError = ((ISOUPTree) this.classifier).getNormalizedError(instance,
							this.classifier.getPredictionForInstance(instance).asDoubleArray());
					for (int j = 0; j < instance.numOutputAttributes(); j++) {
						sum += normalizedError[j];
					}
				} else {
					Prediction prediction = this.classifier.getPredictionForInstance(instance);
					for (int j = 0; j < instance.numOutputAttributes(); j++) {
						sum += Math.abs(prediction.getPrediction(j) - instance.valueOutputAttribute(j));
					}
				}
				sum = sum / instance.numOutputAttributes();
				// return sum;
				// }
				// Check for warning only if useBkgLearner is active
				if (this.useBkgLearner) {
					// Update the warning detection method
					this.warningDetectionMethod.input(sum);
					// Check if there was a change
					if (this.warningDetectionMethod.getChange()) {
						this.lastWarningOn = instancesSeen;
						this.numberOfWarningsDetected++;
						// Create a new bkgTree classifier
						ISOUPTreeRF bkgClassifier = (ISOUPTreeRF) this.classifier.copy();
						...
						this.bkgLearner = new ARFBaseLearnerMTR(indexOriginal, bkgClassifier, bkgEvaluator,
								instancesSeen, this.useBkgLearner, this.useDriftDetector, this.driftOption,
								this.warningOption, true);

						// Update the warning detection object for the current object
						// (this effectively resets changes made to the object while it was still a bkg
						// learner).
						this.warningDetectionMethod = ((ChangeDetector) getPreparedClassOption(this.warningOption))
								.copy();
					}
				}
				...
				this.driftDetectionMethod.input(sum);
				// Check if there was a change
		...
		public double[] getVotesForInstance(Instance instance) {
			DoubleVector vote = new DoubleVector(this.classifier.getPredictionForInstance(instance).asDoubleArray());
	...
	protected class TrainingRunnable implements Runnable, Callable<Integer> {
		final private ARFBaseLearnerMTR learner;
		final private Instance instance;
		final private double weight;
		final private long instancesSeen;

		public TrainingRunnable(ARFBaseLearnerMTR learner, Instance instance, double weight, long instancesSeen) {
