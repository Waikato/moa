...
/**
 * Anticipative and Dynamic Adaptation to Concept Changes. Ensemble method for
 * data streams that adapts to concept changes and deals with concept
 * recurrence.
 *
...
public class ADACC extends DACC {
	 ...
	 */
	public FloatOption stabIndexSizeOption = new FloatOption("StabThr", 'z', "The threshold for stability", 0.8, 0, 1);
	/**
	 * Threshold for concept equivalence
	 */
	public FloatOption equivIndexSizeOption = new FloatOption("CeThr", 'q', "The threshold for concept equivalence",
			0.7, 0, 1);
	/**
	...
	/**
	 * Maximum number of snapshots (copies of classifiers kept in case of
	 * recurrence)
	 */
	...
	/**
	 * If the environment is stable enough, take a snapshot (a copy) of the best
	 * adaptive classifier and keep it for future use, in case of concept recurrence
	 */
	private void takeSnapshot() {
		...
		if (this.index >= this.theta_stab)
			if (addedPermanent == 0) {
				this.ensemble[this.ensemble.length - MAXPERMANENT
						+ addedPermanent] = (Classifier) getBestAdaptiveClassifier().copy();
				addedPermanent++;
			} else {
	...
	/**
	 * Returns the kappa statistics, a statistical measure of agreement in the
	 * predictions of 2 classifiers. Used as a measure of diversity of predictive
	...
	/**
	 * Returns the stability index of the adaptive ensemble of classifiers. The
	 * ensemble is considered stable here if its diversity level and error rates are
	 * low.
	 * 
