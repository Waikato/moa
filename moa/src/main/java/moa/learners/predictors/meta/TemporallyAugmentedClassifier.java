 ...
 * <li>-n : The number of old labels to include</li>
 * </ul>
 *
 * @author Bernhard Pfahringer (bernhard@cs.waikato.ac.nz)
 * @version $Revision: 1 $
...
public class TemporallyAugmentedClassifier extends AbstractClassifier implements Classifier {
	...
	public void initHeader(InstancesHeader dataset) {
		...
		ArrayList<Attribute> attrs = new ArrayList<>(numLabels + dataset.numAttributes());
	...
	public Instance extendWithOldLabels(Instance instance) {
		if (this.header == null) {
			...
			this.baseLearner.setModelContext(this.header.getEmptyHeader());
		}
		int numLabels = this.oldLabels.length;
		if (numLabels == 0) {
			return instance;
