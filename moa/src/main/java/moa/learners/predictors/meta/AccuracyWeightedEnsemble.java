...
public class AccuracyWeightedEnsemble extends AbstractClassifier implements Classifier {
	 ...
	 */
	public FloatOption memberCountOption = new FloatOption("memberCount", 'n',
			"The maximum number of classifier in an ensemble.", 15, 1, Integer.MAX_VALUE);

	/**
	 * Number of classifiers remembered and available for ensemble construction.
	 */
	public FloatOption storedCountOption = new FloatOption("storedCount", 'r',
			"The maximum number of classifiers to store and choose from when creating an ensemble.", 30, 1,
			Integer.MAX_VALUE);
	 ...
	 */
	public IntOption numFoldsOption = new IntOption("numFolds", 'f',
			"Number of cross-validation folds for candidate classifier testing.", 10, 1, Integer.MAX_VALUE);
	...
	protected void processChunk() {
		// Compute weights
		double candidateClassifierWeight = this.computeCandidateWeight(this.candidateClassifier, this.currentChunk,
				this.numFolds);
		...
		if (this.storedWeights != null) {
			...
			for (int i = 0; i < storeSize; i++) {
				if (i < this.ensemble.length) {
					...
					measurements[i] = new Measurement("Stored member weight " + (i + 1),
							this.storedWeights[storeSize - i - 1][0]);
				}
			}
		}
	...
	/**
	 * Removes the classifier at a given index from the model, thus decreasing the
	 * models size.
	 *
	 * @param index
	 */
