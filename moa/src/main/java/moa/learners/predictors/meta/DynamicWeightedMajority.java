...
public class DynamicWeightedMajority extends AbstractClassifier implements Classifier {
	...
	public ClassOption baseLearnerOption = new ClassOption("baseLearner", 'l', "Base classifiers to train.",
	...
	public void trainOnInstanceImpl(Instance inst) {
		...
		for (int i = 0; i < this.experts.size(); i++) {
			double[] pr = this.experts.get(i).getPredictionForInstance(inst).asDoubleArray();
		...
		if (this.epochs % this.periodOption.getValue() == 0) {
			...
			if (yHat != (int) inst.classValue()) {
				...
				Classifier classifier = (Classifier) ((Classifier) getPreparedClassOption(this.baseLearnerOption))
						.copy();
				classifier.resetLearning();
				this.experts.add(classifier);
				this.weights.add(1.0);
	...
	public Prediction getPredictionForInstance(Instance inst) {
		...
		Utils.normalize(Pr);
		return new ClassificationPrediction(Pr);
	}
