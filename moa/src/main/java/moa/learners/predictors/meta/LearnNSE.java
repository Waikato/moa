 ...
 * batches of data that experience constant or variable rate of drift, addition
 * or deletion of concept classes, as well as cyclical drift.
 * </p>
 *
 * <p>
 * Based on: Ryan Elwell and Robi Polikar. Incremental learning of concept drift
 * in non-stationary environments. IEEE Transactions on Neural Networks,
...
public class LearnNSE extends AbstractClassifier implements Classifier {
	...
	public IntOption periodOption = new IntOption("period", 'p', "Size of the environments.", 250, 1,
			Integer.MAX_VALUE);

	public FloatOption sigmoidSlopeOption = new FloatOption("sigmoidSlope", 'a',
	...
	public FloatOption sigmoidCrossingPointOption = new FloatOption("sigmoidCrossingPoint", 'b',
	...
	public IntOption ensembleSizeOption = new IntOption("ensembleSize", 'e', "Ensemble size.", 15, 1,
			Integer.MAX_VALUE);

	public MultiChoiceOption pruningStrategyOption = new MultiChoiceOption("pruningStrategy", 's',
			"Classifiers pruning strategy to be used.", new String[] { "NO", "AGE", "ERROR" },
			new String[] { "Don't prune classifiers", "Age-based", "Error-based" }, 0);
		...
		if (this.trainingWeightSeenByModel > 0.0) {
			for (int i = 0; i < this.ensemble.size(); i++) {
				if (this.ensembleWeights.get(i) > 0.0) {
					DoubleVector vote = this.ensemble.get(i).getPredictionForInstance(inst).asDoubleVector();
