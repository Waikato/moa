...
import moa.learners.predictors.trees.ASHoeffdingTree;
 ...
 * <li>after one node splits, if the number of split nodes of the ASHT tree is
 * higher than the maximum value, then it deletes some nodes to reduce its size
 * </ul>
 ...
 * size s will be reset about twice as often as trees with a size limit of 2s.
 * This creates a set of different reset-speeds for an ensemble of such trees,
 * and therefore a subset of trees that are a good approximation for the current
 ...
 * impact on the ensemble’s predictive performance. When the tree size exceeds
 ...
 * </ul>
 * The maximum allowed size for the n-th ASHT tree is twice the maximum allowed
 * size for the (n-1)-th tree. Moreover, each tree has a weight proportional to
 * the inverse of the square of its error, and it monitors its error with an
 * exponential weighted moving average (EWMA) with alpha = .01. The size of the
 * first tree is 2. <br/>
 * <br/>
 * With this new method, it is attempted to improve bagging performance by
 * increasing tree diversity. It has been observed that boosting tends to
 ...
 * Gavaldà. New ensemble methods for evolving data streams. In 15th ACM SIGKDD
 * International Conference on Knowledge Discovery and Data Mining, 2009.<br/>
 * <br/>
 * The learner must be ASHoeffdingTree, a Hoeffding Tree with a maximum size
 * value.<br/>
 * <br/>
 * Example:<br/>
 * <br/>
 ...
 * @version $Revision: 7 $
 */
public class OzaBagASHT extends AbstractClassifier implements Classifier {
	...
	public ClassOption baseLearnerOption = new ClassOption("baseLearner", 'l', "ASHoeffdingTree to train.",
			ASHoeffdingTree.class, "ASHoeffdingTree");
	...
	public void trainOnInstanceImpl(Instance inst) {
		...
		for (int i = 0; i < this.ensemble.length; i++) {
			...
			if (k > 0) {
				...
				if (Utils.maxIndex(this.ensemble[i].getPredictionForInstance(inst)) == trueClass) {
