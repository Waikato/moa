...
/**
 * Dynamic Adaptation to Concept Changes. Ensemble method for data streams that
 * adapts to concept changes.
 *
 * Reference: JABER, G., CORNUEJOLS, A., and TARROUX, P. A New On-Line Learning
 * Method for Coping with Recurring Concepts: The ADACC System. In : Neural
 * Information Processing. Springer Berlin Heidelberg, 2013. p. 595-604.
 *
 * @author Ghazal Jaber (ghazal.jaber@gmail.com)
 *
 */
...
public class DACC extends AbstractClassifier implements Classifier {
	 ...
	 * Base classifier
	 */
	public ClassOption learnerOption = new ClassOption("baseLearner", 'l', "Classifier to train.", Classifier.class,
			"bayes.NaiveBayes");
	/**
	 * Ensemble size
	 */
	public FloatOption memberCountOption = new FloatOption("ensembleSize", 'n',
			"The maximum number of classifiers in an ensemble.", 20, 1, Integer.MAX_VALUE);
	/**
	 * Maturity age of classifiers
	 */
	public FloatOption maturityOption = new FloatOption("maturity", 'a', "The maturity age.", 20, 0, 100);
	/**
	 * Size of the evaluation window for weights computing
	 */
	public FloatOption evaluationSizeOption = new FloatOption("evalSize", 'e', "The size of the evaluation window.", 20,
			1, 1000);
	/**
	 * Combination functions: MAX and WVD (MAX leads to a faster reactivity to the
	 * change, WVD is more robust to noise)
	 */
	public MultiChoiceOption combinationOption = new MultiChoiceOption("cmb", 'c', "The combination function.",
			new String[] { "MAX", "WVD" }, new String[] { "Maximum", "Weighted Vote of the best" }, 0);
	/**
	...
	/**
	 * Receives a training instance from the stream and updates the adaptive
	...
	/**
	 * Updates the evaluation window of a classifier and returns the updated weight
	 * value.
	 * 
	 * @param index the index of the classifier in the ensemble
	...
	/**
	 * Returns the best (or worst) half of classifiers in the adaptive ensemble. The
	 * best classifiers are used to compute the stability index in ADACC. The worst
	 * classifiers are returned in order to select a classifier for deletion.
	 * 
	 * @param bestHalf boolean value set to true (false) if we want to return the
	 *                 best (worst) half of adaptive classifiers.
	 * @param horizon
	 * @return an array containing the weight values of the corresponding
	 *         classifiers and their indexes in the ensemble.
	 */
	...
	/**
	 * Returns the classifiers that vote for the final prediction when the MAX
	...
	protected ArrayList<Integer> getMAXIndexes() {
		...
		ArrayList<Integer> maxWIndex = new ArrayList<>();
	...
	/**
	 * Returns the classifiers that vote for the final prediction when the WVD
	 * combination function is selected
	 * 
	 * @return the classifiers whose weights lie in the higher half of the
	 *         ensemble's weight interval.
	 */
	...
	/**
	 * Returns the number of classifiers used for prediction which includes the
	 * adaptive learners and the snapshots in ADACC
	 * 
	...
	/**
	 * Returns the number of adaptive classifiers in the ensemble which excludes the
	 * static snapshots in ADACC
	 * 
	...
	/**
	 * This helper class is used to sort an array of pairs of integers: val and
	 * index. The array is sorted based on the val field.
	 * 
	 * @author Ghazal Jaber
	 *
	 */
