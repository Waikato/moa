...
import moa.evaluation.evaluators.BasicRegressionPerformanceEvaluator;
...
import moa.learners.predictors.trees.ARFFIMTDD;
import moa.options.ClassOption;

/**
 * Implementation of AdaptiveRandomForestRegressor, an extension of
 * AdaptiveRandomForest for classification.
 *
 * <p>
 * See details in:<br>
 ...
 * </p>
 */
public class AdaptiveRandomForestRegressor extends AbstractRegressor implements Regressor {
	...
	public ClassOption treeLearnerOption = new ClassOption("treeLearner", 'l', "Random Forest Tree.", ARFFIMTDD.class,
			"ARFFIMTDD");
	...
	public Prediction getPredictionForInstance(Instance instance) {
		...
		return new RegressionPrediction(predicted);
	}
	...
	protected void initEnsemble(Instance instance) {
		...
		for (int i = 0; i < ensembleSize; ++i) {
			...
			this.ensemble[i] = new ARFFIMTDDBaseLearner(i, (ARFFIMTDD) treeLearner.copy(),
					(BasicRegressionPerformanceEvaluator) regressionEvaluator.copy(), this.instancesSeen,
	...
	protected final class ARFFIMTDDBaseLearner extends AbstractMOAObject {
		...
		private void init(int indexOriginal, ARFFIMTDD instantiatedClassifier,
				BasicRegressionPerformanceEvaluator evaluatorInstantiated, long instancesSeen, boolean useBkgLearner,
		...
		public ARFFIMTDDBaseLearner(int indexOriginal, ARFFIMTDD instantiatedClassifier,
				BasicRegressionPerformanceEvaluator evaluatorInstantiated, long instancesSeen, boolean useBkgLearner,
				boolean useDriftDetector, ClassOption driftOption, ClassOption warningOption,
				boolean isBackgroundLearner) {
...
//                boolean correctlyClassifies = this.classifier.correctlyClassifies(instance);
				double prediction = this.classifier.getPredictionForInstance(instance).asDouble();
				...
				if (this.useBkgLearner) {
					...
					if (this.warningDetectionMethod.getChange()) {
						...
						BasicRegressionPerformanceEvaluator bkgEvaluator = (BasicRegressionPerformanceEvaluator) this.evaluator
								.copy();
						bkgEvaluator.reset();

						// Create a new bkgLearner object
						this.bkgLearner = new ARFFIMTDDBaseLearner(indexOriginal, bkgClassifier, bkgEvaluator,
...
//            return vote.getArrayRef();
			return this.classifier.getPredictionForInstance(instance).asDoubleArray();
		}
