...
import moa.learners.predictors.rules.AMRulesRegressor;
...
public class LocalMultiLabelClassifier extends AbstractMultiLabelClassifier {

//	public IntOption randomSeedOption = new IntOption("randomSeedOption",
//			'r', "randomSeedOption",
//			1,Integer.MIN_VALUE, Integer.MAX_VALUE);

	public LocalMultiLabelClassifier() {
		super.randomSeedOption = randomSeedOption;
		init();
	}

	protected void init() {
		baseLearnerOption = new ClassOption("baseLearner", 'l', "Classifier to train.", Classifier.class,
				AMRulesRegressor.class.getName());
	}
	...
	protected Measurement[] getModelMeasurementsImpl() {
		Measurement[] baseLearnerMeasurements = ((Classifier) getPreparedClassOption(this.baseLearnerOption))
	...
	public Prediction getPredictionForInstance(Instance instance) {
		...
		if (this.hasStarted) {
			...
			for (int i = 0; i < this.ensemble.length; i++) {
				...
				double[] votes = this.ensemble[i].getPredictionForInstance(transformInstance(instance, i))
						.asDoubleArray();
