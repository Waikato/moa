...
public abstract class HeterogeneousEnsembleAbstract extends AbstractClassifier {
	...
	public ListOption baselearnersOption = new ListOption("baseClassifiers", 'b',
			...
			new Option[] { new ClassOption("", ' ', "", Classifier.class, "bayes.NaiveBayes"),
	...
	public IntOption gracePerionOption = new IntOption("gracePeriod", 'g',
			"How many instances before we reevalate the best classifier", 1, 1, Integer.MAX_VALUE);

	public IntOption activeClassifiersOption = new IntOption("activeClassifiers", 'k',
			"The number of active classifiers (used for voting)", 1, 1, Integer.MAX_VALUE);

	public FlagOption weightClassifiersOption = new FlagOption("weightClassifiers", 'p',
	...
	public Prediction getPredictionForInstance(Instance inst) {
		...
		for (int i = 0; i < topK.size(); ++i) {
			double[] memberVotes = normalize(ensemble[topK.get(i)].getPredictionForInstance(inst).asDoubleArray());
			double weight = 1.0;
