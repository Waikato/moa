...
/**
 * The online version of the Accuracy Updated Ensemble as proposed by Brzezinski
 * and Stefanowski in "Combining block-based and online methods in learning
 * ensembles from concept drifting data streams", Information Sciences, 2014.
 */
public class OnlineAccuracyUpdatedEnsemble extends AbstractClassifier implements Classifier {
	...
	public FlagOption verboseOption = new FlagOption("verbose", 'v',
			"When checked the algorithm outputs additional information about component classifier weights.");

	/**
	 * Determines whether additional information should be sent to the output.
	 */
	public FlagOption linearOption = new FlagOption("linearFunction", 'f',
			"When checked the algorithm uses a linear weighting function.");
	...
	/**
	 * The mean square residual in a given moment, based on a window of latest
	 * examples.
	 */
	...
	public void prepareForUseImpl(TaskMonitor monitor, ObjectRepository repository) {
		...
		this.candidate = new ClassifierWithMemory(
				(Classifier) ((Classifier) getPreparedClassOption(this.learnerOption)).copy(), this.windowSize);
		...
		if (this.trainingWeightSeenByModel > 0.0) {
			for (int i = 0; i < this.ensemble.length; i++) {
				if (this.weights[i][0] > 0.0) {
					DoubleVector vote = this.ensemble[(int) this.weights[i][1]].classifier
	...
	/**
	 * Checks if the memory limit is exceeded and if so prunes the classifiers in
	 * the ensemble.
	 */
	protected void enforceMemoryLimit() {
		...
		for (int i = 0; i < this.ensemble.length; i++) {
			((HoeffdingTree) this.ensemble[(int) this.weights[i][1]].classifier).maxByteSizeOption
	 ...
	 * 
	 * @param i       the identifier (in terms of array learners) of the classifier
	 *                for which the weight is supposed to be computed
	...
	protected double computeWeight(int i, Instance example) {
		...
		if (t > d) {
			mse_it = this.ensemble[i].mse_it + e_it / d - this.ensemble[i].squareErrors[t % d] / d;
		} else {
			mse_it = this.ensemble[i].mse_it * (t - 1) / t + e_it / t;
		}
