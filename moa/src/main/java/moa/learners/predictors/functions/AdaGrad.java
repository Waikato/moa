...
/**
 * <!-- globalinfo-start --> Implements the AdaGrad oneline optimiser for
 * learning various linear models (binary class SVM, binary class logistic
 * regression and linear regression). For more information, see<br/>
 * <br/>
...
public class AdaGrad extends SGD {
	...
	public AdaGrad() {
		lambdaRegularizationOption = new FloatOption(lambdaRegularizationOption.getName(),
				lambdaRegularizationOption.getCLIChar(), lambdaRegularizationOption.getPurpose(), 0.0);

		learningRateOption = new FloatOption(learningRateOption.getName(), learningRateOption.getCLIChar(),
				learningRateOption.getPurpose(), 0.01);
	}
	...
	public void trainOnInstanceImpl(Instance instance) {
		...
		for (int i = 0; i < n; i++) {
			...
			gradients.setValue(idx,
					instance.valueSparse(i) * dldz + (m_lambda / (m_t + m_epsilon)) * m_weights.getValue(idx));
		}
