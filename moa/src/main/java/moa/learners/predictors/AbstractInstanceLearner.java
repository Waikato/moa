...
public abstract class AbstractInstanceLearner<MLTask extends InstanceLearner> extends AbstractOptionHandler
		implements InstanceLearner {
	...
	/**
	 * Creates an classifier and setups the random seed option if the classifier is
	 * randomizable.
	 */
	...
	public void prepareForUseImpl(TaskMonitor monitor, ObjectRepository repository) {
		...
		// resetLearning should not be called if the classifier has not yet received the
		// context
	...
	public void trainOnInstance(Instance inst) {
		...
		if (!(this instanceof SemiSupervisedInstanceLearner) && inst.missingOutputs()) {
			isTraining = false;
		}
		if (isTraining) {
	 ...
	 * Rule 4: attribute types must stay in the same order (although class can move;
	 * is always skipped over)<br>
	 * <br>
	 *
	...
	/**
	 * Resets this classifier. It must be similar to starting a new classifier from
	 * scratch. <br>
	 * <br>
	 * 
	 * The reason for ...Impl methods: ease programmer burden by not requiring them
	 * to remember calls to super in overridden methods. Note that this will produce
	 * compiler errors if not overridden.
	 */
	...
	/**
	 * Gets the index of the attribute in a set of instances, given the index of the
