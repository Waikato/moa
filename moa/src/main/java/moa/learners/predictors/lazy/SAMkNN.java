 ...
 *
 */
package moa.learners.predictors.lazy;
...
import moa.learners.clusterers.kmeanspm.CoresetKMeans;
 ...
 * -w max instances <br>
 * -m minimum number of instances in the STM <br>
 * -p LTM size relative to max instances <br>
 * -r Recalculation of the STM error <br>
 *
 * @author Viktor Losing (vlosing@techfak.uni-bielefeld.de) Paper: "KNN
 *         Classifier with Self Adjusting Memory for Heterogeneous Concept
 *         Drift" Viktor Losing, Barbara Hammer and Heiko Wersing
 *         http://ieeexplore.ieee.org/document/7837853 PDF can be found at
 *         https://pub.uni-bielefeld.de/download/2907622/2907623 BibTex:
 *         "@INPROCEEDINGS{7837853, author={V. Losing and B. Hammer and H.
 *         Wersing}, booktitle={2016 IEEE 16th International Conference on Data
 *         Mining (ICDM)}, title={KNN Classifier with Self Adjusting Memory for
 *         Heterogeneous Concept Drift}, year={2016}, pages={291-300},
 *         keywords={data mining;optimisation;pattern classification;Big
 *         Data;Internet of Things;KNN classifier;SAM-kNN robustness;data
 *         mining;k nearest neighbor algorithm;metaparameter
 *         optimization;nonstationary data streams;performance evaluation;self
 *         adjusting memory model;Adaptation models;Benchmark testing;Biological
 *         system modeling;Data mining;Heuristic algorithms;Prediction
 *         algorithms;Predictive models;Data streams;concept drift;data
 *         mining;kNN}, doi={10.1109/ICDM.2016.0040}, month={Dec} }"
 */
public class SAMkNN extends AbstractClassifier implements CapabilitiesHandler {
	...
	public IntOption limitOption = new IntOption("limit", 'w', "The maximum number of instances to store", 5000, 1,
			Integer.MAX_VALUE);
	public IntOption minSTMSizeOption = new IntOption("minSTMSize", 'm', "The minimum number of instances in the STM",
			50, 1, Integer.MAX_VALUE);

	public FloatOption relativeLTMSizeOption = new FloatOption("relativeLTMSize", 'p',
			"The allowed LTM size relative to the total limit.", 0.4, 0.0, 1.0);
	...
	private InstancesHeader stm;
	private InstancesHeader ltm;
	...
	protected void init() {
		...
		// store calculated STM distances in a matrix to avoid recalculation, are reused
		// in the STM adaption phase
	...
	public void setModelContext(InstancesHeader context) {
		try {
			this.stm = new InstancesHeader(context, 0);
			this.stm.setClassIndex(context.classIndex());
			this.ltm = new InstancesHeader(context, 0);
	...
	public void trainOnInstanceImpl(Instance inst) {
		...
		if (newWindowSize < oldWindowSize) {
			...
			InstancesHeader discardedSTMInstances = new InstancesHeader(this.stm, 0);
	...
	public Prediction getPredictionForInstance(Instance inst) {
		...
		try {
			...
			return new ClassificationPrediction(new double[inst.numClasses()]);
		}
	...
	/**
	 * Performs classwise kMeans++ clustering for given samples with corresponding
	 * labels. The number of samples is halved per class.
	 */
	private void clusterDown() {
		...
		for (int c = 0; c <= this.maxClassValue; c++) {
			...
			if (classSamples.size() > 0) {
				// used kMeans++ implementation expects the weight of each sample at the first
				// index,
				// make sure that the first value gets the uniform weight 1, overwrite class
				// value
	...
	private void cleanSingle(InstancesHeader cleanAgainst, int cleanAgainstindex, InstancesHeader toClean) {
		InstancesHeader cleanAgainstTmp = cleanAgainst.getEmptyHeader();
	...
	/**
	 * Removes distance-based all instances from the input samples that contradict
	 * those in the STM.
	 */
	private void clean(InstancesHeader cleanAgainst, InstancesHeader toClean, boolean onlyLast) {
	 ...
	 */
	private double[] getDistanceWeightedVotes(double distances[], int[] nnIndices, InstancesHeader instances) {
	...
	private double[] getDistanceWeightedVotesCM(double distances[], int[] nnIndices, InstancesHeader stm,
			InstancesHeader ltm) {
		double v[] = new double[this.maxClassValue + 1];
		for (int nnIdx : nnIndices) {
			if (nnIdx < stm.numInstances()) {
				...
				v[(int) ltm.instance((nnIdx - stm.numInstances())).classValue()] += 1.
						/ Math.max(distances[nnIdx], 0.000000001);
			}
		}
		return v;
	}
	 ...
	 */
	private double[] getCMVotes(double distancesSTM[], InstancesHeader stm, double distancesLTM[],
			InstancesHeader ltm) {
	...
	private int getLabelFct(double distances[], InstancesHeader instances, int startIdx, int endIdx) {
	...
	/**
	 * Returns the Euclidean distance between one sample and a collection of samples
	 * in an 1D-array.
	 */
	private double[] get1ToNDistances(Instance sample, InstancesHeader samples) {
	...
	/**
	 * Removes predictions of the largest window size and shifts the remaining ones
	 * accordingly.
	 */
	 ...
	 */
	private List<Integer> getIncrementalTestTrainPredHistory(InstancesHeader instances, int startIdx,
			List<Integer> predictionHistory) {
		for (int i = startIdx + this.kOption.getValue() + predictionHistory.size(); i < instances.numInstances(); i++) {
			predictionHistory.add((this.getLabelFct(distanceMatrixSTM[i], instances, startIdx, i - 1) == instances
					.get(i).classValue()) ? 1 : 0);
		}
	 ...
	 */
	private List<Integer> getTestTrainPredHistory(InstancesHeader instances, int startIdx) {
	...
	/**
	 * Returns the window size with the minimum Interleaved test-train error, using
	 * bisection (with recalculation of the STM error).
	 */
		...
		if (numSamples < 2 * this.minSTMSizeOption.getValue()) {
			...
			for (Integer numSamplesIt : numSamplesRange) {
				...
				if (this.predictionHistories.containsKey(idx)) {
					predHistory = this.getIncrementalTestTrainPredHistory(this.stm, idx,
							this.predictionHistories.get(idx));
				} else {
	...
	/**
	 * Returns the window size with the minimum Interleaved test-train error, using
	 * bisection (without recalculation using an incremental approximation).
	 */
	...
	/**
	 * Returns the bisected STM size which minimizes the interleaved-test-train
	 * error.
	 */
