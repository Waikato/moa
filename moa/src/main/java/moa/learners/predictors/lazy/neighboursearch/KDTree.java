...
import moa.learners.predictors.lazy.neighboursearch.kdtrees.KDTreeNode;
import moa.learners.predictors.lazy.neighboursearch.kdtrees.KDTreeNodeSplitter;
import moa.learners.predictors.lazy.neighboursearch.kdtrees.SlidingMidPointOfWidestSide;
...
public class KDTree extends NearestNeighbourSearch {
	...
	protected void addInstanceToTree(Instance inst, KDTreeNode node) throws Exception {
		if (node.isALeaf()) {
			...
			try {
				...
				System.err.println("m_InstList.length: " + m_InstList.length + " instList.length: " + instList.length
						+ "node.m_End+1: " + (node.m_End + 1) + "m_InstList.length-node.m_End+1: "
						+ (m_InstList.length - node.m_End - 1));
				throw ex;
			}
			m_InstList = instList;
	 ...
	 */
	public void centerInstances(InstancesHeader centers, int[] assignments, double pc) throws Exception {
	 ...
	 * @throws Exception If there is some problem assigning instances to centers.
	 */
	protected void determineAssignments(KDTreeNode node, InstancesHeader centers, int[] candidates, int[] assignments,
	 ...
	 */
	protected int[] refineOwners(KDTreeNode node, InstancesHeader centers, int[] candidates) throws Exception {
		...
		owner = centers.instance(candidates[ownerIndex]).copy();
		...
		for (int i = 0; i < numCand; i++) {
			...
			if ((inside[i])
				...
				Instance competitor = centers.instance(candidates[i]).copy();
				if
	...
	protected boolean candidateIsFullOwner(KDTreeNode node, Instance candidate, Instance competitor) throws Exception {
		// get extreme point
		Instance extreme = candidate.copy();
		for (int i = 0; i < m_Instances.numAttributes(); i++) {
		...
		boolean isFullOwner = m_EuclideanDistance.distance(extreme, candidate) < m_EuclideanDistance.distance(extreme,
				competitor);
	 ...
	 */
	public void assignSubToCenters(KDTreeNode node, InstancesHeader centers, int[] centList, int[] assignments)
			throws Exception {
	...
	public String normalizeNodeWidthTipText() {
		return "Whether if the widths of the KDTree node should be normalized "
				...
				+ "universe is the range of the split attribute based on all the " + "instances (default: false).";
	}
	...
	public String globalInfo() {
		return "Class implementing the KDTree search algorithm for nearest " + "neighbour search.\n"
				...
				+ "instances in the resulting node are more than " + "<maximal-inst-number> instances.\n"
