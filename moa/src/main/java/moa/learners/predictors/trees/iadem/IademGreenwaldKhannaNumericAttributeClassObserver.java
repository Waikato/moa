...
import moa.learners.predictors.core.attributeclassobservers.GreenwaldKhannaNumericAttributeClassObserver;
import weka.core.Utils;

public class IademGreenwaldKhannaNumericAttributeClassObserver extends GreenwaldKhannaNumericAttributeClassObserver
	...
	public void observeAttributeClass(double attVal, int classVal, double weight) {
		if (Utils.isMissingValue(attVal)) {
			...
			IademGreenwaldKhannaQuantileSummary valDist = (IademGreenwaldKhannaQuantileSummary) this.attValDistPerClass
					.get(classVal);
			if (valDist == null) {
	...
	public double probabilityOfAttributeValueGivenClass(double attVal, int classVal) {
		IademGreenwaldKhannaQuantileSummary obs = (IademGreenwaldKhannaQuantileSummary) this.attValDistPerClass
				.get(classVal);
		if (obs == null) {
			return 0.0;
		} else {
		...
		for (int i = 0; i < cuts.size(); i++) {
			...
			for (int j = 0; j < totalDist.length; j++) {
				...
				if (counts_cut_value[i][0] != 0) {
					lEst = lDist[j] / counts_cut_value[i][0];
				}
				double lError = 0.0;
				...
				if (counts_cut_value[i][1] != 0) {
					rEst = rDist[j] / counts_cut_value[i][1];
				}
