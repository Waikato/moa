...
/**
 * Adaptive decision option tree for streaming data with adaptive Naive Bayes
 * classification at leaves. An Adaptive Hoeffding Option Tree is a Hoeffding
 * Option Tree with the following improvement: each leaf stores an estimation of
 * the current error. It uses an EWMA estimator with alpha = .2. The weight of
 * each node in the voting process is proportional to the square of the inverse
 * of the error. <br/>
 * <br/>
 * Example:<br/>
 * <code>AdaHoeffdingOptionTree -o 50 </code> Parameters:
 * <ul>
 * <li>Same parameters as <code>HoeffdingOptionTreeNB<code>
 * </ul>
 *
 * @author Albert Bifet (abifet at cs dot waikato dot ac dot nz)
 * @version $Revision: 7 $
	...
	public static class AdaLearningNode extends LearningNodeNB {
		...
		public double[] getClassVotes(Instance inst, HoeffdingOptionTree ht) {
			...
			if (distSum * (1.0 - this.CorrectWeight) * (1.0 - this.CorrectWeight) > 0.0) {
				Utils.normalize(dist, distSum * (1.0 - this.CorrectWeight) * (1.0 - this.CorrectWeight)); // Adding
																											// weight
			}
			return dist;
		}
