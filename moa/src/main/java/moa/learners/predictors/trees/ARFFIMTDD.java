...
/**
 * Implementation of ARFFIMTDD, an extension of ARFFIMTDD to be used by
 * AdaptiveRandomForestRegressor.
 *
 * <p>
 * See details in:<br>
 * Heitor Murilo Gomes, Jean Paul Barddal, Luis Eduardo Boiko Ferreira, Albert
 * Bifet. Adaptive random forests for data stream regression. In European
...
public class ARFFIMTDD extends AbstractClassifier implements Regressor {
	...
	public IntOption subspaceSizeOption = new IntOption("subspaceSizeSize", 'k',
			"Number of features per subset for each node split. Negative values = #features - k", 2, Integer.MIN_VALUE,
			Integer.MAX_VALUE);

	// region ================ OPTIONS ================

	public ClassOption splitCriterionOption = new ClassOption("splitCriterion", 's', "Split criterion to use.",
			SplitCriterion.class, "core.splitcriteria.VarianceReductionSplitCriterion");
	...
	public FloatOption learningRatioOption = new FloatOption("learningRatio", 'l',
			"Learning ratio to used for training the Perceptrons in the leaves.", 0.02, 0, 1.00);
	...
	public static class LeafNode extends Node {
		...
		public void learnFromInstance(Instance inst, boolean growthAllowed, ARFFIMTDD arffimtdd) {
			...
			sumOfAbsErrors += inst.weight()
					* Math.abs(tree.normalizeTargetValue(Math.abs(inst.classValue() - getPrediction(inst))));
...
//                        }
	...
	public class FIMTDDPerceptron implements Serializable {
		...
		public DoubleVector normalizedInstance(Instance inst) {
			...
			if (tree.examplesSeen > 1)
				normalizedInstance.setValue(inst.numAttributes() - 1, 1.0); // Value to be multiplied with the constant
																			// factor
			else
	...
	public Prediction getPredictionForInstance(Instance inst) {
		...
		return new RegressionPrediction(prediction);
	}
					...
					if (iNode.lossExamplesSeen - iNode.previousWeight >= alternateTreeTMinOption.getValue()) {
						...
						if (Qi > 0) {
							...
							altTree = false;
								...
								&& iNode.lossExamplesSeen >= (10 * this.gracePeriodOption.getValue()))
