 ...
 * <li>-g : The number of instances a leaf should observe between split
 * attempts</li>
 ...
 * longer to decide</li>
 * <li>-w : The allowable error in secondary split decisions, values closer to 0
 * will take longer to decide</li>
 ...
 * @version $Revision: 7 $
 */
public class HoeffdingOptionTree extends AbstractClassifier implements Classifier, CapabilitiesHandler {
	...
	public IntOption maxOptionPathsOption = new IntOption("maxOptionPaths", 'o',
			"Maximum number of option paths per node.", 5, 1, Integer.MAX_VALUE);
	...
	public FloatOption secondarySplitConfidenceOption = new FloatOption("secondarySplitConfidence", 'w',
			"The allowable error in secondary split decisions, values closer to 0 will take longer to decide.", 0.1,
			0.0, 1.0);
	...
	public FileOption dumpFileOption = new FileOption("dumpFile", 'f', "File to append option table to.", null, "csv",
			true);
	...
	public static class Node extends AbstractMOAObject {
		...
		public FoundNode[] filterInstanceToLeaves(Instance inst, SplitNode parent, int parentBranch,
				boolean updateSplitterCounts) {
			List<FoundNode> nodes = new LinkedList<>();
			filterInstanceToLeaves(inst, parent, parentBranch, nodes, updateSplitterCounts);
			return nodes.toArray(new FoundNode[nodes.size()]);
		}

		public void filterInstanceToLeaves(Instance inst, SplitNode splitparent, int parentBranch,
	...
	private void addToOptionTable(AttributeSplitSuggestion bestSuggestion, SplitNode parent) {
		...
		if (dumpFile != null) {
			...
			immediateResultStream
					.println(this.trainingWeightSeenByModel + "," + treeDepth + "," + splitAtt + "," + splitVal);
