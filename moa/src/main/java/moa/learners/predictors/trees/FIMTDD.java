...
import moa.learners.predictors.core.attributeclassobservers.FIMTDDNumericAttributeClassObserver;
...
public class FIMTDD extends AbstractRegressor implements Regressor {
	...
	public ClassOption splitCriterionOption = new ClassOption("splitCriterion", 's', "Split criterion to use.",
			SplitCriterion.class, "VarianceReductionSplitCriterion");

	public IntOption gracePeriodOption = new IntOption("gracePeriod", 'g',
			"Number of instances a leaf should observe between split attempts.", 200, 0, Integer.MAX_VALUE);

	public FloatOption splitConfidenceOption = new FloatOption("splitConfidence", 'c',
			"Allowed error in split decision, values close to 0 will take long to decide.", 0.0000001, 0.0, 1.0);
	...
	public FloatOption PageHinckleyAlphaOption = new FloatOption("PageHinckleyAlpha", 'a',
			"Alpha value to use in the Page Hinckley change detection tests.", 0.005, 0.0, 1.0);

	public IntOption PageHinckleyThresholdOption = new IntOption("PageHinckleyThreshold", 'h',
			"Threshold value used in the Page Hinckley change detection tests.", 50, 0, Integer.MAX_VALUE);

	public FloatOption alternateTreeFadingFactorOption = new FloatOption("alternateTreeFadingFactor", 'f',
			"Fading factor used to decide if an alternate tree should replace an original.", 0.995, 0.0, 1.0);

	public IntOption alternateTreeTMinOption = new IntOption("alternateTreeTMin", 'y',
			"Tmin value used to decide if an alternate tree should replace an original.", 150, 0, Integer.MAX_VALUE);

	public IntOption alternateTreeTimeOption = new IntOption("alternateTreeTime", 'u',
			"The number of instances used to decide if an alternate tree should be discarded.", 1500, 0,
			Integer.MAX_VALUE);

	public FlagOption regressionTreeOption = new FlagOption("regressionTree", 'e',
			"Build a regression tree instead of a model tree.");

	public FloatOption learningRatioOption = new FloatOption("learningRatio", 'l',
			"Learning ratio to used for training the Perceptrons in the leaves.", 0.003, 0, 1.00);

	public FloatOption learningRateDecayFactorOption = new FloatOption("learningRatioDecayFactor", 'd',
			"Learning rate decay factor (not used when learning rate is constant).", 0.001, 0, 1.00);

	public FlagOption learningRatioConstOption = new FlagOption("learningRatioConst", 'p',
			"Keep learning rate constant instead of decaying.");

	public IntOption maxDepthOption = new IntOption("maxDepthFactor", 'b', "The maximum depth of the tree.",
	...
	public abstract static class Node extends AbstractMOAObject {
		...
		public long calcByteSize() {
			return SizeOf.sizeOf(this)
					+ (originalNode == null && alternateTree != null ? alternateTree.calcByteSize() : 0);
		}
	...
	public static class LeafNode extends Node {
		...
		public double errorModel = 0.0;
		public double errorTargetMean = 0.0;
		 ...
		 */
		public void learnFromInstance(Instance inst, boolean growthAllowed, double prediction) {
			...
			// sum of absolute errors
			sumOfAbsErrors += inst.weight()
					* Math.abs(tree.normalizeTargetValue(Math.abs(inst.classValue() - prediction)));

			if (tree.buildingModelTree()) {
				double predictionModel = getPredictionModel(inst);
				double predictionTargetMean = getPredictionTargetMean(inst);

				errorModel = errorModel * 0.95 + Math.abs(predictionModel - inst.classValue());
				errorTargetMean = errorTargetMean * 0.95 + Math.abs(predictionTargetMean - inst.classValue());
		...
		public void checkForSplit(Instance inst, double prediction) {
			...
			if (this.getLevel() < tree.maxDepthOption.getValue()
					&& examplesSeen - examplesSeenAtLastSplitEvaluation >= tree.gracePeriodOption.getValue()) {
	...
	public class FIMTDDPerceptron implements Serializable {
		...
		public void initializeWeights() {
			...
			instancesSeen = 0;
			for (int j = 0; j < tree.getModelContext().numInputAttributes() + 1; j++) {
				// The last index corresponds to the constant b
				weightAttribute[j] = 2 * tree.classifierRandom.nextDouble() - 1;
			}
			normalizeWeights();
		}
		...
		public void updateWeights(Instance inst, double learningRatio) {
			...
			for (int i = 0; i < weightAttribute.length; i++) {
				weightAttribute[i] += delta * learningRatio * normalizedInstance[i];
			}
		...
		public void normalizeWeights() {
			...
			for (int i = 0; i < this.weightAttribute.length; i++)
				sum += Math.abs(weightAttribute[i]);
			for (int i = 0; i < this.weightAttribute.length; i++)
				weightAttribute[i] /= sum;
		}

		public double[] normalizedInstance(Instance inst) {
			// Normalize Instance
			double[] normalizedInstance = new double[inst.numInputAttributes() + 1];
			for (int j = 0; j < inst.numInputAttributes(); j++) {
				double mean = tree.sumOfAttrValues.getValue(j) / tree.examplesSeen;
				double sd = computeSD(tree.sumOfAttrSquares.getValue(j), tree.sumOfAttrValues.getValue(j),
						tree.examplesSeen);
				if (inst.inputAttribute(j).isNumeric() && tree.examplesSeen > 1 && sd > 0.0000001)
					normalizedInstance[j] = (inst.valueInputAttribute(j) - mean) / (sd);
				else
					normalizedInstance[j] = 0;
			}
			normalizedInstance[inst.numInputAttributes()] = tree.examplesSeen > 1 ? 1 : 0; // Value to be multiplied
		...
		public double prediction(double[] instanceValues) {
			...
			for (int i = 0; i < instanceValues.length; i++)
				ret += instanceValues[i] * weightAttribute[i];
			return ret;
		}
		...
		private double denormalizePrediction(double normalizedPrediction) {
			...
			if (examplesSeen > 1)
				return normalizedPrediction * sd + mean;
			else
				return 0.0;
		}
		...
		public void getModelDescription(StringBuilder out, int indent) {
			...
			if (getModelContext() != null) {
				...
				out.append(" + " + weightAttribute[getModelContext().numInputAttributes()]);
			}
			StringUtils.appendNewline(out);
		}
	}
	...
	public long calcByteSize() {
		long size = (SizeOf.sizeOf(this)) + this.treeRoot.calcByteSize();
		size += (SizeOf.sizeOf(this.sumOfAttrValues)) + (int) (SizeOf.sizeOf(this.sumOfAttrSquares));
		return size;
	}

	@Override
	public Prediction getPredictionForInstance(Instance inst) {
		return new RegressionPrediction((treeRoot == null) ? 0 : treeRoot.getPrediction(inst));
	}
	...
	public void processInstance(Instance inst, Node node, double prediction, double normalError, boolean growthAllowed,
			boolean inAlternate) {
		...
		while (true) {
			if (currentNode instanceof LeafNode) {
					...
					if (iNode.lossExamplesSeen - iNode.previousWeight >= alternateTreeTMinOption.getValue()) {
						...
						if (Qi > 0) {
							...
							altTree = false;
								...
								&& iNode.lossExamplesSeen >= (10 * alternateTreeTMinOption.getValue())
				...
				if (iNode.changeDetection && !inAlternate) {
					if (iNode.PageHinckleyTest(normalError - iNode.sumOfAbsErrors / iNode.examplesSeen
							- PageHinckleyAlphaOption.getValue(), PageHinckleyThresholdOption.getValue())) {
		...
		if (bestSplitSuggestions.length < 2) {
			...
			// structures in each attribute observer
				...
				for (int i = 0; i < node.attributeObservers.size(); i++) {
					...
					if (obs != null) {
						obs.removeBadSplits(splitCriterion, secondBestSuggestion.merit / bestSuggestion.merit,
								bestSuggestion.merit, hoeffdingBound);
					}
				}
			}
		}
