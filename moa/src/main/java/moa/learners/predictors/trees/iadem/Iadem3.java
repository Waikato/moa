...
import moa.learners.predictors.core.conditionaltests.NominalAttributeMultiwayTest;
import moa.learners.predictors.core.conditionaltests.NumericAttributeBinaryTest;
...
public class Iadem3 extends Iadem2 {
	...
	public IntOption maxNestingLevelOption = new IntOption("maxNestingLevel", 'p',
			"Maximum level of nesting for alternative subtrees (-1 => unbounded).", 1, -1, Integer.MAX_VALUE);

	public IntOption maxSubtreesPerNodeOption = new IntOption("maxSubtreesPerNode", 'w',
			"Maximum number of alternative subtrees per split node (-1 => unbounded).", 1, -1, Integer.MAX_VALUE);
	...
	public AbstractChangeDetector getEstimatorCopy() {
		return (AbstractChangeDetector) ((AbstractChangeDetector) getPreparedClassOption(
	...
	public LeafNode newLeafNode(Node parent, long instTreeCountSinceVirtual, long instNodeCountSinceVirtual,
			double[] initialClassCount, Instance instance) {
		switch (this.leafPredictionOption.getChosenIndex()) {
		case 0: {
			return new AdaptiveLeafNode(this, parent, instTreeCountSinceVirtual, instNodeCountSinceVirtual,
					initialClassCount, newNumericClassObserver(), this.estimator,
					this.splitTestsOption.getChosenIndex() == 2, this.splitTestsOption.getChosenIndex() == 0, instance);
		}
		case 1: {
			return new AdaptiveLeafNodeNB(this, parent, instTreeCountSinceVirtual, instNodeCountSinceVirtual,
					initialClassCount, newNumericClassObserver(), this.naiveBayesLimit, this.estimator,
					this.splitTestsOption.getChosenIndex() == 2, this.splitTestsOption.getChosenIndex() == 0, instance);
		}
		case 2: {
			return new AdaptiveLeafNodeNBKirkby(this, parent, instTreeCountSinceVirtual, instNodeCountSinceVirtual,
					initialClassCount, newNumericClassObserver(), this.naiveBayesLimit,
					this.splitTestsOption.getChosenIndex() == 2, this.splitTestsOption.getChosenIndex() == 0,
					this.estimator, instance);
		}
		default: {
			return new AdaptiveLeafNodeWeightedVote(this, parent, instTreeCountSinceVirtual, instNodeCountSinceVirtual,
					initialClassCount, newNumericClassObserver(), this.naiveBayesLimit,
					this.splitTestsOption.getChosenIndex() == 2, this.splitTestsOption.getChosenIndex() == 0,
					this.estimator, instance);
		}
	...
	public class AdaptiveLeafNode extends LeafNode implements Serializable {
		...
		public AdaptiveLeafNode(Iadem3 arbol, Node parent, long instTreeCountSinceVirtual,
				long instNodeCountSinceVirtual, double[] initialClassCount,
				IademNumericAttributeObserver numericAttClassObserver, AbstractChangeDetector estimator,
				boolean onlyMultiwayTest, boolean onlyBinaryTest, Instance instance) {
			super(arbol, parent, instTreeCountSinceVirtual, instNodeCountSinceVirtual, initialClassCount,
			...
			for (int i = 0; i < instance.numAttributes(); i++) {
				if (instance.classIndex() != i && instance.attribute(i).isNominal()) {
					if ((!sort.isEmpty()) && (i == sort.first())) {
						...
						virtualChildren.set(i, new AdaptiveNominalVirtualNode((Iadem3) tree, this, i, onlyMultiwayTest,
								onlyBinaryTest));
					}
				} else if (instance.classIndex() != i && instance.attribute(i).isNumeric()) {
					virtualChildren.set(i,
							new AdaptiveNumericVirtualNode((Iadem3) tree, this, i, numericAttClassObserver));
				} else {
					virtualChildren.set(i, null);
				}
			}
		...
		public AdaptiveLeafNode[] doSplit(IademAttributeSplitSuggestion mejorExpansion, Instance instance) {
			...
			splitNode = (AdaptiveSplitNode) virtualChildren.get(mejorExpansion.splitTest.getAttsTestDependsOn()[0])
					.getNewSplitNode(instTreeCountSinceReal, parent, mejorExpansion, instance);
	...
	public class AdaptiveLeafNodeNB extends AdaptiveLeafNode {
		...
		public AdaptiveLeafNodeNB(Iadem3 tree, Node parent, long instTreeCountSinceVirtual,
				long instNodeCountSinceVirtual, double[] initialClassCount,
				IademNumericAttributeObserver numericAttClassObserver, int limitNaiveBayes,
				AbstractChangeDetector estimator, boolean onlyMultiwayTest, boolean onlyBinaryTest, Instance instance) {
			super(tree, parent, instTreeCountSinceVirtual, instNodeCountSinceVirtual, initialClassCount,
					numericAttClassObserver, estimator, onlyMultiwayTest, onlyBinaryTest, instance);
	...
	public class AdaptiveLeafNodeNBAdaptive extends AdaptiveLeafNodeNB {
		...
		public AdaptiveLeafNodeNBAdaptive(Iadem3 tree, Node parent, long instancesProcessedByTheTree,
			...
			super(tree, parent, instancesProcessedByTheTree, instancesProcessedByThisLeaf, classDist,
					observadorContinuos, naiveBayesLimit, estimator, onlyMultiwayTest, onlyBinaryTest, instance);
	...
	public class AdaptiveLeafNodeNBKirkby extends AdaptiveLeafNodeNB {
		...
		public AdaptiveLeafNodeNBKirkby(Iadem3 tree, Node parent, long instancesProcessedByTheTree,
				long instancesProcessedByThisLeaf, double[] classDist,
				IademNumericAttributeObserver observadorContinuos, int naiveBayesLimit, boolean onlyMultiwayTest,
				boolean onlyBinaryTest, AbstractChangeDetector estimator, Instance instance) {
			super(tree, parent, instancesProcessedByTheTree, instancesProcessedByThisLeaf, classDist,
	...
	public class AdaptiveLeafNodeWeightedVote extends AdaptiveLeafNodeNBAdaptive {
		...
		public AdaptiveLeafNodeWeightedVote(Iadem3 tree, Node parent, long instTreeCountSinceVirtual,
				long instNodeCountSinceVirtual, double[] classDist, IademNumericAttributeObserver observadorContinuos,
				int naiveBayesLimit, boolean onlyMultiwayTest, boolean onlyBinaryTest, AbstractChangeDetector estimator,
				Instance instance) {
			super(tree, parent, instTreeCountSinceVirtual, instNodeCountSinceVirtual, classDist, observadorContinuos,
					naiveBayesLimit, onlyMultiwayTest, onlyBinaryTest, estimator, instance);
		}
		...
		protected boolean isSignificantlyGreaterThan(double mean1, double mean2, int n1, int n2) {
			double m = 1.0 / n1 + 1.0 / n2, confidence = 0.001, log = Math.log(1.0 / confidence),
	...
	public class AdaptiveNominalVirtualNode extends NominalVirtualNode
		...
		public AdaptiveNominalVirtualNode(Iadem3 tree, Node parent, int attID, boolean onlyMultiwayTest,
		...
		@Override
		public SplitNode getNewSplitNode(long counter, Node parent, IademAttributeSplitSuggestion bestSplit,
				Instance instance) {
			AdaptiveSplitNode splitNode = new AdaptiveSplitNode((Iadem3) this.tree, parent, null,
					((LeafNode) this.parent).getMajorityClassVotes(instance), bestSplit.splitTest,
					((AdaptiveLeafNode) this.parent).estimator, (AdaptiveLeafNode) this.parent,
	...
	public class AdaptiveNumericVirtualNode extends NumericVirtualNode
		...
		public AdaptiveNumericVirtualNode(Iadem3 tree, Node parent, int attID,
	...
	public class AdaptiveSplitNode extends SplitNode implements Serializable {
		...
		protected AutoExpandVector<Iadem3Subtree> alternativeTree = new AutoExpandVector<>();
		...
		public AdaptiveSplitNode(Iadem3 tree, Node parent, Node[] child, double[] freq,
				InstanceConditionalTest splitTest, AbstractChangeDetector estimator, AdaptiveLeafNode predictionLeaf,
				int causeOfSplit) {
		...
		private Node checkAlternativeSubtrees(boolean acierto, Instance instance) throws IademException {
			if (this.estimator != null) {
				...
				for (int i = 0; i < this.alternativeTree.size(); i++) {
					...
					double bound = IademCommonProcedures.AverageComparitionByHoeffdingCorollary(
					...
					if (thisError - treeError > bound/**/) {
					...
					} else if (this.estimator.getDelay() > 6000 && subtree.windowWidth() > 6000/**/) {
						{
							if (treeError - thisError > bound) {
								...
								((Iadem3) this.tree).updateNumberOfNodes(-subtree.getNumberOfNodes());
								((Iadem3) this.tree)
										.updateNumberOfNodesSplitByTieBreaking(-subtree.numSplitsByBreakingTies);
								i--;
							} else /**/ {
		...
		void createTree(Instance instance) throws IademException {
			...
			if (iadem3Tree.canCreateSubtree()) {
				...
				if ((maxTreeLevel == -1 || iadem3Tree.getTreeLevel() < maxTreeLevel)
					...
					if (this.estimator != null) {
						Iadem3Subtree subtree = new Iadem3Subtree(this, iadem3Tree.getTreeLevel() + 1,
								(Iadem3) this.tree, instance);
