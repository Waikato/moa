...
import moa.learners.predictors.core.attributeclassobservers.VFMLNumericAttributeClassObserver;
...
public class IademVFMLNumericAttributeClassObserver extends VFMLNumericAttributeClassObserver
		...
		if (Utils.isMissingValue(attVal)) {
			...
			if (this.binList.size() < 1) {
				...
				while ((min <= max) && !found) {
					...
					if (((attVal >= bin.lowerBound) && (attVal < bin.upperBound)) || ((i == this.binList.size() - 1)
				...
				if ((bin.lowerBound == attVal) || (this.binList.size() >= this.numBinsOption.getValue())) {// Option.getValue())
					// {//1000)
					// {
		...
		for (Bin bin : this.binList) {
			...
			if ((bestSuggestion == null) || (merit > bestSuggestion.merit)) {
				bestSuggestion = new AttributeSplitSuggestion(
						new IademNumericAttributeBinaryTest(attIndex, bin.upperBound, false), postSplitDists, merit);
			}
		}
		...
		for (int i = 0; i < binList.size(); i++) {
			if (attVal == this.binList.get(i).lowerBound && classVal == this.binList.get(i).boundaryClass) {
				...
				attValClassValCount = this.binList.get(i).boundaryWeight;
					...
					|| ((i == this.binList.size() - 1) && (attVal >= this.binList.get(i).lowerBound)
		...
		if (Utils.isMissingValue(attVal)) {
			...
			while (min <= max && !found) {
				...
				if ((attVal >= bin.lowerBound && attVal < bin.upperBound)
						|| ((i == numBins - 1) && (attVal >= bin.lowerBound) && (attVal <= bin.upperBound))) {
					found = true;
					index = i;
				} else if (attVal < bin.lowerBound) {
					max = i - 1;
			...
			if (bin.boundaryWeight == 0 && binList.size() > 0) {
				...
				if (index != 0) {
					 ...
					 * oldFirstBin->lowerBound, oldFirstBin->upperBound, oldFirstBin->exampleCount,
					 * oldFirstBin->boundryCount);
					 */
					 ...
					 * newFirstBin->upperBound, newFirstBin->exampleCount,
					 * newFirstBin->boundryCount);
					 */
					binList.remove(0);
				}
			}
	...
	public void computeClassDistProbabilities(double[][][] cut_value_classDist_lower,
			double[][][] cut_value_classDist_upper, double[][] counts_cut_value, boolean withIntervalEstimates) {
		int classes = this.classDist.numValues();
		Bin element = binList.get(0);
	...
	public ArrayList<Double> cutPointSuggestion(int numMaxIntervalos) {
		ArrayList<Double> cutPoint = new ArrayList<>();
	...
	public ArrayList<Double[]> computeConditionalProbPerBin(ArrayList<Double> cortes) {
		ArrayList<Double[]> condProb = new ArrayList<>();
