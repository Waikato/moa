...
import moa.learners.predictors.bayes.NaiveBayes;
...
public class EFDT extends AbstractClassifier implements Classifier {

	private static final long serialVersionUID = 2L;

	public IntOption reEvalPeriodOption = new IntOption("reevaluationPeriod", 'R',
			"The number of instances an internal node should observe between re-evaluation attempts.", 2000, 0,
			Integer.MAX_VALUE);
	 ...
	 * "VFML1000", "BINTREE"}, new String[]{ "Gaussian approximation evaluating 10
	 * splitpoints", "Gaussian approximation evaluating 100 splitpoints",
	 * "Greenwald-Khanna quantile summary with 10 tuples", "Greenwald-Khanna
	 * quantile summary with 100 tuples", "Greenwald-Khanna quantile summary with
	 * 1000 tuples", "VFML method with 10 bins", "VFML method with 100 bins", "VFML
	 * method with 1000 bins", "Exhaustive binary tree"}, 0);
	 */
	...
	public Prediction getPredictionForInstance(Instance inst) {
		if (this.treeRoot != null) {
			...
			return new ClassificationPrediction(new double[numClasses]);
		}
	}
	...
	public void deactivateAllLeaves() {
		...
		for (FoundNode learningNode : learningNodes) {
			if (learningNode.node instanceof ActiveLearningNode) {
				deactivateLearningNode((ActiveLearningNode) learningNode.node, learningNode.parent,
		...
		if (!node.observedClassDistributionIsPure()) {
			...
			for (AttributeSplitSuggestion bestSplitSuggestion : bestSplitSuggestions) {
				...
				if (bestSplitSuggestion.splitTest != null) {
					...
					double currentSum = node.getInfogainSum()
							.get((bestSplitSuggestion.splitTest.getAttsTestDependsOn()[0]));
					node.getInfogainSum().put((bestSplitSuggestion.splitTest.getAttsTestDependsOn()[0]),
			...
			if (bestSplitSuggestions.length < 2) {
				...
				// because this is an unsplit leaf. current average merit should be always zero
				// on the null split.
				...
				if (bestSuggestion.splitTest == null) { // if you have a null split
					...
					bestSuggestionAverageMerit = node.getInfogainSum()
							.get((bestSuggestion.splitTest.getAttsTestDependsOn()[0])) / node.getNumSplitAttempts();
				}
			...
			if (shouldSplit) {
				...
				if (splitDecision.splitTest == null) {
					...
					Node newSplit = newSplitNode(splitDecision.splitTest, node.getObservedClassDistribution(),
							splitDecision.numSplits());
					((EFDTSplitNode) newSplit).attributeObservers = node.attributeObservers; // copy the attribute
																								// observers
	...
	public class EFDTSplitNode extends SplitNode implements EFDTNode {
		...
		// DRY Don't Repeat Yourself... code duplicated from ActiveLearningNode in
		// VFDT.java. However, this is the most practical way to share stand-alone.
		...
		public void learnFromInstance(Instance inst, EFDT ht, EFDTSplitNode parent, int parentBranch) {
			...
			this.observedClassDistribution.addToValue((int) inst.classValue(), inst.weight()); // update prior
																								// (predictor)
			...
			// check if a better split is available. if so, chop the tree at this point,
			// copying likelihood. predictors for children are from parent likelihood.
		...
		protected void reEvaluateBestSplit(EFDTSplitNode node, EFDTSplitNode parent, int parentIndex) {
			...
			// EFDT must transfer over gain averages when replacing a node: leaf to split,
			// split to leaf, or split to split
			...
			if (this.splitTest != null) {
				...
				// given the current implementations in MOA, we're only ever expecting one int
				// to be returned
			}
			...
			for (AttributeSplitSuggestion bestSplitSuggestion : bestSplitSuggestions) {
				...
				else { // handle the null attribute. this is fine to do- it'll always average zero, and
						// we will use this later to potentially burn bad splits.
					double currentSum = node.getInfogainSum().get(-1); // null split
			...
			if (node.splitTest == null) { // current is null- shouldn't happen, check for robustness
				...
				currentAverageMerit = node.getInfogainSum().get(node.splitTest.getAttsTestDependsOn()[0])
			...
			if (deltaG > hoeffdingBound || (hoeffdingBound < tieThreshold && deltaG > tieThreshold / 2)) {
				...
				if (splitDecision.splitTest == null) {
					...
					replacement.setInfogainSum(node.getInfogainSum()); // transfer infogain history, split to
																		// replacement leaf
					...
					((EFDTSplitNode) newSplit).attributeObservers = node.attributeObservers; // copy the attribute
																								// observers
					newSplit.setInfogainSum(node.getInfogainSum()); // transfer infogain history, split to replacement
																	// split

					if (node.splitTest == splitDecision.splitTest
							&& node.splitTest.getClass() == NumericAttributeBinaryTest.class
							&& (argmax(splitDecision.resultingClassDistributions[0]) == argmax(
									node.getChild(0).getObservedClassDistribution())
									|| argmax(splitDecision.resultingClassDistributions[1]) == argmax(
											node.getChild(1).getObservedClassDistribution()))) {
						...
						for (int i = 0; i < splitDecision.numSplits(); i++) {
							...
							if (splitDecision.splitTest.getClass() == NominalAttributeBinaryTest.class
									|| splitDecision.splitTest.getClass() == NominalAttributeMultiwayTest.class) {
								newChild.usedNominalAttributes = new ArrayList<>(node.usedNominalAttributes); // deep
																												// copy
								newChild.usedNominalAttributes.add(splitDecision.splitTest.getAttsTestDependsOn()[0]);
								// no nominal attribute should be split on more than once in the path
							}
		...
		protected void attemptToSplit(ActiveLearningNode node, SplitNode parent, int parentIndex) {
			if (!node.observedClassDistributionIsPure()) {
				...
				for (int i = 0; i < bestSplitSuggestions.length; i++) {
					...
					if (bestSplitSuggestions[i].splitTest != null) {
						if (!node.getInfogainSum()
								.containsKey((bestSplitSuggestions[i].splitTest.getAttsTestDependsOn()[0]))) {
							node.getInfogainSum().put((bestSplitSuggestions[i].splitTest.getAttsTestDependsOn()[0]),
									0.0);
						}
						double currentSum = node.getInfogainSum()
								.get((bestSplitSuggestions[i].splitTest.getAttsTestDependsOn()[0]));
						node.getInfogainSum().put((bestSplitSuggestions[i].splitTest.getAttsTestDependsOn()[0]),
				...
				if (bestSplitSuggestions.length < 2) {
					...
					AttributeSplitSuggestion bestSuggestion = bestSplitSuggestions[bestSplitSuggestions.length - 1];
					AttributeSplitSuggestion secondBestSuggestion = bestSplitSuggestions[bestSplitSuggestions.length
							- 2];
					...
					if (secondBestSuggestion.splitTest == null) { // if you have a null split
						...
						secondBestSuggestionAverageMerit = node.getInfogainSum()
								.get((secondBestSuggestion.splitTest.getAttsTestDependsOn()[0]))
								/ node.getNumSplitAttempts();
					}
