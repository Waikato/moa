...
public class ORTO extends FIMTDD implements Regressor {
	...
	public IntOption maxTreesOption = new IntOption("maxTrees", 'm',
			"The maximum number of trees contained in the option tree.", 10, 1, Integer.MAX_VALUE);

	public IntOption maxOptionLevelOption = new IntOption("maxOptionLevel", 'x',
			"The maximal depth at which option nodes can be created.", 10, 0, Integer.MAX_VALUE);
	...
	public MultiChoiceOption optionNodeAggregationOption = new MultiChoiceOption("optionNodeAggregation", 'o',
				...
				if (currentNode instanceof LeafNode) {
					...
					if (!inAlternate && iNode.alternateTree != null) {
						...
						if (altTree) {
							...
							processInstance(inst, iNode.alternateTree, prediction, normalError, true, true); // growth
																												// is
																												// allowed
																												// in
																												// the
							...
							for (Node child : ((OptionNode) currentNode).children) {
								processInstance(inst, child, child.getPrediction(inst), normalError, growthAllowed,
										inAlternate);
							}
							break;
						}
	...
	public void processInstanceOptionNode(Instance inst, OptionNode node, double prediction, double normalError,
		...
		if (node.changeDetection) {
			...
			if (((InnerNode) node).PageHinckleyTest(
					error - node.sumOfAbsErrors / node.examplesSeen + PageHinckleyAlphaOption.getValue(),
		...
		for (Node child : node.children) {
			...
			for (int i = 0; i < inst.weight(); i++) {
				...
				node.optionFFSSL[index] = node.optionFFSSL[index] * optionFadingFactorOption.getValue()
						+ Math.pow(childPrediction - inst.classValue(), 2);
			}
		}

		for (Node child : node.children) {
			processInstance(inst, child, child.getPrediction(inst), normalError,
	...
	protected void attemptToSplit(LeafNode node, Node parent, int parentIndex, Instance inst, double prediction) {
		...
		List<AttributeSplitSuggestion> acceptedSplits = new LinkedList<>();
		...
		if (bestSplitSuggestions.length == 1) {
			...
			if (secondBestSuggestion.merit / bestSuggestion.merit < 1 - hoeffdingBound) {
				...
				numSplits = 1;
				acceptedSplits.add(bestSuggestion);
			} else { // If the splitting criterion was not met, initiate pruning of the E-BST
				...
				for (int i = 0; i < node.attributeObservers.size(); i++) {
					...
					if (obs != null) {
						((FIMTDDNumericAttributeClassObserver) obs).removeBadSplits(splitCriterion,
								secondBestSuggestion.merit / bestSuggestion.merit, bestSuggestion.merit,
								hoeffdingBound);
					}
				}
			}
		...
		// If the splitting criterion was met, split the current node using the chosen
		// attribute test, and
