 ...
 *
 * <p>
 * This adaptive Hoeffding Tree uses ADWIN to monitor performance of branches on
 * the tree and to replace them with new branches when their accuracy decreases
 * if the new branches are more accurate.
 * </p>
 * See details in:
 * </p>
 * <p>
 ...
 * NaiveBayes adaptive (NBAdaptive).
 * </ul>
 *
 * @author Albert Bifet (abifet at cs dot waikato dot ac dot nz)
 * @version $Revision: 7 $
...
public class HoeffdingAdaptiveTree extends HoeffdingTree {
	 ...
	 * "leafprediction", 'l', "Leaf prediction to use.", new String[]{ "MC", "NB",
	 * "NBAdaptive"}, new String[]{ "Majority class", "Naive Bayes",
	...
	public static class AdaSplitNode extends SplitNode implements NewNode {
		...
		public void learnFromInstance(Instance inst, HoeffdingAdaptiveTree ht, SplitNode parent, int parentBranch) {
			...
			if (filterInstanceToLeaf(inst, parent, parentBranch).node != null) {
				ClassPrediction = Utils
						.maxIndex(filterInstanceToLeaf(inst, parent, parentBranch).node.getClassVotes(inst, ht));
			}
	...
	public static class AdaLearningNode extends LearningNodeNBAdaptive implements NewNode {
		...
		public void learnFromInstance(Instance inst, HoeffdingAdaptiveTree ht, SplitNode parent, int parentBranch) {
			...
			/*
			 * if (this.alternateTree != null) {
			 * this.alternateTree.learnFromInstance(inst,ht); } for (Node child :
			 * this.children) { if (child != null) { child.learnFromInstance(inst,ht); } }
			 */
		}
		...
		public double[] getClassVotes(Instance inst, HoeffdingTree ht) {
			...
			if (predictionOption == 0) { // MC
				...
				if (this.mcCorrectWeight > this.nbCorrectWeight) {
					...
					dist = NaiveBayes.doNaiveBayesPrediction(inst, this.observedClassDistribution,
