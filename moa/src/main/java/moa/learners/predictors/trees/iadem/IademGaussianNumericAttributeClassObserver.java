...
public class IademGaussianNumericAttributeClassObserver extends GaussianNumericAttributeClassObserver
		...
		for (int i = 0; i < cuts.size(); i++) {
			...
			for (int j = 0; j < totalDist.length; j++) {
				...
				if (counts_cut_value[i][0] != 0) {
					leftEst = lDist[j] / counts_cut_value[i][0];
				}
				...
				if (counts_cut_value[i][1] != 0) {
					rightEst = rDist[j] / counts_cut_value[i][1];
				}
	...
	public ArrayList<Double[]> computeConditionalProbPerBin(ArrayList<Double> cuts) {
		ArrayList<Double[]> probClassDistPerCut = new ArrayList<>();
