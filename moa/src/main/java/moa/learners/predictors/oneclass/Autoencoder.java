...
package moa.learners.predictors.oneclass;
...
import moa.learners.predictors.OneClassClassifier;

/**
 * Implements an autoencoder: a neural network that attempts to reconstruct the
 * input. The autoencoder's structure is an input layer with one neuron for each
 * non-class attribute, a hidden layer of two neurons and an output layer with
 * one neuron for each non-class attribute.
 *
 * Partially modeled on Albert Bifet's Perceptron.java, also found in the MOA
 * project.
 *
 * Backpropagation implementation based on Matt Mazur's excellent tutorial on
 * this topic:
...
public class Autoencoder extends AbstractClassifier implements Classifier, OneClassClassifier {
	...
	public FloatOption thresholdOption = new FloatOption("threshold", 't',
			"Determines the threshold for recognizing outliers. Higher values means fewer outliers.", 0.6, 0.001,
			0.999);
	...
	/**
	 * The learning rate; controls the size of the adjustments of the weights and
	 * bias unit.
	 */
	...
	/**
	 * Performs the requisite calculations between the input layer and the hidden
	 * layer.
	 *
	...
	/**
	 * Performs the requisite calculations between the hidden layer and the output
	 * layer.
	 *
	...
	private void backpropagation(Instance inst) {
		...
		RealMatrix adjustmentOne = delta2.transpose().multiply(input.transpose())
	 ...
	 * @return the outputs of the hidden layer and the output vector that attempts
	 *         to reconstruct the input
	 *
	 *         private RealMatrix prediction(RealMatrix input) { RealMatrix output =
	 *         new double[this.numAttributes+2];
	 * 
	 *         // Initialize arrays and compute total net input for the hidden layer
	 *         for(int i = 0 ; i < this.numAttributes ; i++) { input[i] =
	 *         inst.value(i); output[i+2] = this.biasTwo; output[0] +=
	 *         input[i]*this.weightsOne[i][0]; output[1] +=
	 *         input[i]*this.weightsOne[i][1]; }
	 * 
	 *         // Logistic function used for hidden layer activation output[0] = 1.0
	 *         / (1.0 + Math.pow(Math.E,-1.0 * (output[0] + this.biasOne)));
	 *         output[1] = 1.0 / (1.0 + Math.pow(Math.E,-1.0 * (output[1] +
	 *         this.biasOne)));
	 * 
	 *         // Compute total net input for the output layer // Use the logistic
	 *         function for output layer activation. for(int i = 2 ; i <
	 *         this.numAttributes ; i++) { output[i] +=
	 *         (output[0]*this.weightsTwo[0][i])+(output[1]*this.weightsTwo[1][i]);
	...
	/**
	 * Calculates the error between the autoencoder's reconstruction of the input
	 * and the argument instances. This error is converted to vote scores.
	 *
	...
	/**
	 * Returns the squared error between the input value and the reconstructed value
	 * as the anomaly score for the argument instance.
	 *
	 ...
	 *
	 * @param trainingPoints the Collection of instances on which to initialize the
