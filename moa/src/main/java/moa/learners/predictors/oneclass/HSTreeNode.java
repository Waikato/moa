 ...
 * “Fast anomaly detection for streaming data,” in IJCAI
 * Proceedings-International Joint Conference on Artificial Intelligence, 2011,
 * vol. 22, no. 1, pp. 1511–1516.
 *
...
public class HSTreeNode implements Serializable {
	...
	/**
	 * The randomly chosen attribute that this node splits on. In the paper this is
	 * 'q'.
	 */
	...
	/**
	 * If this node is an internal node: <b>true</b>, if it is a leaf node:
	 * <b>false</b>.
	 */
	 ...
	 *
	 * @param min      the minimum values of the attributes for this node's
	 *                 workspace
	 * @param max      the maximum values of the attributes for this node's
	 *                 workspace
	...
	public HSTreeNode(double[] min, double[] max, int k, int maxDepth) {
		...
		// If this node is not at the maximum depth level, then create two subordinate
		// nodes.
		if ((depth < maxDepth)) {
	 ...
	 * @param referenceWindow if the HSTree is in the initial reference window:
	...
	/**
	 * Update the node's model by setting the latest window's mass profile as the
	 * reference window's mass profile, resetting the latest window's mass profile
	 * to zero and updating any subordinates nodes' models.
	 */
	...
	/**
	 * If this node is a leaf node or it has a mass profile of less than sizeLimit,
	 * this returns the anomaly score for the argument instance. Otherwise this node
	 * determines which of its subordinate nodes the argument instance belongs to
	 * and asks it provide the anomaly score.
	 *
	 * @param inst      the instance being passed through the tree
	 * @param sizeLimit the minimum mass profile for a node to calculate the
	 *                  argument instance's anomaly score
	 *
	...
	/**
	 * Prints this node to string and, if it is an internal node, prints its
	 * children nodes as well. Useful for debugging the entire tree structure.
	 */
