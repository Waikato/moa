 ...
 * Section 3.4.2 of D. M. J. Tax, “One-Class Classification: Concept-learning in
 * the absence of counter-examples,” Delft University of Technology, 2001 with
 * adaptations for the data stream environment.
 *
 * @author Richard Hugh Moulton
 *
 */
public class NearestNeighbourDescription extends AbstractClassifier implements Classifier, OneClassClassifier {
	...
	// "Up to the kth nearest neighbour of the argument instance will be considered.
	// //NOT IMPLEMENTED//", 1);

	// public IntOption mOption = new IntOption("m", 'm',
	// "Up to the mth nearest neighbour of the argument's nearest neighbour(s) will
	// be considered. //NOT IMPLEMENTED//", 1);
	...
	/**
	 * Constructor for a Nearest Neighbour Description classifier based on an
	 * argument training set of instances.
	 *
	...
	public void resetLearningImpl() {
		...
		this.neighbourhood = new FixedLengthList<>(nbhdSize);
	}

	/**
	 * The classifier adds the argument instance to its neighbourhood. If the
	 * neighbourhood is full, then the FixedLengthList removes the oldest instance
	 * in it to accommodate the new instance.
	 */
	@Override
	...
	/**
	 * Calculates the distance between the argument instance and its nearest
	 * neighbour as well as the distance between that nearest neighbour and its own
	 * nearest neighbour. The ratio of these distances is compared to the threshold
	 * value, tau, and converted into a vote score.
	 *
	...
	/**
	 * Returns the anomaly score for an argument instance based on the distance from
	 * it to its nearest neighbour compared to the distance from its nearest
	 * neighbour to the neighbour's nearest neighbour.
	 *
	...
	/**
	 * Searches the neighbourhood in order to find the argument instance's nearest
	 * neighbour.
	 *
	 ...
	 * @param inNbhd         if inst is in neighbourhood2: <b>true</b>, else:
	 *                       <b>false</b>
	 *
	...
	private Instance getNearestNeighbour(Instance inst, List<Instance> neighbourhood2, boolean inNbhd) {
		...
		for (Instance candidateNN : neighbourhood2) {
			// If inst is in neighbourhood2 and an identical instance is found, then it is
	...
	/**
	 * Initializes the Nearest Neighbour Distance (NN-d) classifier with the
	 * argument training points.
	 *
