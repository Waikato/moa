 ...
 * S. C. Tan, K. M. Ting, and T. F. Liu, “Fast anomaly detection for streaming
 * data,” in IJCAI Proceedings-International Joint Conference on Artificial
...
public class HSTrees extends AbstractClassifier implements Classifier, OneClassClassifier {
	...
	public FloatOption anomalyThresholdOption = new FloatOption("anomalyThreshold", 'a',
			"Threshold (as a fraction of the maximum mass value) below which an instance is declared an anomaly.", 0.5,
	...
	// Respectively, these variables store: the window size, the number of trees,
	// the maximum depth of the trees,
	// the dimensionality of the data stream and the number of instances seen to
	// date.
	/**
	...
	/**
	 * The minimum mass required in a node (as a fraction of the window size) to
	 * calculate the anomaly score. 'sizeLimit' in the original paper.
	 */
	...
	/**
	 * If the classifier is in the first reference window: <b>true</b>, else:
	 * <b>false</b>
	 */
	...
	/**
	 * Combine the anomaly scores from each HSTree in the forest and convert into a
	 * vote score.
	 *
	...
	public double getAnomalyScore(Instance inst) {
		if (this.referenceWindow)
			...
			accumulatedScore = accumulatedScore / ((this.numTrees));
	 ...
	 *
	 * @param trainingPoints the Collection of instance with which to initialize the
	 *                       Streaming Hs-Trees classifier.
	 */
	@Override
