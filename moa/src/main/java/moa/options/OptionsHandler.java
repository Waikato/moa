...
public class OptionsHandler extends JavaCLIParser {
	...
	public OptionsHandler(Object c, String cliString) {
		...
		// this.prepareForUse();
		/*
		 * int firstSpaceIndex = cliString.indexOf(' ', 0); String classOptions; String
		 * className; if (firstSpaceIndex > 0) { className = cliString.substring(0,
		 * firstSpaceIndex); classOptions = cliString.substring(firstSpaceIndex + 1,
		 * cliString.length()); classOptions = classOptions.trim(); } else { className =
		 * cliString; classOptions = ""; }
		 */
	 ...
	 * 
	 * public Options getOptions() { if (this.options == null) { this.options = new
	 * Options(); Option[] myOptions = discoverOptionsViaReflection(); for (Option
	 * option : myOptions) { this.options.addOption(option); } } return
	 * this.options; }
	 */
	 ...
	 * this.handler.getClass(); Field[] fields = c.getFields(); List<Option> optList
	 * = new LinkedList<Option>(); for (Field field : fields) { String fName =
	 * field.getName(); Class<?> fType = field.getType(); if
