...
/**
 * This class handles the dependency between two options by updating the dependent option whenever
 * the option it is depending on changes.
 *
 * @author Cornelius Styp von Rekowski (cornelius.styp@ovgu.de)
 * @version $Revision: 1 $
 */
public class DependentOptionsUpdater implements ChangeListener, Serializable {
    ...
    /**
     * Refresh the provided choices of an EditableMultiChoiceOption every time a ClassOption (the
     * prequential evaluation task) is changed. This method checks if the chosen task actually
     * changed, before updating the MultiChoiceOption. <br>
     * The method looks for available options in the learner option of the prequential evaluation
     * task and recursively in all of its ClassOptions. <br>
     * Only Int and Float options are shown in the MultiChoiceOption, because only numeric
     * parameters should be variable. <br>
     * If one of the options is named "budget" or its name contains the word "budget", it is
    ...
    public void refreshVariedParamNameOption() {
        ...
        if (this.lastEvalTaskCLIString == null
            ...
            ALPrequentialEvaluationTask evalTask =
                    (ALPrequentialEvaluationTask) this.evalTaskOption.getPreMaterializedObject();
    ...
    private void addRecursiveNumberOptions(
            ClassOption option,
        ...
        for (ClassOption classOption : classOptions) {
            this.addRecursiveNumberOptions(
                    classOption, optionNames, optionDescriptions, newNamePrefix);
        }
    }

    /**
     * Resolve the name of the varied parameter and return the corresponding option. The varied
     * parameter name has the format "learner/suboptions.../numberOption". If no matching parameter
                ...
                if (opt.getName().equals(singleOptions[i])) {
                    if (opt instanceof ClassOption) {
                        currentOptionHandler =
                                (OptionHandler) ((ClassOption) opt).getPreMaterializedObject();
                    } else {
