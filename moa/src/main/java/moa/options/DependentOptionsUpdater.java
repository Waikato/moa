...
public class DependentOptionsUpdater implements ChangeListener, Serializable {
	...
	public DependentOptionsUpdater(ClassOptionWithListenerOption learnerOption,
	...
	/**
	 * Refresh the provided choices of an EditableMultiChoiceOption every time a
	 * ClassOption (the prequential evaluation task) is changed. This method checks
	 * if the chosen task actually changed, before updating the MultiChoiceOption.
	 * <br>
	 * The method looks for available options in the learner option of the
	 * prequential evaluation task and recursively in all of its ClassOptions. <br>
	 * Only Int and Float options are shown in the MultiChoiceOption, because only
	 * numeric parameters should be variable. <br>
	...
	public void refreshVariedParamNameOption() {
		...
		if (this.lastEvalTaskCLIString == null || !this.lastEvalTaskCLIString.equals(currentEvalTaskCLIString)) {
			...
			ALPrequentialEvaluationTask evalTask = (ALPrequentialEvaluationTask) this.evalTaskOption
			...
			this.variedParamNameOption.setOptions(optionNames.toArray(new String[0]),
					optionDescriptions.toArray(new String[0]), defaultIndex >= 0 ? defaultIndex : 0);
		}
	}

	private void addRecursiveNumberOptions(ClassOption option, List<String> optionNames,
			List<String> optionDescriptions, String namePrefix) {
		...
		List<Option> numberOptions = new ArrayList<>(options.length);
		...
		List<String> names = new ArrayList<>(numberOptions.size());
		List<String> descriptions = new ArrayList<>(numberOptions.size());
		...
		List<ClassOption> classOptions = new ArrayList<>(options.length);
		for (Option o : options) {
	 ...
	 * The varied parameter name has the format
	 * "learner/suboptions.../numberOption". If no matching parameter can be found,
