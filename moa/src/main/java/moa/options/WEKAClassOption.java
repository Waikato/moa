...
/**
 * WEKA class option. This option is used to access options in WEKA. For example, WEKAClassifier
 * uses it to set the base learner.
 *
 * @author Richard Kirkby (rkirkby@cs.waikato.ac.nz)
 * @author FracPete (fracpete at waikato dot ac dot nz)
 * @version $Revision$
...
public class WEKAClassOption extends AbstractClassOption {
    ...
    public String getValueAsCLIString() {
        ...
        if (currentValue instanceof weka.core.OptionHandler)
            result +=
                    " " + Utils.joinOptions(((weka.core.OptionHandler) currentValue).getOptions());
        result = result.trim();
    ...
    public static Object cliStringToObject(
        ...
        if (requiredType.isInstance(classInstance)
            ...
            if (classInstance instanceof weka.core.OptionHandler) {
                try {
                    ((weka.core.OptionHandler) classInstance)
