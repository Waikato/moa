...
public abstract class AbstractOptionHandler extends AbstractMOAObject implements OptionHandler {
	...
	public Options getOptions() {
		/*
		 * if (this.options == null) { this.options = new Options(); if (this.config==
		 * null) { this.config = new OptionsHandler(this, "");
		 * this.config.prepareForUse(); } Option[] myOptions =
		 * this.config.discoverOptionsViaReflection(); for (Option option : myOptions) {
		 * this.options.addOption(option); } } return this.options;
		 */
		if (this.config == null) {
	...
	public String getCLICreationString(Class<?> expectedType) {
		return AbstractClassOption.stripPackagePrefix(this.getClass().getName(), expectedType) + " "
	...
	protected void prepareClassOptions(TaskMonitor monitor, ObjectRepository repository) {
		 ...
		 * getOptions().getOptionArray(); for (Option option : optionArray) { if (option
		 * instanceof ClassOption) { ClassOption classOption = (ClassOption) option;
		 * monitor.setCurrentActivity("Materializing option " + classOption.getName() +
		 * "...", -1.0); Object optionObj = classOption.materializeObject(monitor,
		 * repository); if (monitor.taskShouldAbort()) { return; } if (optionObj
		 * instanceof OptionHandler) { monitor.setCurrentActivity("Preparing option " +
		 * classOption.getName() + "...", -1.0); ((OptionHandler)
		 * optionObj).prepareForUse(monitor, repository); if (monitor.taskShouldAbort())
		 * { return; } } if (this.classOptionNamesToPreparedObjects == null) {
