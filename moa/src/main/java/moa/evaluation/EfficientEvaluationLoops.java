...
public class EfficientEvaluationLoops {
    ...
    public static PrequentialResult PrequentialEvaluation(
        ...
        while (stream.hasMoreInstances()
            ...
            if (basicEvaluator != null) basicEvaluator.addResult(instance, prediction);
            if (windowedEvaluator != null) windowedEvaluator.addResult(instance, prediction);
            if (storePredictions) predictions.add(Utils.maxIndex(prediction));
        ...
        return new PrequentialResult(
                windowed_results, cumulative_results, targetValues, predictions);
    }
    ...
    public static PrequentialResult PrequentialSSLEvaluation(
        ...
        List<Pair<Long, Example<Instance>>> delayBuffer =
                new ArrayList<Pair<Long, Example<Instance>>>();
        ...
        while (stream.hasMoreInstances()
            ...
            while (delayBuffer.size() > 0 && delayBuffer.get(0).getKey() == instancesProcessed) {
                ...
                //                System.out.println("[TRAIN][DELAY]
                // "+delayedExample.getData().toString());
            ...
            if (instancesProcessed < initialWindowSize) {
                ...
                // learner).addInitialWarmupTrainingInstances();
                //                System.out.println("[TRAIN][INITIAL_WINDOW]
                // "+instance.getData().toString());
                learner.trainOnInstance(instance);
                instancesProcessed++;
                continue;
            }
            ...
            if (is_labeled && delayLength >= 0) {
                ...
                if (learner instanceof SemiSupervisedLearner) {
                    //                    System.out.println("[TRAIN_UNLABELED][DELAYED] " +
                    // unlabeledExample.getData().toString());
                    pseudoLabel =
                            ((SemiSupervisedLearner) learner)
                                    .trainOnUnlabeledInstance(
                                            (Instance) unlabeledExample.getData());
                }
                ...
                if (learner instanceof SemiSupervisedLearner) {
                    //                    System.out.println("[TRAIN_UNLABELED][IMMEDIATE] " +
                    // unlabeledExample.getData().toString());
                    pseudoLabel =
                            ((SemiSupervisedLearner) learner)
                                    .trainOnUnlabeledInstance(
            ...
            if (trueClass == pseudoLabel) numCorrectPseudoLabeled++;
        ...
        other_measures.put(
                "pseudo_label_accuracy", (double) numCorrectPseudoLabeled / numInstancesTested);

        return new PrequentialResult(
                windowed_results, cumulative_results, targetValues, predictions, other_measures);
    }
    ...
    private static void testPrequentialSSL(
            String file_path,
            Learner learner,
            long maxInstances,
            long windowSize,
        ...
        BasicClassificationPerformanceEvaluator basic_evaluator =
        ...
        WindowClassificationPerformanceEvaluator windowed_evaluator =
        ...
        System.out.println(
                "Number of unlabeled instances: "
                        + result.otherMeasurements.get("num_unlabeled_instances"));

        System.out.println("\tBasic performance");
        for (int i = 0; i < result.cumulativeResults.length; ++i)
            System.out.println(
                    basic_evaluator.getPerformanceMeasurements()[i].getName()
                            + ": "
                            + result.cumulativeResults[i]);
        ...
        for (int j = 0; j < result.windowedResults.size(); ++j) {
            ...
            System.out.println(
                        windowed_evaluator.getPerformanceMeasurements()[i].getName()
                                + ": "
                                + result.windowedResults.get(j)[i]);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        String ELEC_arff =
                "/Users/gomeshe/Dropbox/ciencia_computacao/lecturer/research/ssl_disagreement/datasets/ELEC/elecNormNew.arff";
    ...
    private static void testPrequentialEfficiency1(Learner learner) {
        ...
        ArffFileStream stream =
                new ArffFileStream("/Users/gomeshe/Desktop/data/Hyper100k.arff", -1);
        stream.prepareForUse();
        ...
        PrequentialResult results =
                PrequentialEvaluation(
                        stream, learner, basic_evaluator, null, 100000, 1, false, false);
    ...
    private static void testPrequentialEvaluation_edge_cases1() {
        ...
        //        BasicClassificationPerformanceEvaluator basic_evaluator = new
        // BasicClassificationPerformanceEvaluator();
        //        basic_evaluator.recallPerClassOption.setValue(true);
        //        basic_evaluator.prepareForUse();
        //
        //        WindowClassificationPerformanceEvaluator windowed_evaluator = new
        // WindowClassificationPerformanceEvaluator();
        //        windowed_evaluator.widthOption.setValue(1000);
        //        windowed_evaluator.prepareForUse();

        PrequentialResult results =
                PrequentialEvaluation(stream, learner, null, null, 100000, 1000, false, false);
    ...
    private static void testPrequentialEvaluation_edge_cases3() {
        ...
        PrequentialResult results =
                PrequentialEvaluation(
                        stream, learner, basic_evaluator, windowed_evaluator, 10, 1, false, false);
    ...
    private static void testPrequentialEvaluation_SampleFrequency_TestThenTrain() {
        ...
        PrequentialResult results =
                PrequentialEvaluation(
                        stream, learner, null, basic_evaluator, -1, 10000, false, false);
    ...
    private static void testPrequentialRegressionEvaluation() {
        ...
        ArffFileStream stream =
                new ArffFileStream(
                        "/Users/gomeshe/Desktop/data/metrotraffic_with_nominals.arff", -1);
        stream.prepareForUse();

        BasicRegressionPerformanceEvaluator basic_evaluator =
                new BasicRegressionPerformanceEvaluator();

        WindowRegressionPerformanceEvaluator windowed_evaluator =
                new WindowRegressionPerformanceEvaluator();
        windowed_evaluator.widthOption.setValue(1000);
        //        windowed_evaluator.prepareForUse();
    ...
    private static void testTestThenTrainEvaluation() {
        ...
        PrequentialResult results =
                PrequentialEvaluation(
                        stream, learner, evaluator, null, 100000, 100000, false, false);
        ...
        for (int i = 0; i < results.cumulativeResults.length; ++i)
            System.out.println(
                    evaluator.getPerformanceMeasurements()[i].getName()
                            + ": "
                            + results.cumulativeResults[i]);
    }
    ...
    private static void testWindowedEvaluation() {
        ...
        PrequentialResult results =
                PrequentialEvaluation(
                        stream, learner, null, evaluator, 100000, 10000, false, false);
