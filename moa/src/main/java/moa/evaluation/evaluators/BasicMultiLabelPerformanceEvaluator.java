 ...
 *
 */
public class BasicMultiLabelPerformanceEvaluator extends AbstractMOAObject
		implements MultiLabelClassificationPerformanceEvaluator {
	...
	public void addResult(Example<Instance> example, Prediction y) {
		...
		if (y == null) {
			...
			System.err.println("[WARNING] Only " + y.numOutputAttributes() + " labels found! (Expecting "
					+ x.numOutputAttributes() + ")\n (Ignoring this prediction)");
		} else {
			sumExamples++;
			int correct = 0;
			for (int j = 0; j < y.numOutputAttributes(); j++) {
			...
			// Recall by instance
			if (sumOnesPred > 0) {
				sumRecall += sumInterse / sumOnesPred;
			}
	...
	public Measurement[] getPerformanceMeasurements() {
		...
		// gather measurements
		Measurement m[] = new Measurement[] { new Measurement("Exact Match", sumAccuracy / sumExamples),
				new Measurement("Accuracy", sumAccuracy2 / sumExamples),
				new Measurement("Hamming Score", sumHamming / sumExamples),
				new Measurement("Precision", (sumPrecision) / sumExamples),
				new Measurement("Recall", sumRecall / sumExamples),
				new Measurement("F-Measure", sumFmeasure / sumExamples), };
