...
public class BasicMultiTargetPerformanceEvaluator extends AbstractMOAObject
		...
		if (inst.weight() > 0.0) {
			...
			if (prediction != null) {
				for (int i = 0; i < numberOutputs; i++) {
					double err = inst.classValue(i)
							- ((prediction.numOutputAttributes() == 0) ? 0.0 : prediction.getPrediction(i));
	...
	public double getSquareError() {
		return Math
				.sqrt(this.weightObserved > 0.0 ? this.squareError / (this.weightObserved * this.numberOutputs) : 0.0);
	}
