...
/**
 * Multi-target regression evaluator that updates evaluation results using a
 * sliding window.
 *
 * @author Albert Bifet (abifet at cs dot waikato dot ac dot nz)
 * @version $Revision: 7 $
 */
public class WindowMultiTargetRegressionPerformanceRelativeMeasuresEvaluator extends AbstractOptionHandler
	...
	public void addResult(Example<Instance> testInst, Prediction prediction) {
		Instance inst = testInst.getData();
		...
		if (weight > 0.0) {
			...
			if (prediction != null) {
				for (int i = 0; i < numberOutputs; i++) {
					double error = (inst.valueOutputAttribute(i) - prediction.getPrediction(i));
					this.sumY[i].add(inst.valueOutputAttribute(i));
					double errorTM = (inst.valueOutputAttribute(i)
							- this.sumY[i].total() / this.weightObserved.total());
