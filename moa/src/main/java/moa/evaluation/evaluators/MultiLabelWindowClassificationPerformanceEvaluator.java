...
public class MultiLabelWindowClassificationPerformanceEvaluator extends WindowClassificationPerformanceEvaluator
	...
	/**
	 * Add a Result. NOTE: In theory, the size of y[] could change, although we do
	 * not take into account this possibility *yet*. (for this, we would have to use
	 * y[] differently, another format for y[] e.g. HashMap, or store more info in
	 * x)
	 */
	@Override
	...
	public void addResult(Example<Instance> example, Prediction prediction) {
		...
		Instance inst = (Instance) example.getData();
		if (inst.weight() > 0.0) {
