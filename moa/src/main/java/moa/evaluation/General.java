...
public class General extends MeasureCollection {
    ...
    public void evaluateClustering(
        ...
        addValue(
                "GPrecision",
                (totalCoverage == 0 ? 0 : ((double) trueCoverage / (double) (totalCoverage))));
    ...
    private double computeOverlap() {
        for (int c = 0; c < numFClusters; c++) {
            if (!(clustering.get(c) instanceof SphereCluster)) {
                System.out.println(
                        "Overlap only supports Sphere Cluster. Found: "
    ...
    private double computeCompactness() {
        ...
        for (int c = 0; c < numFClusters; c++) {
            if (!(clustering.get(c) instanceof SphereCluster)) {
                System.out.println(
                        "Compactness only supports Sphere Cluster. Found: "
                                + clustering.get(c).getClass());
                return Double.NaN;
            }
        }
        ...
        for (int c = 0; c < numFClusters; c++) {
            ...
            if (containedPoints.size() > 1) {
                ...
                if (Math.abs(minRadius - cfRadius) < 0.1e-10) {
                    compactness = 1;
                } else if (minRadius < cfRadius) compactness = minRadius / cfRadius;
                else {
                    System.out.println(
                            "Optimal radius bigger then real one ("
                                    + (cfRadius - minRadius)
