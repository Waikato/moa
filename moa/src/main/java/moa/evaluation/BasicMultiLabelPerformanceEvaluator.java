 ...
 * @author Jesse Read (jesse@tsc.uc3m.es)
 * @version $Revision: 1 $
 *     <p>Added instance based F-Measure, Recall, Precision and Accuracy
...
public class BasicMultiLabelPerformanceEvaluator extends AbstractMOAObject
    ...
    public void addResult(Example<Instance> example, Prediction y) {
        ...
        if (y == null) {
            ...
            System.err.println(
                    "[WARNING] Only "
                            + y.numOutputAttributes()
                            + " labels found! (Expecting "
            ...
            for (int j = 0; j < y.numOutputAttributes(); j++) {
                ...
                if (y_true == yp) correct++;

                if (y_true == 1 || yp == 1) sumReunion++;

                if (y_true == 1 && yp == 1) sumInterse++;

                if (y_true == 1) sumOnesTrue++;
