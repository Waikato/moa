...
/**
 * Class that holds separate {@link PreviewCollection}s for mean and standard deviation values. The
 * values can be calculated from a PreviewCollection that contains the results of multiple runs for
...
public class MeanPreviewCollection {
    ...
    /**
     * On creation of a MeanPreviewCollection, the mean Previews and standard deviation Previews are
     * calculated from the given PreviewCollection by averaging the measurements for all entries
     * over the different runs that have been performed. This results in a collection of mean
     * Previews and a separate collection of standard deviation Previews, both containing Previews
    ...
    private void constructMeanStdPreviewsForParam(
        ...
        List<double[]> meanParamMeasurements =
                calculateMeanMeasurementsForParam(numEntriesPerPreview, numParamValues, paramValue);

        // calculate standard deviation
        List<double[]> stdParamMeasurements =
                calculateStdMeasurementsForParam(
                        numEntriesPerPreview, numParamValues, paramValue, meanParamMeasurements);
        ...
        meanMeasurementNames =
                Arrays.copyOfRange(meanMeasurementNames, 4, meanMeasurementNames.length);
        ...
        Preview meanParamValuePreview =
                new PreviewCollectionLearningCurveWrapper(
                        meanLearningCurve, this.origMultiRunPreviews.taskClass);
        Preview stdParamValuePreview =
                new PreviewCollectionLearningCurveWrapper(
                        stdLearningCurve, this.origMultiRunPreviews.taskClass);
        ...
        for (PreviewCollection<Preview> foldPreview : this.origMultiRunPreviews.subPreviews) {
            ...
            if (foldPreview.getPreviews().size() == numParamValues) {
                ...
                this.addPreviewMeasurementsToSum(
                        paramMeasurementsSum, foldParamPreview, numEntriesPerPreview);
            }
        }
    ...
    private void addPreviewMeasurementsToSum(
            List<double[]> measurementsSum, Preview preview, int numEntriesPerPreview) {
