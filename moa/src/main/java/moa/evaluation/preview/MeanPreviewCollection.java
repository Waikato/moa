...
public class MeanPreviewCollection {
	...
	/**
	 * On creation of a MeanPreviewCollection, the mean Previews and standard
	 * deviation Previews are calculated from the given PreviewCollection by
	 * averaging the measurements for all entries over the different runs that have
	 * been performed. This results in a collection of mean Previews and a separate
	...
	public MeanPreviewCollection(PreviewCollection<PreviewCollection<Preview>> multiRunPreviews) {
		...
		this.meanPreviews = new PreviewCollection<>("mean preview entry id", "parameter value id",
				multiRunPreviews.taskClass, multiRunPreviews.variedParamName, multiRunPreviews.variedParamValues);
		this.stdPreviews = new PreviewCollection<>("mean preview entry id", "parameter value id",
				multiRunPreviews.taskClass, multiRunPreviews.variedParamName, multiRunPreviews.variedParamValues);
	...
	/**
	 * Construct the mean and standard deviation Previews for one specific parameter
	 * value.
	 * 
	...
	private void constructMeanStdPreviewsForParam(int numEntriesPerPreview, int numParamValues, int paramValue) {
		// calculate mean
		List<double[]> meanParamMeasurements = calculateMeanMeasurementsForParam(numEntriesPerPreview, numParamValues,
				paramValue);

		// calculate standard deviation
		List<double[]> stdParamMeasurements = calculateStdMeasurementsForParam(numEntriesPerPreview, numParamValues,
				paramValue, meanParamMeasurements);
		...
		Preview meanParamValuePreview = new PreviewCollectionLearningCurveWrapper(meanLearningCurve,
				this.origMultiRunPreviews.taskClass);
		Preview stdParamValuePreview = new PreviewCollectionLearningCurveWrapper(stdLearningCurve,
				this.origMultiRunPreviews.taskClass);
	 ...
	 */
	private List<double[]> calculateMeanMeasurementsForParam(int numEntriesPerPreview, int numParamValues,
			int paramValue) {
		List<double[]> paramMeasurementsSum = new ArrayList<>(numEntriesPerPreview);
		List<double[]> meanParamMeasurements = new ArrayList<>(numEntriesPerPreview);
	 ...
	 */
	private List<double[]> calculateStdMeasurementsForParam(int numEntriesPerPreview, int numParamValues,
			int paramValue, List<double[]> meanParamMeasurements) {
		List<double[]> paramMeasurementsSquaredDiffSum = new ArrayList<>(numEntriesPerPreview);
		List<double[]> paramMeasurementsStd = new ArrayList<>(numEntriesPerPreview);
		...
		for (PreviewCollection<Preview> foldPreview : this.origMultiRunPreviews.subPreviews) {
			...
			if (foldPreview.getPreviews().size() == numParamValues) {
				...
				this.addPreviewMeasurementSquaredDiffsToSum(meanParamMeasurements, paramMeasurementsSquaredDiffSum,
						foldParamPreview, numEntriesPerPreview);
			}
		}
	 ...
	 * @param numEntriesPerPreview
	 */
	private void addPreviewMeasurementsToSum(List<double[]> measurementsSum, Preview preview,
	 ...
	 * @param numEntriesPerPreview
	 */
	private void addPreviewMeasurementSquaredDiffsToSum(List<double[]> meanMeasurements,
			List<double[]> measurementsSquaredDiffSum, Preview preview, int numEntriesPerPreview) {
		List<double[]> previewMeasurements = preview.getData();
