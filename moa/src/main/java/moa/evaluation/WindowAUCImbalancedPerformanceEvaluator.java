...
/**
 * Classification evaluator that updates evaluation results using a sliding window. Performance
 * measures designed for class imbalance problems. Only to be used for binary classification
 * problems with unweighted instances. Class 0 - majority/negative examples, class 1 - minority,
 * positive examples. Prequential AUC calculation as described and analyzed in: D. Brzezinski, J.
 * Stefanowski, "Prequential AUC: Properties of the Area Under the ROC Curve for Data Streams with
 * Concept Drift", Knowledge and Information Systems, 2017.
 *
...
public class WindowAUCImbalancedPerformanceEvaluator extends AbstractOptionHandler
    ...
    public class Estimator {
        ...
        public void add(double score, boolean isPositive, boolean correctPrediction) {
            ...
            if (size > 0 && posWindow >= this.size) {
                ...
                correctPositivePredictions -=
                        window[posWindow % size].isPositive ? predictions[posWindow % size] : 0;
                ...
                int oldestExamplePredictedClass =
                        predictions[posWindow % size] == 1.0
                                ? oldestExampleTrueClass
            ...
            //// posWindow needs to be always incremented to differentiate between examples in the
            // red-black tree
            posWindow++;
        }
        ...
        public double getAccuracy() {
            if (size > 0) {
                return totalObservedInstances > 0.0
                        ? correctPredictions / Math.min(size, totalObservedInstances)
                        : 0.0;
            } else {
                return totalObservedInstances > 0.0
                        ? correctPredictions / totalObservedInstances
                        : 0.0;
            }
        }
        ...
        public double getKappa() {
            ...
            if (size > 0) {
                for (int i = 0; i < numClasses; i++) {
                    pc +=
                            (this.rowKappa[i] / Math.min(size, totalObservedInstances))
                                    * (this.columnKappa[i]
                                            / Math.min(size, totalObservedInstances));
                }
            } else {
                for (int i = 0; i < numClasses; i++) {
                    pc +=
                            (this.rowKappa[i] / totalObservedInstances)
    ...
    public void addResult(Example<Instance> exampleInstance, double[] classVotes) {
        ...
        if (inst.classIsMissing() == false) {
            ...
            if (weight > 0.0) {
                ...
                this.aucEstimator.add(
                        normalizedVote, trueClass == 1, Utils.maxIndex(classVotes) == trueClass);
                this.weightMajorityClassifier.add(
                        (this.aucEstimator.getRatio() <= 1 ? 0 : 1) == trueClass ? weight : 0);
            }
        }
    }
