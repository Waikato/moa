...
public class CMM extends MeasureCollection {
	...
	/**
	 * number of cluster in the adjusted groundtruth clustering that was calculated
	 * through the groundtruth analysis
	 */
	...
	/**
	 * pointInclusionProbFC[p][C] contains the probability of point p being included
	 * in cluster C
	 */
	...
	protected String[] getNames() {
		String[] names = { "CMM", "CMM Basic", "CMM Missed", "CMM Misplaced", "CMM Noise", "CA Seperability",
				"CA Noise", "CA Model" };
		return names;
	}
	...
	/**
	 * calculates the CMM specific matching between found clusters and ground truth
	 * clusters
	 */
	private void calculateMatching() {
		...
		for (int fc = 0; fc < numFClusters; fc++) {
			...
			if (sumsFC[fc] != 0 && matchIndex == -1) {
				...
				// if we have multiple fit candidates, use majority vote of corresponding
				// classes
	...
	private double noiseError(CMMPoint cmdp, int assignedClusterID) {
		...
		// TODO: Noiscluster?
		// also happens when we decrease the radius and there is only a noise point in
		// the center
		if (gtAssignedID == -1) {
			error = 1;
			cmdp.p.setMeasureValue("CMM Type", "noise - cluster");
		} else {
			if (enableModelError && gtAnalysis.getGT0Cluster(gtAssignedID)
					.getInclusionProbability(cmdp) >= pointInclusionProbThreshold) {
