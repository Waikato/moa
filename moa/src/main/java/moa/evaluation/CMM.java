...
public class CMM extends MeasureCollection {
    ...
    /**
     * number of cluster in the adjusted groundtruth clustering that was calculated through the
    ...
    /**
     * enable/disable model error when enabled errors that are caused by the underling cluster model
     * will not be counted
     */
    ...
    public void evaluateClustering(
            Clustering clustering, Clustering trueClustering, ArrayList<DataPoint> points)
            throws Exception {
    ...
    private void calculateMatching() {
        ...
        for (int p = 0; p < numPoints; p++) {
            ...
            for (int fc = 0; fc < numFClusters; fc++) {
                ...
                if (pointInclusionProbFC[p][fc] >= pointInclusionProbThreshold) {
                    // make sure we don't count points twice that are contained in two merged
                    // clusters
        ...
        for (int fc = 0; fc < numFClusters; fc++) {
            ...
            for (int hc0 = 0; hc0 < numGT0Classes; hc0++) {
                if (mapFC[fc][hc0] != 0) {
                    if (matchIndex == -1) matchIndex = hc0;
                    else {
                        matchIndex = -1;
                        break;
                    }
            ...
            int realMatch = -1;
            if (matchIndex == -1) {
                if (debug) System.out.println("No cluster match: needs to be implemented?");
            } else {
    ...
    private void calculateError() {
        ...
        /**
         * mainly iterate over all points and find the right error value for the point. within the
         * same run calculate various other stuff like coverage etc...
         */
        for (int p = 0; p < numPoints; p++) {
            CMMPoint cmdp = gtAnalysis.getPoint(p);
        ...
        addValue(
                "CMM Misplaced",
                (errorMisplacedMax != 0) ? 1 - errorMisplaced / errorMisplacedMax : 1);
            ...
            for (int fc = 0; fc < numFClusters; fc++) {
                ...
                if (matchMap[fc] != -1 && matchMap[fc] == cmdp.workclass()) {
                    if (clustering.get(fc) instanceof SphereCluster) {
                        ...
                        if (hullDist < minHullDist) minHullDist = hullDist;
                    } else {
                        double min = 1;
                        double max = 1;
        ...
        if (gtAssignedID == -1) {
            System.out.println(
                    "Point "
                            ...
                            + " from gtcluster "
                            + cmdp.trueClass
                            + " assigned to noise cluster "
                            + assignedClusterID);
            return 1;
        }

        if (gtAssignedID == cmdp.workclass()) return 0;
        else {
