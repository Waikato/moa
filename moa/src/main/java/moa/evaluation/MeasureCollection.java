...
public abstract class MeasureCollection extends AbstractMOAObject {
    ...
    public double getMean(int index) {
        if (corrupted[index] || values[index].size() < 1) return Double.NaN;
    ...
    private void updateSortedValues(int index) {
        ...
        for (int i = sortedValues[index].size(); i < values[index].size(); i++) {
            ...
            int insertIndex = 0;
            while (!sortedValues[index].isEmpty()
                    && insertIndex < sortedValues[index].size()
                    && v > sortedValues[index].get(insertIndex)) insertIndex++;
    ...
    public double getMedian(int index) {
        ...
        if (size > 0) {
            if (size % 2 == 1) return sortedValues[index].get((int) (size / 2));
            else
                return (sortedValues[index].get((size - 1) / 2)
                                + sortedValues[index].get((size - 1) / 2 + 1))
                        / 2.0;
        }
        return Double.NaN;
    }
    ...
    public double getMeanRunningTime() {
        if (values[0].size() != 0) return (time / 10e5 / values[0].size());
        else return 0;
    }
    ...
    protected abstract void evaluateClustering(
     ...
     */
    public double evaluateClusteringPerformance(
            Clustering clustering, Clustering trueClustering, ArrayList<DataPoint> points)
            throws Exception {
    ...
    public String getEventType(int index) {
        if (index < events.size()) return events.get(index);
        else return null;
    }
}
