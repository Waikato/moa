...
public class CMM_GTAnalysis {
	...
	/**
	 * number of classes of the original ground truth, in case of a micro clustering
	 * ground truth this differs from numGTClusters
	 */
	...
	/**
	 * mapping between true cluster ID/class label of the original ground truth and
	 * the internal cluster ID/working class label.
	 * 
	...
	/**
	 * the threshold which defines when ground truth clusters will be merged. set to
	 * 1 to disable merging
	 */
	private double tauConnection = 0.5;

	/**
	 * experimental (default: disabled) separate k for points to cluster and cluster
	 * to cluster
	 */
	...
	protected class CMMPoint extends DataPoint {
		...
		/**
		 * Retruns the current working label of the cluster the point belongs to. The
		 * label can change due to merging of clusters.
		 * 
	...
	protected class GTCluster {
		...
		/**
		 * calculate knn distances of points within own cluster + average knn distance
		 * and average knn distance deviation of all points
		 */
		private void calculateKnn() {
			...
			knnMeanAvg = knnMeanAvg / points.size();
			knnDevAvg = knnDevAvg / points.size();
		...
		private void mergeCluster(int mergeID) {
			if (mergeID < gt0Clusters.size()) {
				...
				if (debug)
					System.out.println("Merging C" + gtcMerge.workclass + " into C" + workclass + " with Con "
							+ connections.get(mergeID) + " / " + gtcMerge.connections.get(workclass));
	 ...
	 * @param enableClassMerge allow class merging (should be set to true on
	 *                         default)
	 */
	 ...
	 * @param knnDist       sorted list of smallest knn distances (can already be
	 *                      filled to make updates possible)
	 * @param knnPointIndex list of corresponding knn indices
	 */
	private void getKnnInCluster(CMMPoint cmmp, int k, ArrayList<Integer> pointIDs, AutoExpandVector<Double> knnDist,
			AutoExpandVector<Integer> knnPointIndex) {

		// iterate over every point in the choosen cluster, cal distance and insert into
		// list
	...
	/**
	 * Calculate connections between clusters and merge clusters accordingly as long
	 * as connections exceed threshold
	 */
			...
			if (debug) {
				...
				for (int c = 0; c < gt0Clusters.size(); c++) {
					...
					for (int c1 = 0; c1 < gt0Clusters.get(c).connections.size(); c1++) {
						System.out.print(
								" C" + gt0Clusters.get(c1).label + ": " + gt0Clusters.get(c).connections.get(c1));
					}
					System.out.println("");
				}
				System.out.println("");
