 ...
 * [CMM_GTAnalysis.java]
 *
 * <p>CMM: Ground truth analysis
 *
 * <p>Reference: Kremer et al., "An Effective Evaluation Measure for Clustering on Evolving Data
 * Streams", KDD, 2011
 *
...
public class CMM_GTAnalysis {
    ...
    /**
     * number of classes of the original ground truth, in case of a micro clustering ground truth
     * this differs from numGTClusters
     */
    ...
    /**
     * mapping between true cluster ID/class label of the original ground truth and the internal
     * cluster ID/working class label.
     *
     * <p>different original cluster IDs might map to the same new cluster ID due to merging of two
     * clusters
     */
    ...
    /**
     * number of noise points that will create an error due to the underlying clustering model (e.g.
     * noise point being covered by a cluster)
     */
    ...
    /**
     * the threshold which defines when ground truth clusters will be merged. set to 1 to disable
     * merging
     */
    ...
    /**
     * experimental (default: disabled) use exponential connectivity function to model different
     * behavior: closer points will have a stronger connection compared to the linear function. Use
     * ConnRefXValue and ConnX to better parameterize lambda, which controls the decay of the
     * connectivity
     */
    ...
    protected class CMMPoint extends DataPoint {
        ...
        /**
         * Retruns the current working label of the cluster the point belongs to. The label can
         * change due to merging of clusters.
         *
         * @return the current working class label
         */
        protected int workclass() {
            if (trueClass == -1) return -1;
            else return mapTrueLabelToWorkLabel.get(trueClass);
        }
    }
    ...
    protected class GTCluster {
        ...
        /**
         * a new GT cluster consists of one or more "old" GT clusters. Connected/overlapping
         * clusters cannot be merged directly because of the underlying cluster model. E.g. for
         * merging two spherical clusters the new cluster sphere can cover a lot more space then two
         * separate smaller spheres. To keep the original coverage we need to keep the orignal
        ...
        protected double getInclusionProbability(CMMPoint point) {
            ...
            for (int c = 0; c < clusterRepresentations.size(); c++) {
                double tmp_prob =
                        gtClustering
                                .get(clusterRepresentations.get(c))
                                .getInclusionProbability(point);
        ...
        /**
         * calculate knn distances of points within own cluster + average knn distance and average
         * knn distance deviation of all points
         */
        private void calculateKnn() {
            for (int p0 : points) {
                ...
                if (!cmdp.isNoise()) {
                    ...
                    if (knnDist.size() != 0) avgKnn /= knnDist.size();
        ...
        private void calculateClusterConnection(int otherCid, boolean initial) {
            ...
            if (workclass == otherCid) {
                ...
                for (int p : points) {
                    ...
                    if (kmax.size() < clusterConnectionMaxPoints
                            || connection > kmax.get(kmax.size() - 1)) {
                        int index = 0;
            ...
            if (otherCid < connections.size()) {
                ...
                System.out.println(
                        "Something is going really wrong with the connection listing!"
                                + knnNeighbourhood
                                + " "
                                + tauConnection);
        }
        ...
        private void mergeCluster(int mergeID) {
            if (mergeID < gt0Clusters.size()) {
                ...
                for (int i = 0; i < numGTClasses; i++) {
                    if (mergeMap[i] == mergeID) mergeMap[i] = workclass;
                    if (mergeMap[i] > mergeID) mergeMap[i]--;
                }
                ...
                while (iterator.hasNext()) {
                    ...
                    if (value == mergeID) mapTrueLabelToWorkLabel.put(key, workclass);
                    if (value > mergeID) mapTrueLabelToWorkLabel.put(key, value - 1);
                }
    ...
    public CMM_GTAnalysis(
            Clustering trueClustering, ArrayList<DataPoint> points, boolean enableClassMerge) {
        if (debug) System.out.println("GT Analysis Debug Output");
        ...
        if (debug) System.out.println("GT Analysis Debug End");
    }
    ...
    protected double getConnectionValue(CMMPoint cmmp, int clusterID) {
        ...
        getKnnInCluster(
                cmmp, knnNeighbourhood, gt0Clusters.get(clusterID).points, knnDist, knnPointIndex);
        ...
        if (knnDist.size() != 0) avgDist /= knnDist.size();
        else return 0;

        // get the upper knn distance of the cluster
        double upperKnn =
                gt0Clusters.get(clusterID).knnMeanAvg + gt0Clusters.get(clusterID).knnDevAvg;
     ...
     * @param knnDist sorted list of smallest knn distances (can already be filled to make updates
     *     possible)
     * @param knnPointIndex list of corresponding knn indices
     */
    private void getKnnInCluster(
            CMMPoint cmmp,
            int k,
    ...
    /**
     * Calculate connections between clusters and merge clusters accordingly as long as connections
     * exceed threshold
     */
        ...
        while (changedConnection) {
            ...
            for (int c0 = 0; c0 < gt0Clusters.size(); c0++) {
                for (int c1 = c0 + 1; c1 < gt0Clusters.size(); c1++) {
                    ...
                    double min =
                            Math.min(
                                    gt0Clusters.get(c0).connections.get(c1),
    ...
    /**
     * Calculates how well the original clusters are separable. Small values indicate bad
     * separability, values close to 1 indicate good separability
     *
    ...
    /**
     * Calculates how well noise is separable from the given clusters Small values indicate bad
     * separability, values close to 1 indicate good separability
     *
     * @return index of noise separability
     */
    public double getNoiseSeparability() {
        if (noise.isEmpty()) return 1;
        ...
        for (int p : noise) {
            ...
            for (int c = 0; c < gt0Clusters.size(); c++) {
                ...
                if (connection > maxConnection) maxConnection = connection;
            }
    ...
    public double getModelQuality() {
            ...
            for (int hc = 0; hc < numGTClusters; hc++) {
                if (gtClustering.get(hc).getGroundTruth() != cmdp.trueClass) {
                    if (gtClustering.get(hc).getInclusionProbability(cmdp) >= 1) {
                        if (!cmdp.isNoise()) pointErrorByModel++;
                        else noiseErrorByModel++;
                        break;
                    }
                }
            }
        ...
        if (debug)
            System.out.println(
                    "Error by model: noise " + noiseErrorByModel + " point " + pointErrorByModel);
