...
/**
 * This task individually evaluates an active learning classifier for each element of a set of
 * parameter values. The individual evaluation is done by prequential evaluation (testing, then
...
public class ALMultiParamTask extends ALMainTask {
    ...
    /**
     * Default constructor which sets up the refresh mechanism between the learner and the
     * variedParamName option.
     */
    public ALMultiParamTask() {
        ...
        new DependentOptionsUpdater(
                this.prequentialEvaluationTaskOption, this.variedParamNameOption);
    }

    /**
     * Constructor that sets the color coding for the subtasks additionally to the default
     * constructor.
     *
    ...
    protected void prepareForUseImpl(TaskMonitor monitor, ObjectRepository repository) {
            ...
            if (ownTaskResultFile != null) {
                this.outputFileOption.setValue(
                        this.insertFileNameExtension(
                                ownTaskResultFile.getAbsolutePath(), "_p" + this.partitionIdx));
            }
        }
        ...
        String baseTaskResultName =
                baseTaskResultFile != null ? baseTaskResultFile.getAbsolutePath() : null;
            ...
            if (learnerVariedParamOption != null) {
                ...
                if (baseDumpFileName != null) {
                    paramValueTask.dumpFileOption.setValue(
                            this.insertFileNameExtension(baseDumpFileName, outputFileExtension));
                }

                // adapt the task result file name
                if (baseTaskResultName != null) {
                    paramValueTask.outputFileOption.setValue(
                            this.insertFileNameExtension(baseTaskResultName, outputFileExtension));
                }
            }
    ...
    protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
        ...
        PreviewCollection<PreviewCollectionLearningCurveWrapper> previewCollection =
                new PreviewCollection<>(
                        "multi param entry id",
                        "learner id",
                        this.getClass(),
