...
import moa.learners.predictors.active.ALClassifier;
...
public class ALMultiParamTask extends ALMainTask {
	...
	public ClassOptionWithListenerOption prequentialEvaluationTaskOption = new ClassOptionWithListenerOption(
			"prequentialEvaluationTask", 'e',
			"Prequential evaluation task to be performed for each " + "parameter value.",
			ALPrequentialEvaluationTask.class, "moa.tasks.meta.ALPrequentialEvaluationTask");

	public EditableMultiChoiceOption variedParamNameOption = new EditableMultiChoiceOption("variedParamName", 'p',
			"Name of the parameter to be varied.", new String[] { "budget" },
			new String[] { "default varied parameter name" }, 0);

	public ListOption variedParamValuesOption = new ListOption("variedParamValues", 'v',
			"List of parameter values to train classifiers for.",
			new FloatOption("value", ' ', "Parameter value.", 0.0),
			new FloatOption[] { new FloatOption("", ' ', "", 0.1), new FloatOption("", ' ', "", 0.2),
					new FloatOption("", ' ', "", 0.5) },
			',');
	...
	/**
	 * Default constructor which sets up the refresh mechanism between the learner
	 * and the variedParamName option.
	 */
	...
	/**
	 * Constructor that sets the color coding for the subtasks additionally to the
	 * default constructor.
	 *
	...
	protected void prepareForUseImpl(TaskMonitor monitor, ObjectRepository repository) {
		...
		ALPrequentialEvaluationTask evalTask = (ALPrequentialEvaluationTask) this.prequentialEvaluationTaskOption
		...
		if (this.partitionIdx >= 0) {
			...
			if (ownTaskResultFile != null) {
				this.outputFileOption.setValue(
						this.insertFileNameExtension(ownTaskResultFile.getAbsolutePath(), "_p" + this.partitionIdx));
			}
		}
		...
		for (int i = 0; i < paramValues.length; i++) {
			...
			if (learnerVariedParamOption != null) {
				...
				if (baseDumpFileName != null) {
					paramValueTask.dumpFileOption
							.setValue(this.insertFileNameExtension(baseDumpFileName, outputFileExtension));
				}

				// adapt the task result file name
				if (baseTaskResultName != null) {
					paramValueTask.outputFileOption
							.setValue(this.insertFileNameExtension(baseTaskResultName, outputFileExtension));
				}
			}

			paramValueTask.learnerOption
					.setValueViaCLIString(ClassOption.objectToCLIString(learner, ALClassifier.class));
	...
	protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
		...
		PreviewCollection<PreviewCollectionLearningCurveWrapper> previewCollection = new PreviewCollection<>(
				"multi param entry id", "learner id", this.getClass(), this.variedParamNameOption.getValueAsCLIString(),
				variedParamValues);
		...
		while (!allThreadsCompleted) {
			...
			for (int i = 0; i < numSubtaskThreads; ++i) {
				...
				PreviewCollectionLearningCurveWrapper latestPreview = (PreviewCollectionLearningCurveWrapper) currentTaskThread
				...
				if (latestPreview != null && latestPreview.numEntries() > 0) {
					...
					PreviewCollectionLearningCurveWrapper finalPreview = (PreviewCollectionLearningCurveWrapper) currentTaskThread
							.getFinalResult();
