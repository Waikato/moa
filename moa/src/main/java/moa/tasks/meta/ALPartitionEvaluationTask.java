...
/**
 * This task extensively evaluates an active learning classifier on a stream. First, the given data
 * set is partitioned into subsets, each leaving out a different part of the overall data. On each
 * subset, the ALMultiParamTask is performed which individually evaluates the active learning
 * classifier for each element of a set of parameter values. The individual evaluation is done by
...
public class ALPartitionEvaluationTask extends ALMainTask {
    ...
    public ClassOption multiParamTaskOption =
            new ClassOption(
                    ...
                    ALMultiParamTask.class,
                    "moa.tasks.meta.ALMultiParamTask");

    public IntOption numPartitionsOption =
            new IntOption("numPartitions", 'k', "Number of data set partitions.", 10);

    public IntOption randomSeedOption =
            new IntOption(
                    "randomSeed",
                    'r',
                    "random seed which is used for partitioning of the stream.",
                    0);
    ...
    protected void prepareForUseImpl(TaskMonitor monitor, ObjectRepository repository) {
        ...
        // get subtask objects
        ALMultiParamTask multiParamTask =
                (ALMultiParamTask) this.multiParamTaskOption.getPreMaterializedObject();
        ALPrequentialEvaluationTask evalTask =
                (ALPrequentialEvaluationTask)
                        multiParamTask.prequentialEvaluationTaskOption.getPreMaterializedObject();
        ...
        for (int i = 0; i < this.numPartitionsOption.getValue(); i++) {
            ...
            ALPrequentialEvaluationTask partitionEvalTask =
                    (ALPrequentialEvaluationTask)
                            partitionTask.prequentialEvaluationTaskOption
    ...
    protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
        ...
        while (!allThreadsCompleted) {
            ...
            for (int i = 0; i < numSubtaskThreads; ++i) {
                ...
                PreviewCollection<PreviewCollectionLearningCurveWrapper> latestPreview =
                ...
                if (latestPreview != null && latestPreview.numEntries() > 0) {
                    ...
                    @SuppressWarnings("unchecked")
                    PreviewCollection<PreviewCollectionLearningCurveWrapper> finalPreview =
                            (PreviewCollection<PreviewCollectionLearningCurveWrapper>)
