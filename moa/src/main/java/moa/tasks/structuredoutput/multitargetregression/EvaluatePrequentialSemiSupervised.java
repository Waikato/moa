...
import moa.evaluation.evaluators.MultiTargetRegressionPerformanceEvaluator;
import moa.evaluation.evaluators.WindowClassificationPerformanceEvaluator;
import moa.evaluation.preview.LearningCurve;
import moa.learners.predictors.SemiSupervisedMultiTargetRegressor;
 ...
 * 
 * Phase1: Creates a initial model with <dbInitialModelPercentage> of the
 * instances in the dataset Phase2: When an instance is received: A binary
 * random process with a binomial distribution selects if the instance should be
...
public class EvaluatePrequentialSemiSupervised extends MainTask implements MultiTargetRegressionMainTask {
	...
	public ClassOption learnerOption = new ClassOption("learner", 'l', "Learner to train.",
			SemiSupervisedMultiTargetRegressor.class, "rules.multilabel.AMRulesMultiTargetRegressorSemiSuper");
	public ClassOption streamOption = new ClassOption("stream", 's', "Stream to learn from.",
			MultiTargetInstanceStream.class, "MultiTargetArffFileStream");
	public ClassOption evaluatorOption = new ClassOption("evaluator", 'e',
			"Classification performance evaluation method.", MultiTargetRegressionPerformanceEvaluator.class,
	...
	public IntOption widthOption = new IntOption("width", 'w', "Size of Window", 1000); // New for prequential method
																						// DEPRECATED
	public FloatOption alphaOption = new FloatOption("alpha", 'a', "Fading factor or exponential smoothing factor",
			.01);
	public FloatOption unlabeledPercentage = new FloatOption("WithoutTarget", 'z', "Without target percentage(%)", 50);
	...
	protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
		...
		SemiSupervisedMultiTargetRegressor learner = (SemiSupervisedMultiTargetRegressor) getPreparedClassOption(
				this.learnerOption);
		ExampleStream<Example<Instance>> stream = (ExampleStream<Example<Instance>> ) getPreparedClassOption(this.streamOption);
		LearningPerformanceEvaluator<Example<Instance>> evaluator = (LearningPerformanceEvaluator<Example<Instance>>) getPreparedClassOption(
		...
		List<Double> slidingWindow = new LinkedList<>();
		...
		while (examplesCounter < dbInitialModelPercentage.getValue() / 100 * StrmDtSz) {
			...
			if (examplesCounter > 1) {
				Prediction trainPrediction = learner.getTrainingPrediction(inst);
				double sumDenominator = 0;
				double sumNumerator = 0;
				for (int m = 0; m < inst.numOutputAttributes(); m++) {
					// sumNumerator+=Math.pow( inst.valueOutputAttribute(m) -
					// learner.prediction.getVote(m,0) , 2 );
