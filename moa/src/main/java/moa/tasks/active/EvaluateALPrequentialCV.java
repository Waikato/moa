 ...
 *
 * This class is basically a copy of EvaluatePrequentialCV except for some limitations in selecting the evaluator 
 * and learner. The main difference is that this class enables the counting of acquisitions of an active learner.
 *
 * @author Daniel Kottke (daniel.kottke@ovgu.de)
 * @author Richard Kirkby (rkirkby@cs.waikato.ac.nz)
 * @author Albert Bifet (abifet at cs dot waikato dot ac dot nz)
...
public class EvaluateALPrequentialCV extends ALMainTask {
    ...
    public String getPurposeString() {
        return "Evaluates an active learning classifier on a stream by doing prequential evaluation (testing then" +
        ...
        while (stream.hasMoreInstances()
            ...
            for (int i = 0; i < learners.length; i++) {
                ...
                if (k > 0) {
                	...
                	int labelAcquired = 0;
                	try {
                    	labelAcquired = ((ALClassifier) learners[i]).getLastLabelAcqReport();
