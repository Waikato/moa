 ...
 * @version $Revision: 7 $
 */
public class EvaluatePeriodicHeldOutTest extends MainTask implements ClassificationMainTask, CapabilitiesHandler {
	...
	protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
		...
		if (this.cacheTestOption.isSet()) {
			...
			InstancesHeader testInstances = new InstancesHeader(stream.getHeader(), this.testSizeOption.getValue());
			...
			testStream = stream;
			/*
			 * monitor.setCurrentActivity("Skipping test examples...", -1.0); for (int i =
			 * 0; i < testSize; i++) { stream.nextInstance(); }
			 */
		}
		...
		while ((this.trainSizeOption.getValue() < 1 || instancesProcessed < this.trainSizeOption.getValue())
			...
			for (instCount = 0; instCount < testSize; instCount++) {
				...
				Example testInst = testStream.nextInstance(); // .copy();
			...
			measurements
					.add(new Measurement("last train speed", this.sampleFrequencyOption.getValue() / lastTrainTime));
			...
			learningCurve
					.insertEntry(new LearningEvaluation(measurements.toArray(new Measurement[measurements.size()])));
