...
import moa.tasks.AbstractEvaluatePrequentialCV;
import moa.tasks.TaskMonitor;

/**
 * Task for delayed cross-validation evaluation of a classifier on a stream by
 * testing and only training with the example after the arrival of other k
 * examples (delayed labeling).
 *
 * <p>
 * See details in:<br>
 ...
 * <br>
 * <p>
 * Cross-validation for data streams was originally proposed in:<br>
 ...
 * @version $Revision: 7 $
 */
public class EvaluatePrequentialDelayedCV extends AbstractEvaluatePrequentialCV<Classifier>
	...
	public ClassOption learnerOption = new ClassOption("learner", 'l', "Learner to train.", Classifier.class,
			"bayes.NaiveBayes");
	...
	public IntOption delayLengthOption = new IntOption("delay", 'k',
			"Number of instances before test instance is used for training", 1000, 1, Integer.MAX_VALUE);
	...
	public MultiChoiceOption validationMethodologyOption = new MultiChoiceOption("validationMethodology", 'a',
			"Validation methodology to use.",
			new String[] { "Cross-Validation", "Bootstrap-Validation", "Split-Validation" },
			new String[] { "k-fold distributed Cross Validation", "k-fold distributed Bootstrap Validation",
	...
	protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
		...
		MLTask[] learners = new MLTask[this.numFoldsOption.getValue()];
		MLTask baseLearner = (MLTask) getPreparedClassOption(this.learnerOption);
		baseLearner.resetLearning();

		LearningPerformanceEvaluator[] evaluators = new LearningPerformanceEvaluator[this.numFoldsOption.getValue()];
		LearningPerformanceEvaluator baseEvaluator = (LearningPerformanceEvaluator) getPreparedClassOption(
				this.evaluatorOption);
		for (int i = 0; i < learners.length; i++) {
			learners[i] = (MLTask) baseLearner.copy();
		...
		while (stream.hasMoreInstances() && ((maxInstances < 0) || (instancesProcessed < maxInstances))
			...
			for (int i = 0; i < learners.length; i++) {
				...
				double[] prediction = learners[i].getPredictionForInstance(testInst).asDoubleArray();
			...
			if (instancesProcessed % this.sampleFrequencyOption.getValue() == 0 || stream.hasMoreInstances() == false) {
				...
				learningCurve
						.insertEntry(new LearningEvaluation(getEvaluationMeasurements(
								new Measurement[] {
										new Measurement("learning evaluation instances", instancesProcessed),
										new Measurement(
												"evaluation time (" + (preciseCPUTiming ? "cpu " : "") + "seconds)",
												time),
