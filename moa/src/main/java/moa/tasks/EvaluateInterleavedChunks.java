...
public class EvaluateInterleavedChunks extends ClassificationMainTask {
    ...
    public String getPurposeString() {
        return "Evaluates a classifier on a stream by testing then training with chunks of data in"
                + " sequence.";
    }
    ...
    protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
        ...
        while (stream.hasMoreInstances()
            ...
            if (instancesProcessed % this.sampleFrequencyOption.getValue() == 0) {
                ...
                RAMHoursIncrement *=
                        (TimingUtils.nanoTimeToSeconds(sampleTrainTime + sampleTestTime)
                                / 3600.0); // Hours
                RAMHours += RAMHoursIncrement;

                double avgTrainTime =
                        TimingUtils.nanoTimeToSeconds(sampleTrainTime)
                                / ((double) this.sampleFrequencyOption.getValue()
                                        / chunkInstances.numInstances());
                double avgTestTime =
                        TimingUtils.nanoTimeToSeconds(sampleTestTime)
                                / ((double) this.sampleFrequencyOption.getValue()
                                        / chunkInstances.numInstances());
                ...
                learningCurve.insertEntry(
                        new LearningEvaluation(
                                new Measurement[] {
                                    ...
                                    new Measurement(
                                            "average chunk train speed",
                                            chunkInstances.numInstances() / avgTrainTime),
                                    new Measurement("average chunk test time", avgTestTime),
                                    new Measurement(
                                            "average chunk test speed",
                                            chunkInstances.numInstances() / avgTestTime),
