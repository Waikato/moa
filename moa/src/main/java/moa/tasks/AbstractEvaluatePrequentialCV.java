/*
 *    AbstractEvaluatePrequentialCV.java
...
/**
 * Task for prequential cross-validation evaluation of a classifier on a stream
 * by testing then training with each example in sequence and doing
 ...
 * @version $Revision: 7 $
 */
public abstract class AbstractEvaluatePrequentialCV<MLTask extends InstanceLearner> extends MainTask {
	...
	protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
		...
		ArrayList<MLTask> learners = new ArrayList<>(this.numFoldsOption.getValue());
		...
		for (int i = 0; i < learners.size(); i++) {
			learners.set(i, (MLTask) baseLearner.copy());
			learners.get(i).setModelContext(stream.getHeader());
		...
		while (stream.hasMoreInstances() && ((maxInstances < 0) || (instancesProcessed < maxInstances))
			...
			for (int i = 0; i < learners.size(); i++) {
				evaluators[i].addResult(testInst, learners.get(i).getPredictionForInstance(testInst));
			}
			...
			for (int i = 0; i < learners.size(); i++) {
				...
				case 0: // Cross-Validation;
					k = instancesProcessed % learners.size() == i ? 0 : 1; // Test all except one
					break;
				case 1: // Bootstrap;
					k = MiscUtils.poisson(1, random);
					break;
				case 2: // Split-Validation;
					k = instancesProcessed % learners.size() == i ? 1 : 0; // Test only one
					break;
				}
				if (k > 0) {
					Example weightedInst = trainInst.copy();
					weightedInst.setWeight(trainInst.weight() * k);
					learners.get(i).trainOnInstance(weightedInst);
				}
			}
			...
			if (instancesProcessed % this.sampleFrequencyOption.getValue() == 0 || stream.hasMoreInstances() == false) {
				...
				for (int i = 0; i < learners.size(); i++) {
					double RAMHoursIncrement = learners.get(i).measureByteSize() / (1024.0 * 1024.0 * 1024.0); // GBs
