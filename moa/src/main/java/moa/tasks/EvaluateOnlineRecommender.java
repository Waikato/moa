...
/**
 * Test for evaluating a recommender by training and periodically testing on samples from a rating
 * dataset. When finished, it will show the learning curve of the recommender rating predictor.
 *
 * <p>Parameters:
 *
 * <ul>
 *   <li>d: dataset - the dataset to be used to train/test the rating predictor.
 *   <li>f: sample frequency - the frequency in which a rating from the dataset will be used to test
 *       the model
 * </ul>
 *
 * @author Alex Catarineu (a.catarineu@gmail.com)
...
public class EvaluateOnlineRecommender extends AuxiliarMainTask {
    ...
    public ClassOption datasetOption =
            new ClassOption(
                    "dataset",
                    'd',
                    "Dataset to evaluate.",
    ...
    public ClassOption ratingPredictorOption =
            new ClassOption(
                    "ratingPredictor",
                    's',
                    "Rating Predictor to evaluate on.",
            ...
            if (n % sampleFrequency == sampleFrequency - 1) {
                ...
                learningCurve.insertEntry(
                        new LearningEvaluation(
                                new Measurement[] {
                                    new Measurement("n", n),
                                    new Measurement("RMSE", Math.sqrt(sum / (double) n)),
                                    new Measurement(
                                            "trainingTime",
                                            (int)
                                                    ((System.currentTimeMillis() - start - evalTime)
                                                            / 1000)),
                                    new Measurement("evalTime", (int) (evalTime / 1000))
                                }));
                if (monitor.resultPreviewRequested()) {
                    monitor.setLatestResultPreview(
                            learningCurve.headerToString()
                                    + "\n"
