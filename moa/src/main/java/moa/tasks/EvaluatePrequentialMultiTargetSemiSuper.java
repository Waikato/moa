 ...
 *
 * <p>Phase1: Creates a initial model with <dbInitialModelPercentage> of the instances in the
 * dataset Phase2: When an instance is received: A binary random process with a binomial
 * distribution selects if the instance should be labeled or unlabeled with probability
 * <unlabeledPercentage>.
 *
 * @author RSousa
 * @version $Revision: 2 $
...
public class EvaluatePrequentialMultiTargetSemiSuper extends MultiTargetMainTask {
    ...
    public String getPurposeString() {
        return "Multi-target Prequential semi-supervised evaluation \n"
                ...
                + " Phase2: When a instance is received: A binary random process with a binomial"
                + " distribution selects if the instance should be  labeled or unlabeled with"
                + " probability <unlabeledPercentage>.";
    }
    ...
    public ClassOption learnerOption =
            new ClassOption(
                    ...
                    'l',
                    "Learner to train.",
                    MultiTargetLearnerSemiSupervised.class,
                    "moa.classifiers.rules.multilabel.AMRulesMultiTargetRegressorSemiSuper");
    ...
    public IntOption widthOption =
            new IntOption(
                    "width", 'w', "Size of Window", 1000); // New for prequential method DEPRECATED
    ...
    public FloatOption unlabeledPercentage =
            new FloatOption("WithoutTarget", 'z', "Without target percentage(%)", 50);
    public FloatOption dbInitialModelPercentage =
            new FloatOption("DBPercent", 'D', "Initial dataset (%)", 30);
    ...
    protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
        ...
        MultiTargetLearnerSemiSupervised learner =
                (MultiTargetLearnerSemiSupervised) getPreparedClassOption(this.learnerOption);
        ...
        if (evaluator instanceof FadingFactorClassificationPerformanceEvaluator) {
            ...
            if (alphaOption.getValue() != .01) {
                System.out.println(
                        "DEPRECATED! Use EvaluatePrequential -e"
                                + " (FadingFactorClassificationPerformanceEvaluator -a "
                                + alphaOption.getValue()
                                + ")");
                return learningCurve;
            }
        ...
        while (examplesCounter < dbInitialModelPercentage.getValue() / 100 * StrmDtSz) {
            ...
            if (examplesCounter > 1) {
                ...
                for (int m = 0; m < inst.numOutputAttributes(); m++) {
                    ...
                    sumNumerator +=
                            Math.pow(
                                    inst.valueOutputAttribute(m) - trainPrediction.getVote(m, 0),
                                    2);
        ...
        while (examplesCounter < StrmDtSz - 1) {
            ...
            if (instancesProcessed % this.sampleFrequencyOption.getValue()
                    == 0) { // || stream.hasMoreInstances() == false
