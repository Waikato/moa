...
public abstract class AbstractLearnModel<MLTask extends InstanceLearner> extends MainTask
	...
	public IntOption maxInstancesOption = new IntOption("maxInstances", 'm',
			"Maximum number of instances to train on per pass over the data.", 10000000, 0, Integer.MAX_VALUE);

	public IntOption numPassesOption = new IntOption("numPasses", 'p', "The number of passes to do over the data.", 1,
			1, Integer.MAX_VALUE);
	...
	public AbstractLearnModel(MLTask learner, InstanceStream stream, int maxInstances, int numPasses) {
	...
	public Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
		...
		for (int pass = 0; pass < numPasses; pass++) {
			...
			monitor.setCurrentActivity("Training learner"
					+ (numPasses > 1 ? (" (pass " + (pass + 1) + "/" + numPasses + ")") : "") + "...", -1.0);
			if (pass > 0) {
				stream.restart();
			}
