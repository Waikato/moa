...
/**
 * Abstract Main Task. All tasks that want to write their result to a file must
 * extend this class.
 *
 * @author Richard Kirkby (rkirkby@cs.waikato.ac.nz)
 * @version $Revision: 7 $
...
public abstract class MainTask extends AbstractTask {
	...
	protected Object doTaskImpl(TaskMonitor monitor, ObjectRepository repository) {
		...
		if (outputFile != null) {
			if (result instanceof Serializable) {
				monitor.setCurrentActivity("Saving result of task " + getTaskName() + " to file " + outputFile + "...",
						-1.0);
				try {
					SerializeUtils.writeToFile(outputFile, (Serializable) result);
				} catch (IOException ioe) {
					throw new RuntimeException(
							"Failed writing result of task " + getTaskName() + " to file " + outputFile, ioe);
				}
			} else {
				throw new RuntimeException("Result of task " + getTaskName()
						+ " is not serializable, so cannot be written to file " + outputFile);
			}
		}
		return result;
	}
	 ...
	 * <code>doTask</code>, that uses <code>doTaskImpl</code>. <code>MainTask</code>
	 * implements <code>doTaskImpl</code> using <code>doMainTask</code> so its
	 * extensions only need to implement <code>doMainTask</code>.
	 *
