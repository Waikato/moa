...
public class Plot extends MainTask {
    ...
    public StringOption gnuplotPathOption =
            new StringOption(
                    "gnuplotPath",
                    'e',
                    "Directory of the gnuplot executable. For example"
                            + " \"C:\\Tools\\Gnuplot\\binary\".",
                    "");

    /** FileOption for selecting the plot output file. */
    public FileOption plotOutputOption =
            new FileOption(
                    "plotOutputFile", 'r', "File with the result plot (image).", null, "eps", true);

    /**
     * Comma separated list of input *csv files. The file paths can be absolute or relative to the
     * executing directory (moa.jar directory).
     */
    ...
    /**
     * Comma separated list of aliases for the input *csv files. If a legend is added to the plot,
     * aliases will be presented in the legend.
     */
    public ListOption fileAliasesOption =
            new ListOption(
                    "aliases",
                    'a',
                    "Aliases for files stated in the inputFiles parameter. Aliases will be"
                            + " presented in the plot's legend.",
                    new StringOption("alias", ' ', "File alias.", "MyAlg"),
                    new StringOption[] {
                        new StringOption("", ' ', "", "OZABag"),
    ...
    public MultiChoiceOption outputTypeOption =
            new MultiChoiceOption(
                    ...
                    Terminal.getStringValues(),
                    Terminal.getDescriptions(),
                    8);
    ...
    public MultiChoiceOption plotStyleOption =
            new MultiChoiceOption(
                    ...
                    'p',
                    "Plot style.",
                    PlotStyle.getStringValues(),
                    PlotStyle.getDescriptions(),
                    2);
    ...
    public StringOption xTitleOption =
            new StringOption("xTitle", 'm', "Title of the plots' x-axis.", "Processed instances");

    /** Units displayed next to x-axis values. */
    public StringOption xUnitOption =
            new StringOption("xUnit", 'g', "Units displayed next to x-axis values.", "");
    ...
    public StringOption yTitleOption =
            new StringOption("yTitle", 'n', "Title of the plots' y-axis.", "Accuracy");

    /** Units displayed next to y-axis values. */
    public StringOption yUnitOption =
            new StringOption("yUnit", 'u', "Units displayed next to y-axis values.", "%");

    /** Plotted line width. */
    public IntOption lineWidthOption =
            new IntOption("lineWidth", 'w', "Determines the thickness of plotted lines", 2);
    ...
    public IntOption pointIntervalOption =
            new IntOption(
                    "pointInterval",
                    'v',
                    "Determines the inteval between plotted data points. Used for LINESPOINTS plots"
                            + " only.",
                    0,
                    0,
                    Integer.MAX_VALUE);
    ...
    public FlagOption smoothOption =
            new FlagOption(
                    "smooth", 's', "Determines whether to smooth the plot with bezier curves.");
    ...
    public MultiChoiceOption legendLocationOption =
            new MultiChoiceOption(
                    ...
                    'l',
                    "Legend (key) location on the plot.",
                    LegendLocation.getStringValues(),
                    LegendLocation.getDescriptions(),
                    8);
    ...
    public MultiChoiceOption legendTypeOption =
            new MultiChoiceOption(
                    ...
                    'k',
                    "Legend elements' alignment.",
                    LegendType.getStringValues(),
                    LegendType.getDescriptions(),
                    1);

    /**
     * Addition pre-plot gunplot commands. For example "set tics out" will change the default tic
     * option and force outward facing tics. See the gnuplot manual for more commands.
     */
    public StringOption additionalSetOption =
            new StringOption(
                    "additionalCommands",
                    'c',
                    "Additional commands that should be added to the gnuplot script before the plot"
                        + " command. For example \"set tics out\" will change the default tic"
                        + " option and force outward facing tics. See the gnuplot manual for more"
                        + " commands.",
                    " ");

    /**
     * Additional plot options. For example \"[] [0:]\" will force the y-axis to start from 0. See
     * the gnuplot manual for more options.
     */
    public StringOption additionalPlotOption =
            new StringOption(
                    "additionalPlotOptions",
                    'z',
                    "Additional options that should be added to the gnuplot script in the plot"
                        + " statement. For example \"[] [0:]\" will force the y-axis to start from"
                        + " 0. See the gnuplot manual for more options.",
                    " ");
    ...
    public enum PlotStyle {
        ...
        private static String[] descriptions =
                new String[] {
                    ...
                    "Symbols are shown at the data point location, can be used to plot experimental"
                            + " data.",
                    "Draws lines and symbols at the same time.",
                    "Draw vertical lines from each data point to X-axis. This is a bar-graph"
                            + " without width.",
                    "Histogram type 1",
                    "Histogram type 2",
                    "Histogram type 3",
                    "It displays dots, can be used when there many data points, but hard to see"
                            + " though.",
                };
    ...
    protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
        ...
        if (inputFilesOption.getList().length > fileAliasesOption.getList().length) {
            ...
            for (int i = 0; i < inputFilesOption.getList().length; i++) {
                File inputFile =
                        new File(((StringOption) inputFilesOption.getList()[i]).getValue());
        ...
        String gnuplotCommand =
                gnuPlotPath + File.separator + "gnuplot \"" + gnuplotScriptPath + "\"";
    ...
    private File writeScriptToFile(String gnuplotScriptPath, String script) {
        ...
        try {
            ...
            throw new RuntimeException(
                    "Unable to create or write to script file: " + scriptFile, ex);
        }
        return scriptFile;
    }
    ...
    private String createScript(File resultFile) {
        ...
        String script =
                "set term "
                        + terminalOptions(Terminal.valueOf(outputTypeOption.getChosenLabel()))
                        + newLine;
        ...
        if (!legendTypeOption.getChosenLabel().equals(LegendType.NONE)) {
            script +=
                    "set key "
                            + legendTypeOption.getChosenLabel().toLowerCase().replace('_', ' ')
                            + " "
                            + legendLocationOption.getChosenLabel().toLowerCase().replace('_', ' ')
                            + newLine;
        }
        ...
        for (int i = 0; i < inputFilesOption.getList().length; i++) {
            ...
            script +=
                    "'"
                            + ((StringOption) inputFilesOption.getList()[i]).getValue()
                            + "' using "
                            + xColumnOption.getValue()
                            + ":"
                            + yColumnOption.getValue();
            ...
            script +=
                    " with "
                            + plotStyleOption.getChosenLabel().toLowerCase()
                            + " ls "
                            + sourceFileIdx
                            + " lw "
