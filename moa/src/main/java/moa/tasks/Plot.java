...
public class Plot extends MainTask {
	 ...
	 */
	public FileOption plotOutputOption = new FileOption("plotOutputFile", 'r', "File with the result plot (image).",
			null, "eps", true);

	/**
	 * Comma separated list of input *csv files. The file paths can be absolute or
	 * relative to the executing directory (moa.jar directory).
	 */
	public ListOption inputFilesOption = new ListOption("inputFiles", 'i',
			...
			new StringOption[] { new StringOption("", ' ', "", "algorithm1.csv"),
					new StringOption("", ' ', "", "algorithm2.csv"), new StringOption("", ' ', "", "algorithm3.csv") },
			',');
	 ...
	 */
	public ListOption fileAliasesOption = new ListOption("aliases", 'a',
			...
			new StringOption[] { new StringOption("", ' ', "", "OZABag"), new StringOption("", ' ', "", "HOT"),
	 ...
	 */
	public MultiChoiceOption outputTypeOption = new MultiChoiceOption("outputType", 't', "Gnuplot output terminal.",
			Terminal.getStringValues(), Terminal.getDescriptions(), 8);

	/**
	 * Type of plot - dots, points, lines ets.
	 */
	public MultiChoiceOption plotStyleOption = new MultiChoiceOption("plotStyle", 'p', "Plot style.",
			PlotStyle.getStringValues(), PlotStyle.getDescriptions(), 2);

	/**
	 * Index of the csv column from which values for the x-axis should be taken.
	 */
	public IntOption xColumnOption = new IntOption("xColumn", 'x',
			"Index of the csv column from which values for the x-axis should be taken.", 1);

	/**
	 * Title of the plots' x-axis.
	 */
	public StringOption xTitleOption = new StringOption("xTitle", 'm', "Title of the plots' x-axis.",
			"Processed instances");
	 ...
	 */
	public IntOption yColumnOption = new IntOption("yColumn", 'y',
			"Index of the column from which values for the y-axis should be taken", 9);
	 ...
	 */
	public IntOption pointIntervalOption = new IntOption("pointInterval", 'v',
			"Determines the inteval between plotted data points. Used for LINESPOINTS plots only.", 0, 0,
			Integer.MAX_VALUE);
	/**
	 ...
	 */
	public FlagOption deleteScriptsOption = new FlagOption("deleteScripts", 'd',
			"Determines whether to delete gnuplot scripts after plotting.");

	/**
	 * Legend (key) location on the plot.
	 */
	public MultiChoiceOption legendLocationOption = new MultiChoiceOption("legendLocation", 'l',
			"Legend (key) location on the plot.", LegendLocation.getStringValues(), LegendLocation.getDescriptions(),
			8);

	/**
	 * Legend elements' alignment.
	 */
	public MultiChoiceOption legendTypeOption = new MultiChoiceOption("legendType", 'k', "Legend elements' alignment.",
			LegendType.getStringValues(), LegendType.getDescriptions(), 1);

	/**
	 * Addition pre-plot gunplot commands. For example "set tics out" will change
	 * the default tic option and force outward facing tics. See the gnuplot manual
	 * for more commands.
	 */
	public StringOption additionalSetOption = new StringOption("additionalCommands", 'c',
	 ...
	 */
	public StringOption additionalPlotOption = new StringOption("additionalPlotOptions", 'z',
	...
	public enum Terminal {
		...
		private static String[] descriptions = new String[] { "HTML Canvas object",
				"LaTeX picture environment using graphicx package", "GIF images using libgd and TrueType fonts",
				"JPEG images using libgd and TrueType fonts", "LaTeX picture environment",
				"pdf terminal based on cairo", "PNG images using libgd and TrueType fonts",
				"PostScript graphics, including EPSF embedded files (*.eps)",
				"Color PostScript graphics, including EPSF embedded files (*.eps)",
				"LaTeX picture environment with PostScript specials", "plain TeX with PostScript specials",
				"LaTeX picture environment with PSTricks macros", "W3C Scalable Vector Graphics driver" };
	...
	public enum LegendLocation {
		TOP_LEFT_INSIDE, TOP_CENTER_INSIDE, TOP_RIGHT_INSIDE, LEFT_INSIDE, CENTER_INSIDE, RIGHT_INSIDE,
		BOTTOM_LEFT_INSIDE, BOTTOM_CENTER_INSIDE, BOTTOM_RIGHT_INSIDE, TOP_LEFT_OUTSIDE, TOP_CENTER_OUTSIDE,
		TOP_RIGHT_OUTSIDE, LEFT_OUTSIDE, CENTER_OUTSIDE, RIGHT_OUTSIDE, BOTTOM_LEFT_OUTSIDE, BOTTOM_CENTER_OUTSIDE,
	...
	public enum PlotStyle {
		...
		private static String[] descriptions = new String[] {
				...
				"Histogram type 1", "Histogram type 2", "Histogram type 3",
	...
	private String createScript(File resultFile) {
		...
		if (!legendTypeOption.getChosenLabel().equals(LegendType.NONE)) {
			script += "set key " + legendTypeOption.getChosenLabel().toLowerCase().replace('_', ' ') + " "
					+ legendLocationOption.getChosenLabel().toLowerCase().replace('_', ' ') + newLine;
		}
		...
		for (int i = 0; i < inputFilesOption.getList().length; i++) {
			...
			sourceFileIdx++;
			script += "'" + ((StringOption) inputFilesOption.getList()[i]).getValue() + "' using "
			...
			script += " with " + plotStyleOption.getChosenLabel().toLowerCase() + " ls " + sourceFileIdx + " lw "
					+ lineWidthOption.getValue();
			if (plotStyleOption.getChosenLabel().equals(PlotStyle.LINESPOINTS.toString())
