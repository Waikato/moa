...
public class EvaluateConceptDrift extends ConceptDriftMainTask {
	...
	public ClassOption learnerOption = new ClassOption("learner", 'l', "Change detector to train.",
			ChangeDetectorLearner.class, "ChangeDetectorLearner");

	public ClassOption streamOption = new ClassOption("stream", 's', "Stream to learn from.",
			ConceptDriftGenerator.class, "GradualChangeGenerator");
	...
	public IntOption instanceLimitOption = new IntOption("instanceLimit", 'i',
			"Maximum number of instances to test/train on  (-1 = no limit).", 1000, -1, Integer.MAX_VALUE);
	...
	public IntOption sampleFrequencyOption = new IntOption("sampleFrequency", 'f',
			"How many instances between samples of the learning performance.", 10, 0, Integer.MAX_VALUE);

	/*
	 * public IntOption memCheckFrequencyOption = new IntOption(
	 * "memCheckFrequency", 'q', "How many instances between memory bound checks.",
	...
	/*
	 * public FileOption outputPredictionFileOption = new
	 * FileOption("outputPredictionFile", 'o',
	...
	protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
		 ...
		 * PrintStream outputPredictionResultStream = null; if (outputPredictionFile !=
		 * null) { try { if (outputPredictionFile.exists()) {
		 * outputPredictionResultStream = new PrintStream( new
		 * FileOutputStream(outputPredictionFile, true), true); } else {
		 * outputPredictionResultStream = new PrintStream( new
		 * FileOutputStream(outputPredictionFile), true); } } catch (Exception ex) {
		 * throw new RuntimeException( "Unable to open prediction result file: " +
		 * outputPredictionFile, ex); } }
		 */
