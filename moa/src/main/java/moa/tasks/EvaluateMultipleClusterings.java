...
public class EvaluateMultipleClusterings extends AuxiliarMainTask {
    ...
    public ClassOption streamOption =
            new ClassOption(
                    "stream",
                    's',
                    "Base stream to learn from (must use FileStream).",
    ...
    public IntOption numStreamsOption =
            new IntOption(
                    "numStreams",
                    'n',
                    "The number of streams to iterate through (must be named according to"
                            + " WriteMultipleStreamsToARFF format.",
                    100,
                    2,
                    Integer.MAX_VALUE);
    ...
    public FileOption dumpFileOption =
            new FileOption(
                    "dumpFile",
                    'd',
                    "File to append intermediate csv reslts to.",
                    "dumpClustering.csv",
                    "csv",
                    true);

    public FlagOption mergeResultsOption =
            new FlagOption(
                    "mergeResults",
                    'm',
                    "Produce a csv file that contains all of the individual results merged"
                            + " together.");

    public FlagOption generalEvalOption =
            new FlagOption(
                    "General", 'g', "GPrecision, GRecall, Redundancy, numCluster, numClasses");

    public FlagOption f1Option = new FlagOption("F1", 'f', "F1-P, F1-R, Purity.");

    public FlagOption entropyOption =
            new FlagOption(
                    "Entropy",
                    'e',
                    "GT cross entropy, FC cross entropy, Homogeneity, Completeness, V-Measure,"
                            + " VarInformation.");

    public FlagOption cmmOption =
            new FlagOption(
                    "CMM",
                    'c',
                    "CMM, CMM Basic, CMM Missed, CMM Misplaced, CMM Noise, CA Seperability, CA"
                            + " Noise, CA Model.");

    public FlagOption ssqOption = new FlagOption("SSQ", 'q', "SSQ.");

    public FlagOption separationOption =
            new FlagOption("Separation", 'p', "BSS, BSS-GT, BSS-Ratio.");

    public FlagOption silhouetteOption = new FlagOption("Silhouette", 'h', "SilhCoeff.");

    public FlagOption statisticalOption =
            new FlagOption("Statistical", 't', "van Dongen, Rand statistic.");
    ...
    protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
        ...
        for (int i = 0; i < this.numStreamsOption.getValue(); i++) {
            ...
            this.task.streamOption.setValueViaCLIString(
                    fStream.getCLICreationString(fStream.getClass()));
            ...
            System.out.println(
                    "Evaluation #"
                            + (i + 1)
                            + " of "
                            + this.numStreamsOption.getValue()
                            + ": "
            ...
            if (this.mergeResultsOption.isSet()) {
                ...
                if (i == 0) writeToMergeFile(mergedFile, outputFile);
                else mergeFiles(mergedFile, outputFile, tempFile);
            }
        }
    ...
    private void mergeFiles(String mergedFile, String outputFile, String tempFile) {
        ...
        try {
            ...
            reader1 =
                    new CSVReaderBuilder(new FileReader(mergedFile)).withCSVParser(parser1).build();
            parser2 = new CSVParserBuilder().withSeparator(';').withIgnoreQuotations(false).build();
            reader2 =
                    new CSVReaderBuilder(new FileReader(outputFile)).withCSVParser(parser2).build();
    ...
    private String[] mergeArrays(String[] nextLine1, String[] nextLine2) {
        ...
        if (nextLine1[nextLine1.length - 1].equals("")) length1--;
        if (nextLine2[nextLine2.length - 1].equals("")) length2--;
