...
import moa.evaluation.PredictionIntervalPerformanceEvaluator;
 ...
 * @version $Revision: 7 $
 */
public class EvaluatePrequentialPredictionIntervals extends RegressionMainTask {
...
//            "Learner to train.", Regressor.class, "moa.classifiers.meta.AdaptiveRandomForestRegression");

    public ClassOption learnerOption = new ClassOption("interval",'p',
            "Prediction Interval method to use", PredictionIntervalLearner.class, "moa.classifiers.predictioninterval.MVEPredictionInterval");
    ...
    public ClassOption evaluatorOption = new ClassOption("evaluator", 'e',
            "Prediction Interval Evaluation Method.",
            PredictionIntervalPerformanceEvaluator.class,
    ...
    protected Object doMainTask(TaskMonitor monitor, ObjectRepository repository) {
        ...
        PredictionIntervalLearner intervalLearner = (PredictionIntervalLearner)getPreparedClassOption(this.learnerOption);
        ExampleStream stream = (ExampleStream) getPreparedClassOption(this.streamOption);
        PredictionIntervalPerformanceEvaluator evaluator = (PredictionIntervalPerformanceEvaluator) getPreparedClassOption(this.evaluatorOption);
...
//        }
        ...
        if (outputPredictionFile != null) {
            try {
                ...
                outputPredictionResultStream.println("lower bound, prediction, upper bound, true value");
            } catch (Exception ex) {
                throw new RuntimeException(
...
//            Prediction prediction = learner instanceof PredictionIntervalLearner ? ((PredictionIntervalLearner)
//                    learner).getPredictionForInstance(testInst) : new Prediction(new double[]{0,0,0}) ;

            // Output prediction
            if (outputPredictionFile != null) {
                double trueClass = ((Instance) trainInst.getData()).classValue();
                if (prediction.length == 3)
                    outputPredictionResultStream.println(prediction[0]+ ", " +prediction[1] + ", " + prediction[2]+ ", " + trueClass);
                else
                    outputPredictionResultStream.println("Prediction Interval Not Available:" + trueClass);

            }


            //evaluator.addClassificationAttempt(trueClass, prediction, testInst.weight());
//            intervalLearner.updateStatistics(prediction.getVotes()[0]);
