...
public final class ImmutableCapabilities extends Capabilities {
     ...
     * CapabilitiesHandlers that have identical sets of capabilities to share a single unique set,
     * rather than having one each.
     */
    static class StaticLookup {

        /** The set of unique immutable capabilities objects. */
        private static Map<ImmutableCapabilities, ImmutableCapabilities> m_UniqueSet =
                new HashMap<>();

        /** The mapping from handlers to their canonical set of capabilities. */
        private static Map<CapabilitiesHandler, ImmutableCapabilities> m_Lookup =
                new WeakHashMap<>();

        /**
         * Whether the given capabilities handler has defined an immutable set of capabilities.
         *
        ...
        public static void define(CapabilitiesHandler owner, ImmutableCapabilities capabilities) {
            ...
            if (capabilities == null) capabilities = new ImmutableCapabilities();

            // Substitute the canonical object, or make canonical
            if (m_UniqueSet.containsKey(capabilities)) capabilities = m_UniqueSet.get(capabilities);
            else m_UniqueSet.put(capabilities, capabilities);
        ...
        public static ImmutableCapabilities get(CapabilitiesHandler owner) {
            ...
            if (!isDefined(owner))
                throw new IllegalArgumentException(
                        "Illegal attempt to get capabilities for undefined owner.");
