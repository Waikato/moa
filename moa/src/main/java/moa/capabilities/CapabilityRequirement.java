...
public class CapabilityRequirement {
    ...
    /**
     * The capabilities to assume a class has if it does not implement the CapabilitiesHandler
     * interface.
     */
    public static final ImmutableCapabilities NON_HANDLER_CAPABILITIES =
            new ImmutableCapabilities(Capability.VIEW_STANDARD, Capability.VIEW_LITE);

    /** The function to use to test if the requirement is met. */
    private Predicate<Capabilities> m_Test;

    /**
     * Creates a capabilities requirement with the given predicate as its method of checking if the
     * requirement is met.
     *
     ...
     * @return True if the handler meets the requirements, false if not.
     */
     ...
     * @return True if the class meets the requirements, false if not.
     */
    public boolean isMetBy(Class<?> klass) {
        ...
        try {
            ...
            throw new RuntimeException(
                    "Couldn't instantiate CapabilitiesHandler " + klass.getSimpleName(), e);
        }
    ...
    /**
     * Creates a requirement that a given set of capabilities have all of the specified
     * capabilities.
     *
        ...
        return new CapabilityRequirement(
                c -> {
                    for (Capability capability : capabilities) {
                        if (!c.hasCapability(capability)) return false;
                    }
                    return true;
                });
    }
    ...
    /**
     * Creates a requirement that a given set of capabilities have at least on of the specified
     * capabilities.
     *
     * @param capabilities The capabilities that a tested set must have at least one of.
     * @return The requirement.
     */
    public static CapabilityRequirement hasAny(Capability... capabilities) {
        return new CapabilityRequirement(
                c -> {
                    for (Capability capability : capabilities) {
                        if (c.hasCapability(capability)) return true;
                    }
                    return false;
                });
    }
