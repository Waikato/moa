...
/**
 * Stream generator that adds recurrent concept drifts to examples in a stream. <br>
 * <br>
 * Example: <br>
 * <br>
 ...
 * x : Width of recurrence (number of instances during which new concept is used) y : Number of
 * stability period (number of instances between drifts) z : Number of appearances of drift
 *
...
public class RecurrentConceptDriftStream extends ConceptDriftStream {
    ...
    public IntOption widthRecurrenceOption =
            new IntOption(
                    "widthRecurrence",
                    'x',
                    "Number of instances during which new concept is used",
                    100);

    public IntOption stabPeriodOption =
            new IntOption("stabPeriod", 'y', "Number of instances between drifts", 200);

    public IntOption numRepOption =
            new IntOption("numRep", 'z', "Number of instances between drifts", 4);
    ...
    public Example nextInstance() {
        ...
        for (int iNumRep = 0; iNumRep < this.numRepOption.getValue(); iNumRep++) {
            probabilityDrift +=
                    1.0 / (1.0 + Math.exp(-4 * (x - iPos) / this.widthOption.getValue()))
                            - 1.0
                                    / (1.0
                                            + Math.exp(
                                                    -4
