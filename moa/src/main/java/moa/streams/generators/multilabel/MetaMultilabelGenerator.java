...
public class MetaMultilabelGenerator extends AbstractOptionHandler implements InstanceStream {
	...
	public ClassOption binaryGeneratorOption = new ClassOption("binaryGenerator", 's',
			"Binary Generator (specify the number of attributes here, but only two classes!).", InstanceStream.class,
			"generators.RandomTreeGenerator");

	public IntOption metaRandomSeedOption = new IntOption("metaRandomSeed", 'm',
			"Random seed (for the meta process). Use two streams with the same seed and r > 0.0 in the second stream if you wish to introduce drift to the label dependencies without changing the underlying concept.",
			1);

	public IntOption numLabelsOption = new IntOption("numLabels", 'c', "Number of labels.", 10, 2, Integer.MAX_VALUE);

	public IntOption skewOption = new IntOption("skew", 'k',
			"Skewed label distribution: 1 (default) = yes; 0 = no (relatively uniform) @NOTE: not currently implemented.",
			1, 0, 1);

	public FloatOption labelCardinalityOption = new FloatOption("labelCardinality", 'z',
			"Desired label cardinality (average number of labels per example).", 1.5, 0.0, Integer.MAX_VALUE);

	public FloatOption labelCardinalityVarOption = new FloatOption("labelCardinalityVar", 'v',
			"Desired label cardinality variance (variance of z) @NOTE: not currently implemented.", 1.0, 0.0,
			Integer.MAX_VALUE);

	public FloatOption labelCardinalityRatioOption = new FloatOption("labelDependency", 'u',
			"Specifies how much label dependency from 0 (total independence) to 1 (full dependence).", 0.25, 0.0, 1.0);

	public FloatOption labelDependencyChangeRatioOption = new FloatOption("labelDependencyRatioChange", 'r',
			"Each label-pair dependency has a 'r' chance of being modified. Use this option on the second of two streams with the same random seed (-m) to introduce label-dependence drift.",
			0.0, 0.0, 1.0);
	...
	public void restart() {
		...
		this.priors = generatePriors(m_MetaRandom, m_L, labelCardinalityOption.getValue(),
				(skewOption.getValue() >= 1));
		// printVector(this.priors);

		// Generate the matrix marking the label-dependencies
		boolean DependencyMatrix[][] = modifyDependencyMatrix(new boolean[m_L][m_L],
				labelCardinalityRatioOption.getValue(), m_MetaRandom);

		// Modify the dependency matrix (and the priors) if they are to change in this
		// stream
		if (labelDependencyChangeRatioOption.getValue() > 0.0) {
			priors = modifyPriorVector(priors, labelDependencyChangeRatioOption.getValue(), m_MetaRandom,
					(skewOption.getValue() >= 1));
	...
	protected MultilabelInstancesHeader generateMultilabelHeader(InstancesHeader si) {
		...
		this.multilabelStreamTemplate.setRelationName("SYN_Z" + this.labelCardinalityOption.getValue() + "L" + this.m_L
				+ "X" + m_A + "S" + metaRandomSeedOption.getValue() + ": -C " + this.m_L);
	...
	private Instance getNextWithBinary(int i) {
		...
		if (queue[i].size() <= 0) {
			...
			System.err
					.println("[Overflow] The binary stream is too skewed, could not get an example of class " + i + "");
			System.exit(1);
			return null;
		} else {
	...
	/**
	 * ModifyDependencyMatrix. A certain number of values will be altered. @NOTE a
	 * future improvement would be to detect cycles, since this may lead to
	 * inconsistencies. However, due to the rarity of this occurring, and the
	 * minimal problems it would cause (and considering that inconsistencies also
	 * occr in real datasets) we don't implement this.
	 *
	 ...
	 * dependencies, generate a conditional distribution matrix Q; such that: P(i) =
	 * Q[i][i] P(i|j) = Q[i][j]
	 *
	...
	/**
	 * GetTopCombinations. Calculating the full joint probability distribution is
	 * too complex. - sample from the approximate joint many times - record the the
	 * n most commonly ocurring Y and their frequencies - create a map based on
	 * these frequencies
	 *
	...
	private HashSet[] getTopCombinations(int n) {
		...
		final HashMap<HashSet, Integer> count = new HashMap<>();
		HashMap<HashSet, Integer> isets = new HashMap<>();
		...
		List<HashSet> top_set = new ArrayList<>(count.keySet());
	...
	private ArrayList<Integer> getShuffledListToLWithoutK(int L, int k) {
		ArrayList<Integer> list = new ArrayList<>(L - 1);
		for (int j = 0; j < L; j++) {
			if (j != k) {
				list.add(j);
			}
