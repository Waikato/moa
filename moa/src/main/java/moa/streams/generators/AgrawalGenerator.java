 ...
 * Rakesh Agrawal, Tomasz Imielinksi, and Arun Swami, "Database Mining: A
 * Performance Perspective", IEEE Transactions on Knowledge and Data
 * Engineering, 5(6), December 1993. <br/>
 * <br/>
 *
 ...
 *
 * Notes:<br/>
 * The built in functions are based on the paper (page 924), which turn out to
 * be functions pred20 thru pred29 in the public C implementation. Perturbation
 * function works like C implementation rather than description in paper.
 *
 * @author Richard Kirkby (rkirkby@cs.waikato.ac.nz)
 * @version $Revision: 7 $
 */
public class AgrawalGenerator extends AbstractOptionHandler implements InstanceStream, CapabilitiesHandler {
	...
	protected interface ClassFunction {
		...
		int determineClass(double salary, double commission, int age, int elevel, int car, int zipcode, double hvalue,
				int hyears, double loan);
	}
	...
	public InstanceExample nextInstance() {
		...
		while (!desiredClassFound) {
			...
			// determine class
			group = classificationFunctions[this.functionOption.getValue() - 1].determineClass(salary, commission, age,
