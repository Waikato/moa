 ...
 * @version $Revision: 7 $
 */
public class HyperplaneGenerator extends AbstractOptionHandler implements InstanceStream, CapabilitiesHandler {
	...
	public IntOption sigmaPercentageOption = new IntOption("sigmaPercentage", 's',
			"Percentage of probability that the direction of change is reversed.", 10, 0, 100);
	...
	private void addDrift() {
		for (int i = 0; i < this.numDriftAttsOption.getValue(); i++) {
			this.weights[i] += (sigma[i]) * (this.magChangeOption.getValue());
