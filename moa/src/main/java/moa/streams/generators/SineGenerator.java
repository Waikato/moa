 ...
 * positive. After the context change the classification is reversed. 2.SINE2.
 * The same two relevant attributes. The classification function is y &lt; 0.5 +
 * 0.3 sin(3 * PI * x). After the context change the classification is reversed.
 * 3.SINIRREL1. Presence of irrelevant attributes. The same classification
 * function of SINE1 but the examples have two more random attributes with no
 * influence on the classification function. 4.SINIRREL2. The same
 * classification function of SINE2 but the examples have two more random
 * attributes with no influence on the classification function. Based on
 * proposal by "Gama, Joao, et al. "Learning with drift detection." Advances in
 * artificial intelligence–SBIA 2004. Springer Berlin Heidelberg, 2004.
 * 286-295."
 *
 * @author Paulo Gonçalves (paulogoncalves@recife.ifpe.edu.br)
 * @version $Revision: 1 $
...
public class SineGenerator extends AbstractOptionHandler implements InstanceStream {
	...
	protected interface ClassFunction {
		...
		int determineClass(double x, double y);
	}
