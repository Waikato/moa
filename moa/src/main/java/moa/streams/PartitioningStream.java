...
public class PartitioningStream extends AbstractOptionHandler implements ExampleStream {
    ...
    public ClassOption streamOption =
            new ClassOption(
                    "baseStream",
                    's',
                    "Stream which is used for partitioning.",
                    ExampleStream.class,
    ...
    public IntOption partitionIndexOption =
            new IntOption(
                    "partitionIndex",
                    'i',
                    "The index of the partition, which should be used as output.",
                    0);
    ...
    public IntOption numPartitionsOption =
            new IntOption(
                    "numPartitions",
                    'n',
                    "The number of total streams the base stream is split into.",
                    1);
    ...
    public IntOption randomSeedOption =
            new IntOption(
                    "randomSeed",
                    'r',
                    "The seed which is used (all other partitions should use the same one to"
                            + " guarentee).",
                    0);
    ...
    public long estimatedRemainingInstances() {
        ...
        long numRemaining =
                (baseEstimatedRemainingInstances * (numPartitions - 1)) / (numPartitions);
        if (numPartitions == 1) {
    ...
    /**
     * discarding all instances which are exluded until an instance which can be seen by this stream
     * or the stream is empty
     */
    protected void discardNexInstancesNotFromPartition() {
        while (!isNextInstanceFromPartition()
                && baseStream.hasMoreInstances()
                && numPartitions > 1) {
