...
/**
 * This stream partitions the base stream into n distinct streams and outputs
 * one of them
 *
 * @author Tuan Pham Minh (tuan.pham@ovgu.de)
 * @version $Revision: 1 $
...
public class PartitioningStream extends AbstractOptionHandler implements ExampleStream {
	...
	public ClassOption streamOption = new ClassOption("baseStream", 's', "Stream which is used for partitioning.",
			ExampleStream.class, "generators.RandomTreeGenerator");

	public IntOption partitionIndexOption = new IntOption("partitionIndex", 'i',
			"The index of the partition, which should be used as output.", 0);

	public IntOption numPartitionsOption = new IntOption("numPartitions", 'n',
	...
	/**
	 * discarding all instances which are exluded until an instance which can be
	 * seen by this stream or the stream is empty
	 */
