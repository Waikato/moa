...
/**
 * Replaces the missing values with another value according to the selected strategy. Available
 * strategies for numerical attributes are: 1. Nothing: Does nothing (doesn't replace missing
 * values) 2. LastKnownValue: Replaces with the last non missing value 3. Mean: Replaces with mean
 * of the processed instances so far 4. Max: Replaces with maximum of the processed instances so far
 * 5. Min: Replaces with minimum of the processed instances so far 6. Constant: Replaces with a
 * constant value (default: zero)
 *
 * <p>Available strategies for nominal attributes are: 1. Nothing: Does nothing (doesn't replace
 * missing values) 2. LastKnownValue: Replaces with the last non missing value 3. Mode: Replaces
 * with the mode of the processed instances so far (most frequent value)
 *
 * <p>Beware of numerical strategies 2 to 5: if no previous non-missing values were processed,
...
public class ReplacingMissingValuesFilter extends AbstractStreamFilter {
    ...
    public MultiChoiceOption numericReplacementStrategyOption =
            new MultiChoiceOption(
                    "numericReplacementStrategy",
                    's',
                    "Replacement strategy for numeric attributes",
                    new String[] {"Nothing", "LastKnownValue", "Mean", "Max", "Min", "Constant"},
                    new String[] {
                        "Does nothing (doesn't replace missing values)",
    ...
    public MultiChoiceOption nominalReplacementStrategyOption =
            new MultiChoiceOption(
                    "nominalReplacementStrategy",
                    't',
                    "Replacement strategy for nominal attributes",
                    new String[] {"Nothing", "LastKnownValue", "Mode"},
                    new String[] {
                        "Does nothing (doesn't replace missing values)",
                        "Replaces with the last non missing value",
                        "Replaces with the mode of the processed instances so far (most frequent"
                                + " value)"
                    },
                    0);
    ...
    public FloatOption numericalConstantValueOption =
            new FloatOption(
                    ...
                    'c',
                    "Value used to replace missing values during the numerical constant strategy",
                    0.0);
    ...
    public InstanceExample nextInstance() {
        ...
        if (numAttributes < 0) {
            ...
            for (int i = 0; i < inst.numAttributes(); i++) {
                if (inst.attribute(i).isNominal()) frequencies[i] = new HashMap<Double, Integer>();
            }
                ...
                // Update statistics with non-missing values
                    ...
                    switch (numericalSelectedStrategy) {
                        ...
                        case 2: // MEAN
                            ...
                            columnsStatistics[i] =
                                    columnsStatistics[i]
                                            + (inst.value(i) - columnsStatistics[i])
                                                    / numberOfSamples[i];
                            break;
                        case 3: // MAX
                            columnsStatistics[i] =
                    ...
                    switch (nominalSelectedStrategy) {
                        ...
                        case 2: // MODE
                            if (!frequencies[i].isEmpty()) {
                                ...
                                Map<Double, Integer> sortedMap =
                                        MapUtil.sortByValue(frequencies[i]);
