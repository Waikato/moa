 ...
 * strategy. Available strategies for numerical attributes are: 1. Nothing: Does
 * nothing (doesn't replace missing values) 2. LastKnownValue: Replaces with the
 * last non missing value 3. Mean: Replaces with mean of the processed instances
 * so far 4. Max: Replaces with maximum of the processed instances so far 5.
 * Min: Replaces with minimum of the processed instances so far 6. Constant:
 * Replaces with a constant value (default: zero)
 *
 * Available strategies for nominal attributes are: 1. Nothing: Does nothing
 * (doesn't replace missing values) 2. LastKnownValue: Replaces with the last
 * non missing value 3. Mode: Replaces with the mode of the processed instances
 * so far (most frequent value)
 *
 * Beware of numerical strategies 2 to 5: if no previous non-missing values were
 * processed, missing values will be replaced by 0.
 * 
 * @author Manuel Martin Salvador <draxus@gmail.com>
 *
 */
public class ReplacingMissingValuesFilter extends AbstractStreamFilter {
	...
	public MultiChoiceOption numericReplacementStrategyOption = new MultiChoiceOption("numericReplacementStrategy", 's',
			"Replacement strategy for numeric attributes",
			new String[] { "Nothing", "LastKnownValue", "Mean", "Max", "Min", "Constant" },
			new String[] { "Does nothing (doesn't replace missing values)", "Replaces with the last non missing value",
	...
	public MultiChoiceOption nominalReplacementStrategyOption = new MultiChoiceOption("nominalReplacementStrategy", 't',
			"Replacement strategy for nominal attributes", new String[] { "Nothing", "LastKnownValue", "Mode" },
			new String[] { "Does nothing (doesn't replace missing values)", "Replaces with the last non missing value",
