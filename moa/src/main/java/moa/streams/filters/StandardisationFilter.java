 ...
 *
 * <p>Standardisation is a scaling technique where the values are centered around the mean with a
 ...
 *
 * <p>Z-SCORE is used to standardise the values of a normal distribution. For more information:
 * https://en.wikipedia.org/wiki/Standard_score. The formula is: z=(z-μ)/σ μ is the mean of the
 * population. σ is the standard deviation of the population, as the square root of variance.
 *
 * <p>There are three algorithms for calculating variance. For more information:
 * https://en.wikipedia.org/wiki/Algorithms_for_calculating_variance#Computing_shifted_data 1. Naive
...
public class StandardisationFilter extends AbstractStreamFilter {
    ...
    public MultiChoiceOption AlgorithmOption =
            new MultiChoiceOption(
                    "AlgorithmOption",
                    'a',
                    "Standardisation Algorithm Option",
                    new String[] {"Naive", "Welford", "Two-pass", "Weighted"},
                    new String[] {"Naive(default)", "Welford", "Two-pass", "Weighted"},
                    0);
    public FloatOption WeightedOptionFloat =
            new FloatOption(
                    "WeightedOptionFloat",
                    'w',
                    "The weight of weighted incremental algorithm",
                    0.0);
                ...
                switch (AlgorithmIndex) {
                    case 0: // Naive
                        ...
                        if (sumOfSquare[i] / count != 0)
                            // Standardisation
                            standardisedInstance.setValue(
                                    i,
                                    (inst.value(i) - sum[i] / count)
                                            / Math.sqrt(
                                                    (sumOfSquare[i] - (sum[i] * sum[i]) / count)
                                                            / (count - 1)));
                        // Standard deviation
                        //                            standardisedInstance.setValue(i,
                        // Math.sqrt((sumOfSquare[i] - (sum[i] * sum[i]) / count) / (count - 1)));
                        else {
                    ...
                    case 1: // Welford
                        ...
                        if (M2[i] / count != 0)
                            ...
                            standardisedInstance.setValue(
                                    i,
                                    (inst.value(i) - mean[i]) / (Math.sqrt(M2[i] / (count - 1))));
                        // Standard deviation
                        //                            standardisedInstance.setValue(i,
                        // Math.sqrt(M2[i] / (count-1)));
                        else standardisedInstance.setValue(i, 0);
                        break;
                    case 2: // Two-pass
                        ...
                        if (delta2[i] / count != 0)
                            ...
                            standardisedInstance.setValue(
                                    i,
                                    (inst.value(i) - mean[i]) / Math.sqrt(delta2[i] / (count - 1)));
                        // Standard deviation
                        //                            standardisedInstance.setValue(i,
                        // Math.sqrt(delta2[i] / (count-1)));
                        else standardisedInstance.setValue(i, 0);
                        break;
                }
            }
