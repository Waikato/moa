...
public class ReLUFilter extends AbstractStreamFilter {
	...
	/**
	 * ReLUFilter - A random projection of the filter space. NOTE: Data should be
	 * standardized.
	 */
	...
	public IntOption numLatentOption = new IntOption("numLatent", 'h',
			"Percent of basis functions (wrt number of input attributes).", 10);
	private int H = 200;
	...
	/**
	 * Filter an instance. Assume that the instance has a single class label, as the
	 * final attribute. Note that this may not always be the case!
	 *
	...
	private void initialize(Instance instance) {
		...
		List<Attribute> v = new ArrayList<>(H);
		List<Integer> indexValues = new ArrayList<>(H);
