 ...
 *
 * @author Richard Kirkby (rkirkby@cs.waikato.ac.nz) - RandomRBFGenerator Timm Jansen
 *     (moa@cs.rwth-aachen.de) - Events
 * @editor Yunsu Kim
 *     <p>Last edited: 2013/06/02
...
public class RandomRBFGeneratorEvents extends ClusteringStream {
    ...
    public IntOption modelRandomSeedOption =
            new IntOption("modelRandomSeed", 'm', "Seed for random generation of model.", 1);

    public IntOption instanceRandomSeedOption =
            new IntOption("instanceRandomSeed", 'i', "Seed for random generation of instances.", 5);
    ...
    public IntOption speedRangeOption =
            new IntOption(
                    "speedRange", 'v', "Speed/Velocity point offset", 0, 0, Integer.MAX_VALUE);

    public FloatOption noiseLevelOption =
            new FloatOption("noiseLevel", 'N', "Noise level", 0.1, 0, 1);

    public FlagOption noiseInClusterOption =
            new FlagOption("noiseInCluster", 'n', "Allow noise to be placed within a cluster");
    ...
    private class GeneratorCluster implements Serializable {
        ...
        public GeneratorCluster(int label) {
            ...
            if (tryCounter < maxOverlapFitRuns) {
                ...
                System.out.println(
                        "Tried "
                                + maxOverlapFitRuns
                                + " times to create kernel. Reduce average radii.");
            }
        }
        ...
        public int getWorkID() {
            for (int c = 0; c < kernels.size(); c++) {
                if (kernels.get(c).equals(this)) return c;
            }
            return -1;
        }
        ...
        private void addInstance(Instance instance) {
            ...
            if (!inserted) {
                ...
                if (minMicroIndex == -1) {
                    ...
                    SphereCluster s;
                    if (alt == 0) s = new SphereCluster(microPoints, numAttsOption.getValue());
                    else s = new SphereCluster(generator.getCenter(), generator.getRadius(), 1);
        ...
        void setMoveVector(double[] vector) {
            ...
            totalMovementSteps =
                    (int)
                            (length
                                    / (maxDistanceMoveThresholdByStep * kernelMovePointFrequency)
                                    * speedInPoints);
            for (int d = 0; d < moveVector.length; d++) {
        ...
        private String splitKernel() {
            ...
            if (spcluster != null) {
                ...
                System.out.println(
                        "Tried to split new kernel from C"
                                + generator.getId()
                                + ". Not enough room for new cluster, decrease average radii,"
                                + " number of clusters or enable overlap.");
                return "";
            }
        }
        ...
        for (int c = 0; c < kernels.size(); c++) {
            for (int m = 0; m < kernels.get(c).microClusters.size(); m++) {
                ...
                kernels.get(c)
                        .microClusters
                        .get(m)
                        .setGroundTruth(kernels.get(c).generator.getId());
    ...
    private int getNextEvent() {
        ...
        if (!lowerLimit || !upperLimit) {
            ...
            if (mode == 0 || (mode == -1 && eventMergeSplitOption.isSet())) {
                ...
                if (!lowerLimit && !upperLimit) choice = instanceRandom.nextInt(2);
                else
                // we have a limit. if lower limit, choose split
                if (lowerLimit) choice = 1;
            ...
            if (mode == 1 || (mode == -1 && eventDeleteCreateOption.isSet())) {
                // have we reached a limit? if not free choice
                if (!lowerLimit && !upperLimit) choice = instanceRandom.nextInt(2) + 2;
                else
                // we have a limit. if lower limit, choose create
                if (lowerLimit) choice = 3;
    ...
    private String changeWeight(boolean increase) {
        ...
        String message;
        if (increase) message = "Increase ";
        else message = "Decrease ";
        message +=
                " weight on Cluster " + id + " from " + weight_old + " to " + (weight_old + delta);
        return message;
    }
    ...
    private String mergeKernels(int steps) {
        if (numActiveKernels > 1 && ((mergeClusterA == null && mergeClusterB == null))) {
                ...
                for (int j = 0; j < i; j++) {
                    if (kernels.get(i).kill != -1 || kernels.get(j).kill != -1) {
                        ...
                        double kernelDist =
                                kernels.get(i)
                                        .generator
                                        .getCenterDistance(kernels.get(j).generator);
            ...
            if (mergeClusterA != null && mergeClusterB != null) {
                ...
                if (debug) {
                    System.out.println(
                            "Center1" + Arrays.toString(mergeClusterA.generator.getCenter()));
                    System.out.println(
                            "Center2" + Arrays.toString(mergeClusterB.generator.getCenter()));
