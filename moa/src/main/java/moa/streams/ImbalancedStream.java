 ...
 * Imbalanced Stream.
 *
 * <p>This is a meta-generator that produces class imbalance in a stream. Only two parameters are
 * required to be set: - The original stream - The ratio (proportion) of each class in the stream.
 *
 * <p>The second parameter determines the ratio of each class in the output stream. The ratio of
 * each class should be provided as a real number between 0.0 and 1.0, each being followed by a
 * semicolon, and their sum should add up to 1.0. The default value of 0.9;0.1 stands for an output
 * stream where approximately 90% of the instances belonging to the first class while the remainder
 * 10% would belong to the secondary class.
 *
 * @author Jean Paul Barddal (jean.barddal@ppgia.pucpr.br)
 * @version 1.0
 */
public class ImbalancedStream extends AbstractOptionHandler implements InstanceStream {
    ...
    public StringOption classRatioOption =
            new StringOption(
                    "classRatio",
                    'c',
                    "Determine the ratio of each class in the output stream. The ratio of each"
                        + " class should be given as a real number between 0 and 1, each followed"
                        + " by a semicolon, and their sum should be equal to 1. The default value"
                        + " of \"0.9;0.1\" stands for an output stream with approximately 90% of"
                        + " the instances belonging to the first class and 10% to the second"
                        + " class.",
                    "0.9;0.1");
        ...
        for (int i = 0; i < probs.length; i++) {
            if (i > probPerClass.length - 1)
                throw new IllegalArgumentException(
                        "Please make sure the number of class ratios provided is less or equal the"
            ...
            if (!p.isNaN() && !p.isInfinite() && p.doubleValue() >= 0.0 && p.doubleValue() <= 1.0) {
                ...
                throw new IllegalArgumentException(
                        "Please make sure only numbers between 0.0 and 1.0 are inputted.");
            }
        }
    ...
    public Example<Instance> nextInstance() {
        ...
        int iClass = -1;
        // loops over all class probabilities to see from which class the next instance should be
        // from
        while (p > 0.0) {
            iClass++;
