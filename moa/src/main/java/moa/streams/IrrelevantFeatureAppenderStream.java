 ...
 *
 * This is a meta-generator that appends irrelevant features in a stream. The
 * following parameters are required to be set: - The original stream, - The
 * number of numeric features to be appended, - The number of categorical
 * features to be appended, and - The number of values for each categorical
 * feature.
 *
 * The values for each new feature are drawn from an uniform distribution and
 * are not related with the class anyhow. This means that no correlation between
 * each feature and the class will be observed, and the entropy will be maximum.
 *
 * @author Jean Paul Barddal (jean.barddal@ppgia.pucpr.br)
 * @version 1.0
 */
public class IrrelevantFeatureAppenderStream extends AbstractOptionHandler implements InstanceStream {
	...
	/**
	 * Constructs the new header according to: 1- The original header 2- The number
	 * of numeric and categorical features given by the user.
	 */
	private void buildHeader() {
		...
		for (int i = 0; i < numValuesCategoricalFeatureOption.getValue(); i++)
			catVals.add("v" + i);
		...
		InstancesHeader format = new InstancesHeader(getCLICreationString(InstanceStream.class), attributes, 0);
