 ...
 *
 * <p>Adaptive Random Forest (ARF). The 3 most important aspects of this ensemble classifier are:
 * (1) inducing diversity through resampling; (2) inducing diversity through randomly selecting
 * subsets of features for node splits (See moa.classifiers.trees.ARFHoeffdingTree.java); (3) drift
 * detectors per base tree, which cause selective resets in response to drifts. It also allows
 * training background trees, which start training if a warning is detected and replace the active
 ...
 * @version $Revision: 1 $
 */
public class AdaptiveRandomForestMB extends AbstractClassifierMiniBatch
    ...
    public double[] getVotesForInstance(Instance instance) {
        ...
        if (this.trainers == null) initEnsemble(testInstance);
            ...
            DoubleVector vote =
                    new DoubleVector(
                            this.trainers
                                    ...
                                    .learner
                                    .classifier
                                    .getVotesForInstance(testInstance));
    ...
    protected void initEnsemble(Instance instance) {
        ...
        switch (this.mFeaturesModeOption.getChosenIndex()) {
            case AdaptiveRandomForestMB.FEATURES_SQRT:
                this.subspaceSize = (int) Math.round(Math.sqrt(n)) + 1;
                break;
            case AdaptiveRandomForestMB.FEATURES_SQRT_INV:
                this.subspaceSize = n - (int) Math.round(Math.sqrt(n) + 1);
                break;
            case AdaptiveRandomForestMB.FEATURES_PERCENT:
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
        if (this.getClass() == AdaptiveRandomForestMB.class)
    ...
    public Classifier[] getSublearners() {
        ...
        Classifier[] forest = new Classifier[this.trainers.size()];
        for (int i = 0; i < forest.length; ++i) forest[i] = this.trainers.get(i).learner.classifier;
        return forest;
    }
