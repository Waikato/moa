...
public class AdaptiveRandomForestMB extends AbstractClassifierMiniBatch implements MultiClassClassifier {
    ...
    public double[] getVotesForInstance(Instance instance) {
        ...
        for(int i = 0 ; i < this.trainers.size() ; ++i) {
            DoubleVector vote = new DoubleVector(this.trainers.get(i).learner.classifier.getVotesForInstance(testInstance));
...
//        BasicClassificationPerformanceEvaluator classificationEvaluator = (BasicClassificationPerformanceEvaluator) getPreparedClassOption(this.evaluatorOption);
    ...
    public Classifier[] getSublearners() {
        ...
        for(int i = 0 ; i < forest.length ; ++i)
            forest[i] = this.trainers.get(i).learner.classifier;
        return forest;
