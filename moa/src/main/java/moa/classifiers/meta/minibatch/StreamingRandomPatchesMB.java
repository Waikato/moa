 ...
 * @version $Revision: 1 $
 */
public class StreamingRandomPatchesMB extends AbstractClassifierMiniBatch
    ...
    public double[] getVotesForInstance(Instance instance) {
        ...
        for (int i = 0; i < this.trainers.size(); ++i) {
            DoubleVector vote =
                    new DoubleVector(
                            this.trainers.get(i).learner.getVotesForInstance(testInstance));
    ...
    protected void initEnsemble(Instance instance) {
        ...
        if (this.trainingMethodOption.getChosenIndex()
                != StreamingRandomPatchesMB.TRAIN_RESAMPLING) {
            ...
            if (this.trainingMethodOption.getChosenIndex()
                            == StreamingRandomPatchesMB.TRAIN_RANDOM_SUBSPACES
                    || this.trainingMethodOption.getChosenIndex()
                            == StreamingRandomPatchesMB.TRAIN_RANDOM_PATCHES) {
                if (k != 0 && k < n) {
                    ...
                    // subspaces without worrying about repetitions.
                        ...
                        this.subspaces =
                                StreamingRandomPatchesMB.localRandomKCombinations(
                                        k,
                                        n,
    ...
    protected class TrainingRunnable implements Runnable, Callable<Integer> {
        ...
        public TrainingRunnable(
                StreamingRandomPatchesClassifier learner, double lambdaOption, int seed) {
