 ...
 *
 * <p>published as: Roberto Souto Maior de Barros, Silas Garrido T. de Carvalho Santos, and Paulo
 * Mauricio Goncalves Jr.: A Boosting-like Online Learning Ensemble. In Proceedings of IEEE
 * International Joint Conference on Neural Networks (IJCNN), Vancouver, Canada, 2016. DOI:
...
public class BOLE extends AbstractClassifier implements MultiClassClassifier {
    ...
    public ClassOption baseLearnerOption =
            new ClassOption(
                    ...
                    "Classifier to train.",
                    Classifier.class,
                    "drift.SingleClassifierDrift -l trees.HoeffdingTree -d (DDM -n 7 -w 1.2 -o"
                            + " 1.95)");
    ...
    public FlagOption breakVotesOption =
            new FlagOption("breakVotes", 'b', "Break Votes? unchecked=no, checked=yes");

    public FloatOption errorBoundOption =
            new FloatOption(
                    "errorBound",
                    'e',
                    "Error bound percentage for allowing experts to vote.",
                    0.5,
                    0.1,
                    1.0);

    public FloatOption weightShiftOption =
            new FloatOption(
                    "weightShift",
                    'w',
                    "Weight shift associated with the error bound.",
                    0.0,
                    0.0,
                    5.0);
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        for (i = 0; i < this.ensemble.length; i++) {
            ...
            if (this.pureBoostOption.isSet()) k = lambda_d;
            else k = MiscUtils.poisson(lambda_d, this.classifierRandom);
