...
public class AutoClass extends HeterogeneousEnsembleAbstract implements MultiClassClassifier {
    ...
    HashMap<String, AdaptiveRandomForestRegressor> ARFregs =
            new HashMap<String, AdaptiveRandomForestRegressor>();
    ...
    public FileOption fileOption =
            new FileOption(
                    ...
                    "/Users/mbahri/Desktop/Dell/moa/src/main/java/moa/classifiers/meta/AutoML/settings.json",
                    ".json",
                    false);
        ...
        if (this.numberOfCores == 1) {
            ...
            for (int i = 0; i < this.ensemble.size(); i++) {
                EnsembleRunnable trainer =
                        new EnsembleRunnable(this.ensemble.get(i).classifier, inst);
            ...
            if (this.settings.useTestEnsemble && this.candidateEnsemble.size() > 0) {
                ...
                for (int i = 0; i < this.candidateEnsemble.size(); i++) {
                    EnsembleRunnable trainer =
                            new EnsembleRunnable(this.candidateEnsemble.get(i).classifier, inst);
                    trainers.add(trainer);
                }
            }
            try {
    ...
    public double[] getVotesForInstance(Instance inst) {
        ...
        // DoubleVector(this.ensemble.get(i).classifier.getVotesForInstance(inst));
            ...
            System.out.println(
                    "Classifier "
                            ...
                            + this.ensemble
                                    .get(this.bestModel)
                                    .classifier
                                    .getCLICreationString(Classifier.class)
                            + ") is the active classifier with performance: "
                            + this.performanceMeasures.get(this.bestModel));
        }
    ...
    protected void evaluatePerformance() {
        ...
        for (int i = 0; i < this.ensemble.size(); i++) {
            ...
            if (!bestPerformanceIdxMap.containsKey(algorithm)
                    || performance > bestPerformanceValMap.get(algorithm)) {
    ...
    protected double computePerformanceMeasure(Algorithm algorithm) {
        ...
        for (int i = 0; i < this.windowPoints.numInstances(); i++) {
            try {
                ...
                double[] votes =
                        algorithm.classifier.getVotesForInstance(this.windowPoints.instance(i));
                boolean correct =
                        (maxIndex(votes) * 1.0 == this.windowPoints.instance(i).classValue());

                if (correct) {
                    acc += 1.0 / this.windowPoints.numInstances();
                } else acc -= 1.0 / this.windowPoints.numInstances();
     ...
     *
     * @param algorithm Classifier being trained.
     */
    ...
    protected void promoteCandidatesIntoEnsemble() {
        for (int i = 0; i < this.candidateEnsemble.size(); i++) {
            ...
            if (this.verbose >= 1) {
                System.out.println(
                        "Test "
                                + i
                                + ") "
                                + newAlgorithm.classifier.getCLICreationString(Classifier.class)
                                + "\t => \t Performance: "
                                + performance);
            }
            ...
            if (this.ensemble.size() < this.settings.ensembleSize) {
                ...
                int replaceIdx =
                        AutoClass.sampleProportionally(
                                replace, !this.settings.performanceMeasureMaximisation); // false
                if (this.verbose >= 1) {
                    System.out.println(
                            "Promote "
                                    + newAlgorithm.classifier.getCLICreationString(Classifier.class)
                                    + " from test ensemble to the ensemble by replacing "
                                    + replaceIdx);
                }
    ...
    protected void trainRegressor(Algorithm algortihm, double performance) {
        ...
        dataset.setClassIndex(
                dataset.numAttributes()); // set class index to our performance feature
    ...
    protected void updateRemovalFlags(
            HashMap<String, Double> bestPerformanceValMap,
            HashMap<String, Integer> bestPerformanceIdxMap,
            HashMap<String, Integer> algorithmCount) {
    ...
    protected int sampleParent(ArrayList<Double> performM) {
        ...
        int parentIdx =
                AutoClass.sampleProportionally(
                        parents, this.settings.performanceMeasureMaximisation); // true
    ...
    protected Algorithm sampleNewConfiguration(int parentIdx) {
        ...
        Algorithm newAlgorithm =
                new Algorithm(
                        this.ensemble.get(parentIdx),
                        this.settings.lambda,
                        this.settings.resetProbability,
                        this.settings.keepCurrentModel,
                        this.verbose);
    ...
    protected double predictPerformance(Algorithm newAlgorithm) {
        ...
        // random forest
        double prediction =
                this.ARFregs.get(newAlgorithm.algorithm).getVotesForInstance(newInst)[0];
    ...
    public void setModelContext(InstancesHeader ih) {
        ...
        // This will cause issues in case setModelContext is invoked before the ensemble has been
        // created.
        //		It is likely safe to not perform this action due to how the context can be acquired
        // later by the learners.
        //		However, it is worth reviewing this in the future. See also
        // HeterogeneousEnsembleAbstract.setModelContext
        //            for (int i = 0; i < this.ensemble.size(); ++i) {
    ...
    public void prepareForUseImpl(TaskMonitor monitor, ObjectRepository repository) {
        ...
        try {
            ...
            BufferedReader bufferedReader =
                    new BufferedReader(new FileReader(fileOption.getValue()));
            Gson gson = new Gson();
