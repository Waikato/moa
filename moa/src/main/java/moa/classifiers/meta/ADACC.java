...
/**
 * Anticipative and Dynamic Adaptation to Concept Changes. Ensemble method for data streams that
 * adapts to concept changes and deals with concept recurrence.
 *
...
public class ADACC extends DACC implements MultiClassClassifier {
    ...
    public IntOption tauSizeOption =
            new IntOption(
                    "tau",
                    't',
                    "The size of the evaluation window for the meta-learning.",
                    100,
                    1,
                    10000);
    ...
    public FloatOption stabIndexSizeOption =
            new FloatOption("StabThr", 'z', "The threshold for stability", 0.8, 0, 1);

    /** Threshold for concept equivalence */
    public FloatOption equivIndexSizeOption =
            new FloatOption("CeThr", 'q', "The threshold for concept equivalence", 0.7, 0, 1);
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        if (recentChunk == null) recentChunk = new Instances(this.getModelContext());

        if (recentChunk.size() < this.tau_size) recentChunk.add(inst);
        else recentChunk.set(this.nbInstances % this.tau_size, inst);

        trainAndClassify(inst);

        if ((this.nbInstances % this.tau_size) == 0) takeSnapshot();
    }

    /**
     * If the environment is stable enough, take a snapshot (a copy) of the best adaptive classifier
     * and keep it for future use, in case of concept recurrence
     */
    private void takeSnapshot() {
        ...
        if (this.index >= this.theta_stab)
            if (addedPermanent == 0) {
                this.ensemble[this.ensemble.length - MAXPERMANENT + addedPermanent] =
                        getBestAdaptiveClassifier().copy();
                addedPermanent++;
            } else {
                ...
                for (int j = 0; j < Math.min(MAXPERMANENT, addedPermanent); j++) {
                    Classifier lastSnapshot =
                            this.ensemble[this.ensemble.length - MAXPERMANENT + j];

                    int[][] votes = new int[2][tau_size];
                    for (int k = 0; k < tau_size; k++) {
                        votes[0][k] =
                                Utils.maxIndex(candidate.getVotesForInstance(recentChunk.get(k)));
                        votes[1][k] =
                                Utils.maxIndex(
                                        lastSnapshot.getVotesForInstance(recentChunk.get(k)));
                    }
    ...
    /**
     * Returns the kappa statistics, a statistical measure of agreement in the predictions of 2
     * classifiers. Used as a measure of diversity of predictive models: the higher the kappa value,
    ...
    private double computeKappa(int[] y1, int[] y2) {
        ...
        if (theta1 == theta2 && theta2 == 1) return 1;

        return (theta1 - theta2) / (1 - theta2);
    }

    /**
     * Returns the stability index of the adaptive ensemble of classifiers. The ensemble is
     * considered stable here if its diversity level and error rates are low.
     *
    ...
    private double computeStabilityIndex() {
        ...
        for (int i = 0; i < m; i++) {
            for (int j = 0; j < tau_size; j++) {
                votes[i][j] =
                        Utils.maxIndex(
                                this.ensemble[arr[i].index].getVotesForInstance(
                                        recentChunk.get(j)));
