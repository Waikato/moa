...
/**
 * The online version of the Accuracy Updated Ensemble as proposed by Brzezinski and Stefanowski in
 * "Combining block-based and online methods in learning ensembles from concept drifting data
 * streams", Information Sciences, 2014.
 */
public class OnlineAccuracyUpdatedEnsemble extends AbstractClassifier
    ...
    public FloatOption windowSizeOption =
            new FloatOption(
                    "windowSize",
                    'w',
                    "The window size used for classifier creation and evaluation.",
                    500,
                    1,
                    Integer.MAX_VALUE);
    ...
    public IntOption maxByteSizeOption =
            new IntOption(
                    "maxByteSize",
                    'm',
                    "Maximum memory consumed by ensemble.",
                    33554432,
                    0,
                    Integer.MAX_VALUE);
    ...
    public FlagOption verboseOption =
            new FlagOption(
                    "verbose",
                    'v',
                    "When checked the algorithm outputs additional information about component"
                            + " classifier weights.");
    ...
    /**
     * The weights of stored classifiers. weights[x][0] = weight weights[x][1] = classifier number
     * in learners
     */
    ...
    public void prepareForUseImpl(TaskMonitor monitor, ObjectRepository repository) {
        ...
        this.candidate =
                new ClassifierWithMemory(
                        ((Classifier) getPreparedClassOption(this.learnerOption)).copy(),
                        this.windowSize);
        ...
        if (this.trainingWeightSeenByModel > 0.0) {
            for (int i = 0; i < this.ensemble.length; i++) {
                if (this.weights[i][0] > 0.0) {
                    DoubleVector vote =
                            new DoubleVector(
                                    this.ensemble[(int) this.weights[i][1]].classifier
    ...
    protected void enforceMemoryLimit() {
        double memoryLimit =
                this.maxByteSizeOption.getValue() / (double) (this.ensemble.length + 1);

        for (int i = 0; i < this.ensemble.length; i++) {
            ((HoeffdingTree) this.ensemble[(int) this.weights[i][1]].classifier)
                    .maxByteSizeOption.setValue((int) Math.round(memoryLimit));
            ((HoeffdingTree) this.ensemble[(int) this.weights[i][1]].classifier)
                    .enforceTrackerLimit();
        }
    }
     ...
     *
     * @param i the identifier (in terms of array learners) of the classifier for which the weight
     *     is supposed to be computed
        ...
        if (this.verboseOption.isSet()) {
            ...
            if (this.weights != null) {
                for (int i = 0; i < this.weights.length; i++) {
                    measurements[i] =
                            new Measurement("Member weight " + (i + 1), this.weights[i][0]);
                }
            }
