...
/**
 * The revised version of the Accuracy Updated Ensemble as proposed by Brzezinski and Stefanowski in
 * "Reacting to Different Types of Concept Drift: The Accuracy Updated Ensemble Algorithm", IEEE
 * Trans. Neural Netw, 2013.
 */
public class AccuracyUpdatedEnsemble extends AbstractClassifier implements MultiClassClassifier {
    ...
    /**
     * Determines the maximum size of model (evaluated after every chunk). In effect only for
     * HoeffdingTree-based component classifiers.
     */
        ...
        if (this.trainingWeightSeenByModel > 0.0) {
            for (int i = 0; i < this.learners.length; i++) {
                if (this.weights[i][0] > 0.0) {
                    DoubleVector vote =
                            new DoubleVector(
                                    this.learners[(int) this.weights[i][1]].getVotesForInstance(
                                            inst));
        ...
        for (int i = 0; i < this.learners.length; i++) {
            this.weights[i][0] =
                    1.0
                            / (mse_r
                                    + this.computeMse(
                                            this.learners[(int) this.weights[i][1]],
                                            this.currentChunk)
                                    + Double.MIN_VALUE);
        }
    ...
    protected void enforceMemoryLimit() {
        double memoryLimit =
                this.maxByteSizeOption.getValue() / (double) (this.learners.length + 1);

        for (int i = 0; i < this.learners.length; i++) {
            ((HoeffdingTree) this.learners[(int) this.weights[i][1]])
