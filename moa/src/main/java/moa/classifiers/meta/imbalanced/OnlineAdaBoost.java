 ...
 * classifier :math:`h_m` are given more weights in the training set of the following learner
 * :math:`h_{m+1}`. In the online context, since there is no training dataset, but a stream of
 * samples, the drawing of samples with replacement can't be trivially executed. The strategy
 * adopted by the Online Boosting algorithm is to simulate this task by training each arriving
 * sample K times, which is drawn by the binomial distribution. Since we can consider the data
 * stream to be infinite, and knowing that with infinite samples the binomial distribution
 * :math:`Binomial(p, N)` tends to a :math:`Poisson(\lambda)` distribution, where :math:`\lambda =
 * Np`. :math:`\lambda` is computed by tracking the total weights of the correctly and misclassified
 * examples.
 *
 ...
 * @version $Revision: 1 $
 */
public class OnlineAdaBoost extends AbstractClassifier
        ...
        for (int i = 0; i < this.ensemble.size(); i++) {
            ...
            if (k > 0) {
                ...
                if (Utils.maxIndex(this.ensemble.get(i).getVotesForInstance(instance))
                    ...
                    this.epsilon.set(
                            i,
                            this.lambdaSw.get(i) / (this.lambdaSw.get(i) + this.lambdaSc.get(i)));
