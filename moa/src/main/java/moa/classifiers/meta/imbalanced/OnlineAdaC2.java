 ...
 *
 * <p>AdaC2 is a boosting algorithm that takes the different misclassification costs into
 * consideration when calculating the classier weights, and updates the sample weight. AdaC2
 * increases more the weights on the misclassified positive samples, compared to misclassified
 * negative samples.:math:`C_P` and :math:`C_N` are the positive and negative costs.
 *
 ...
 *   <li>-p : The cost of misclassifying a positive sample.
 *   <li>-n : The cost of misclassifying a negative sample.
 ...
 * @version $Revision: 1 $
 */
public class OnlineAdaC2 extends AbstractClassifier
        implements MultiClassClassifier, CapabilitiesHandler {

    @Override
    public String getPurposeString() {
        return "OnlineAdaC2 is the adaptation of the ensemble learner to data streams from B. Wang"
                + " and J. Pineau";
    }
    ...
    public void trainOnInstanceImpl(Instance instance) {
        ...
        for (int i = 0; i < this.ensemble.size(); i++) {
            ...
            if (k > 0) {
                ...
                if (Utils.maxIndex(this.ensemble.get(i).getVotesForInstance(instance)) == 1.0
                        && instance.classValue() == 1.0) {
                    this.lambdaTP.set(i, this.lambdaTP.get(i) + (this.costPositive * lambda));
                    this.wAcc.set(
                            i,
                            (this.lambdaTP.get(i) + this.lambdaTN.get(i)) / this.lambdaSum.get(i));
                    this.wErr.set(
                            i,
                            (this.lambdaFP.get(i) + this.lambdaFN.get(i)) / this.lambdaSum.get(i));
                    lambda = (this.costPositive * lambda) / (2 * this.wAcc.get(i));
                } else if (Utils.maxIndex(this.ensemble.get(i).getVotesForInstance(instance)) == 0.0
                        && instance.classValue() == 0.0) {
                ...
                } else if (Utils.maxIndex(this.ensemble.get(i).getVotesForInstance(instance)) == 1.0
                        && instance.classValue() == 0.0) {
        ...
        for (int i = 0; i < this.ensemble.size(); ++i) {
            ...
            if (vote.sumOfValues() > 0.0) {
                for (int v = 0; v < vote.numValues(); ++v) {
                    vote.setValue(
                            v, vote.getValue(v) * Math.log(this.wAcc.get(i) / this.wErr.get(i)));
                }
                vote.normalize();
