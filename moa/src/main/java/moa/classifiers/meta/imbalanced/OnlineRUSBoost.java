 ...
 *
 * <p>RUSBoost follows the techniques of UnderOverBagging and SMOTEBagging by introducing sampling
 * techniques as a postprocessing step before each iteration of the standard AdaBoost algorithm.
 * RUSBoost uses three implementations:
 *
 * <p>- RUSBoost 1: fixes the class ration. - RUSBoosT 2: fixes the example distribution. - RUSBoost
 * 3: fixes the sampling rate.
 *
 * <p>This online ensemble learner method is improved by the addition of an ADWIN change detector.
 * ADWIN stands for Adaptive Windowing. It works by keeping updated statistics of a variable sized
 * window, so it can detect changes and perform cuts in its window to better adapt the learning
 * algorithms.
 *
 * <p>See details in:<br>
 * B. Wang and J. Pineau, "Online Bagging and Boosting for Imbalanced Data Streams," in IEEE
 * Transactions on Knowledge and Data Engineering, vol. 28, no. 12, pp. 3353-3366, 1 Dec. 2016. doi:
 * 10.1109/TKDE.2016.2609424
 *
 * <p>Parameters:
 *
 * <ul>
 *   <li>-l : Each classiﬁer to train of the ensemble is an instance of the base estimator.
 *   <li>-s : The size of the ensemble, in other words, how many classifiers to train.
 *   <li>-i : The sampling rate of the positive instances.
 *   <li>-a : The implementation of RUSBoost to use {1, 2, 3}.
 ...
 *   <li>-r : Seed for the random state.
 * </ul>
 *
 * @author Alessio Bernardo (alessio dot bernardo at polimi dot dot it)
 * @version $Revision: 1 $
 */
public class OnlineRUSBoost extends AbstractClassifier
    ...
    public IntOption ensembleSizeOption =
            new IntOption(
                    "ensembleSize", 's', "The size of the ensemble.", 10, 1, Integer.MAX_VALUE);

    public IntOption samplingRateOption =
            new IntOption(
                    "samplingRate", 'i', "The sampling rate of the positive instances.", 3, 1, 10);
    ...
    public MultiChoiceOption algorithmImplementationOption =
            new MultiChoiceOption(
                    ...
                    new String[] {
                        "Fixed class ration", "Fixed example distribution", "Fixed sampling rate"
                    },
                    new String[] {"ClassRation", "ExampleDistribution", "SamplingRate"},
                    0);
    ...
    public void trainOnInstanceImpl(Instance instance) {
        ...
        for (int i = 0; i < this.ensemble.size(); i++) {
            ...
            if (this.algorithmImplementation == 0) {
                        ...
                        lambdaRus =
                                lambda
                                        * ((this.lambdaPos.get(i) + this.lambdaNeg.get(i))
                                                / (this.lambdaPos.get(i)
                                                        + this.lambdaNeg.get(i)
                                                                * (this.samplingRate
                                                                        * (this.nPositive
                                                                                / this.nNegative)))
                                                * (((this.samplingRate + 1) * this.nPositive)
                                                        / (this.nPositive + this.nNegative)));
                    }
                } else {
                    if (this.nPositive != 0) {
                        lambdaRus =
                ...
                if (instance.classValue() == 1.0) {
                    lambdaRus =
                            ((lambda * this.nPositive) / (this.nPositive + this.nNegative))
                                    / (this.lambdaPos.get(i)
                                            / (this.lambdaPos.get(i) + this.lambdaNeg.get(i)));
                } else {
                    lambdaRus =
                            ((lambda * this.samplingRate * this.nPositive)
                                            / (this.nPositive + this.nNegative))
                                    / (this.lambdaNeg.get(i)
                                            / (this.lambdaPos.get(i) + this.lambdaNeg.get(i)));
                }
            ...
            if (Utils.maxIndex(this.ensemble.get(i).getVotesForInstance(instance))
                    == instance.classValue()) {
                this.lambdaSc.set(i, this.lambdaSc.get(i) + lambda);
                this.epsilon.set(
                        i, this.lambdaSw.get(i) / (this.lambdaSc.get(i) + this.lambdaSw.get(i)));
    ...
    public double[] getVotesForInstance(Instance instance) {
        ...
        for (int i = 0; i < this.ensemble.size(); ++i) {
            DoubleVector vote =
                    new DoubleVector(this.ensemble.get(i).getVotesForInstance(testInstance));
            if (vote.sumOfValues() > 0.0) {
                for (int v = 0; v < vote.numValues(); ++v) {
                    vote.setValue(
                            v,
                            vote.getValue(v)
                                    * Math.log((1 - this.epsilon.get(i)) / this.epsilon.get(i)));
                }
                vote.normalize();
