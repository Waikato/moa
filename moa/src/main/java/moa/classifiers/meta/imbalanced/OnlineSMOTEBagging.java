 ...
 *
 * <p>This method works by re-sampling the negative class with replacement rate at 100%, while
 * :math:`CN^+` positive examples are generated for each base learner, among which :math:`a\%` of
 * them are created by re-sampling and the rest of the examples are created by the synthetic
 ...
 * @version $Revision: 1 $
 */
public class OnlineSMOTEBagging extends AbstractClassifier
    ...
    public IntOption samplingRateOption =
            new IntOption(
                    "samplingRate", 'i', "The sampling rate of the positive instances.", 1, 1, 10);
            ...
            try {
                Instances neighbours =
                        search.kNearestNeighbours(
                                x, Math.min(k, this.posSamples.numInstances() - 1));
                ...
                Enumeration attrEnum =
                        this.samoaToWeka
                                .wekaInstance(this.posSamples.instance(0))
                                .enumerateAttributes();
                while (attrEnum.hasMoreElements()) {
                    ...
                    if (!attr.equals(
                        ...
                        if (attr.isNumeric()) {
                            double dif =
                                    this.samoaToWeka
                                                    .wekaInstance(neighbours.instance(nn))
                                                    .value(attr)
                                            - this.samoaToWeka.wekaInstance(x).value(attr);
