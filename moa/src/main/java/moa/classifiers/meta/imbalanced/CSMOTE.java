 ...
 * CSMOTE
 *
 * <p>This strategy save all the minority samples in a window managed by ADWIN. In the meantime, a
 * model is trained with the data in input. When the minority sample ratio is less than a certain
 * threshold, an online SMOTE version is applied. A random minority sample is chosen from the window
 * and a new synthetic sample is generated until the minority sample ratio is greater or equal than
 * the threshold. The model is then trained with the new samples generated.
 *
 * <p>See details in:<br>
 * Alessio Bernardo, Heitor Murilo Gomes, Jacob Montiel, Bernhard Pfharinger, Albert Bifet, Emanuele
 * Della Valle. C-SMOTE: Continuous Synthetic Minority Oversampling for Evolving Data Streams. In
 * BigData, IEEE, 2020.
 *
 * <p>Parameters:
 *
 * <ul>
 *   <li>-l : Classifier to train. Default is ARF
 *   <li>-k : Number of neighbors for SMOTE. Default is 5
 *   <li>-t : Threshold for the minority samples. Default is 0.5
 *   <li>-m : Minimum number of samples in the minority class for applying SMOTE. Default is 100
 *   <li>-d : Should use ADWIN as drift detector? If enabled it is used by the method to track the
 *       performance of the classifiers and adapt when a drift is detected.
 * </ul>
 *
 * @author Alessio Bernardo (alessio dot bernardo at polimi dot com)
 * @version $Revision: 1 $
...
public class CSMOTE extends AbstractClassifier implements MultiClassClassifier {
    ...
    public String getPurposeString() {
        return "OnlineSMOTE strategy that saves the data in a sliding window and when the minority"
                + " class ratio is less than a threshold it generates some synthetic new samples"
                + " using SMOTE";
    }
    ...
    public IntOption neighborsOption =
            new IntOption(
                    "neighbors", 'k', "Number of neighbors for SMOTE.", 5, 1, Integer.MAX_VALUE);

    public FloatOption thresholdOption =
            new FloatOption("threshold", 't', "Minority class samples threshold.", 0.5, 0.1, 0.5);
    ...
    public FlagOption disableDriftDetectionOption =
            new FlagOption("disableDriftDetection", 'd', "Should use ADWIN as drift detector?");
    ...
    private double calculateRatio() {
        ...
        if ((this.nMinorityTotal + this.nGeneratedMinorityTotal)
            ...
            ratio =
                    ((double) this.nMinorityTotal + (double) this.nGeneratedMinorityTotal)
                            / ((double) this.nMinorityTotal
                                    + (double) this.nGeneratedMinorityTotal
                                    + (double) this.nGeneratedMajorityTotal
                                    + (double) this.nMajorityTotal);
        }
        // class 1 is the real minority
        else {
            ratio =
                    ((double) this.nMajorityTotal + (double) this.nGeneratedMajorityTotal)
                            / ((double) this.nMinorityTotal
                                    + (double) this.nGeneratedMinorityTotal
                                    + (double) this.nGeneratedMajorityTotal
                                    + (double) this.nMajorityTotal);
    ...
    private Instance generateNewInstance(Instances minoritySamples) {
        ...
        try {
            Instances neighbours =
                    search.kNearestNeighbours(
                            instanceI,
                            Math.min(this.neighbors, minoritySamples.numInstances() - 1));
            ...
            Enumeration attrEnum =
                    this.samoaToWeka
                            .wekaInstance(minoritySamples.instance(0))
                            .enumerateAttributes();
            while (attrEnum.hasMoreElements()) {
                ...
                if (!attr.equals(
                    ...
                    if (attr.isNumeric()) {
                        double dif =
                                this.samoaToWeka.wekaInstance(neighbours.instance(nn)).value(attr)
                        ...
                        for (int nnEx = 0; nnEx < neighbours.numInstances(); nnEx++) {
                            int val =
                                    (int)
                                            this.samoaToWeka
                                                    .wekaInstance(neighbours.instance(nnEx))
                                                    .value(attr);
                            valueCounts[val]++;
                        }
                        int maxIndex = 0;
