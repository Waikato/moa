 ...
 * RebalanceStream
 *
 * <p>The RebalanceStream algorithm trains a model called learner as the standard algorithm
 * implementation. Moreover, it saves every new data in input in a batch. If ADWIN detects a warning
 * level, it starts saving the new data in input also in another batch called resetBatch. Then, when
 * ADWIN detects a change, so there is a concept drift, it trains in parallel 3 models called
 * learnerBal, learnerReset, learnerResetBal. The learnerBal model uses the batch data rebalanced by
 * SMOTE, the learnerReset model uses the resetBatch data and the learnerResetBal model uses the
 * resetBatch data rebalanced by SMOTE. Then the best model based on k-statistic is chosen to
 * continue the experiment with a new sample.
 *
 * <p>See details in:<br>
 * Alessio Bernardo, Albert Bifet, Emanuele Della Valle. Incremental Rebalancing Learning \\on
 * Evolving Data Streams. In ICDM Workshop, 2020.
 *
 * <p>Parameters:
 *
 * <ul>
 *   <li>-l : Classiﬁer to train. Default is TemporallyAugmentedClassifier
 *   <li>-c : Minimum number of samples in the batch for applying SMOTE. Default is -1 (no limit)
 *   <li>-g : Maximum number of samples in the batch for applying SMOTE. Default is -1 (no limit)
 *   <li>-h : Minimum number of samples in the ResetBatch for applying SMOTE. Default is -1 (no
 *       limit)
 *   <li>-g : Maximum number of samples in the ResetBatch for applying SMOTE. Default is -1 (no
 *       limit)
 * </ul>
 *
 * @author Alessio Bernardo (alessio dot bernardo at polimi dot com)
...
public class RebalanceStream extends AbstractClassifier implements MultiClassClassifier {
    ...
    public ClassOption baseLearnerOption =
            new ClassOption(
                    ...
                    "Classifier to train.",
                    Classifier.class,
                    "meta.TemporallyAugmentedClassifier");

    public IntOption minInstanceLimitBatchOption =
            new IntOption(
                    "minInstanceLimitBatch",
                    'c',
                    "Minimum number of instances in the batch in order to rebalance it  (-1 = no"
                            + " limit).",
                    -1,
                    -1,
                    Integer.MAX_VALUE);
    ...
    public IntOption maxInstanceLimitBatchOption =
            new IntOption(
                    ...
                    'g',
                    "Maximum number of instances in the batch in order to rebalance it  (-1 = no"
                            + " limit).",
                    -1,
                    -1,
                    Integer.MAX_VALUE);
    ...
    public IntOption minInstanceLimitResetBatchOption =
            new IntOption(
                    ...
                    'h',
                    "Minimum number of instances in the Resetbatch in order to rebalance it  (-1 ="
                            + " no limit).",
                    -1,
                    -1,
                    Integer.MAX_VALUE);
    ...
    public IntOption maxInstanceLimitResetBatchOption =
            new IntOption(
                    ...
                    'm',
                    "Maximum number of instances in the Resetbatch in order to rebalance it  (-1 ="
                            + " no limit).",
                    -1,
                    -1,
                    Integer.MAX_VALUE);
    ...
    public void resetLearningImpl() {
        ...
        if (this.minInstanceLimitBatch != -1 && this.maxInstanceLimitBatch != -1) {
            if (this.minInstanceLimitBatch > this.maxInstanceLimitBatch) {
                System.out.println(
                        "The minimum number of instances in the batch cannot be greater than the"
        ...
        if (this.minInstanceLimitResetBatch != -1 && this.maxInstanceLimitResetBatch != -1) {
            if (this.minInstanceLimitResetBatch > this.maxInstanceLimitResetBatch) {
                System.out.println(
                        "The minimum number of instances in the resetBatch cannot be greater than"
    ...
    private void checkADWINWidth(Instance instance) throws Exception {
        ...
        if (this.adwin.getChange()) {
            boolean minBatchSizeCheck =
                    checkMinConstraints(this.batch.size(), this.minInstanceLimitBatch);
            boolean minResetBatchSizeCheck =
                    checkMinConstraints(this.resetBatch.size(), this.minInstanceLimitResetBatch);
            ...
            boolean maxBatchSizeCheck =
                    checkMaxConstraints(this.batch.size(), this.maxInstanceLimitBatch);
            boolean maxResetBatchCheck =
                    checkMaxConstraints(this.resetBatch.size(), this.maxInstanceLimitResetBatch);
    ...
    private void trainsParallelModels() throws Exception {
        ...
        if (this.nMinorityTotal > 1 && this.nMajorityTotal > 1) {
            ...
            while (0.5 > ratio) {
                ...
                if ((this.nMinorityTotal + this.nGeneratedMinorityTotal)
                        < (this.nMajorityTotal + this.nGeneratedMajorityTotal)) {
        ...
        ratio =
                calculateRatio(
                        this.resetBatchMajority.size(),
                        this.resetBatchMinority.size(),
                        maxGenerated,
                        minGenerated);
        // if the ratio is less than threshold,
        // I apply the online SMOTE version
        ...
        if (this.resetBatchMinority.size() > 1 && this.resetBatchMajority.size() > 1) {
            ...
            while (0.5 > ratio) {
                Instance newInstance;
                // class 0 is the real minority
                if ((this.resetBatchMinority.size() + minGenerated)
                        < (this.resetBatchMajority.size() + maxGenerated)) {
                ...
                // prequential evaluation
                fillConfusionMatrix(
                        newInstance, this.confusionMatrixResetBal, this.learnerResetBal);
                // model training
        ...
        if (rebalance) {
            ...
            this.kStatResetBal =
                    calculateKStatistic(this.confusionMatrixResetBal, this.accResetBal);
        } else {
    ...
    private Instance generateNewInstance(
            ArrayList<Instance> minoritySamples, boolean newInstanceBatch) {
                ...
                while (attrEnum.hasMoreElements()) {
                    ...
                    if (!attr.equals(this.minorityInstances.classAttribute())) {
                        ...
                        if (attr.isNumeric()) {
                            ...
                            distance +=
                                    ((double[][]) this.vdmMap.get(attr))[(int) iVal][(int) jVal];
                        }
                    }
                }
        ...
        while (attrEnum.hasMoreElements()) {
            ...
            if (!attr.equals(this.minorityInstances.classAttribute())) {
                if (attr.isNumeric()) {
                    double dif =
                            this.samoaToWeka.wekaInstance(nnArray[nn]).value(attr)
                                    - this.samoaToWeka.wekaInstance(instanceI).value(attr);
        ...
        if (newInstanceBatch == true) {
            if (this.instanceGenerated.get(instanceI) != null) {
                this.instanceGenerated.replace(
                        instanceI, this.instanceGenerated.get(instanceI) + 1);
            } else {
            ...
            if (!attr.equals(this.minorityInstances.classAttribute())) {
                if (attr.isNominal() || attr.isString()) {
                    ...
                    int[][] featureValueCountsByClass =
                            new int[this.minorityInstances.classAttribute().numValues()]
                                    [attr.numValues()];
                    ...
                    for (int valueIndex1 = 0; valueIndex1 < attr.numValues(); valueIndex1++) {
                        for (int valueIndex2 = 0; valueIndex2 < attr.numValues(); valueIndex2++) {
                            ...
                            for (int classValueIndex = 0;
                                    classValueIndex < this.minorityInstances.numClasses();
                                    classValueIndex++) {
                                double c1i =
                                        (double)
                                                featureValueCountsByClass[classValueIndex][
                                                        valueIndex1];
                                double c2i =
                                        (double)
                                                featureValueCountsByClass[classValueIndex][
    ...
    private double calculateRatio(
            int nMajority, int nMinority, int nMajorityGenerated, int nMinorityGenerated) {
        double ratio = 0.0;
        // class 0 is the real minority
        if ((nMinority + nMinorityGenerated) <= (nMajority + nMajorityGenerated)) {
            ratio =
                    ((double) nMinority + (double) nMinorityGenerated)
                            / ((double) nMinority
                                    + (double) nMinorityGenerated
                                    + (double) nMajority
                                    + (double) nMajorityGenerated);
        }
        // class 1 is the real minority
        else {
            ratio =
                    ((double) nMajority + (double) nMajorityGenerated)
                            / ((double) nMinority
                                    + (double) nMinorityGenerated
                                    + (double) nMajority
                                    + (double) nMajorityGenerated);
        ...
        double p0 =
                (((double) confusionMatrix[0][0] + (double) confusionMatrix[0][1])
                        ...
                        * (((double) confusionMatrix[0][0] + (double) confusionMatrix[1][0])
                                / (double) numberSamples);
        double p1 =
                (((double) confusionMatrix[1][0] + (double) confusionMatrix[1][1])
                                / (double) numberSamples)
                        * (((double) confusionMatrix[0][1] + (double) confusionMatrix[1][1])
                                / (double) numberSamples);
        double pc = p0 + p1;
    ...
    private void fillConfusionMatrix(
            Instance instance, int[][] confusionMatrix, Classifier learner) {
    ...
    public String toString() {
        return "RebalanceStream strategy trains in parallel four models and each one of them uses a"
