...
/**
 * Comparing with the original AdaptiveRandomForest, AdaptiveRandomForestRE with an extra option to turn off resampling function.
 * The original is a classifier designed to deal with imbalanced datasets.
 *
 * @author Heitor Murilo Gomes, Ethan Wang
 * @version $Revision: 1 $
 */
public class AdaptiveRandomForestRE extends AbstractClassifier implements MultiClassClassifier,
    ...
    public MultiChoiceOption streamBalanceOption = new MultiChoiceOption("streamBalanceOption", 'r', "The option of stream balance", new String[]{"Imbalance", "Balance"},
            new String[]{"Default (no balance)", "Balanced"}, 0);
    ...
    protected static final int IMBALANCE_STREAM_RESAMPLING_ON = 0;
    protected static final int BALANCE_STREAM_RESAMPLING_OFF = 1;
    ...
    public void resetLearningImpl() {
        ...
        if (numberOfJobs != AdaptiveRandomForestRE.SINGLE_THREAD && numberOfJobs != 1)
...
//        BasicClassificationPerformanceEvaluator classificationEvaluator = (BasicClassificationPerformanceEvaluator) getPreparedClassOption(this.evaluatorOption);
        ...
        switch (this.mFeaturesModeOption.getChosenIndex()) {
            case AdaptiveRandomForestRE.FEATURES_SQRT:
                this.subspaceSize = (int) Math.round(Math.sqrt(n)) + 1;
                break;
            case AdaptiveRandomForestRE.FEATURES_SQRT_INV:
                this.subspaceSize = n - (int) Math.round(Math.sqrt(n) + 1);
                break;
            case AdaptiveRandomForestRE.FEATURES_PERCENT:
        ...
        //  AdaptiveRandomForestRE.FEATURES_M then nothing is performed in the
    ...
    protected final class ARFBaseLearner {
        ...
        public void trainOnInstance(Instance instance, double weight, long instancesSeen) {
            ...
            switch (this.resampling.getChosenIndex()) {
                case AdaptiveRandomForestRE.BALANCE_STREAM_RESAMPLING_OFF:
                    //Turn off resampling for balance dataset.
                    // The same as original AdaptiveRandomForest
                    break;
                // Default (no balance)
                case AdaptiveRandomForestRE.IMBALANCE_STREAM_RESAMPLING_ON:
                    if (instancesSeen <= 1) {
