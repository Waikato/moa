...
public class RandomRules extends AbstractClassifier implements Regressor {
    ...
    public double[] getVotesForInstance(Instance inst) {
        ...
        for (int i = 0; i < this.ensemble.length; i++) {
            DoubleVector vote =
                    new DoubleVector(
                            this.ensemble[i].getVotesForInstance(transformInstance(inst, i)));
            if (VerbosityOption.getValue() > 1) sb.append(vote.getValue(0) + ", ");
    ...
    protected Instance transformInstance(Instance inst, int classifierIndex) {
        if (this.listAttributes == null) {
            //  this.numAttributes = (int) (this.numAttributesPercentageOption.getValue() *
            // inst.numAttributes()/100.0); // JD: number of attributes should not consider class
            this.numAttributes =
                    (int)
                            (this.numAttributesPercentageOption.getValue()
            ...
            for (int ensembleIndex = 0; ensembleIndex < this.ensemble.length; ensembleIndex++) {
                for (int attributeIndex = 0;
                        attributeIndex < this.numAttributes;
                        attributeIndex++) {
                    boolean isUnique = false;
                    while (isUnique == false) {
                        this.listAttributes[attributeIndex][ensembleIndex] =
                                this.classifierRandom.nextInt(inst.numAttributes() - 1);
                        isUnique = true;
                        for (int k = 0; k < attributeIndex; k++) {
                            if (this.listAttributes[attributeIndex][ensembleIndex]
                                    == this.listAttributes[k][ensembleIndex]) {
                                isUnique = false;
                                break;
                            }
                        }
                ...
                for (int attributeIndex = 0;
                    ...
                    attributes.addElement(
                            inst.attribute(this.listAttributes[attributeIndex][ensembleIndex]));
        ...
        for (int attributeIndex = 0; attributeIndex < this.numAttributes; attributeIndex++) {
            // instance.setValue(attributeIndex,
            // inst.value(this.listAttributes[attributeIndex][classifierIndex]));
            attVals[attributeIndex] =
                    inst.value(this.listAttributes[attributeIndex][classifierIndex]);
        }
