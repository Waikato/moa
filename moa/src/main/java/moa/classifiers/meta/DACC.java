...
/**
 * Dynamic Adaptation to Concept Changes. Ensemble method for data streams that adapts to concept
 * changes.
 *
 * <p>Reference: JABER, G., CORNUEJOLS, A., and TARROUX, P. A New On-Line Learning Method for Coping
 * with Recurring Concepts: The ADACC System. In : Neural Information Processing. Springer Berlin
 * Heidelberg, 2013. p. 595-604.
 *
 * @author Ghazal Jaber (ghazal.jaber@gmail.com)
 */
public class DACC extends AbstractClassifier implements MultiClassClassifier {
    ...
    public FloatOption memberCountOption =
            new FloatOption(
                    "ensembleSize",
                    'n',
                    "The maximum number of classifiers in an ensemble.",
                    20,
                    1,
                    Integer.MAX_VALUE);
    ...
    public FloatOption maturityOption =
            new FloatOption("maturity", 'a', "The maturity age.", 20, 0, 100);

    /** Size of the evaluation window for weights computing */
    public FloatOption evaluationSizeOption =
            new FloatOption("evalSize", 'e', "The size of the evaluation window.", 20, 1, 1000);

    /**
     * Combination functions: MAX and WVD (MAX leads to a faster reactivity to the change, WVD is
    ...
    public MultiChoiceOption combinationOption =
            new MultiChoiceOption(
                    ...
                    new String[] {"MAX", "WVD"},
                    new String[] {"Maximum", "Weighted Vote of the best"},
                    0);
    ...
    public double[] getVotesForInstance(Instance inst) {
        ...
        if (cmb == 0) arr = getMAXIndexes();
        else arr = getWVDIndexes();
        ...
        if (this.trainingWeightSeenByModel > 0.0) {
            ...
            for (int i = 0; i < arr.size(); i++) {
                if (this.ensembleWeights[arr.get(i)].val > 0.0) {
                    ...
                    DoubleVector vote =
                            new DoubleVector(this.ensemble[arr.get(i)].getVotesForInstance(inst));
    ...
    protected void trainAndClassify(Instance inst) {
        ...
        for (int i = 0; i < getNbActiveClassifiers(); i++) {
            ...
            if (this.ensembleAges[i] < this.maturityOption.getValue()
                    && i < getNbAdaptiveClassifiers()) mature = false;

            // check if all adaptive learners are not mature
            if (this.ensembleAges[i] >= this.maturityOption.getValue()
                    && i < getNbAdaptiveClassifiers()) unmature = false;

            if (this.nbInstances >= this.ensembleWeights[i].index + 1) {

                // train adaptive learners
                if (i < getNbAdaptiveClassifiers()) this.ensemble[i].trainOnInstance(inst);
    ...
    protected double updateEvaluationWindow(int index, int val) {
        ...
        int wsize =
                (int) Math.min(this.evaluationSizeOption.getValue(), this.ensembleAges[index] + 1);
    ...
    /**
     * Returns the best (or worst) half of classifiers in the adaptive ensemble. The best
     * classifiers are used to compute the stability index in ADACC. The worst classifiers are
     * returned in order to select a classifier for deletion.
     *
     * @param bestHalf boolean value set to true (false) if we want to return the best (worst) half
     *     of adaptive classifiers.
     * @return an array containing the weight values of the corresponding classifiers and their
     *     indexes in the ensemble.
     */
    protected Pair[] getHalf(boolean bestHalf) {
        ...
        if (bestHalf) Arrays.sort(newEnsembleWeights, Collections.reverseOrder());
        else Arrays.sort(newEnsembleWeights);
    ...
    /**
     * Returns the classifiers that vote for the final prediction when the MAX combination function
     * is selected
     *
    ...
    protected ArrayList<Integer> getMAXIndexes() {
        ...
        for (int i = newEnsembleWeights.length - 1; i >= 0; i--) {
            if (newEnsembleWeights[i].val != maxWVal) break;
            else maxWIndex.add(newEnsembleWeights[i].index);
        }
        return maxWIndex;
    }

    /**
     * Returns the classifiers that vote for the final prediction when the WVD combination function
     * is selected
     *
     * @return the classifiers whose weights lie in the higher half of the ensemble's weight
     *     interval.
     */
    protected ArrayList<Integer> getWVDIndexes() {
        ...
        for (int i = newEnsembleWeights.length - 1; i >= 0; i--)
            if (newEnsembleWeights[i].val < med) break;
            else maxWIndex.add(newEnsembleWeights[i].index);
    ...
    /**
     * Returns the number of classifiers used for prediction which includes the adaptive learners
     * and the snapshots in ADACC
     *
    ...
    /**
     * Returns the number of adaptive classifiers in the ensemble which excludes the static
     * snapshots in ADACC
     *
    ...
    /**
     * This helper class is used to sort an array of pairs of integers: val and index. The array is
     * sorted based on the val field.
     *
     * @author Ghazal Jaber
     */
