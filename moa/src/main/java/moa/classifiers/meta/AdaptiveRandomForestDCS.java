...
import moa.classifiers.trees.HoeffdingTreeDCS;
 ...
 * @version $Revision: 1 $
 */
public class AdaptiveRandomForestDCS extends AbstractClassifier implements MultiClassClassifier,
                                                                        CapabilitiesHandler {
    ...
    public MultiChoiceOption votingMethodOption = new MultiChoiceOption("votingMethod", 'V',
            "", new String[]{"Tree Depth", "ORACLE"}, new String[]{"Tree Depth", "ORACLE"},
            0);

    public ClassOption treeLearnerOption = new ClassOption("treeLearner", 'l',
            "Random Forest Tree.", ARFHoeffdingTreeDCS.class,
            "ARFHoeffdingTreeDCS -e 2000000 -g 50 -c 0.01");
    ...
    protected ARFBaseLearnerDCS[] ensemble;
    ...
    public void resetLearningImpl() {
        ...
        if(numberOfJobs != AdaptiveRandomForestDCS.SINGLE_THREAD && numberOfJobs != 1)
    ...
    public void trainOnInstanceImpl(Instance instance) {
        ...
        for (int i = 0 ; i < this.ensemble.length ; i++) {
            HoeffdingTreeDCS.Vote vote = this.ensemble[i].getVotesForInstance(instance);
            DoubleVector voteVector = new DoubleVector(vote.getVotes());
            InstanceExample example = new InstanceExample(instance);
            this.ensemble[i].evaluator.addResult(example, voteVector.getArrayRef());
...
//            System.out.println(testInstance.classAttribute().getAttributeValues());
    ...
    protected void initEnsemble(Instance instance) {
        ...
        this.ensemble = new ARFBaseLearnerDCS[ensembleSize];
...
//        BasicClassificationPerformanceEvaluator classificationEvaluator = (BasicClassificationPerformanceEvaluator) getPreparedClassOption(this.evaluatorOption);
        ...
        switch(this.mFeaturesModeOption.getChosenIndex()) {
            case AdaptiveRandomForestDCS.FEATURES_SQRT:
                this.subspaceSize = (int) Math.round(Math.sqrt(n)) + 1;
                break;
            case AdaptiveRandomForestDCS.FEATURES_SQRT_INV:
                this.subspaceSize = n - (int) Math.round(Math.sqrt(n) + 1);
                break;
            case AdaptiveRandomForestDCS.FEATURES_PERCENT:
        ...
        ARFHoeffdingTreeDCS treeLearner = (ARFHoeffdingTreeDCS) getPreparedClassOption(this.treeLearnerOption);
        treeLearner.resetLearning();
        
        for(int i = 0 ; i < ensembleSize ; ++i) {
            treeLearner.subspaceSizeOption.setValue(this.subspaceSize);
            this.ensemble[i] = new ARFBaseLearnerDCS(
                i, 
                (ARFHoeffdingTreeDCS) treeLearner.copy(),
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
        if (this.getClass() == AdaptiveRandomForestDCS.class)
     ...
     */
    protected final class ARFBaseLearnerDCS extends AbstractMOAObject {
        public int indexOriginal;
        public long createdOn;
        public long lastDriftOn;
        public long lastWarningOn;
        public ARFHoeffdingTreeDCS classifier;
        ...
        // Bkg learner
        protected ARFBaseLearnerDCS bkgLearner;
        // Statistics
        ...
        private void init(int indexOriginal, ARFHoeffdingTreeDCS instantiatedClassifier, BasicClassificationPerformanceEvaluator evaluatorInstantiated,
        ...
        public ARFBaseLearnerDCS(int indexOriginal, ARFHoeffdingTreeDCS instantiatedClassifier, BasicClassificationPerformanceEvaluator evaluatorInstantiated,
                    long instancesSeen, boolean useBkgLearner, boolean useDriftDetector, ClassOption driftOption, ClassOption warningOption, boolean isBackgroundLearner) {
            ...
            if(this.useDriftDetector && !this.isBackgroundLearner) {
                ...
                if(this.useBkgLearner) {
                    ...
                    if(this.warningDetectionMethod.getChange()) {
                        ...
                        ARFHoeffdingTreeDCS bkgClassifier = (ARFHoeffdingTreeDCS) this.classifier.copy();
                        ...
                        this.bkgLearner = new ARFBaseLearnerDCS(indexOriginal, bkgClassifier, bkgEvaluator, instancesSeen,
        ...
        public HoeffdingTreeDCS.Vote getVotesForInstance(Instance instance) {
            HoeffdingTreeDCS.Vote votesDCS = this.classifier.getVotesForInstanceDCS(instance);
            return votesDCS;
        }
    ...
    protected class TrainingRunnable implements Runnable, Callable<Integer> {
        final private ARFBaseLearnerDCS learner;
        final private Instance instance;
        final private double weight;
        final private long instancesSeen;

        public TrainingRunnable(ARFBaseLearnerDCS learner, Instance instance,
