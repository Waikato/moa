 ...
 *
 * <p>Probabilistic Exact Adaptive Random Forest with Lossy Counting (PEARL), utilises an exact
 * technique and a probabilistic technique to replace drifted trees in an ensemble with relevant
 * trees from a repository of trees for learning from a data stream with recurrent concept drifts.
 *
 * <p>See details in:<br>
 * Ocean Wu, Yun Sing Koh, Gillian Dobbie, Thomas Lacombe Probabilistic exact adaptive random forest
 * for recurrent concepts in data streams. International Journal of Data Science and Analytics,
 * 2022. <a href="https://doi.org/10.1007/s41060-021-00273-1">DOI</a>.
 *
 * <p>Parameters:
 *
 * <ul>
 *   <li>-l : Classiﬁer to train. Must be set to ARFHoeffdingTree
 *   <li>-s : The number of trees in the ensemble
 ...
 *       - m
 *   <li>-a : The lambda value for bagging (lambda=6 corresponds to levBag)
 *   <li>-j : Number of threads to be used for training
 *   <li>-x : Change detector for drifts and its parameters
 *   <li>-p : Change detector for warnings (start training bkg learner)
 *   <li>-w : Should use weighted voting?
 *   <li>-u : Should use drift detection? If disabled then bkg learner is also disabled
 *   <li>-q : Should use bkg learner? If disabled then reset tree immediately
 *   <li>-r : The number of trees in tree pool (default 100000)
 *   <li>-c : The number of candidate trees. (default 120)
 *   <li>-k : The kappa parameter for tree swapping. (default 0.0)
 *   <li>-e : The edit distance parameter for tree swapping (default 100)
 *   <li>-f : The size of LRU state queue (default 10000000)
 *   <li>-z : The window size for tracking candidate trees' performance (default 50)
 *   <li>-g : Enable lossy state graph
 *   <li>-d : Lossy window size (default 100000)
 *   <li>-v : Candidate tree reuse rate (default 1.0 [0.0 - 1.0])
 *   <li>-y : The reuse window size (default 100000)
 * </ul>
 *
 ...
 * @version $Revision: 1 $
 */
public class PEARL extends AbstractClassifier implements MultiClassClassifier, CapabilitiesHandler {
    ...
    public IntOption treeRepoSizeOption =
            new IntOption(
                    "treeRepoSize",
                    'r',
                    "The number of trees in tree pool.",
                    100000,
                    60,
                    Integer.MAX_VALUE);

    public IntOption candidatePoolSizeOption =
            new IntOption(
                    "candidatePoolSize",
                    'c',
                    "The number of candidate trees.",
                    120,
                    60,
                    Integer.MAX_VALUE);

    public FloatOption cdKappaOption =
            new FloatOption(
                    "cdKappa",
                    'k',
                    "The kappa parameter for tree swapping.",
                    0.0,
                    0.0,
                    Float.MAX_VALUE);

    public IntOption editDistanceOption =
            new IntOption(
                    "editDistance",
                    'e',
                    "The edit distance parameter for tree swapping",
                    100,
                    60,
                    120);

    public IntOption lruQueueSize =
            new IntOption(
                    "lruQueueSize",
                    'f',
                    "The size of LRU state queue",
                    10000000,
                    100,
                    Integer.MAX_VALUE);

    public IntOption performanceEvalWindowSize =
            new IntOption(
                    "performanceEvalWindowSize",
                    'z',
                    "The window size for tracking candidate trees' performance",
                    50,
                    1,
                    Integer.MAX_VALUE);

    public FlagOption enableStateGraph =
            new FlagOption("enableStateGraph", 'g', "Is lossy state graph enabled");

    public IntOption lossyWindowSizeSizeOption =
            new IntOption(
                    "lossyWindowSize", 'd', "Lossy window size", 100000, 100, Integer.MAX_VALUE);

    // according to the paper though v does not have a significan effect on predictive capability of
    // the model
    // using the default used in the Agrawal experiments
    // https://github.com/ingako/PEARL/blob/master/run/run-agrawal-3.sh
    public FloatOption candidateTreeReuseRate =
            new FloatOption(
                    "candidateTreeReuseRate", 'v', "Candidate tree reuse rate.", 0.4, 0.0, 1.0);

    public IntOption reuseWindowSizeOption =
            new IntOption(
                    "reuseWindowSize", 'y', "The reuse window size", 100000, 1, Integer.MAX_VALUE);
    ...
    public void resetLearningImpl() {
        ...
        else numberOfJobs = this.numberOfJobsOption.getValue();
            ...
            if (k > 0) {
                if (this.executor != null) {
                    TrainingRunnable trainer =
                            new TrainingRunnable(this.ensemble[i], instance, k, this.instancesSeen);
    ...
    void patternMatchCandidateTrees(ArrayList<Integer> warningTreePosList) {
        ...
        for (int i : closestState) {
            if ((i < this.curState.size())
                    && (i < this.treePool.size())
                    && !this.curState.contains(i)
                    && !this.treePool.get(i).isCandidate) {
    ...
    private void adaptState(Instance instance, ArrayList<Integer> driftedTreePosList) {
        ...
        Collections.sort(
                this.candidateTrees, (tree1, tree2) -> Double.compare(tree1.kappa, tree2.kappa));
        ...
        for (int i = 0; i < driftedTreePosList.size(); i++) {
            ...
            if (swap_tree == null) {
                ...
                if (bkgLearner == null) {
                    ...
                    if (!bkgLearner.isEvalReady
                            || !drifted_tree.isEvalReady
    ...
    protected void initEnsemble(Instance instance) {
        ...
        // (BasicClassificationPerformanceEvaluator) getPreparedClassOption(this.evaluatorOption);
        BasicClassificationPerformanceEvaluator classificationEvaluator =
        ...
        this.stateGraph =
                new LossyStateGraph(
                        this.treeRepoSizeOption.getValue(),
                        this.lossyWindowSizeSizeOption.getValue());
        this.graphSwitch =
                new StateGraphSwitch(
                        this.stateGraph,
    ...
    protected final class ARFBaseLearner extends AbstractMOAObject {
        ...
        public DriftInfo trainOnInstance(Instance instance, double weight, long instancesSeen) {
            ...
            if (this.bkgLearner != null) {
                ...
                if (this.bkgLearner.predictedLabelsWindow.size()
                        >= performanceEvalWindowSize.getValue()) {
        ...
        private double computeKappa(
                int[][] confusionMatrix, double accuracy, int sample_count, int classCount) {
    ...
    protected class LRUState extends AbstractMOAObject {
        ...
        protected LRUState(int capacity, int editDistanceThreshold) {
            ...
            this.map =
                    new LinkedHashMap<String, State>(capacity, 0.75f, true) {
    ...
    class StateGraphSwitch extends AbstractMOAObject {
        ...
        public StateGraphSwitch(LossyStateGraph state_graph, int window_size, double reuse_rate) {
