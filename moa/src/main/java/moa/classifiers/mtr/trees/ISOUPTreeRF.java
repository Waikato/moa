...
public class ISOUPTreeRF extends ISOUPTree {
	...
	public MultiChoiceOption subspaceSizeOption = new MultiChoiceOption(
			"randomForestFunAttr",
			'z',
	...
	public int numRFAttrs() {
		if (subspaceSize == null) {
			switch (subspaceSizeOption.getChosenLabel()) {
				case "sqrt": this.subspaceSize = 1 + (int) Math.ceil(Math.sqrt(this.getModelContext().numInputAttributes()));
				case "log": this.subspaceSize = 1 + (int) Math.ceil(Math.log(this.getModelContext().numInputAttributes()));
				case "percent": this.subspaceSize = (int) Math.ceil(0.1 * this.getModelContext().numInputAttributes());
			}
		}
