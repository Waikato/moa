...
/**
 * <!-- globalinfo-start -->
 * Class for building and using a multinomial Naive Bayes classifier. Performs text classic bayesian
 * prediction while making naive assumption that all inputs are independent. For more information
 * see,<br>
 * <br>
...
public class NaiveBayesMultinomial extends AbstractClassifier implements MultiClassClassifier {
    ...
    public String getPurposeString() {
        return "Multinomial Naive Bayes classifier: performs classic bayesian prediction while"
                + " making naive assumption that all inputs are independent.";
    }
     ...
     * format: m_wordTotalForClass[wordAttribute][class]
     */
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        for (int i = 0; i < inst.numValues(); i++) {
            ...
            if (index != classIndex && !inst.isMissing(i)) {
                ...
                m_wordTotalForClass[classValue].addToValue(
                        index, w * inst.valueSparse(i) + laplaceCorrection);
            }
        }
    }
    ...
    public double[] getVotesForInstance(Instance instance) {
        ...
        for (int i = 0; i < m_numClasses; i++) {
            probOfClassGivenDoc[i] =
                    Math.log(m_probOfClass[i]) - totalSize * Math.log(m_classTotals[i]);
        }
        ...
        for (int i = 0; i < instance.numValues(); i++) {
            ...
            for (int c = 0; c < m_numClasses; c++) {
                ...
                probOfClassGivenDoc[c] +=
                        wordCount
                                * Math.log(
                                        value == 0
                                                ? this.laplaceCorrectionOption.getValue()
                                                : value);
            }
        }
