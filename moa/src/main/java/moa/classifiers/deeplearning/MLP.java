...
class MiniBatch {
    ...
    public void addToMiniBatch(double[] featureValues, double[] classValue) {
        if (itemsInMiniBatch == 0) {
            ...
            if (itemsInMiniBatch == 1) {
                trainMiniBatchData =
                        trainMiniBatchData.stack(
                                trainingNDManager.create(featureValues).toType(FLOAT32, false), 0);
                trainMiniBatchLabels =
                        trainMiniBatchLabels.stack(trainingNDManager.create(classValue), 0);
            } else {
                trainMiniBatchData =
                        trainMiniBatchData.concat(
                                trainingNDManager
                                        .create(featureValues, new Shape(1, featureValues.length))
                                        .toType(FLOAT32, false),
                                0);
                trainMiniBatchLabels =
                        trainMiniBatchLabels.concat(
                                trainingNDManager.create(
                                        classValue, new Shape(1, classValue.length)),
                                0);
            }
        }
        itemsInMiniBatch++;
    ...
    public void addToMiniBatch(Instance inst) {
        ...
        NDArray label =
                dataAndLabel.get(
                        Integer.toString(featureLength)
                                + ":"
                                + Integer.toString(featureLength + 1));
        ...
        if (itemsInMiniBatch == 0) {
            ...
            if (itemsInMiniBatch == 1) {
                ...
                trainMiniBatchData =
                        trainMiniBatchData.concat(data.reshape(new Shape(1, featureLength)), 0);
                trainMiniBatchLabels =
                        trainMiniBatchLabels.concat(label.reshape(new Shape(1, classLength)), 0);
            }
        }
        itemsInMiniBatch++;
        if (itemsInMiniBatch == miniBatchSize) {
...
public class MLP extends AbstractClassifier implements MultiClassClassifier {
    ...
    public FloatOption learningRateOption =
            new FloatOption("learningRate", 'r', "Learning Rate", 0.03, 0.0000001, 1.0);
    ...
    public IntOption numberOfLayers =
            new IntOption("numberOfLayers", 'L', "Number of layers", 1, 1, 4);
    ...
    public double[] getVotesForFeatureValues(Instance inst, double[] featureValues) {
        ...
        try {
            NDManager testingNDManager =
                    NDManager.newBaseManager(nnmodel.getNDManager().getDevice());
    ...
    public static double getNormalizedValue(
            double value, double sumOfValues, double sumOfSquares, long samplesSeen) {
        // Normalize data
    ...
    public static void setFeatureValuesArray(
            Instance inst,
        ...
        for (int i = 0; i < inst.numInputAttributes(); i++) {
            ...
            if (useOneHotEncoding
                ...
                if (inst.attribute(i).isNumeric()
                        && (normalizeInfo != null)
                        && (normalizeInfo[index] != null)) {
                    // Normalize data
                    if (testing) {
                        normalizeInfo[index].sumOfSquares += inst.value(i) * inst.value(i);
                        normalizeInfo[index].sumOfValues += inst.value(i);
                    }
                    featureValuesArrayToSet[index] =
                            getNormalizedValue(
                                    inst.value(i),
                                    normalizeInfo[index].sumOfValues,
                                    normalizeInfo[index].sumOfSquares,
                                    samplesSeen);
                } else {
    ...
    public void initializeNetwork(Instance inst) {
        ...
        modelName =
                "L"
                        + numberOfLayers.getValue()
                        + "_N"
                        + numberOfNeuronsInEachLayerInLog2.getValue()
                        + "_"
                        + optimizerTypeOption.getChosenLabel()
                        + "_"
                        + decimalFormat.format(learningRateOption.getValue());
    }
    ...
    protected void setModel() {
        try {
            ...
            nnmodel =
                    Model.newInstance(
                            "mlp",
                            (deviceTypeOption.getChosenIndex() == deviceTypeOptionGPU)
                                    ? Device.gpu()
                                    : Device.cpu());
            // Construct a neural network and set it in the block
            Integer[] neuronsInEachHiddenLayer = {};
            List<Integer> neuronsInEachHiddenLayerArrayList =
                    new ArrayList<Integer>(Arrays.asList(neuronsInEachHiddenLayer));
            for (int i = 0; i < numberOfLayers.getValue(); i++) {
                neuronsInEachHiddenLayerArrayList.add(
                        (int) Math.pow(2, numberOfNeuronsInEachLayerInLog2.getValue()));
            }
            neuronsInEachHiddenLayer =
                    neuronsInEachHiddenLayerArrayList.toArray(neuronsInEachHiddenLayer);
            Block block =
                    new Mlp(
                            featureValuesArraySize,
                            numberOfClasses,
                            Arrays.stream(neuronsInEachHiddenLayer)
            ...
            System.out.println(
                    "System GPU count: "
                            + gpuCount
                            + " Model using Device: "
                            + nnmodel.getNDManager().getDevice());
        ...
        try {
            ...
            switch (this.optimizerTypeOption.getChosenIndex()) {
                ...
                case MLP.OPTIMIZER_RMSPROP:
                    optimizer =
                            Optimizer.rmsprop().optLearningRateTracker(learningRateTracker).build();
                    break;
                case MLP.OPTIMIZER_ADAGRAD_RESET:
                case MLP.OPTIMIZER_ADAGRAD:
                    optimizer =
                            Optimizer.adagrad().optLearningRateTracker(learningRateTracker).build();
                    break;
                case MLP.OPTIMIZER_ADAM_RESET:
