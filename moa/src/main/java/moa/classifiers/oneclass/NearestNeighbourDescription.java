...
/**
 * Implements David Tax's Nearest Neighbour Description method described in Section 3.4.2 of D. M.
 * J. Tax, “One-Class Classification: Concept-learning in the absence of counter-examples,” Delft
 * University of Technology, 2001 with adaptations for the data stream environment.
 *
 * @author Richard Hugh Moulton
 */
public class NearestNeighbourDescription extends AbstractClassifier
        implements Classifier, OneClassClassifier {
    ...
    public IntOption neighbourhoodSizeOption =
            new IntOption(
                    "neighbourhoodSize",
                    'n',
                    "The number of instances to store in the neighbourhood.",
                    200,
                    1,
                    Integer.MAX_VALUE);

    // public IntOption kOption = new IntOption("k", 'k',
    //		"Up to the kth nearest neighbour of the argument instance will be considered. //NOT
    // IMPLEMENTED//", 1);

    // public IntOption mOption = new IntOption("m", 'm',
    //		"Up to the mth nearest neighbour of the argument's nearest neighbour(s) will be considered.
    // //NOT IMPLEMENTED//", 1);
    ...
    public FloatOption thresholdOption =
            new FloatOption(
                    "tau",
                    't',
                    "The threshold value to determine whether an instance is a member of the target"
                            + " class or an outlier",
                    1.0);
    ...
    /**
     * Constructor for a Nearest Neighbour Description classifier based on an argument training set
     * of instances.
     *
    ...
    /**
     * The classifier adds the argument instance to its neighbourhood. If the neighbourhood is full,
     * then the FixedLengthList removes the oldest instance in it to accommodate the new instance.
     */
    @Override
    public void trainOnInstanceImpl(Instance inst) {
        if (this.getAnomalyScore(inst) < 0.5) this.neighbourhood.add(inst);
    }

    /**
     * Calculates the distance between the argument instance and its nearest neighbour as well as
     * the distance between that nearest neighbour and its own nearest neighbour. The ratio of these
     * distances is compared to the threshold value, tau, and converted into a vote score.
     *
    ...
    /**
     * Returns the anomaly score for an argument instance based on the distance from it to its
     * nearest neighbour compared to the distance from its nearest neighbour to the neighbour's
     * nearest neighbour.
     *
    ...
    public double getAnomalyScore(Instance inst) {
        if (this.neighbourhood.size() < 2) return 1.0;

        Instance nearestNeighbour = getNearestNeighbour(inst, this.neighbourhood, false);
        Instance nnNearestNeighbour =
                getNearestNeighbour(nearestNeighbour, this.neighbourhood, true);

        double indicatorArgument =
                distance(inst, nearestNeighbour) / distance(nearestNeighbour, nnNearestNeighbour);
     ...
     */
    private Instance getNearestNeighbour(
            Instance inst, List<Instance> neighbourhood2, boolean inNbhd) {
        double dist = Double.MAX_VALUE;
        Instance nearestNeighbour = null;

        for (Instance candidateNN : neighbourhood2) {
            // If inst is in neighbourhood2 and an identical instance is found, then it is no longer
            // required to
    ...
    /**
     * Initializes the Nearest Neighbour Distance (NN-d) classifier with the argument training
     * points.
     *
    ...
    public void initialize(Collection<Instance> trainingPoints) {
        ...
        if (trgPtsIterator.hasNext()) this.trainOnInstance(trgPtsIterator.next());
    }
}
