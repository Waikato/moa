...
/**
 * Implements the Streaming Half-Space Trees one-class classifier described in S. C. Tan, K. M.
 * Ting, and T. F. Liu, “Fast anomaly detection for streaming data,” in IJCAI
 * Proceedings-International Joint Conference on Artificial Intelligence, 2011, vol. 22, no. 1, pp.
 * 1511–1516.
 *
 * @author Richard Hugh Moulton
 */
public class HSTrees extends AbstractClassifier implements Classifier, OneClassClassifier {
    ...
    public IntOption windowSizeOption =
            new IntOption("windowSize", 'p', "The size of the landmark windows used.", 250);
    public IntOption numTreesOption =
            new IntOption("numberOfTrees", 't', "The number of trees in the ensemble.", 25);
    public IntOption maxDepthOption =
            new IntOption("maxDepth", 'h', "The maximum depth of the trees", 15);
    public FloatOption anomalyThresholdOption =
            new FloatOption(
                    "anomalyThreshold",
                    'a',
                    "Threshold (as a fraction of the maximum mass value) below which an instance is"
                            + " declared an anomaly.",
                    0.5,
                    Double.MIN_VALUE,
                    1.0);
    public FloatOption sizeLimitOption =
            new FloatOption(
                    "sizeLimit",
                    's',
                    "The minimum mass required in a node (as a "
                            + "fraction of the window size) to calculate the anomaly score.",
                    0.1,
                    Double.MIN_VALUE,
                    1.0);

    // Respectively, these variables store: the window size, the number of trees, the maximum depth
    // of the trees,
    ...
    /**
     * The minimum mass required in a node (as a fraction of the window size) to calculate the
     * anomaly score. 'sizeLimit' in the original paper.
     */
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        if (this.numInstances > 50) referenceWindow = false;
    ...
    public double getAnomalyScore(Instance inst) {
        if (this.referenceWindow) return 0.5;
        else {
     ...
     *
     * @param trainingPoints the Collection of instance with which to initialize the Streaming
     *     Hs-Trees classifier.
     */
    @Override
