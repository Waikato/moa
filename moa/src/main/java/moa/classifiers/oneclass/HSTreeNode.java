...
/**
 * A node in an HSTree. Based on the work S. C. Tan, K. M. Ting, and T. F. Liu, “Fast anomaly
 * detection for streaming data,” in IJCAI Proceedings-International Joint Conference on Artificial
 * Intelligence, 2011, vol. 22, no. 1, pp. 1511–1516.
 *
 * <p>Made use of by HSTrees.java.
 *
 * @author Richard Hugh Moulton
 */
public class HSTreeNode implements Serializable {
    ...
    public HSTreeNode(double[] min, double[] max, int k, int maxDepth, Random classifierRandom) {
        ...
        if ((depth < maxDepth)) {
            ...
            this.splitAttribute =
                    (int) Math.floor((this.classifierRandom.nextDouble() * (this.dimensions)));
     ...
     * @param referenceWindow if the HSTree is in the initial reference window: <b>true</b>, else:
     *     <b>false</b>
     */
    public void updateMass(Instance inst, boolean referenceWindow) {
        if (referenceWindow) r++;
        else l++;

        if (internalNode) {
            if (inst.value(this.splitAttribute) > this.splitValue)
                right.updateMass(inst, referenceWindow);
            else left.updateMass(inst, referenceWindow);
        }
    }

    /**
     * Update the node's model by setting the latest window's mass profile as the reference window's
     * mass profile, resetting the latest window's mass profile to zero and updating any
     * subordinates nodes' models.
     */
    ...
    /**
     * If this node is a leaf node or it has a mass profile of less than sizeLimit, this returns the
     * anomaly score for the argument instance. Otherwise this node determines which of its
     * subordinate nodes the argument instance belongs to and asks it provide the anomaly score.
     *
     * @param inst the instance being passed through the tree
     * @param sizeLimit the minimum mass profile for a node to calculate the argument instance's
     *     anomaly score
    ...
    public double score(Instance inst, int sizeLimit) {
        ...
        if (this.internalNode && this.r > sizeLimit) {
            ...
            else anomalyScore = left.score(inst, sizeLimit);
        } else {
    ...
    protected void printNode() {
        System.out.println(
                this.depth + ", " + this.splitAttribute + ", " + this.splitValue + ", " + this.r);
