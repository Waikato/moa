...
/**
 * Implements an autoencoder: a neural network that attempts to reconstruct the input. The
 * autoencoder's structure is an input layer with one neuron for each non-class attribute, a hidden
 * layer of two neurons and an output layer with one neuron for each non-class attribute.
 *
 * <p>Partially modeled on Albert Bifet's Perceptron.java, also found in the MOA project.
 *
 * <p>Backpropagation implementation based on Matt Mazur's excellent tutorial on this topic:
...
public class Autoencoder extends AbstractClassifier implements Classifier, OneClassClassifier {
    ...
    public IntOption hiddenLayerOption =
            new IntOption(
                    "hiddenLayer",
                    'h',
                    "The number of neurons in the hidden layer. Should be less than the"
                        + " dimensionality of the data stream to ensure that the identity function"
                        + " is not learned.",
                    2,
                    1,
                    100);
    ...
    public FloatOption learningRateOption =
            new FloatOption(
                    "learningRate",
                    'l',
                    "The rate to adapt the autoencoder's weights after each instance.",
                    0.5);
    ...
    public FloatOption thresholdOption =
            new FloatOption(
                    "threshold",
                    't',
                    "Determines the threshold for recognizing outliers. Higher values means fewer"
                            + " outliers.",
                    0.6,
                    0.001,
                    0.999);
    ...
    private void backpropagation(Instance inst) {
        ...
        RealMatrix adjustmentTwo =
                (delta.multiply(hidden.transpose())).scalarMultiply(-1.0 * this.learningRate);
        ...
        for (int i = 0; i < this.hiddenLayerSize; i++) {
            delta2.setEntry(
                    0, i, delta2.getEntry(0, i) * hidden2.getEntry(i, 0) * hidden.getEntry(i, 0));
            adjustBiasOne -= this.learningRate * delta2.getEntry(0, i) * this.biasOne;
        }

        RealMatrix adjustmentOne =
                delta2.transpose()
                        .multiply(input.transpose())
                        .scalarMultiply(-1.0 * this.learningRate);
     ...
     * @return the outputs of the hidden layer and the output vector that attempts to reconstruct
     *     the input
     *     <p>private RealMatrix prediction(RealMatrix input) { RealMatrix output = new
     *     double[this.numAttributes+2];
     *     <p>// Initialize arrays and compute total net input for the hidden layer for(int i = 0 ;
     *     i < this.numAttributes ; i++) { input[i] = inst.value(i); output[i+2] = this.biasTwo;
     *     output[0] += input[i]*this.weightsOne[i][0]; output[1] += input[i]*this.weightsOne[i][1];
     *     }
     *     <p>// Logistic function used for hidden layer activation output[0] = 1.0 / (1.0 +
     *     Math.pow(Math.E,-1.0 * (output[0] + this.biasOne))); output[1] = 1.0 / (1.0 +
     *     Math.pow(Math.E,-1.0 * (output[1] + this.biasOne)));
     *     <p>// Compute total net input for the output layer // Use the logistic function for
     *     output layer activation. for(int i = 2 ; i < this.numAttributes ; i++) { output[i] +=
     *     (output[0]*this.weightsTwo[0][i])+(output[1]*this.weightsTwo[1][i]); output[i] = 1.0 /
     *     (1.0 + Math.pow(Math.E,-1.0 * output[i])); }
     *     <p>return output; }
     */

    /**
     * Calculates the error between the autoencoder's reconstruction of the input and the argument
     * instances. This error is converted to vote scores.
     *
    ...
    /**
     * Returns the squared error between the input value and the reconstructed value as the anomaly
     * score for the argument instance.
     *
     ...
     *
     * @param trainingPoints the Collection of instances on which to initialize the Autoencoder
     *     classifier.
     */
    @Override
