...
/**
 * SeqDrift1ChangeDetector.java. This extends Abstract Change Detector as required by MOA.
 *
 * <p>Sakthithasan, S., Pears, R., {@literal &} Koh, Y. (2013). One Pass Concept Change Detection
 * for Data Streams. In J. Pei, V. Tseng, L. Cao, H. Motoda, {@literal &} G. Xu (Eds.), Advances in
...
public class SeqDrift1ChangeDetector extends AbstractChangeDetector {
    ...
    public FloatOption deltaOption =
            new FloatOption(
                    "deltaSeqDrift1", 'd', "Delta of SeqDrift1 change detection", 0.01, 0.0, 1.0);

    public FloatOption deltaWarningOption =
            new FloatOption(
                    "deltaWarningOption",
                    'w',
                    "Delta of SeqDrift1 change detector to declare warning state",
                    0.1,
                    0.0,
                    1.0);
    ...
    public IntOption blockSeqDriftOption =
            new IntOption(
                    "blockSeqDrift1Option",
                    'b',
                    "Block size of SeqDrift1 change detector",
                    200,
                    100,
                    10000);
    ...
    public void resetLearning() {
        seqDrift1 =
                new SeqDrift1(
                        (double) this.deltaOption.getValue(),
                        ((int) this.blockSeqDriftOption.getValue()),
                        ((double) this.deltaWarningOption.getValue()));
    }
    ...
    /**
     * SeqDrift1 uses sliding window to build a sequential change detection model that uses
     * statistically sound guarantees defined using Bernstein Bound on false positive and false
     * negative rates. This is a block based approach and checks for changes in the data values only
     * at block boundaries as opposed to the methods on per instance basis. SeqDrift1 maintains a
     * sliding window and repository. Repository gathers the new instances and sliding window stores
     * only the data values that are statistically not different, in other words from the same
     * distribution. If the data values in the repository are consistent with the values in sliding
     * window the data values of the repository are copied to the sliding window applying reservoir
     * algorithm. The hypothesis is that the mean values of the sliding window and right repository
     * are not statistically different. In addition, SeqDrift1 declares a warning state depending on
     * warning significance level and increases sample size to get a statistically more rigorous
     * mean value
     *
     * <p>Sakthithasan, S., Pears, R., {@literal &} Koh, Y. (2013). One Pass Concept Change
     * Detection for Data Streams. In J. Pei, V. Tseng, L. Cao, H. Motoda, {@literal &} G. Xu
     * (Eds.), Advances in Knowledge Discovery and Data Mining (Vol. 7819, pp. 461-472): Springer
     * Berlin Heidelberg.
     *
     * @author Sakthithasan Sripirakas sripirakas363 at yahoo dot com
     */
    public class SeqDrift1 extends AbstractMOAObject {
        ...
        public SeqDrift1(
                double _significanceLevel, int _blockSize, double _significanceWarningLevel) {
            // Parameters
            significanceLevel = _significanceLevel;
            blockSize = _blockSize;
            sampleSize = _blockSize;
            slidingWindowBlockCount =
                    (int) (1 / _significanceLevel); // Sliding window size is automaticall
            // set as 1/delta
        ...
        public boolean setInput(double _inputValue) {
            ...
            if ((instanceCount % sampleSize) == 0) // Drift point check
            {
                ...
                if (iDriftType == DRIFT) {
                    ...
                    return false; // In warning state no instance is moved from sliding window to
                    // repository. Thus returning now
                } /*
                  ...
                  */ else {
                    isWarning = false;
                    moveValuesFromRightToLeft(); // All instances in sliding window should be moved
                    // to repository if no drift or if drift but not in
                    // warning
                    return false;
                }
            }
        ...
        private void addToRightRepository(double _inputValue) {
            if ((rightRepository.getSize() < sampleSize)
                    || isWarning) // By default sliding window should have maximum of blockSize
            // instances except in warning state
            {
                rightRepository.add(new Double(_inputValue));
            } else {
                System.out.println(
                        "request to add to sliding window sliding window size :"
                                + rightRepository.getSize()
                                + " Warning :"
                                + isWarning);
            }
        }
        ...
        /**
         * Removes excess instances in sliding window when the number of blocks is more than the
         * sliding window size threshold
         *
         * <p>
         *
         * @return void
        ...
        private void moveValuesFromRightToLeft() {
            for (int iIndex = 0;
                    iIndex < rightRepository.getSize();
                    iIndex++) // Copy all instances from sliding window to repository
            {
            ...
            blockCount =
                    blockCount
                            + rightRepository.getSize()
                                    / blockSize; // Determine the block counter value after adding
            // the instances

            if (slidingWindowBlockCount > 0) // Sliding is enabled
            {
                while (blockCount
                        > slidingWindowBlockCount) // Remove old instances from  repository
                {
            ...
            if (!isWarning) // If in warning do not remove the instances from  sliding window
            {
                ...
                System.out.println(
                        "ERROR: requested to move instances from  sliding window to  repository");
                System.exit(2);
            }
        }
         ...
         *
         * <p>After deletion the block counter is set to zero
         *
         * @return void
         */
        ...
        private int getDriftType() {
            if (getWidth() > blockSize) {
                leftRepositoryMean =
                        getLeftRepositorySampleMean(); // Get the subsample mean from  repository
                rightRepositoryMean =
                        getRightRepositorySampleMean(); // Get the subsample mean from  sliding
                // window
                epsilon = getEpsilon();
