...
/**
 * SeqDriftChangeDetector.java. This extends Abstract Change Detector as required by MOA.
 *
 * <p>Pears, R., Sakthithasan, S., {@literal &} Koh, Y. (2014). Detecting concept change in dynamic
 * data streams. Machine Learning, 97(3), 259-293. doi:10.1007/s10994-013-5433-9
 *
 * @author Sakthithasan Sripirakas sripirakas363 at yahoo dot com
 * @version $Revision: 7 $
 */
public class SeqDrift2ChangeDetector extends AbstractChangeDetector {
    ...
    public FloatOption deltaSeqDrift2Option =
            new FloatOption(
                    "deltaSeq2Drift", 'd', "Delta of SeqDrift2 change detection", 0.01, 0.0, 1.0);

    public IntOption blockSeqDrift2Option =
            new IntOption(
                    "blockSeqDrift2Option",
                    'b',
                    "Block size of SeqDrift2 change detector",
                    200,
                    100,
                    10000);
    ...
    public void resetLearning() {
        seqdrift =
                new SeqDrift2(
                        (double) this.deltaSeqDrift2Option.getValue(),
                        ((int) this.blockSeqDrift2Option.getValue()));
    }
    ...
    /**
     * SeqDrift2 uses reservoir sampling to build a sequential change detection model that uses
     ...
     * at block boundaries as opposed to the methods on per instance basis. SeqDrift maintains a
     * reservoir and a repository. Repository gathers the new instances and reservoir stores only
     * the data values that are statistically not different, in other words from the same
     * distribution. If the data values in the repository are consistent with the values in
     * reservoir, the data values of the repository are copied to the reservoir applying reservoir
     * algorithm.The hypothesis is that the mean values of the reservoir and right repository are
     * not statistically different
     *
     * <p>Pears, R., Sakthithasan, S., {@literal &} Koh, Y. (2014). Detecting concept change in
    ...
    public class SeqDrift2 extends AbstractMOAObject {
         ...
         * Significance level controls the false positive rate and block size sets the interval of
         * two consecutive hypothesis tests Block Size is a positive integer and significance level
        ...
        /**
         * This method can be used to directly interface with SeqDrift change detector. This method
         * requires a numerical value as an input. The return value indicates whether there is a
        ...
        /**
         * This method copies the data values of the repository to the reservoir applying reservoir
         * algorithm
         *
         * @return void
         */
        ...
        /**
         * This method removes all elements from the reservoir after a drift is detected.
         *
         * @return void
         */
        ...
        /**
         * This method returns the type of drift detected The possible values are: DRIFT,
         * INTERNAL_DRIFT and NODRIFT
         *
         * @param
        ...
        private void optimizeEpsilon() {
            ...
            if (tests >= 1) {
                ...
                while (IsNotOptimized) {
                    ...
                    previousStepEpsilon =
                            (1.0 / (3 * rightRepositorySize * (1 - ktemp))) * (x + squareRootValue);
                    ktemp = 3 * ktemp / 4;
                    currentStepEpsilon =
                            (1.0 / (3 * rightRepositorySize * (1 - ktemp))) * (x + squareRootValue);

                    if (((previousStepEpsilon - currentStepEpsilon) / previousStepEpsilon)
                            < 0.0001) {
    ...
    public class Repository {
        ...
        public int getNumOfTests() {
            int iNumTests = 0;
            for (int iIndex = 0; iIndex < blocks.size(); iIndex++) {
                if (blocks.get(iIndex).IsTested()) iNumTests++;
            }
            return iNumTests;
        }
    }
