...
/**
 * Online drift detection method based on McDiarmid's bounds. HDDM<sub><i>W</i>-test</sub> uses the
 * EWMA statistic as estimator. It receives as input a stream of real values and returns the
 * estimated status of the stream: STABLE, WARNING or DRIFT.
 *
 ...
 *   <li>-m : Controls how much weight is given to more recent data compared to older data. Smaller
 *       values mean less weight given to recent data
...
public class HDDM_W_Test extends AbstractChangeDetector {
    ...
    public void input(double value) {
        ...
        if (total.EWMA_Estimator < 0) {
            ...
            total.independentBoundedConditionSum =
                    lambda * lambda
    ...
    public boolean detectMeanIncrement(SampleInfo sample1, SampleInfo sample2, double confidence) {
        ...
        double bound =
                Math.sqrt(
                        (sample1.independentBoundedConditionSum
                                        + sample2.independentBoundedConditionSum)
    ...
    void updateIncrStatistics(double valor, double confidence) {
        ...
        double bound =
                Math.sqrt(
                        total.independentBoundedConditionSum * Math.log(1.0 / driftConfidence) / 2);
        ...
        if (total.EWMA_Estimator + bound < incrCutPoint) {
            ...
            sample1_IncrMonitoring.independentBoundedConditionSum =
                    total.independentBoundedConditionSum;
            ...
            if (sample2_IncrMonitoring.EWMA_Estimator < 0) {
                ...
                sample2_IncrMonitoring.EWMA_Estimator =
                        lambda * valor + auxDecay * sample2_IncrMonitoring.EWMA_Estimator;
                sample2_IncrMonitoring.independentBoundedConditionSum =
                        lambda * lambda
                                + auxDecay
                                        * auxDecay
                                        * sample2_IncrMonitoring.independentBoundedConditionSum;
    ...
    void updateDecrStatistics(double valor, double confidence) {
        ...
        if (total.EWMA_Estimator - epsilon > decrCutPoint) {
            ...
            sample1_DecrMonitoring.independentBoundedConditionSum =
                    total.independentBoundedConditionSum;
            ...
            if (sample2_DecrMonitoring.EWMA_Estimator < 0) {
                ...
                sample2_DecrMonitoring.EWMA_Estimator =
                        lambda * valor + auxDecay * sample2_DecrMonitoring.EWMA_Estimator;
                sample2_DecrMonitoring.independentBoundedConditionSum =
                        lambda * lambda
                                + auxDecay
                                        * auxDecay
                                        * sample2_DecrMonitoring.independentBoundedConditionSum;
