...
/**
 * ADaptive sliding WINdow method. This method is a change detector and estimator. It keeps a
 * variable-length window of recently seen items, with the property that the window has the maximal
 * length statistically consistent with the hypothesis "there has been no change in the average
...
public class ADWIN extends AbstractMOAObject {
    ...
    public int deleteElement() {
        ...
        double incVariance =
                Node.Variance(0)
                        + n1 * WIDTH * (u1 - TOTAL / WIDTH) * (u1 - TOTAL / WIDTH) / (n1 + WIDTH);
        VARIANCE -= incVariance;
            ...
            while (blnReduceWidth) // Diference
            {
                ...
                do {
                    for (int k = 0; k <= (cursor.bucketSizeRow - 1); k++) {
                        ...
                        double absvalue =
                                (double) (u0 / n0) - (u1 / n1); // n1<WIDTH-mintMinWinLength-1
    ...
    private boolean blnCutexpression(
        ...
        double m =
                ((double) 1 / ((n0 - mintMinWinLength + 1)))
