...
/**
 * Drift detection method as published in: David Tse Jung Huang, Yun Sing Koh, Gillian Dobbie, and
 * Russel Pears: Detecting Volatility Shift in Data Streams. ICDM 2014: 863-868 Usage: {@link
 * #input(double)}
 *
 * @author David T.J. Huang - The University of Auckland
 * @version 1.0
...
public class SEEDChangeDetector extends AbstractChangeDetector {
    ...
    public FloatOption deltaSEEDOption =
            new FloatOption("deltaSEED", 'd', "Delta value of SEED Detector", 0.05, 0.0, 1.0);
    public IntOption blockSizeSEEDOption =
            new IntOption("blockSizeSEED", 'b', "BlockSize value of SEED Detector", 32, 32, 256);
    public FloatOption epsilonPrimeSEEDOption =
            new FloatOption(
                    "epsilonPrimeSEED",
                    'e',
                    "EpsilonPrime value of SEED Detector",
                    0.01,
                    0.0025,
                    0.01);
    public FloatOption alphaSEEDOption =
            new FloatOption("alphaSEED", 'a', "Alpha value of SEED Detector", 0.8, 0.2, 0.8);
    public IntOption compressTermSEEDOption =
            new IntOption(
                    "compressTermSEED", 'c', "CompressTerm value of SEED Detector", 75, 50, 100);
    ...
    public void resetLearning() {
        seed =
                new SEED(
                        (double) this.deltaSEEDOption.getValue(),
    ...
    public class SEEDWindow {
        ...
        public void compressBlock(SEEDBlock cursor) {
            ...
            cursor.getPrevious()
                    .setItemCount(cursor.getItemCount() + cursor.getPrevious().getItemCount());
            cursor.getPrevious()
                    .setVariance(cursor.getVariance() + cursor.getPrevious().getVariance());
            cursor.getPrevious()
                    .setBlockSize(cursor.getBlockSize() + cursor.getPrevious().getBlockSize());
        ...
        public boolean checkHomogeneity(SEEDBlock block) {
            ...
            double epsilonPrime =
                    getADWINBound(block.getItemCount(), block.getPrevious().getItemCount());
