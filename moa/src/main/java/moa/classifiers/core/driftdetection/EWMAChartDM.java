...
public class EWMAChartDM extends AbstractChangeDetector {
    ...
    public FloatOption lambdaOption =
            new FloatOption(
                    "lambda",
                    'l',
                    "Lambda parameter of the EWMA Chart Method",
                    0.2,
                    0.0,
                    Float.MAX_VALUE);
    ...
    public void input(double prediction) {
        ...
        m_s =
                Math.sqrt(
                        m_p
                                * (1.0 - m_p)
                                * lambda
                                * (1.0 - Math.pow(1.0 - lambda, 2.0 * m_n))
                                / (2.0 - lambda));
        ...
        // double L_t = 2.76 - 6.23 * m_p + 18.12 * Math.pow(m_p, 3) - 312.45 * Math.pow(m_p, 5) +
        // 1002.18 * Math.pow(m_p, 7); //%1 FP
        double L_t =
                3.97
                        - 6.56 * m_p
                        + 48.73 * Math.pow(m_p, 3)
                        - 330.13 * Math.pow(m_p, 5)
                        + 848.18 * Math.pow(m_p, 7); // %1 FP
        // double L_t = 1.17 + 7.56 * m_p - 21.24 * Math.pow(m_p, 3) + 112.12 * Math.pow(m_p, 5) -
        // 987.23 * Math.pow(m_p, 7); //%1 FP
