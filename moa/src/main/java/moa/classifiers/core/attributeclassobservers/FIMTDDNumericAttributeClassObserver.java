...
public class FIMTDDNumericAttributeClassObserver extends BinaryTreeNumericAttributeClassObserver
    ...
    protected AttributeSplitSuggestion searchForBestSplitOption(
            ...
            int attIndex) {
        // Return null if the current node is null or we have finished looking through all the
        // possible splits
        ...
        if (currentNode.left != null) {
            currentBestOption =
                    searchForBestSplitOption(
                            currentNode.left, currentBestOption, criterion, attIndex);
        }
        ...
        double[][] postSplitDists =
                new double[][] {
                    {countLeftTotal, sumTotalLeft, sumSqTotalLeft},
                    {countRightTotal, sumTotalRight, sumSqTotalRight}
                };
        double[] preSplitDist =
                new double[] {
                    (countLeftTotal + countRightTotal),
        ...
        if (currentNode.right != null) {
            currentBestOption =
                    searchForBestSplitOption(
                            currentNode.right, currentBestOption, criterion, attIndex);
        }
        sumTotalLeft -= currentNode.leftStatistics.getValue(1);
        sumTotalRight += currentNode.leftStatistics.getValue(1);
        sumSqTotalLeft -= currentNode.leftStatistics.getValue(2);
    ...
    /**
     * A method to remove all nodes in the E-BST in which it and all it's children represent 'bad'
     * split points
     */
    public void removeBadSplits(
            SplitCriterion criterion,
    ...
    /**
     * Recursive method that first checks all of a node's children before deciding if it is 'bad'
     * and may be removed
     */
