...
/**
 * Self-training classifier: it is trained with a limited number of labeled data at first, then it
 * predicts the labels of unlabeled data, the most confident predictions are used for training in
 * the next iteration.
 */
public class SelfTrainingClassifier extends AbstractClassifier implements SemiSupervisedLearner {
    ...
    public MultiChoiceOption confidenceOption =
            new MultiChoiceOption(
                    "confidenceComputation",
                    's',
                    "Choose the method to estimate the prediction uncertainty",
                    new String[] {"DistanceMeasure", "FromLearner"},
                    new String[] {
                        "Confidence score from pair-wise distance with the ground truth",
                        "Confidence score estimated by the learner itself"
                    },
                    1);
    ...
    public String getPurposeString() {
        return "A self-training classifier";
    }
    ...
    private void trainOnUnlabeledBatch() {
        ...
        for (AbstractMap.SimpleEntry<Instance, Double> x : mostConfident) {
            ...
            if (x.getKey().classValue() == x.getValue()) ++this.instancesCorrectPseudoLabeled;
    ...
    /**
     * Dynamically updates the confidence threshold at the end of each labeledInstancesBuffer
     * horizon
     */
    ...
    /**
     * Dynamically updates the confidence threshold: adapt the threshold if the last confidence
     * score falls out of z-index = 1 zone
     */
    ...
    private List<AbstractMap.SimpleEntry<Instance, Double>> predictOnBatch(List<Instance> batch) {
        ...
        for (Instance instance : batch) {
            Instance copy =
                    instance.copy(); // use copy because we do not want to modify the original data
            double classValue = -1.0;
            if (!instance
                    .classIsMissing()) // if it is not missing, assume this is a debug execution and
                // store it for checking pseudo-labelling accuracy.
                classValue = instance.classValue();

            copy.setClassValue(Utils.maxIndex(learner.getVotesForInstance(copy)));
            batchWithPredictions.add(
                    new AbstractMap.SimpleEntry<Instance, Double>(copy, classValue));
        }
     ...
     */
    private List<AbstractMap.SimpleEntry<Instance, Double>> getMostConfidentFromLearner(
            List<AbstractMap.SimpleEntry<Instance, Double>> batch) {
     ...
     * @return mostConfident the result containing the most confident prediction from the given
     *     batch
     */
    //    private List<AbstractMap.SimpleEntry<Instance, Double>>
    // getMostConfidentDistanceBased(List<AbstractMap.SimpleEntry<Instance, Double>> batch) {
    //        /*
    ...
    //                    conf += Clusterer.distance(XL.toDoubleArray(), X.getKey().toDoubleArray())
    // / this.L.size();
    //                }
    //            }
    //            conf = (1.0 / conf > 1.0 ? 1.0 : 1 / conf); // reverse so the distance becomes the
    // confidence
    //            confidences[i++] = conf;
    ...
    //        }
    //
    //        for (double confidence : confidences) lastConfidenceScore += confidence /
    // confidences.length;
    //
