...
/**
 * A simple semi-supervised classifier that serves as a baseline. The idea is to group the incoming
 * data into micro-clusters, each of which is assigned a label. The micro-clusters will then be used
 * for classification of unlabeled data.
 */
public class ClusterAndLabelClassifier extends AbstractClassifier implements SemiSupervisedLearner {

    private static final long serialVersionUID = 1L;

    public ClassOption clustererOption =
            new ClassOption(
                    "clustream", 'c', "Used to configure clustream", Clustream.class, "Clustream");

    /** Lets user decide if they want to use pseudo-labels */
    public FlagOption usePseudoLabelOption =
            new FlagOption("pseudoLabel", 'p', "Using pseudo-label while training");

    public FlagOption debugModeOption =
            new FlagOption("debugMode", 'e', "Print information about the clusters on stdout");

    /** Decides the labels based on k-nearest cluster, k defaults to 1 */
    public IntOption kNearestClusterOption =
            new IntOption(
                    "kNearestCluster",
                    'k',
    ...
    public int trainOnUnlabeledInstance(Instance instance) {
        ...
        // In case the label is available for debugging purposes (i.e. checking the pseudoLabel
        // accuracy),
        ...
        if (this.usePseudoLabel) {
            ClustreamKernel closestCluster =
                    getNearestClustreamKernel(this.clustream, unlabeledInstance, false);
            pseudoLabel =
                    (closestCluster != null ? Utils.maxIndex(closestCluster.classObserver) : -1);
    ...
    public double[] getVotesForInstance(Instance instance) {
        Objects.requireNonNull(this.clustream, "Cluster must not be null!");
        // Creates a copy of the instance to be used in here (avoid changing the instance passed to
        // this method)
        Instance unlabeledInstance = instance.copy();
        ...
        if (clustering != null) {
            if (k == 1) {
                ClustreamKernel closestKernel =
                        getNearestClustreamKernel(clustream, unlabeledInstance, false);
                if (closestKernel != null) votes = closestKernel.classObserver;
            } else {
    ...
    private ClustreamKernel[] findKNearestClusters(Instance instance, int k) {
        ...
        if (clustering == null || clustering.size() == 0) return new ClustreamKernel[0];
        ...
        //////////////////////////////////
        if (this.debugModeOption.isSet()) debugVotingScheme(clustering, instance, topK, true);
        //////////////////////////////////
    ...
    class DistanceKernelComparator implements Comparator<ClustreamKernel> {
        ...
        public int compare(ClustreamKernel C1, ClustreamKernel C2) {
            double distanceC1 =
                    Clustream.distanceIgnoreNaN(C1.getCenter(), instance.toDoubleArray());
            double distanceC2 =
                    Clustream.distanceIgnoreNaN(C2.getCenter(), instance.toDoubleArray());
            return Double.compare(distanceC1, distanceC2);
        }
    }

    private ClustreamKernel getNearestClustreamKernel(
            Clustream clustream, Instance instance, boolean includeClass) {
        double minDistance = Double.MAX_VALUE;
        ClustreamKernel closestCluster = null;

        List<Integer> excluded = new ArrayList<>();
        if (!includeClass) excluded.add(instance.classIndex());
        ...
        for (int i = 0; i < kernels.size(); ++i) {
            ...
            if (this.debugModeOption.isSet())
                debugClustreamMicroCluster(
                        (ClustreamKernel) kernels.get(i), clusterCenter, distance, true);
            //////////////////////////////
        ...
        ///////////////////////////
        if (this.debugModeOption.isSet()) debugShowInstance(instance);
        ///////////////////////////
    ...
    protected Measurement[] getModelMeasurementsImpl() {
        ...
        return new Measurement[] {
            ...
            new Measurement(
                    ...
                    this.instancesCorrectPseudoLabeled / (double) this.instancesPseudoLabeled * 100)
        };
    }
    ...
    private void debugClustreamMicroCluster(
            ClustreamKernel cluster,
            double[] clusterCenter,
            double distance,
            boolean showMicroClusterValues) {
    ...
    private void debugVotingScheme(
            Clustering clustering,
            Instance instance,
            ClustreamKernel[] topK,
            boolean showAllClusters) {
        ...
        if (showAllClusters) {
            ...
            for (int x = 0; x < kernels.size(); ++x) {
                ...
                double distance = Clustream.distanceIgnoreNaN(arrayInstance, clusterCenter);
                debugClustreamMicroCluster(
                        (ClustreamKernel) kernels.get(x), clusterCenter, distance, true);
            }
        }
    }
}
