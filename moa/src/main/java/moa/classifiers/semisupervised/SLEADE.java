...
public class SLEADE extends AbstractClassifier
        implements SemiSupervisedLearner, MultiClassClassifier {

    public ClassOption baseEnsembleOption =
            new ClassOption(
                    "baseEnsemble",
                    'l',
                    "Ensemble to train on instances.",
                    StreamingRandomPatches.class,
                    "StreamingRandomPatches");

    public MultiChoiceOption confidenceStrategyOption =
            new MultiChoiceOption(
                    "confidenceStrategy",
                    'b',
                    "Defines the strategy to calculate the confidence. SumVotes = uses votes"
                            + " directly, ArgMax = sets 1 to the argmax of each vote array",
                    new String[] {"Sum", "ArgMax"},
                    new String[] {"Sum", "ArgMax"},
                    1);

    public FlagOption enableRandomThresholdOption =
            new FlagOption(
                    "enableRandomThreshold",
                    'q',
                    "If set, the minConfidenceThreshold is ignored and a random value is used.");

    public MultiChoiceOption autoWeightShrinkageOption =
            new MultiChoiceOption(
                    "autoWeightShrinkage",
                    'e',
                    "Different strategies for automatically setting the weight shrinkage (ws)"
                            + " value",
                    new String[] {"Constant", "LabeledDivTotal", "LabeledNoWarmupDivTotal"},
                    new String[] {"Constant", "LabeledDivTotal", "LabeledNoWarmupDivTotal"},
                    2);

    public MultiChoiceOption SSLStrategyOption =
            new MultiChoiceOption(
                    "SSLStrategy",
                    'p',
    ...
    public FloatOption SSL_minConfidenceOption =
            new FloatOption(
                    "SSL_minConfidence",
                    'm',
                    "Minimum confidence to use unlabeled instance to train.",
                    0.00,
                    0.0,
                    1.0);

    public MultiChoiceOption weightFunctionOption =
            new MultiChoiceOption(
                    "weightFunction",
                    'w',
    ...
    public MultiChoiceOption pairingFunctionOption =
            new MultiChoiceOption(
                    "pairingFunction",
                    't',
    ...
    public FloatOption SSL_weightShrinkageOption =
            new FloatOption(
                    "SSL_weightShrinkageOption",
                    'n',
                    "The pseudo-labeled instances will be weighted according to instance weight *"
                            + " 1/WS.",
                    100.0,
                    0.0,
                    Integer.MAX_VALUE);

    public FlagOption useUnsupervisedDriftDetectionOption =
            new FlagOption(
                    "useUnsupervisedDriftDetection",
                    's',
    ...
    public ClassOption studentLearnerForUnsupervisedDriftDetectionOption =
            new ClassOption(
                    "studentLearnerForUnsupervisedDriftDetection",
                    'g',
                    "Student to mimic the ensemble. It is used for unsupervised drift detection.",
                    Classifier.class,
    ...
    public IntOption unsupervisedDetectionWeightWindowOption =
            new IntOption(
                    "unsupervisedDetectionWeightWindow",
                    'z',
                    "The labeledInstancesBuffer length of the sigmoid functions for the"
                            + " unsupervised drift detection pseudo-labeling weighting.",
                    20,
                    0,
                    Integer.MAX_VALUE);

    public IntOption labeledWindowLimitOption =
            new IntOption(
                    "labeledWindowLimit",
                    'j',
                    "The maximum number of labeled instances to store in the sliding window used"
                            + " for quick start training learners",
                    100,
                    0,
                    Integer.MAX_VALUE);

    public FlagOption debugEnsembleDiversityOption =
            new FlagOption(
                    "debugEnsembleDiversity",
                    'v',
                    "Calculates the kappa statistic among ensemble members' predictions");

    public FileOption debugOutputFileOption =
            new FileOption(
                    "debugOutputFile", 'o', "File to append debug information.", null, "csv", true);

    public FileOption debugOutputDriftFileOption =
            new FileOption(
                    "debugOutputDriftFile",
                    'c',
                    "File to append drift debug information.",
                    null,
                    "csv",
                    true);

    public FileOption debugOutputConfidencePredictionsFileOption =
            new FileOption(
                    "debugOutputConfidencePredictionsFile",
                    'f',
                    "File to append confidences and predictions information.",
                    null,
                    "csv",
                    true);

    public IntOption debugFrequencyOption =
            new IntOption(
                    "debugFrequency",
                    'i',
                    "The labeledInstancesBuffer length of the sigmoid functions for the"
    ...
    public FlagOption debugPerfectPseudoLabelingOption =
            new FlagOption(
                    "debugPerfectPseudoLabeling",
                    'd',
                    "CAUTION: THIS OVERRIDES THE PSEUDO-LABELING STRATEGY AND ALWAYS CHOOSE THE"
                            + " CORRECT LABEL AS THE PSEUDO-LABEL, FOR DEBUG ONLY!");

    public StringOption gtDriftLocationOption =
            new StringOption(
                    "gtDriftLocation",
                    'a',
                    "If set, it is used to calculate drift detection accuracy. format:"
                            + " 'dd_location1,dd_location2,...,dd_locationX",
                    "");
    ...
    /**
     * This is only called during supervised training, thus it is a fair estimation of the
    ...
    public static final int AUTO_WEIGHT_SHRINKAGE_CONSTANT =
            0; // use the default constant value from SSL_weightShrinkageOption
    ...
    protected long lastDriftDetectedAt; // last drift detected using the instance counter that only
    // considers labeled data
    protected long
            lastDriftDetectedAtUnlabelled; // similar to the previous one, but using all instances
    // (including unlabeled).
    ...
    /**
     * IMPORTANT: These evaluators are used for debugging purposes only, they assume immediate
     * access to labels *
     */
    ...
    public void setModelContext(InstancesHeader context) {
        try {
            this.labeledInstancesBuffer =
                    new Instances(context, 0); // new StringReader(context.toString())
    ...
    protected void reactToDrift(Instance instance) {
        ...
        /**
         * Use the estimated accuracy (based only on the labeled instances) for each member of the
         * ensemble *
         */
        for (int i = 0; i < this.evaluatorTestThenTrainEnsembleMembers.length; ++i) {
            double acc =
                    this.evaluatorTestThenTrainEnsembleMembers[i].getPerformanceMeasurements()[1]
                            .getValue();
            if (this.evaluatorTestThenTrainEnsembleMembers[minIndex].getPerformanceMeasurements()[1]
                            .getValue()
                    > acc) minIndex = i;
            if (this.evaluatorTestThenTrainEnsembleMembers[maxIndex].getPerformanceMeasurements()[1]
                            .getValue()
                    <= acc) maxIndex = i;
            sum += acc;
        }
        ...
        StreamingRandomPatches.StreamingRandomPatchesClassifier[] ensemble =
                this.baseEnsemble.getEnsembleMembers();
        ArrayList<Integer> resetIndexes = new ArrayList<>();

        for (int i = 0; i < ensemble.length; ++i) {
            if (this.evaluatorTestThenTrainEnsembleMembers[i].getPerformanceMeasurements()[1]
                            .getValue()
                    < mean) {
        ...
        for (int i = 0; i < resetIndexes.size(); ++i) {
            for (int j = 0; j < this.labeledInstancesBuffer.numInstances(); ++j) {
                ensemble[resetIndexes.get(i)].trainOnInstance(
                        this.labeledInstancesBuffer.get(j),
                        1,
                        this.instancesSeen,
    ...
    private boolean trainAndUpdateStudentDetector(Instance instance) {
        ...
        boolean correctlyClassifies =
                this.detectionStudent.correctlyClassifies(instanceWithEnsemblePrediction);
        ...
        if (this.driftDetectionMethod.getChange()) {
            ...
            //                this.outputAccuracyStatisticsDebugStream.println(this.allInstancesSeen
            // + ", " + this.instancesSeen + ", "+
            // this.evaluatorWindowEnsembleDebug.getPerformanceMeasurements()[1].getValue() +",
            // SLEAD");

            //            if(this.outputDriftDetectionDebugStream != null)
            //                this.outputDriftDetectionDebugStream.println(this.allInstancesSeen +
            // ", " + this.instancesSeen + "," + this.numberOfUnsupervisedDriftsDetected);

            // should change order?
            this.detectionStudent =
                    (Classifier)
                            getPreparedClassOption(
                                    this.studentLearnerForUnsupervisedDriftDetectionOption);
            this.detectionStudent.resetLearning();

            this.driftDetectionMethod =
                    (ChangeDetector) getPreparedClassOption(this.driftDetectionMethodOption);
    ...
    public void resetLearningImpl() {
        this.baseEnsemble =
                (StreamingRandomPatches) getPreparedClassOption(this.baseEnsembleOption);
        ...
        // Define whether to calculate the kappa diversity or not. If we are using the pairing
        // function that relies on
        // minKappa, then it must be updated. Another option is to just calculate it to have the
        // ensemble diversity
        // statistics.
        ...
        this.evaluatorWindowEnsembleDebug.widthOption.setValue(
                this.debugFrequencyOption.getValue());
        ...
        if (outputDebugFile != null) {
            try {
                if (outputDebugFile.exists()) {
                    outputAccuracyStatisticsDebugStream =
                            new PrintStream(new FileOutputStream(outputDebugFile, true), true);
                } else {
                    outputAccuracyStatisticsDebugStream =
                            new PrintStream(new FileOutputStream(outputDebugFile), true);
                }
            } catch (Exception ex) {
                throw new RuntimeException("Unable to open debug file: " + outputDebugFile, ex);
            }
        }
        ...
        if (outputDebugDriftFile != null) {
            try {
                if (outputDebugDriftFile.exists()) {
                    outputDriftDetectionDebugStream =
                            new PrintStream(new FileOutputStream(outputDebugDriftFile, true), true);
                } else {
                    outputDriftDetectionDebugStream =
                            new PrintStream(new FileOutputStream(outputDebugDriftFile), true);
                }
            } catch (Exception ex) {
        ...
        File outputConfidencePredictionsDebugFile =
                this.debugOutputConfidencePredictionsFileOption.getFile();
        if (outputConfidencePredictionsDebugFile != null) {
            try {
                if (outputConfidencePredictionsDebugFile.exists()) {
                    outputConfidencePredictionDebugStream =
                            new PrintStream(
                                    new FileOutputStream(
                                            outputConfidencePredictionsDebugFile, true),
                                    true);
                } else {
                    outputConfidencePredictionDebugStream =
                            new PrintStream(
                                    new FileOutputStream(outputConfidencePredictionsDebugFile),
                                    true);
                }
            } catch (Exception ex) {
                throw new RuntimeException(
                        "Unable to open confidence prediction debug file: "
                                + outputConfidencePredictionsDebugFile,
                        ex);
            }
        }
    }
    ...
    private void initDebugMode() {
        ...
        if (this.evaluatorWindowEnsembleMembersDebug == null)
            ...
            this.evaluatorWindowEnsembleMembersDebug =
                    new WindowClassificationPerformanceEvaluator
                            [this.baseEnsemble.getEnsembleMembers().length];

        // Gambi!
        if (this.outputDriftDetectionDebugStream != null) {
            this.outputDriftDetectionDebugStream.println(
                    "#instances,#labeled_instances,#drift_window,#drift_total,gtDrift");
        }

        if (this.outputAccuracyStatisticsDebugStream != null) {
            for (int i = 0; i < this.baseEnsemble.getEnsembleMembers().length; ++i) {
                // nao precisa fazer isso! Os membros do ensemble nao precisam escrever na saida de
                // debug...
                //                this.baseEnsemble.getEnsembleMembers()[i].outputDebugStream =
                // this.outputDebugStream;
                this.evaluatorWindowEnsembleMembersDebug[i] =
                        new WindowClassificationPerformanceEvaluator();
                this.evaluatorWindowEnsembleMembersDebug[i].widthOption.setValue(
                        this.debugFrequencyOption.getValue());
            }

            // Even more imaginative code
            this.outputAccuracyStatisticsDebugStream.print(
                    "#instances-labeled,#instances,#labeled_instances,windowed_accuracy(w="
                            + this.debugFrequencyOption.getValue()
                            + "),");
        ...
        if (outputConfidencePredictionDebugStream != null) {
            ...
            this.learnerPseudoLabeledCorrectSeen =
                    new long[baseEnsemble.ensembleSizeOption.getValue()];
            ...
            this.outputConfidencePredictionDebugStream.print(
                    "#instance,groundTruthLabel,Ensemble_Pred,Ensemble_Conf,EnsembleAvgLabel,EnsembleAvgPseudoLabel,"
    ...
    public void trainOnInstanceImpl(Instance instance) {
        ...
        if (this.baseEnsemble.getEnsembleMembers() == null) {
            ...
            initDebugMode();
            this.evaluatorTestThenTrainEnsembleMembers =
                    new BasicClassificationPerformanceEvaluator
                            [this.baseEnsemble.getEnsembleMembers().length];
            for (int i = 0; i < this.baseEnsemble.getEnsembleMembers().length; ++i)
                this.evaluatorTestThenTrainEnsembleMembers[i] =
                        new BasicClassificationPerformanceEvaluator();
        }
        ...
        if (this.baseEnsemble != null && this.baseEnsemble.getEnsembleMembers() != null) {
            if (this.evaluatorTestThenTrainEnsembleMembers != null) {
                ...
                for (int i = 0; i < this.baseEnsemble.getEnsembleMembers().length; ++i) {
                    double[] voteEnsembleMember =
                            this.baseEnsemble.getEnsembleMembers()[i].getVotesForInstance(instance);
                    this.evaluatorTestThenTrainEnsembleMembers[i].addResult(
                            example, voteEnsembleMember);
                }
            } else {
        ...
        // **** DEBUG CODE **** //
        //        if(/*this.instancesSeen >= 1000 && */ this.instancesSeen == 1970) { // &&
        // this.instancesSeen <= 2000) {
        //            System.out.println("x, prob_from_drift_point (1000),
        // prob_from_instancesSeen_point (1600), min(pfdp, pfisp)");
        ...
        //                        weightAge(x, 1000, 1600/*this.lastDriftDetectedAt,
        // this.instancesSeen*/, 20));
        //            }
        //        }
        // **** DEBUG CODE **** //
        ...
        if (shouldUpdateKappaDiversity) changeStatus = kappaUpdateFirst(instance, ensembleMembers);

        // Store the predictions of each learner.
        HashMap<Integer, Integer> classifiersExactPredictions =
                classifiersPredictions(instance, ensembleMembers);
        ...
        /** Debug * */
        if (outputConfidencePredictionDebugStream != null)
            for (int i = 0; i < this.learnerLabeledSeen.length; ++i) ++this.learnerLabeledSeen[i];
        /** Debug * */
        if (useUnsupervisedDriftDetectionOption.isSet()) {
            ...
            if (this.labeledWindowLimitOption.getValue() > 0) {
                if (this.labeledWindowLimitOption.getValue()
                        <= this.labeledInstancesBuffer.numInstances()) {
            ...
            if (driftDetected) reactToDrift(instance);
        }
    ...
    public int trainOnUnlabeledInstance(Instance instance) {
        ...
        if (debugPerfectPseudoLabelingOption.isSet() && instance.classIsMissing()) {
            throw new RuntimeException(
                    "Pseudo-labeling debug is on, but the true class is not available");
        }
        ...
        for (int i = 0; i < ensembleMembers.length; ++i) indicesNoReposition.add(i);
        ...
        for (int i = 0; i < ensembleMembers.length; ++i) {
            ...
            switch (this.pairingFunctionOption.getChosenIndex()) {
                ...
                case SSL_PAIRING_MIN_KAPPA:
                    for (int j = 0; j < ensembleMembers.length; ++j) {
                        if (j != i) {
                            ...
                            if (idxSelectedByPairing == -1
                                    || currentKappa < this.getKappa(i, idxSelectedByPairing))
                                idxSelectedByPairing = j;
                        }
                    }
                    votesRawOthers =
                            ensembleMembers[idxSelectedByPairing].getVotesForInstance(instance);
                    minIdxEstimatedAccuracy =
                            ensembleMembers[idxSelectedByPairing].evaluator
                ...
                case SSL_PAIRING_RANDOM:
                    ...
                    do {
                        idxSelectedByPairing =
                                Math.abs(this.classifierRandom.nextInt()) % ensembleMembers.length;
                ...
                case SSL_PAIRING_MAJORITY_TRAINS_MINORITY:
                    // TODO: Extremely naive approach, getting the same predictions (Ensemble size -
                    // 1) times.
                    // Could create an array with predictions the first time and lazily select which
                    // learners are used.
                    // For the future! Make it faster once it is right!
                    // Versao antiga!
                    //                    votesRawOthers =
                    // getVotesForComplementSubEnsemble(instance, i,
                    // this.baseEnsemble.getEnsembleMembers());
                    // Assumption: the complement ensemble will not yield a random prediction! Thus
                    // we can assume a performance of 0.5
                    // We could also estimate it by summing all the performance measurements for it
                    // and dividing by the ensemble size

                    /****** OLD strategy for the confidence ******/
                    if (confidenceStrategyOption.getChosenIndex() == SSL_CONFIDENCE_STRATEGY_SUM) {
                        votesRawOthers =
                                getVotesForComplementSubEnsemble(
                                        instance, i, this.baseEnsemble.getEnsembleMembers());
                    } else {
                        /****** NEW strategy for the confidence ******/
                        votesRawOthers =
                                getExactVotesForComplementSubEnsemble(
                                        instance, i, this.baseEnsemble.getEnsembleMembers());
                    }
                    DoubleVector votes = new DoubleVector(votesRawOthers);
                    int predictedIndexByVotes = votes.maxIndex();
                    confidenceForMajMin =
                            votes.getValue(predictedIndexByVotes) / votes.sumOfValues();

                    minIdxEstimatedAccuracy =
                            0.5; // ensembleMembers[idxMinKappa].evaluator.getPerformanceMeasurements()[1].getValue() / 100.0;
                    break;
            }

            //            if(this.pairingFunctionOption.getChosenIndex() !=
            // SSL_PAIRING_MAJORITY_TRAINS_MINORITY) ;
            ...
            else instanceWithPseudoLabel.setClassValue(predictedIndexByMinKappa);

            // Sanity check...
            if (predictedIndexByMinKappa >= 0
                    && votesMinKappa.sumOfValues() > 0.0
                    && minIdxEstimatedAccuracy > 0.0) {
                ...
                switch (autoWeightShrinkageOption.getChosenIndex()) {
                    ...
                    case AUTO_WEIGHT_SHRINKAGE_LABELED_DIV_TOTAL:
                        ...
                        break;
                    /**
                     * If there is a warmup period, this is taken into account when calculating the
                     * auto-shrinkage *
                     */
                    case AUTO_WEIGHT_SHRINKAGE_LABELED_IGNORE_WARMUP_DIV_TOTAL:
                        weightShrinkage =
                                (this.instancesSeen - this.initialWarmupTrainingCounter)
                                        / (this.allInstancesSeen
                                                - this.initialWarmupTrainingCounter
                                                + 0.0000001);
                        break;
                ...
                switch (weightFunctionOption.getChosenIndex()) {
                    ...
                    case SSL_WEIGHT_CONFIDENCE:
                        weightMinKappa =
                                votesMinKappa.getValue(predictedIndexByMinKappa)
                                        / votesMinKappa.sumOfValues();
                        break;
                    ...
                    case SSL_WEIGHT_UNSUPERVISED_DETECTION_WEIGHT_SHRINKAGE:
                        weightMinKappa =
                                votesMinKappa.getValue(predictedIndexByMinKappa)
                                        / votesMinKappa.sumOfValues()
                                        * weightAge(
                                                ensembleMembers[idxSelectedByPairing].createdOn,
                                                this.lastDriftDetectedAt,
                                                this.instancesSeen,
                                                this.unsupervisedDetectionWeightWindowOption
                                                        .getValue());
                        break;
                }
                ...
                switch (SSLStrategyOption.getChosenIndex()) {
                    ...
                    case SSL_STRATEGY_NO_CHECKS:
                        ...
                        if (!instance.classIsMissing()
                                && ((int) instanceWithPseudoLabel.classValue())
                    ...
                    case SSL_STRATEGY_CHECK_CONFIDENCE_AND_OTHERS:
                        ...
                        //                        double confidenceByMinKappa = -1; //
                        // votesMinKappa.getValue(predictedIndexByMinKappa) /
                        // votesMinKappa.sumOfValues();

                        DoubleVector votesByCurrent =
                                new DoubleVector(ensembleMembers[i].getVotesForInstance(instance));
                        int predictedByCurrent = votesByCurrent.maxIndex();
                        double confidenceByCurrent =
                                votesByCurrent.getValue(predictedByCurrent)
                                        / votesByCurrent.sumOfValues();

                        double minConfidenceThreshold = this.SSL_minConfidenceOption.getValue();

                        // NEW: this overwrite the minimum confidence threshold set by the user and
                        // apply a random one.
                        ...
                        if (this.outputConfidencePredictionDebugStream != null) {
                            ...
                            learnerTTT_acc[i] =
                                    this.baseEnsemble.getEnsembleMembers()[i].evaluator
                                            .getPerformanceMeasurements()[1].getValue();
                            learnerWIN_acc[i] =
                                    this.evaluatorWindowEnsembleMembersDebug[i]
                                            .getPerformanceMeasurements()[1].getValue();
        ...
        if (this.outputConfidencePredictionDebugStream != null) {
            ...
            printConfidenceAccuracyDebug(
                    ...
                    complementConf,
                    pseudoUsed,
                    this.learnerLabeledSeen,
                    this.learnerPseudoLabeledSeen,
                    this.learnerPseudoLabeledCorrectSeen,
                    learnerTTT_acc,
                    learnerWIN_acc,
                    learnersWeight);
    ...
    public double[] getVotesForInstance(Instance instance) {
        ...
        if (this.evaluatorWindowEnsembleMembersDebug != null
                && this.evaluatorWindowEnsembleMembersDebug[0] != null)
    ...
    private void printConfidenceAccuracyDebug(
            int groundTruthLabel,
            int ensemblePred,
            double ensembleConf,
            double ensembleAvgLabel,
            double ensembleAvgPseudoLabel,
            double[] learnersPred,
            double[] learnersConf,
            double[] complementPred,
            double[] complementConf,
            int[] pseudoUsed,
            long[] labeledSeen,
            long[] pseudoLabeledTotal,
            long[] pseudoLabeledCorrect,
            double[] learnerTTT_acc,
            double[] learnerWIN_acc,
            double[] learnersWeight) {
        // DEBUG
        if (this.outputConfidencePredictionDebugStream != null) {
            // #instance Ensemble_TTT_acc Ensemble_WIN_acc Pred(0)	Conf(0)	Pred(L\0) Conf(L\0)
            //	Pseudo(0) TTT_acc(0) WIN_acc(0)
            ...
            outputConfidencePredictionDebugStream.print(
                    this.evaluatorTestThenTrainEnsembleDebug.getPerformanceMeasurements()[1]
                                    .getValue()
                            + ",");
            ...
            //
            //            for(int i = 0 ; i < this.baseEnsemble.getEnsembleMembers().length ; ++i) {
            //                this.outputDebugStream.print((this.instancesSeen -
            // this.baseEnsemble.getEnsembleMembers()[i].createdOn) + ",");
            //            }
            this.outputConfidencePredictionDebugStream.println();
        }
    }
    ...
    protected Measurement[] getModelMeasurementsImpl() {
        ...
        //                this.instancesPseudoLabeled / (double) (this.instancesSeen *
        // this.baseEnsemble.ensembleSizeOption.getValue()) * 100),
        //                new Measurement("correct pseudo labeled",
        //                        this.instancesPseudoLabeled != 0 ?
        // this.instancesCorrectPseudoLabeled / (double) this.instancesPseudoLabeled * 100 : 0)
        //        };
        ...
        // DEBUG
        if (
        /*this.debugShowTTTAccuracyConfidenceOption.isSet() &&*/ this
                        .outputAccuracyStatisticsDebugStream
                != null) {
            //            if(this.allInstancesSeen % this.debugFrequencyOption.getValue() == 0)
            outputAccuracyStatisticsDebugStream.print(
                    (this.allInstancesSeen - this.instancesSeen)
                            + ","
                            + this.allInstancesSeen
                            + ","
                            + this.instancesSeen
                            + ","
                            + this.evaluatorWindowEnsembleDebug.getPerformanceMeasurements()[1]
                                    .getValue()
                            + ",,"); // + ", avg conf incorr ???, avg conf corr ???");
            ...
            for (int i = 0; i < this.baseEnsemble.getEnsembleMembers().length; ++i) {
                this.outputAccuracyStatisticsDebugStream.print(
                        this.evaluatorWindowEnsembleMembersDebug[i].getPerformanceMeasurements()[1]
                                        .getValue()
                                + ",");
            }
            ...
            for (int i = 0; i < this.baseEnsemble.getEnsembleMembers().length; ++i) {
                this.outputAccuracyStatisticsDebugStream.print(
                        (this.instancesSeen - this.baseEnsemble.getEnsembleMembers()[i].createdOn)
                                + ",");
            }
            this.outputAccuracyStatisticsDebugStream.println();
        }
        return new Measurement[] {
            ...
            new Measurement(
                    ...
                    this.instancesCorrectPseudoLabeled
                            / (double) this.instancesPseudoLabeled
                            * 100),
            ...
            //                new Measurement("accuracy supervised learner",
    ...
    private double[] getVotesForComplementSubEnsemble(
            Instance instance,
            int idxLearner,
            StreamingRandomPatches.StreamingRandomPatchesClassifier[] ensembleMembers) {
        // Store the predictions of each learner.
        ...
        for (int i = 0; i < ensembleMembers.length; i++) {
            ...
            //          TODO: This is internal to SRP, when we move to a generic approach, we may
            // need to do that in here as well, but with a dedicated evaluator.
            //          ensembleMembers[i].evaluator.addResult(example, vote.getArrayRef());
        }
        return votes.getArrayCopy();
    }

    private double[] getExactVotesForComplementSubEnsemble(
            Instance instance,
            int idxLearner,
            StreamingRandomPatches.StreamingRandomPatchesClassifier[] ensembleMembers) {
        // Store the predictions of each learner.
        ...
        for (int i = 0; i < ensembleMembers.length; i++) {
            if (i != idxLearner) {
                ...
                if (vote.getValue(maxIndex) > 0) votes.addValues(dummyVote);
                else votes.addValues(vote); // add the array with all 0's
            }
            //          TODO: This is internal to SRP, when we move to a generic approach, we may
            // need to do that in here as well, but with a dedicated evaluator.
            //          ensembleMembers[i].evaluator.addResult(example, vote.getArrayRef());
    ...
    private void kappaUpdateSecond(
            int[][] changeStatus,
            StreamingRandomPatches.StreamingRandomPatchesClassifier[] ensembleMembers,
            HashMap<Integer, Integer> classifiersExactPredictions) {
        /******************* K STATISTIC *******************/
        ...
        for (int i = 0; i < ensembleMembers.length; ++i) {
            for (int j = i + 1; j < ensembleMembers.length; ++j) {
                if (currentDriftStatus[i] - changeStatus[0][i] == 0)
                    this.pairsOutputsKappa[i][j].update(
                            classifiersExactPredictions.get(i), classifiersExactPredictions.get(j));
                else this.pairsOutputsKappa[i][j].reset();
            }
        }
        ...
        for (int i = 0; i < ensembleMembers.length; ++i) {
            for (int j = i + 1; j < ensembleMembers.length; ++j) {
                ...
                if (kappa < this.currentMinKappa) this.currentMinKappa = kappa;
                if (kappa > this.currentMaxKappa) this.currentMaxKappa = kappa;
    ...
    private HashMap<Integer, Integer> classifiersPredictions(
            Instance instance,
            StreamingRandomPatches.StreamingRandomPatchesClassifier[] ensembleMembers) {
        // Store the predictions of each learner.
    ...
    private double weightAge(
            long createdOn, long lstDriftDetectedOn, long labelledInstancesSeen, int window) {
        // lastDrift
        ...
        if (this.baseEnsemble != null
            ...
            if (this.evaluatorWindowEnsembleMembersDebug != null) {
                for (int i = 0; i < this.baseEnsemble.getEnsembleMembers().length; ++i) {
                    double[] voteEnsembleMember =
                            this.baseEnsemble.getEnsembleMembers()[i].getVotesForInstance(instance);
                    this.evaluatorWindowEnsembleMembersDebug[i].addResult(
                            example, voteEnsembleMember);
                }
            }
        }
            ...
            if (gtDriftLocations != null) {
                for (int i = 0; i < this.gtDriftLocations.length; ++i) {
                    if (this.gtDriftLocations[i] == this.allInstancesSeen) {
                        ...
                        //
                        // this.outputDriftDetectionDebugStream.println(this.allInstancesSeen + ", "
                        // + this.instancesSeen + ",-1,"+this.numberOfUnsupervisedDriftsDetected);
                    }
                }
            }
            if (this.allInstancesSeen % this.debugFrequencyOption.getValue() == 0
                ...
                int numberDriftsSinceLastOutput =
                        this.numberOfUnsupervisedDriftsDetected
                                - this.currentNumberOfUnsupervisedDriftsDetected;
                this.outputDriftDetectionDebugStream.println(
                        this.allInstancesSeen
                                + ","
                                + this.instancesSeen
                                + ","
                                + numberDriftsSinceLastOutput
                                + ","
                                + this.numberOfUnsupervisedDriftsDetected
                                + ","
                                + gtDrift);
    ...
    protected double getKappa(int i, int j) {
        assert (i != j);
        if (this.pairsOutputsKappa == null) return -6;
    ...
    protected class C {
        ...
        public void reset() {
            for (int i = 0; i < C.length; ++i) for (int j = 0; j < C.length; ++j) C[i][j] = 0;
            this.instancesSeen = 0;
        }
        ...
        public double theta1() {
            double sum = 0.0;
            for (int i = 0; i < C.length; ++i) sum += C[i][i];
        ...
        public double theta2() {
            ...
            for (int i = 0; i < C.length; ++i) {
                ...
                //      System.out.println("column = (" + sum1 + "," + (sum1 / (double)
                // instancesSeen) + ") row = (" + sum2 + "," + (sum2 / (double) instancesSeen) +
                // ")");
