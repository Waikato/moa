...
public class AdaptivePredictionInterval extends AbstractClassifier
        implements PredictionIntervalLearner {
    ...
    public FloatOption confidenceLevelOption =
            new FloatOption("confidence", 'c', "confidence level", 0.95, 0, 1);

    public FloatOption scalarLimitOption =
            new FloatOption("limit", 't', "lower limit of the scalar", 0.1, 0, 1);
    ...
    public void resetLearningImpl() {
        if (this.regressor == null)
            this.regressor = (Regressor) getPreparedClassOption(this.learnerOption);
    ...
    public double[] getVotesForInstance(Instance inst) {
        ...
        double prediction =
                ((Learner) this.regressor).getVotesForInstance(example).length > 0
                        ? ((Learner) this.regressor).getVotesForInstance(example)[0]
                        : 0;
    ...
    private double calculateBounds() {
        ...
        return getScalar(this.metaEvaluator.getCoverage())
                * this.normalDistribution.inverseCumulativeProbability(
                        0.5 + this.confidenceLevelOption.getValue() / 2)
                * this.evaluator.getSquareError();
    }
                ...
                scalar =
                        (Math.log(
                                                                -(1
                                                                        ...
                                                                        * (coverage - 100.0))
                                                        / Math.log(
                                                                this.confidenceLevelOption
                                                                                .getValue()
                                                                        * 100)
                                                + 1)
                                        * (1 - this.scalarLimitOption.getValue())
                                + this.scalarLimitOption.getValue();
            } else if (coverage > 200 * this.confidenceLevelOption.getValue() - 100.0
                    && coverage < this.confidenceLevelOption.getValue() * 100.0) {
                scalar =
                        -(100.0 - coverage)
                                        * Math.log(
                                                1
                                                        ...
                                                        * (coverage
                                                                - (200
                                                                                * this
                                                                                        ...
                                                                                        .getValue()
                                                                        - 100.0)))
                                        / Math.log(this.confidenceLevelOption.getValue() * 100)
                                + 1;
            }
        } else {
            scalar = 1.0;
        }
        if (scalar <= this.scalarLimitOption.getValue()) return this.scalarLimitOption.getValue();
        else return scalar;
    }
