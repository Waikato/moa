 ...
 * @version $Revision: 1 $
 */
public class MEKAClassifier extends AbstractMultiLabelClassifier implements MultiLabelClassifier, Serializable {
    ...
    public void trainOnInstanceImpl(StructuredInstance samoaInstance) {
		// This is here so MEKAClassifier can act as a multi-target regressor
    	trainOnInstance((MultiLabelInstance) samoaInstance);
    }
    
	public void trainOnInstanceImpl(Instance samoaInstance) {
		// Convert Samoa instance to Weka instance (as used in Meka)
		L = samoaInstance.numOutputAttributes();
	...
	public Prediction getPredictionForInstance(Instance samoaInstance) {
		...
		return new MultiLabelClassificationPrediction(votes);
	}

	public Prediction getPredictionForInstance(StructuredInstance samoaInstance) {
		// This is here so MEKAClassifier can act as a multi-target regressor
		return getPredictionForInstance((MultiLabelInstance) samoaInstance);
	}
	
	public Prediction getPredictionForInstance(MultiLabelInstance samoaInstance) {

		MultiLabelClassificationPrediction prediction = new MultiLabelClassificationPrediction(L);

		if (isClassificationEnabled == true) { 

			double votes[] = getPredictionForInstance(samoaInstance).asDoubleArray();
