package moa.classifiers.mlc.core.splitcriteria;
...
public class PCTWeightedICVarianceReduction extends WeightedICVarianceReduction {
	...
	public double getMeritOfSplit(DoubleVector[] preSplitTargetDist, DoubleVector[][] postSplitTargetDists, DoubleVector[] preSplitInputDist, DoubleVector[][] postSplitInputDists) {
		...
		for (int i=0; i<numTargets; i++)
			targetError += weights.getValue(i) * getMeritOfSplitForAttribute(preSplitTargetDist[i],postSplitTargetDists[i]);
		for (int i=0; i<numInputs; i++)
			targetError += inputWeights.getValue(i) * getMeritOfSplitForAttribute(preSplitInputDist[i],postSplitInputDists[i]);

		
		return targetInputTradeoff * targetError / weights.sumOfValues() + (1 - targetInputTradeoff) * inputError / inputWeights.sumOfValues();
	}
