...
public class MultilabelHoeffdingTree extends HoeffdingTreeClassifLeaves
        implements MultiLabelLearner, MultiTargetRegressor {
    // Needs to use InfoGainSplitCriterionMultiLabel, since multilabel entropy is calculated in a
    // different way
    ...
    public class MultilabelLearningNodeClassifier extends LearningNodeClassifier {
        ...
        public MultilabelLearningNodeClassifier(
                double[] initialClassObservations, Classifier cl, MultilabelHoeffdingTree ht) {
            super(initialClassObservations);
        ...
        public void learnFromInstance(Instance inst, HoeffdingTree ht) {
            ...
            for (int i = 0; i < inst.numInputAttributes(); i++) {
                ...
                if (obs == null) {
                    obs =
                            inst.inputAttribute(i).isNominal()
                                    ? mht.newNominalClassObserver()
                                    : mht.newNumericClassObserver();
        ...
        public void describeSubtree(HoeffdingTree ht, StringBuilder out, int indent) {
            ...
            this.observedClassDistribution.getSingleLineDescription(
                    out, this.observedClassDistribution.numValues());
    ...
    protected void deactivateLearningNode(
        ...
        Node newLeaf =
                new MultilabelInactiveLearningNode(toDeactivate.getObservedClassDistribution());
        if (parent == null) {
    ...
    public Prediction getPredictionForInstance(MultiLabelInstance inst) {
        ...
        if (this.treeRoot != null) {
            ...
            MultilabelLearningNodeClassifier multilabelLeafNode =
                    (MultilabelLearningNodeClassifier) leafNode;
            if (leafNode == null) {
