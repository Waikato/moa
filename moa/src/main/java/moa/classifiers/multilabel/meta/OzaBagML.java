    ...
    public static Prediction combinePredictions(Prediction[] predictions, Instance inst) {
        ...
        for (int i = 0; i < predictions.length; i++) {
            try {
                ...
                if (more_votes != null) {
                    for (int numOutputAttribute = 0;
                            numOutputAttribute < inst.numOutputAttributes();
                            numOutputAttribute++) {
                        int length = 0;
                        if (more_votes.getVotes(numOutputAttribute) != null)
                            length = more_votes.getVotes(numOutputAttribute).length;
                        for (int numValueAttribute = 0;
                                numValueAttribute < length;
                                numValueAttribute++) {
                            result.setVote(
                                    numOutputAttribute,
                                    numValueAttribute,
                                    (result.getVote(numOutputAttribute, numValueAttribute)
                                            + more_votes.getVote(
                                                            numOutputAttribute, numValueAttribute)
