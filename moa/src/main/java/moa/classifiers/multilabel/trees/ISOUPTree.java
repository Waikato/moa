 ...
 * @version $Revision: 1 $
 */
// public class ISOUPTree extends AbstractMultiTargetRegressor implements MultiTargetRegressor,
// SemiSupervisedMultiTargetRegressor {
public class ISOUPTree extends AbstractMultiLabelLearner
        implements MultiTargetRegressor, MultiTargetLearnerSemiSupervised {
    ...
    public FloatOption alternateTreeFadingFactorOption =
            new FloatOption(
                    ...
                    'f',
                    "The fading factor to use when deciding if an alternate tree should replace an"
                            + " original.",
                    0.995,
                    0.0,
                    1.0);
    ...
    public IntOption alternateTreeTMinOption =
            new IntOption(
                    "alternateTreeTMin",
                    'y',
                    "The Tmin value to use when deciding if an alternate tree should replace an"
                            + " original.",
                    150,
                    0,
                    Integer.MAX_VALUE);
    ...
    public IntOption alternateTreeTimeOption =
            new IntOption(
                    "alternateTreeTime",
                    'u',
                    "The 'time' (in terms of number of instances) value to use when deciding if an"
                            + " alternate tree should be discarded.",
                    1500,
                    0,
                    Integer.MAX_VALUE);
    ...
    public FlagOption regressionTreeOption =
            new FlagOption(
                    "regressionTree", 's', "Build a regression tree instead of a model tree.");
    ...
    public FlagOption learningRatioConstOption =
            new FlagOption(
                    "learningRatioConst",
                    'o',
                    "Keep learning rate constant instead of decaying (if kept constant learning"
                            + " ratio is suggested to be 0.001).");

    public FlagOption runAsPCTOption =
            new FlagOption(
                    "runAsPCT",
                    'p',
                    "Run as a predictive clustering tree, i.e., use input attributes in heuristic"
                            + " calculation.");

    public FlagOption doNotNormalizeOption =
            new FlagOption("doNotNormalize", 'n', "Don't normalize.");

    public FileOption weightFile =
            new FileOption(
                    "targetWeightFile",
                    'w',
                    "File with the weights of the targets.",
                    null,
                    null,
                    false);
    ...
    private static class InstanceWrapper extends InstanceImpl {
        ...
        public boolean isInputMissing(int index) {
            return this.instanceData.isMissing(
                    this.instanceHeader.getInstanceInformation().inputAttributeIndex(index));
        }

        public boolean isOutputMissing(int index) {
            return this.instanceData.isMissing(
                    this.instanceHeader.getInstanceInformation().outputAttributeIndex(index));
        }

        public boolean missingOutputs() {
            if (this.instanceHeader.numOutputAttributes() == 1) return classIsMissing();
            else {
    ...
    public abstract static class Node implements Serializable {
        ...
        public long calcByteSize() {
            long size =
                    SizeOf.sizeOf(this)
                            + SizeOf.sizeOf(sumOfSquares)
                            + SizeOf.sizeOf(sumOfValues)
                            + SizeOf.sizeOf(examplesSeen);
            if (tree.runAsPCTOption.isSet()) {
                size +=
                        SizeOf.sizeOf(sumOfInputSquares)
                                + SizeOf.sizeOf(sumOfInputValues)
    ...
    public static class LeafNode extends Node {
        ...
        public void learnFromInstance(
                Instance instance, double[] prediction, boolean growthAllowed) {
            ...
            for (int i = 0; i < tree.numOutputAttributes; i++) {
                if (!inst.isOutputMissing(i)) {
                    ...
                    if (tree.buildingModelTree()) {
                        errorP.setValue(
                                i, errorP.getValue(i) * 0.95 + Math.abs(predictionP[i] - outVal));
                        errorM.setValue(
                                i, errorM.getValue(i) * 0.95 + Math.abs(predictionM[i] - outVal));
                    }

                    observations[i] =
                            new DoubleVector(
                                    new double[] {
                                        weight, weight * outVal, weight * outVal * outVal
                                    });
                }
            }
                ...
                for (int i : inputIndexes) {
                    if (!inst.isInputMissing(i)) {
                        ...
                        inputObservations[i] =
                                new DoubleVector(
                                        new double[] {
                                            weight, weight * inVal, weight * inVal * inVal
                                        });
                    }
                }
            }
            ...
            for (int i : inputIndexes) {
                ...
                if (!tree.runAsPCTOption.isSet() || !inst.inputAttribute(i).isNumeric()) {
                    ...
                    ((MultiLabelBSTreePCT) obs)
                            .observeAttribute(
                                    inst.valueInputAttribute(i), observations, inputObservations);
                }
            }
        ...
        public AttributeExpansionSuggestion[] getBestSplitSuggestions(
            ...
            for (int i : inputIndexes) {
                ...
                if (obs != null) {
                    ...
                    if (tree.runAsPCTOption.isSet()
                            && tree.modelContext.inputAttribute(i).isNumeric()) {
                        ...
                        bestSuggestion =
                                ((MultiLabelBSTreePCT) obs)
                                        .getBestEvaluatedSplitSuggestion(
                                                criterion,
                                                preSplitStatistics,
                                                preSplitInputStatistics,
                                                i);
                    } else {
                        bestSuggestion =
                                obs.getBestEvaluatedSplitSuggestion(
                                        criterion, preSplitStatistics, i);
                    }
            ...
            return bestSuggestions.toArray(
                    new AttributeExpansionSuggestion[bestSuggestions.size()]);
        }
        ...
        public void checkForSplit() {
            ...
            if (learningWeight - examplesSeenAtLastSplitEvaluation
    ...
    public abstract static class InnerNode extends Node {
        ...
        protected DoubleVector sumOfAbsErrors =
                new DoubleVector(); // Needed for PH tracking of mean error
        ...
        public long calcByteSize() {
            ...
            for (Node child : children) size += child.calcByteSize();
            return size;
        }
            ...
            for (int branch = 0; branch < children.size(); branch++) {
                ...
                if (child != null) {
                    if (branch == 0) {
                        ...
                        this.predicate.getDescription(
                                out, 0, tree.getModelContext().getInstanceInformation());
                    } else {
    ...
    public class MultitargetPerceptron implements Serializable {
        ...
        public void updatePerceptron(Instance inst) {
            ...
            if (tree.learningRatioConstOption.isSet()) {
                ...
                learningRatio =
                        tree.learningRatioOption.getValue()
                                / (1
                                        + instancesSeen
        ...
        public void updateWeights(Instance inst, double learningRatio) {
            if (instancesSeen > 1.0) {
                ...
                for (int i = 0; i < tree.numOutputAttributes; i++) {
                    if (!Double.isNaN(normalizedTarget[i])) {
                        /* to account for missing target values */
        ...
        private double[] denormalizePrediction(double[] normalizedPrediction) {
            ...
            if (tree.normalize()) {
                for (int i = 0; i < tree.numOutputAttributes; i++) {
                    ...
                    double sd =
                            computeSD(
                                    tree.sumOfSquares.getValue(i),
                                    tree.sumOfValues.getValue(i),
                                    tree.examplesSeen.getValue(i));
                    if (examplesSeen.getValue(i) > 1) out[i] = normalizedPrediction[i] * sd + mean;
                    else out[i] = 0;
                }
                return out;
        ...
        public void getModelDescription(StringBuilder out, int indent) {
            if (getModelContext() != null) {
                for (int i = 0; i < tree.numOutputAttributes; i++) {
                    ...
                    out.append(
                            (weights[i][tree.numInputAttributes] < 0 ? " - " : " + ")
                                    + String.format(
                                            "%.4f", Math.abs(weights[i][tree.numInputAttributes])));
                }
                StringUtils.appendNewline(out);
            }
        }
    ...
    public void resetLearningImpl() {
        ...
        if (this.getModelContext() != null) checkRoot();
    ...
    public void loadWeights() {
        ...
        try {
            List<String> lines =
                    Files.readAllLines(Paths.get(weightFile.getValue()), Charset.defaultCharset());
            for (int i = 0; i < lines.size(); i++)
                targetWeights.setValue(i, Double.valueOf(lines.get(i)));
        } catch (Exception e) {
            for (int i = 0; i < this.numOutputAttributes; i++) targetWeights.setValue(i, 1.0);
        }
    }
    ...
    public Prediction getPredictionForInstance(MultiLabelInstance inst) {
        ...
        MultiLabelPrediction prediction =
                new MultiLabelPrediction(getModelContext().numOutputAttributes());
    ...
    public double[] normalizedInputVector(Instance inst) {
        ...
        if (normalize()) {
            for (int j = 0; j < this.numInputAttributes; j++) {
                ...
                double sd =
                        computeSD(
                                sumOfAttrSquares.getValue(j),
                                sumOfAttrValues.getValue(j),
                                weightOfInputs.getValue(j));
                if (weightOfInputs.getValue(j) > 1 && sd > 0.00001) {
                    if (attr.isNumeric()) normalizedInput[j] = (value - mean) / sd;
                    else normalizedInput[j] = value;
                } else normalizedInput[j] = 0;
            }
            normalizedInput[this.numInputAttributes] =
    ...
    public double[] normalizedTargetVector(Instance instance) {
        ...
        if (normalize()) {
            for (int i = 0; i < this.numOutputAttributes; i++) {
                double value =
                        (inst.isOutputMissing(i)) ? Double.NaN : inst.valueOutputAttribute(i);
                if (!Double.isNaN(value)) {
                    double sd =
                            computeSD(
                                    sumOfSquares.getValue(i),
                                    sumOfValues.getValue(i),
                                    examplesSeen.getValue(i));
                    double average = sumOfValues.getValue(i) / examplesSeen.getValue(i);
                    if (sd > 0 && examplesSeen.getValue(i) > 1) out[i] = (value - average) / (sd);
                    else out[i] = 0;
                } else out[i] = value;
            }
        } else {
            ...
            if (examplesSeen.getValue(i) > 1) {
                ...
                double sd =
                        computeSD(
                                ...
                                examplesSeen.getValue(i));
                double average = sumOfValues.getValue(i) / examplesSeen.getValue(i);
                if (sd > 0) return (value - average) / sd;
                else return 0;
            }
            return 0;
    ...
    protected void attemptToSplit(LeafNode node, InnerNode parent, int parentIndex) {
        ...
        AttributeExpansionSuggestion[] bestSplitSuggestions =
                node.getBestSplitSuggestions(splitCriterion); // TODO
        // update
        // with
        // split
        // criterion
        ...
        if (bestSplitSuggestions.length < 2) {
            ...
            double hoeffdingBound =
                    computeHoeffdingBound(1, splitConfidenceOption.getValue(), numExamples);
        ...
        if (shouldSplit) {
            AttributeExpansionSuggestion splitDecision =
                    bestSplitSuggestions[bestSplitSuggestions.length - 1];
            ...
            for (int i = 0;
                    i < 2;
                    i++) { // Hardcoded for 2 values (due to the use of the Predicate class)
                ...
                if (buildingModelTree()) {
                    ...
                    for (int j = 0; j < getModelContext().numOutputAttributes(); j++) {
                        newChild.examplesSeen.setValue(
                                j, splitDecision.getResultingNodeStatistics()[j][i].getValue(0));
                        newChild.sumOfValues.setValue(
                                j, splitDecision.getResultingNodeStatistics()[j][i].getValue(1));
                        newChild.sumOfSquares.setValue(
                                j, splitDecision.getResultingNodeStatistics()[j][i].getValue(2));
                    }
                }
