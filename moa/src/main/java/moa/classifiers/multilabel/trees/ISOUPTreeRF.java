...
public class ISOUPTreeRF extends ISOUPTree {
    ...
    public MultiChoiceOption subspaceSizeOption =
            new MultiChoiceOption(
                    "randomForestFunAttr",
                    'z',
                    "The function of the number of input attribtues to use in the random forest"
                            + " construction.",
                    new String[] {"sqrt", "log", "percent"},
                    new String[] {"Square root", "Logarithm", "10% percent"},
                    0);
    ...
    public int numRFAttrs() {
        if (subspaceSize == null) {
            switch (subspaceSizeOption.getChosenLabel()) {
                ...
                case "percent":
                    this.subspaceSize =
                            (int) Math.ceil(0.1 * this.getModelContext().numInputAttributes());
            }
        }
    ...
    public DoubleVector getNodeFeatureScore(ISOUPTree.Node node) {
        if (node instanceof ISOUPTree.SplitNode) {
            DoubleVector scores =
                    new DoubleVector(new double[getModelContext().numInputAttributes()]);
            for (ISOUPTree.Node child : ((ISOUPTree.SplitNode) node).children) {
                scores.addValues(getNodeFeatureScore(child));
            }
            scores.setValue(
                    ((ISOUPTree.SplitNode) node).predicate.getAttributeIndex(),
                    Math.pow(0.5, node.getLevel()));
            return scores;
        } else {
