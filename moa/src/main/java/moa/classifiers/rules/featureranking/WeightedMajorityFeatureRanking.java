...
/**
 * Weighted Majority Feature Ranking method João Duarte, João Gama,Feature ranking in hoeffding
 * algorithms for regression. SAC 2017: 836-841
 */
public class WeightedMajorityFeatureRanking extends AbstractFeatureRanking
    ...
    public FloatOption meritThresholdOption =
            new FloatOption(
                    "meritThreshold",
                    'm',
                    "Merit threshold value. If the merit of an input attribute is below the"
                            + " threshold its importance will decrease",
                    0.01,
                    0.0,
                    1.0);
    ...
    public void update(ObservableMOAObject o, Object arg) {
        ...
        if (arg instanceof MeritCheckMessage) {
            ...
            boolean[] attributesMask =
                    msg.getLearningAttributes().clone(); // because it will be modified
            ...
            if (ri == null) {
                ...
                for (int i = 0; i < numAttributes; i++)
                    attributeImportance[i] =
                            (attributeImportance[i] * numRules + 1) / (numRules + 1);
            }
            // Merit check of an existing rule
                ...
                while (it.hasNext()) {
                    ...
                    this.attributeImportance[attIndex] =
                            (attributeImportance[attIndex] * numRules
                                            - old[attIndex]
                                            + ri.getAttributeImportance(attIndex))
                                    / numRules;
                }
            }
        }
        // Rule expanded
            ...
            this.attributeImportance[attIndex] =
                    (attributeImportance[attIndex] * numRules - oldValue + 1) / numRules;
        ...
        // Rule will be removed
            ...
            for (int i = 0; i < this.attributeImportance.length; i++)
                attributeImportance[i] =
                        (attributeImportance[i] * numRules - attribImportance[i]) / (numRules - 1);
    ...
    public DoubleVector getFeatureRankings() {
        ...
        return normRankings;*/
        if (attributeImportance == null) return new DoubleVector();
    ...
    public class RuleInformation {
        ...
        public RuleInformation(int numAttributes) {
            ...
            depth = 0;
            for (int i = 0; i < numAttributes; i++) attributeImportance[i] = 1.0;
        }
        ...
        public void addLiteralAttribute(int attribIndex) {
            ...
            if (!literalAttributes.contains(attribIndex)) literalAttributes.add(attribIndex);
        }
