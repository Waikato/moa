...
/**
 * A modified ActiveLearningNode that uses a Perceptron as the leaf node model, and ensures that the
 * class values sent to the attribute observers are not truncated to ints if regression is being
 * performed
 */
public class RuleActiveRegressionNode extends RuleActiveLearningNode {
    ...
    public RuleActiveRegressionNode(Rule.Builder builder) {
        ...
        this.perceptron.learningRatioOption =
                ((AMRulesRegressorOld) this.amRules).learningRatioOption;
        this.perceptron.constantLearningRatioDecayOption =
                ((AMRulesRegressorOld) this.amRules).constantLearningRatioDecayOption;

        if (this.predictionFunction != 1) {
            this.targetMean = new TargetMean();
            if (builder.statistics[0] > 0)
                this.targetMean.reset(
                        builder.statistics[1] / builder.statistics[0],
                        (long) builder.statistics[0]);
        }
        this.predictionFunction = builder.predictionFunction;
        if (builder.statistics != null) this.nodeStatistics = new DoubleVector(builder.statistics);
    }
    ...
    public void updateStatistics(Instance instance) {
        ...
        if (this.predictionFunction
                != 1) { // Train target mean if prediction function is not Perceptron
    ...
    public double[] getPrediction(Instance instance, int predictionMode) {
        ...
        if (predictionMode == 1) ret = this.perceptron.getVotesForInstance(instance);
        else ret = this.targetMean.getVotesForInstance(instance);
        return ret;
    }
    ...
    public int getLearnerToUse(Instance instance, int predMode) {
        ...
        if (predictionMode == 0) {
            ...
            debug(
                    "Rule"
                            + this.owner.ruleNumberID
                            + " P:"
                            + this.perceptron.getVotesForInstance(instance)[0]
                            + " ("
                            + perceptronError
                            + ")"
                            + " M:"
                            + this.targetMean.getVotesForInstance(instance)[0]
                            + " ("
                            + meanTargetError
                            + ")",
                    3); // Commented by JD
    ...
    private double normalize(double value) {
        ...
        double sdY =
                computeSD(
                        this.nodeStatistics.getValue(2),
    ...
    public boolean isAnomaly(
        ...
        if (perceptronIntancesSeen >= numberOfInstanceesForAnomaly) {
            ...
            for (int x = 0; x < instance.numAttributes() - 1; x++) {
                ...
                // odds ratio
                if (probability > 0) anomaly += Math.log(probability / (1 - probability));
    ...
    public void initialize(RuleActiveLearningNode oldLearningNode) {
        ...
        if (((RuleActiveRegressionNode) oldLearningNode).perceptron != null) {
            this.perceptron =
                    new Perceptron(((RuleActiveRegressionNode) oldLearningNode).perceptron);
            this.perceptron.resetError();
            this.perceptron.setLearningRatio(
                    ((AMRulesRegressorOld) this.amRules).learningRatioOption.getValue());
        }

        if (((RuleActiveRegressionNode) oldLearningNode).targetMean != null) {
            this.targetMean =
                    new TargetMean(((RuleActiveRegressionNode) oldLearningNode).targetMean);
    ...
    public double[] getSimplePrediction() {
        if (this.targetMean != null) return this.targetMean.getVotesForInstance((Instance) null);
    ...
    public boolean tryToExpand(double splitConfidence, double tieThreshold) {
        ...
        // SplitCriterion splitCriterion = new SDRSplitCriterionAMRulesNode();//JD for assessing
        // only best branch
        ...
        if (bestSplitSuggestions.length < 2) {
            shouldSplit =
                    ((bestSplitSuggestions.length > 0) && (bestSplitSuggestions[0].merit > 0));
        ...
        if (shouldSplit == true) {
            ...
            double[] branchMerits =
                    splitCriterion.computeBranchSplitMerits(
                            bestSuggestion.resultingClassDistributions);
    ...
    public void learnFromInstance(Instance inst) {
                    ...
                    (int)
                                    Math.round(
                                            (inst.numAttributes() - 1)
                                                    * this.amRules.getAttributesPercentage())
                            / 100;

            attributesMask = new boolean[inst.numAttributes()];
            ArrayList<Integer> indices = new ArrayList<Integer>();
            for (int i = 0; i < inst.numAttributes(); i++)
                if (i != inst.classIndex()) indices.add(i);
        ...
        for (int i = 0, ct = 0; i < attributesMask.length; i++) {
            if (attributesMask[i]) {
                ...
                if (obs != null) {
                    ((FIMTDDNumericAttributeClassObserver) obs)
                            .observeAttributeClass(inst.value(i), inst.classValue(), inst.weight());
                }
                ++ct;
            }
        }
