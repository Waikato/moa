...
public class NumericRulePredicate extends AbstractMOAObject implements Predicate {
    ...
    public NumericRulePredicate(
            int inputAttributeIndex, double attributeValue, boolean isEqualOrLower) {
    ...
    public void getDescription(StringBuilder sb, int indent) {
        if (isEqualOrLower)
            StringUtils.appendIndented(
                    sb, indent + 1, "In" + inputAttributeIndex + " <= " + attributeValue);
        else
            StringUtils.appendIndented(
                    sb, indent + 1, "In" + inputAttributeIndex + " > " + attributeValue);
    }
        ...
        if (instInformation != null) {
            if (isEqualOrLower)
                StringUtils.appendIndented(
                        sb,
                        indent + 1,
                        instInformation.inputAttribute(inputAttributeIndex).name()
                                + " <= "
                                + attributeValue);
            else
                StringUtils.appendIndented(
