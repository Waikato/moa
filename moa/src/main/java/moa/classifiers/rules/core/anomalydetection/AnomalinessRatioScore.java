 ...
 *
 * <p>percentageAnomalousAttributesOption - Percentage of anomalous attributes.
                    ...
                    if (sd > MINSTD) {
                        prob =
                                probabilityFunction.getProbability(
                                        stats[0] / weightSeen,
                                        Utils.computeSD(stats[1], stats[0], weightSeen),
                                        val);
                        if ((1 - prob) > univariateThreshold) anomalousTotal++;
                        total++;
                    }
                }
            }
            if (total > 0) anomaly = anomaly / total;
