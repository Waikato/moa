...
/**
 * Class that contains several utilities Variance Standard deviation Vector operations(copy, etc)
 * Entropy Complementary set
 */
public final class Utils {
    ...
    public static double computeVariance(DoubleVector statistics) {
        return computeVariance(
                statistics.getValue(0), statistics.getValue(1), statistics.getValue(2));
    }
    ...
    public static DoubleVector[] copy(DoubleVector[] toCopy) {
        DoubleVector[] copy = new DoubleVector[toCopy.length];
        for (int i = 0; i < toCopy.length; i++) copy[i] = new DoubleVector(toCopy[i]);
        return copy;
    }
    ...
    public static double computeEntropy(double count, double sum) {
        if (sum / count == 1 || sum == 0) return 0;
        else
            return -(sum / count * Math.log(sum / count)
                    + (1 - sum / count) * Math.log(1 - sum / count));
    }
}
