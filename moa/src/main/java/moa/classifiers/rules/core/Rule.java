...
/**
 * Class that stores an arrayList of predicates of a rule and the observers (statistics). This class
 * implements a function that evaluates a rule.
 *
 * <p>Learning Decision Rules from Data Streams, IJCAI 2011, J. Gama, P. Kosina
 *
 * @author A. Bifet, J. Duarte, J. Gama
 * @version $Revision: 2 $
 */
...
public class Rule extends AbstractMOAObject {
    ...
    /**
     * getLearningNode Method This is the way to pass info for other classes. Implements
     * getLearningNode() in class RuleActiveLearningNode
     *
     * @return
     */
    ...
    public void split() {
        ...
        if (st instanceof NumericAttributeBinaryTest) {
            ...
            NumericAttributeBinaryRulePredicate predicate =
                    new NumericAttributeBinaryRulePredicate(
                            splitTest.getAttsTestDependsOn()[0],
                            splitTest.getSplitValue(),
                            splitIndex + 1);
            RuleSplitNode ruleSplitNode =
                    new RuleSplitNode(predicate, this.learningNode.getStatisticsBranchSplit());

            if (this.nodeListAdd(ruleSplitNode) == true) {
                // create a new learning node
                RuleActiveLearningNode newLearningNode =
    ...
    private boolean nodeListAdd(RuleSplitNode ruleSplitNode) {
        ...
        for (RuleSplitNode node : nodeList) {
            ...
            if (nodeTest.isUsingSameAttribute(ruleSplitNodeTest)) {
                ...
                if (nodeTest.isIncludedInRuleNode(ruleSplitNodeTest)
                        == true) { // remove this line to keep the most recent attribute value
                    // replace the value
                    nodeTest.setAttributeValue(ruleSplitNodeTest);
                    isUpdated =
                            true; // if is updated (i.e. an expansion happened) a new learning node
                    // should be created
                }
            }
        }
    ...
    public String printRule() {
        ...
        StringUtils.appendIndented(
                ...
                "Rule Nr."
                        + this.ruleNumberID
                        + " Instances seen:"
                        + this.learningNode.getInstancesSeen()
                        + "\n"); // AC
        for (RuleSplitNode node : nodeList) {
        ...
        if (this.learningNode instanceof RuleActiveRegressionNode) {
            if (((RuleActiveRegressionNode) this.learningNode).perceptron != null) {
                ((RuleActiveRegressionNode) this.learningNode)
                        .perceptron.getModelDescription(out, 0);
                StringUtils.appendNewline(out);
            }
        }
        return (out.toString());
    ...
    public boolean isAnomaly(
        ...
        return this.learningNode.isAnomaly(
                instance,
