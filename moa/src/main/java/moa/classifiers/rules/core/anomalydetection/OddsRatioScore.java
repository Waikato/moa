 ...
 *
 * <p>thresholdOption - The threshold value for detecting anomalies minNumberInstancesOption - The
 * minimum number of instances required to perform anomaly detection probabilityFunctionOption -
...
public class OddsRatioScore extends AbstractAnomalyDetector {
    ...
    public boolean updateAndCheckAnomalyDetection(MultiLabelInstance instance) {
        ...
        if (probabilityFunction == null) {
            ...
            probabilityFunction =
                    (ProbabilityFunction) getPreparedClassOption(probabilityFunctionOption);
            // free memory
            ...
            for (int i = 0; i < instance.numInputAttributes(); i++) {
                ...
                if (instance.attribute(i).isNumeric()) {
                    ...
                    if (sd > MINSTD) {
                        ...
                        else if (prob < MINPROB) prob = MINPROB;
                        anomaly += Math.log(prob) - Math.log(1 - prob);
                        ct++;
                    }
                }
            }
            if (ct > 0) anomaly = anomaly / ct;
            ...
            for (int i = 0; i < instance.numInputAttributes(); i++) {
                ...
                if (instance.attribute(i).isNumeric()) {
                    ...
                    if (stats != null) {
                        ...
                        stats =
                                new double[] {
                                    instance.weight() * val, instance.weight() * val * val
                                };
    ...
    protected void printAnomaly(Instance inst, double anomaly) {
        ...
        for (int i = 0; i < inst.numInputAttributes(); i++) {
            if (inst.attribute(i).isNumeric()) {
                ...
                double prob =
                        probabilityFunction.getProbability(
                                mean, Utils.computeSD(stats[1], stats[0], weightSeen), val);
                // Mean
