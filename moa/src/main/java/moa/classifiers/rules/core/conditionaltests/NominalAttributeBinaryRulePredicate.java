 ...
 * @version $Revision: 7 $
 */
public class NominalAttributeBinaryRulePredicate extends NominalAttributeBinaryTest
    ...
    public boolean evaluate(Instance inst) {
        if (state) return (branchForInstance(inst) == 0);
        else return (branchForInstance(inst) != 0);
    }
    ...
    public void getDescription(StringBuilder sb, int indent) {
        if (state) StringUtils.appendIndented(sb, indent + 1, "In" + attIndex + " = " + attValue);
        else StringUtils.appendIndented(sb, indent + 1, "In" + attIndex + " <> " + attValue);
    }
