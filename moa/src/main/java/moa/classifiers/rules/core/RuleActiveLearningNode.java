...
public abstract class RuleActiveLearningNode extends ActiveLearningNode {
    ...
    protected AttributeClassObserver newNumericClassObserver() {
        ...
        return (AttributeClassObserver)
                ((AttributeClassObserver)
                                this.amRules.numericObserverOption.getPreMaterializedObject())
                        .copy();
    }
    ...
    public double computeProbability(double mean, double sd, double value) {
        ...
        if (sd > 0.0) {
            double k =
                    (Math.abs(value - mean) / sd); // One tailed variant of Chebyshev's inequality
