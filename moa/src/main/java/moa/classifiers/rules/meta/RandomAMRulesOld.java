...
public class RandomAMRulesOld extends AbstractClassifier implements Regressor {
    ...
    public ClassOption baseLearnerOption =
            new ClassOption(
                    ...
                    'l',
                    "Classifier to train.",
                    AbstractAMRules.class,
                    AMRulesRegressorOld.class.getName());
    ...
    public MultiChoiceOption votingTypeOption =
            new MultiChoiceOption(
                    "votingTypeOption",
                    'C',
                    "Select whether the base learner error is computed as the overall error os only"
                            + " the error of the rules that cover the example.",
                    new String[] {"Overall", "Only rules covered"},
    ...
    public FloatOption fadingErrorFactorOption =
            new FloatOption(
                    "fadingErrorFactor",
                    'e',
                    "Fading error factor for the accumulated error",
                    0.99,
                    0,
                    1);
    ...
    public void resetLearningImpl() {
        ...
        AbstractAMRules baseLearner =
                (AbstractAMRules) getPreparedClassOption(this.baseLearnerOption);
    ...
    public void trainOnInstanceImpl(Instance instance) {
        ...
        for (int i = 0; i < this.ensemble.length; i++) {
            ...
            if (k > 0) {
                ...
                // estimate error
                double error =
                        Math.abs(inst.classValue() - ensemble[i].getVotesForInstance(inst)[0]);
    ...
    public double[] getVotesForInstance(Instance inst) {
        ...
        ErrorWeightedVote combinedVote =
                (ErrorWeightedVote)
                        ((ErrorWeightedVote) getPreparedClassOption(this.votingFunctionOption))
                                .copy();
    ...
    protected Measurement[] getModelMeasurementsImpl() {
        Measurement[] baseLearnerMeasurements =
                ((AbstractAMRules) getPreparedClassOption(this.baseLearnerOption))
