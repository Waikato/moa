...
/**
 * This classifier learn ordered and unordered rule set from data stream. This algorithm also does
 * the detection of anomalies.
 *
 * <p>Learning Decision RuleClassifications from Data Streams, IJCAI 2011, J. Gama, P. Kosina
 *
 * <p>Parameters:
 *
 * <ul>
 *   <li>-p: Minimum value of p
 *   <li>-t: Tie Threshold
 *   <li>-c: Split Confidence
 *   <li>-g: GracePeriod, the number of instances a leaf should observe between split attempts
 *   <li>-o: Prediction function to use. Ex:FirstHit
 *   <li>-r: Learn ordered or unordered rule
 * </ul>
 *
 * @author P. Kosina, E. Almeida, J. Gama
 * @version $Revision: 2 $
...
public class RuleClassifier extends AbstractClassifier implements MultiClassClassifier {
    ...
    protected DoubleVector saveBestEntropy =
            new DoubleVector(); // Saves the best value of entropy, cut_Point and symbol.

    protected DoubleVector saveBestEntropyNominalAttrib =
            new DoubleVector(); // Saves the best value of entropy and their cut_Point.

    protected DoubleVector ruleClassIndex =
            new DoubleVector(); // The index of the class for each rule.

    protected DoubleVector saveBestGlobalEntropy = new DoubleVector();

    protected ArrayList<ArrayList<Double>> saveBestValGlobalEntropy =
            new ArrayList<
                    ArrayList<
                            Double>>(); // For each attribute contains the best value of entropy and
    // its cutPoint.

    protected ArrayList<Double> saveTheBest =
            new ArrayList<Double>(); // Contains the best attribute.
    ...
    protected ArrayList<RuleClassification> ruleSetAnomaliesSupervised =
            new ArrayList<RuleClassification>();

    protected ArrayList<Integer> ruleAnomaliesIndexSupervised = new ArrayList<Integer>();

    protected ArrayList<ArrayList<Integer>> caseAnomalySupervised =
    ...
    public FloatOption PminOption =
            new FloatOption(
                    "Pmin",
                    'p',
                    "Percentage of the total number of example seen in the node.",
                    0.1,
                    0.0,
                    1.0);
    ...
    public FloatOption anomalyProbabilityThresholdOption =
            new FloatOption(
                    "anomalyprobabilityThreshold", 'o', "The threshold value.", 0.99, 0.0, 1.0);

    public FloatOption probabilityThresholdOption =
            new FloatOption("probabilityThreshold", 'k', "The threshold value.", 0.10, 0.0, 1.0);
    ...
    public MultiChoiceOption predictionFunctionOption =
            new MultiChoiceOption(
                    ...
                    new String[] {"firstHit", "weightedSum", "weightedMax"},
                    new String[] {"first Hit", "weighted Sum", "weighted Max"},
                    0);

    public FlagOption orderedRulesOption = new FlagOption("orderedRules", 'r', "orderedRules.");

    public FlagOption anomalyDetectionOption =
            new FlagOption("anomalyDetection", 'u', "anomaly Detection.");
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        for (int j = 0; j < ruleSet.size(); j++) {
            if (this.ruleSet.get(j).ruleEvaluate(inst) == true) {
                ...
                if (this.Supervised.isSet()) {
                    anomaly =
                            computeAnomalySupervised(
                                    this.ruleSet.get(j),
                                    j,
                                    inst); // compute anomaly (Supervised method)
                } else if (this.Unsupervised.isSet()) {
                    anomaly =
                            computeAnomalyUnsupervised(
                                    this.ruleSet.get(j),
                                    j,
                                    inst); // compute anomaly (Unsupervised method)
                }
                ...
                if ((this.ruleSet.get(j).instancesSeen
                    ...
                    this.ruleSet
                            .get(j)
                            .obserClassDistrib
                            .addToValue((int) inst.classValue(), inst.weight());
                        ...
                        if (!inst.isMissing(instAttIndex)) {
                            AttributeClassObserver obs =
                                    this.ruleSet
                                            .get(j)
                                            .observers
                                            .get(i); // Nominal and binary tree.
                            AttributeClassObserver obsGauss =
                                    this.ruleSet.get(j).observersGauss.get(i); // Gaussian.
                            if (obs == null) {
                                obs =
        ...
        if (ruleFired
                == false) { // If none of the rules cover the example update sufficient statistics
            // of the default rule
    ...
    public void getModelDescription(StringBuilder out, int indent) {
        if (this.anomalyDetectionOption.isSet()) {
            if (this.Supervised.isSet()) {
                this.printAnomaliesSupervised(
                        out, indent); // Get Model Description (Supervised method)
            } else if (this.Unsupervised.isSet()) {
                this.printAnomaliesUnsupervised(
                        out, indent); // Get Model Description (Unsupervised method)
            }
        } else {
            this.getModelDescriptionNoAnomalyDetection(
                    out, indent); // Get Model Description no Anomaly detection
        }
    }

    public void printAnomaliesUnsupervised(
            StringBuilder out, int indent) { // Get Model Description (Unsupervised method)

        StringUtils.appendNewline(out);
        StringUtils.appendIndented(
                out, indent, "**********************UNSUPERVISED*****************");
        StringUtils.appendNewline(out);
        StringUtils.appendNewline(out);
        for (int k = 0; k < this.ruleSetAnomalies.size(); k++) {
            ...
            for (int i = 0; i < this.ruleSetAnomalies.get(k).predicateSet.size(); i++) {
                if (this.ruleSetAnomalies.get(k).predicateSet.size() == 1) {
                                                ...
                                                .value(
                                                        (int)
                                                                getRuleMajorityClassIndex(
                                                                        this.ruleSetAnomalies.get(
                                                                                k))));
                        StringUtils.appendNewline(out);
                    } else if (this.ruleSetAnomalies.get(k).predicateSet.get(i).getSymbol()
                            == -1.0) {
                        String nam =
                                this.instance
                                        .attribute(
                        ...
                        StringUtils.appendIndented(
                                ...
                                nam
                                        ...
                                        + this.instance
                                                ...
                                                .value(
                                                        (int)
                                                                ruleClassIndex.getValue(
                                                                        ruleAnomaliesIndex.get(k)
                                                                                - 1)));
                        StringUtils.appendNewline(out);
                    } else {
                        String nam =
                    ...
                    if (i < this.ruleSetAnomalies.get(k).predicateSet.size() - 1) {
                        ...
                        int count =
                                getCountNominalAttrib(this.ruleSetAnomalies.get(k).predicateSet);
                        if ((this.ruleSetAnomalies.get(k).predicateSet.get(i).getSymbol() == 0.0)
                                || (count != 0)) {
                            StringUtils.appendIndented(
                                    out,
            ...
            for (int z = 0; z < this.ruleAttribAnomalyStatistics.get(k).size(); z++) {
                if (this.ruleAttribAnomalyStatistics.get(k).get(z).size() == 5) {
                    String s =
                            String.format(
                                    "%.3e", this.ruleAttribAnomalyStatistics.get(k).get(z).get(4));
                    StringUtils.appendIndented(
                            out,
                            indent,
                            instance.attribute(
                                                    this.ruleAttribAnomalyStatistics
                                                            .get(k)
                                                            .get(z)
                                                            .get(0)
                                                            .intValue())
                                    ...
                                    + " +- "
                                    + round(this.ruleAttribAnomalyStatistics.get(k).get(z).get(3))
                                    + ")   P="
                                    + s);
                    StringUtils.appendNewline(out);
                } else {
                    String s =
                            String.format(
                                    "%.3e", this.ruleAttribAnomalyStatistics.get(k).get(z).get(2));
                    String val =
                            this.instance
                                    .attribute(
                                            this.ruleAttribAnomalyStatistics
    ...
    public void printAnomaliesSupervised(
            StringBuilder out, int indent) { // Get Model Description (Supervised method)

        StringUtils.appendNewline(out);
        StringUtils.appendIndented(
                out, indent, "************************SUPERVISED*******************");
        StringUtils.appendNewline(out);
        StringUtils.appendNewline(out);
        for (int k = 0; k < this.ruleSetAnomaliesSupervised.size(); k++) {
            StringUtils.appendIndented(
                    ...
                    indent,
                    "Case: "
                            + this.caseAnomalySupervised.get(k).get(0)
                            + "   Anomaly Score: "
                            + this.caseAnomalySupervised.get(k).get(1)
                            + "%");
            StringUtils.appendNewline(out); // Ver outra parte default
            StringUtils.appendIndented(
                    out, indent, "Rule " + this.ruleAnomaliesIndexSupervised.get(k) + ": ");
            for (int i = 0; i < this.ruleSetAnomaliesSupervised.get(k).predicateSet.size(); i++) {
                if (this.ruleSetAnomaliesSupervised.get(k).predicateSet.size() == 1) {
                    if (this.ruleSetAnomaliesSupervised.get(k).predicateSet.get(i).getSymbol()
                            == 0.0) {
                        String nam =
                                this.instance
                                        .attribute(
                                                (int)
                                                        this.ruleSetAnomaliesSupervised
                                                                .get(k)
                                                                .predicateSet
                                                                .get(i)
                                                                .getAttributeValue())
                                                        ...
                                                        (int)
                                                                getRuleMajorityClassIndex(
                                                                        this
                                                                                ...
                                                                                .get(k))));
                        StringUtils.appendNewline(out);
                    } else if (this.ruleSetAnomaliesSupervised
                                    .get(k)
                                    .predicateSet
                                    .get(i)
                                    .getSymbol()
                    ...
                    if (i < this.ruleSetAnomaliesSupervised.get(k).predicateSet.size() - 1) {
                        ...
                        int count =
                                getCountNominalAttrib(
                                        this.ruleSetAnomaliesSupervised.get(k).predicateSet);
                        if ((this.ruleSetAnomaliesSupervised.get(k).predicateSet.get(i).getSymbol()
                                        == 0.0)
                                || (count != 0)) {
                            StringUtils.appendIndented(
            ...
            for (int z = 0; z < this.ruleAttribAnomalyStatisticsSupervised.get(k).size(); z++) {
                if (this.ruleAttribAnomalyStatisticsSupervised.get(k).get(z).size() == 5) {
                    String s =
                            String.format(
                                    "%.3e",
                                    this.ruleAttribAnomalyStatisticsSupervised
                                            .get(k)
                                            .get(z)
                                            .get(4));
                    StringUtils.appendIndented(
    ...
    public void getModelDescriptionNoAnomalyDetection(
            StringBuilder out, int indent) { // Get Model Description no Anomaly detection
        StringUtils.appendNewline(out);
                ...
                if (this.ruleSet.get(k).predicateSet.size() == 1) {
                    if (this.ruleSet.get(k).predicateSet.get(i).getSymbol() == 0.0) {
                        ...
                        StringUtils.appendIndented(
                                ...
                                nam
                                        + " <= "
                                        + this.ruleSet.get(k).predicateSet.get(i).getValue()
                                        + " --> "
                                        + this.instance
                                                .classAttribute()
                                                .value((int) ruleClassIndex.getValue(k)));
                        StringUtils.appendNewline(out);
                    } else {
                        String nam =
                                this.instance
                        ...
                        StringUtils.appendIndented(
                                ...
                                nam
                                        ...
                                        + this.instance
                                                .classAttribute()
                                                .value((int) this.ruleClassIndex.getValue(k)));
                        StringUtils.appendNewline(out);
                    }
                } else {
                    if (this.ruleSet.get(k).predicateSet.get(i).getSymbol() == 0.0) {
    ...
    public void updateRuleAttribStatistics(Instance inst, RuleClassification rl, int ruleIndex) {
        rl.instancesSeen++;
        if (rl.squaredAttributeStatisticsSupervised.size() == 0
                && rl.attributeStatisticsSupervised.size() == 0) {
    ...
    public double computeAnomalyUnsupervised(
            RuleClassification rl, int ruleIndex, Instance inst) { // Unsupervised
        ArrayList<Integer> caseAnomalyTemp = new ArrayList<Integer>();
        ArrayList<ArrayList<Double>> AttribAnomalyStatisticTemp2 =
                ...
                if (!inst.isMissing(x)) {
                    ...
                    if (inst.attribute(x).isNumeric()) { // Numeric Attributes
                        if ((rl.instancesSeen - rl.attributeMissingValues.getValue(x)) > 30) {
                            ...
                            double sd =
                                    computeSD(
                                            rl.squaredAttributeStatistics.getValue(x),
                                            rl.attributeStatistics.getValue(x),
                                            rl.instancesSeen);
                            double probability = computeProbability(mean, sd, inst.value(x));
                            if (probability != 0.0) {
                                D = D + Math.log(probability);
                                if (probability
                                        < this.probabilityThresholdOption.getValue()) { // 0.10
                        ...
                        AutoExpandVector<DoubleVector> attribNominal =
                                ((NominalAttributeClassObserver) rl.observers.get(x))
                                        .attValDistPerClass; // Attributes values distribution per
                        // class
                        ...
                        for (int i = 0; i < attribNominal.size(); i++) {
                            if (attribNominal.get(i) != null) {
                                numbAttribValue =
                                        numbAttribValue
                                                + attribNominal.get(i).getValue((int) attribVal);
                            }
                        }
    ...
    // Compute anomalies unsupervised
    public double computeAnomalySupervised(
            RuleClassification rl, int ruleIndex, Instance inst) { // Not supervised
        ArrayList<Integer> caseAnomalyTemp = new ArrayList<Integer>();
        ArrayList<ArrayList<Double>> AttribAnomalyStatisticTemp2 =
                new ArrayList<ArrayList<Double>>();
        double D = 0.0;
                    ...
                    if (inst.attribute(x).isNumeric()) { // Numeric Attributes
                        if ((rl.instancesSeen - rl.attributeMissingValues.getValue(x)) > 30) {
                            double mean =
                                    computeMean(
                                            (double)
                                                    rl.attributeStatisticsSupervised
                                                            .get(x)
                                                            .get((int) inst.classValue()),
                                            (int)
                                                    rl.obserClassDistrib.getValue(
                                                            (int) inst.classValue()));
                            double sd =
                                    computeSD(
                                            (double)
    ...
    public void createRule(Instance inst) {
        ...
        if (remainder == 0) {
            ...
            if (HB == true) {
                ...
                Rl.predicateSet.add(pred);
                this.ruleSet.add(Rl);
                if (Rl.predicateSet.get(0).getSymbol() == -1.0
                        || Rl.predicateSet.get(0).getSymbol() == 1.0) {
    ...
    public void expandeRule(RuleClassification rl, Instance inst, int ruleIndex) {
        int remainder = (int) Double.MAX_VALUE;
        int numInstanciaObservers =
                (int)
                        rl.obserClassDistrib
                                .sumOfValues(); // Number of instances for this rule observers.
        ...
        if (remainder == 0) {
            ...
            boolean HB =
                    checkBestAttrib(
                            numInstanciaObservers,
                            rl.observers,
                            rl.obserClassDistrib); // Check if the best attribute value is really
            // the best.
            if (HB == true) {
                double attributeValue = this.saveTheBest.get(3);
                double symbol = this.saveTheBest.get(2); // =, <=, > (0.0, -1.0, 1.0).
                ...
                int countPred = 0;
                for (int i = 0;
                        i < rl.predicateSet.size();
                        i++) { // Checks if the new predicate is not yet in the predicateSet.
                    if (this.pred.getSymbol() == 0.0) { // Nominal Attribute.
                        if (rl.predicateSet.get(i).getAttributeValue()
                                != this.pred.getAttributeValue()) {
                    ...
                    for (int f = 0; f < this.ruleSet.size(); f++) {
                        ...
                        if (this.ruleSet.get(f).predicateSet.size() == predicSetTemp.size()) {
                            for (int x = 0; x < this.ruleSet.get(f).predicateSet.size(); x++) {
                                if (this.ruleSet.get(f).predicateSet.get(x).getAttributeValue()
                                                == predicSetTemp.get(x).getAttributeValue()
                                        && this.ruleSet.get(f).predicateSet.get(x).getSymbol()
                                                == predicSetTemp.get(x).getSymbol()
                                        && this.ruleSet.get(f).predicateSet.get(x).getValue()
                                                == predicSetTemp.get(x).getValue()) {
                    ...
                    if (countDifPred == this.ruleSet.size()) {
                        if (this.pred.getSymbol() == 0.0) {
                            ...
                            for (int f = 0; f < rl.predicateSet.size(); f++) {
                                if (this.pred.getAttributeValue()
                                                == rl.predicateSet.get(f).getAttributeValue()
                                        && this.pred.getSymbol()
                                                == rl.predicateSet.get(f).getSymbol()) {
    ...
    public void theBestAttributes(
            Instance instance, AutoExpandVector<AttributeClassObserver> observersParameter) {
        for (int z = 0; z < instance.numAttributes() - 1; z++) {
            if (!instance.isMissing(z)) {
                ...
                if (instance.attribute(instAttIndex).isNominal()) {
                    this.minEntropyNominalAttrib = Double.MAX_VALUE;
                    AutoExpandVector<DoubleVector> attribNominal =
                            ((NominalAttributeClassObserver) observersParameter.get(z))
                                    .attValDistPerClass;
                    findBestValEntropyNominalAtt(
                            attribNominal,
                            instance.attribute(z)
                                    .numValues()); // The best value (lowest entropy) of a nominal
                    // attribute.
                    ...
                    this.saveBestGlobalEntropy.setValue(
                            z, this.saveBestEntropyNominalAttrib.getValue(1));
                } else {
                    this.root =
                            ((BinaryTreeNumericAttributeClassObserver) observersParameter.get(z))
                                    .root;
                    mainFindBestValEntropy(
                            this.root); // The best value (lowest entropy) of a numeric attribute.
    ...
    public void findBestValEntropy(
            Node node,
        ...
        if (this.root != null) {
            if (node != null) {
                ...
                if (node.cut_point != this.root.cut_point) {
                    for (int i = 0; i < numClass; i++) {
                        if (status == true) { // Left node.
                            double parentss =
                                    parentCCL.getValue(i)
                                            - (node.classCountsLeft.getValue(i)
                                                    + node.classCountsRight.getValue(i));
                            double left =
                                    classCountL.getValue(i)
                                            - node.classCountsRight.getValue(i)
                                            - parentss;
                            double right =
                                    classCountR.getValue(i)
                                            + node.classCountsRight.getValue(i)
                                            + parentss;
                        ...
                        if (status == false) { // Right node.
                            double left =
                                    classCountL.getValue(i) + node.classCountsLeft.getValue(i);
                            double right =
                                    classCountR.getValue(i) - node.classCountsLeft.getValue(i);
                            classCountLTemp.addToValue(i, left);
                            classCountRTemp.addToValue(i, right);
                        }
                    }
                ...
                double numInst =
                        this.root.classCountsLeft.sumOfValues()
                                + this.root.classCountsRight.sumOfValues();
                ...
                if ((classCountLSum > this.PminOption.getValue() * numInst)
                    ...
                    double classCountLEntropy =
                            entropy(classCountLTemp); // Entropy of class count left.
                    double classCountREntropy =
                            entropy(classCountRTemp); // Entropy of class count right
                    if (((classCountLSum / numInst) * classCountLEntropy
                                    + (classCountRSum / numInst) * classCountREntropy)
                            <= minEntropy) {
                        this.minEntropyTemp =
                                (classCountLSum / numInst) * classCountLEntropy
                                        + (classCountRSum / numInst) * classCountREntropy;
    ...
    public void mainFindBestValEntropy(Node root) {
        if (root != null) {
            ...
            double numInst =
                    root.classCountsLeft.sumOfValues() + root.classCountsRight.sumOfValues();
            ...
            findBestValEntropy(
                    root, classCountL, classCountR, true, this.minEntropyTemp, parentClassCL);
        }
    }

    // Find best value of entropy for nominal attributes
    public void findBestValEntropyNominalAtt(
            AutoExpandVector<DoubleVector> attrib, int attNumValues) {
    ...
    public boolean checkBestAttrib(
            double n,
        ...
        double hoeffdingBound =
                ComputeHoeffdingBound(range, this.splitConfidenceOption.getValue(), n);
        if ((h0 > bestEntropy)
                && ((secondBestEntropy - bestEntropy > hoeffdingBound)
    ...
    protected double[] firstHit(Instance inst) {
        ...
        for (int j = 0; j < this.ruleSet.size(); j++) {
            if (this.ruleSet.get(j).ruleEvaluate(inst) == true) {
                ...
                for (int z = 0; z < this.numClass; z++) {
                    votes[z] =
                            this.ruleSet.get(j).obserClassDistrib.getValue(z)
