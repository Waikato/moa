...
public class TargetMean extends AbstractClassifier implements AMRulesRegressorFunction {
    ...
    public FloatOption fadingErrorFactorOption =
            new FloatOption(
                    "fadingErrorFactor",
                    'e',
                    "Fading error factor for the TargetMean accumulated error",
                    0.99,
                    0,
                    1);
    ...
    public double[] getVotesForInstance(Instance inst) {
        ...
        if (n > 0) currentMean[0] = sum / n;
        else currentMean[0] = 0;
        return currentMean;
    }
    ...
    protected void updateAccumulatedError(Instance inst) {
        ...
        if (n > 0) mean = sum / n;
        errorSum =
                Math.abs(inst.classValue() - mean) * inst.weight() + fadingErrorFactor * errorSum;
    }
    ...
    public double getCurrentError() {
        if (this.nError > 0) return this.errorSum / this.nError;
