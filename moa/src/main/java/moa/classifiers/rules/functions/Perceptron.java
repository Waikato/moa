...
public class Perceptron extends AbstractClassifier implements AMRulesRegressorFunction {
    ...
    private final double SD_THRESHOLD =
            0.0000001; // THRESHOLD for normalizing attribute and target values
    ...
    public FloatOption learningRateDecayOption =
            new FloatOption(
                    "learningRateDecay",
                    'm',
                    " Learning Rate decay to use for training the Perceptron.",
                    0.001);
    ...
    public FloatOption fadingFactorOption =
            new FloatOption(
                    "fadingFactor",
                    'e',
                    "Fading factor for the Perceptron accumulated error",
                    0.99,
                    0,
                    1);
    ...
    public Perceptron(Perceptron p) {
        ...
        if (p.weightAttribute != null) this.weightAttribute = p.weightAttribute.clone();
        ...
        this.squaredperceptronattributeStatistics =
                new DoubleVector(p.squaredperceptronattributeStatistics);
    ...
    public void trainOnInstanceImpl(Instance inst) {
        accumulatedError =
                Math.abs(this.prediction(inst) - inst.classValue()) * inst.weight()
        ...
        if (this.initialisePerceptron == true) {
            ...
            for (int i = 0; i < inst.numAttributes(); i++)
                if (inst.attribute(i).isNumeric() && i != inst.classIndex()) numericIndices.add(i);
            ...
            this.initialisePerceptron =
                    false; // not in resetLearningImpl() because it needs Instance!
        ...
        if (constantLearningRatioDecayOption.isSet() == false) {
            learningRatio =
                    learningRatioOption.getValue()
                            / (1 + perceptronInstancesSeen * learningRateDecay);
        }
    ...
    private double denormalizedPrediction(double normalizedPrediction) {
        if (this.initialisePerceptron == false) {
            ...
            if (sdY > SD_THRESHOLD) return normalizedPrediction * sdY + meanY;
            else return normalizedPrediction + meanY;
        } else
            return normalizedPrediction; // Perceptron may have been "reseted". Use old weights to
        // predict
    }
    ...
    public void updateWeights(Instance inst, double learningRatio) {
        ...
        for (int j = 0; j < numericAttributesIndex.length; j++) {
            ...
            this.weightAttribute[j] +=
                    learningRatio * delta * normalizedInstance[j] * inst.weight();
            sumWeights += Math.abs(this.weightAttribute[j]);
            // }
        }
        this.weightAttribute[numericAttributesIndex.length] +=
                learningRatio * delta * inst.weight();
        ...
        if (sumWeights > numericAttributesIndex.length) { // Lasso regression
            for (int j = 0; j < numericAttributesIndex.length; j++) {
                // int instAttIndex = modelAttIndexToInstanceAttIndex(numericAttributesIndex[j],
                // inst);
                // if(inst.attribute(instAttIndex).isNumeric()) {
            ...
            this.weightAttribute[numericAttributesIndex.length] =
                    this.weightAttribute[numericAttributesIndex.length] / sumWeights;
        }
    ...
    public double getCurrentError() {
        if (nError > 0) return accumulatedError / nError;
