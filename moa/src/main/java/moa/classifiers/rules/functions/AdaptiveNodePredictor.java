...
public class AdaptiveNodePredictor extends AbstractClassifier implements AMRulesRegressorFunction {
    ...
    public double getCurrentError() {
        if (p.getCurrentError() < tm.getCurrentError()) return p.getCurrentError();
        else return tm.getCurrentError();
    }
    ...
    public double[] getVotesForInstance(Instance inst) {
        double[] ret = null;
        if (hasStarted) {
            if (p.getCurrentError() < tm.getCurrentError()) ret = p.getVotesForInstance(inst);
            else ret = tm.getVotesForInstance(inst);
        } else ret = new double[] {0};
        return ret;
    }
    ...
    public String getPurposeString() {
        return "Returns the prediction of a perceptron or the target mean, according to the current"
                + " mean absolute error.";
    }
}
