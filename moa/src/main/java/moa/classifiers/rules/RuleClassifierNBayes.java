    ...
    protected double[] firstHitNB(Instance inst) {
        ...
        for (int j = 0; j < this.ruleSet.size(); j++) {
            if (this.ruleSet.get(j).ruleEvaluate(inst) == true) {
                ...
                if (this.ruleSet.get(j).obserClassDistrib.sumOfValues()
                        >= this.nbThresholdOption.getValue()) {
                    votes =
                            NaiveBayes.doNaiveBayesPredictionLog(
                                    inst,
                                    this.ruleSet.get(j).obserClassDistrib,
                                    this.ruleSet.get(j).observers,
                                    this.ruleSet.get(j).observersGauss);
