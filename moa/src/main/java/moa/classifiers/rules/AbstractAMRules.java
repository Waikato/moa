 ...
 * @version $Revision: 2 $*
 *     <p>This algorithm learn ordered and unordered rule set from data stream. Each rule in AMRules
 *     use a Page-Hinkley test to detect changes in the processing generating data and react to
 *     changes by pruning the rule set. This algorithm also does the detection of anomalies.
 */
...
public abstract class AbstractAMRules extends AbstractClassifier {
    ...
    public FlagOption DriftDetectionOption =
            new FlagOption("DoNotDetectChanges", 'H', "Drift Detection. Page-Hinkley.");
    ...
    public IntOption pageHinckleyThresholdOption =
            new IntOption(
                    ...
                    'l',
                    "The threshold value (Lambda) to be used in the Page Hinckley change detection"
                            + " tests.",
                    35,
                    0,
                    Integer.MAX_VALUE);
    public FlagOption noAnomalyDetectionOption =
            new FlagOption("noAnomalyDetection", 'A', "Disable anomaly Detection.");
    ...
    /**
     * description of the Methods used. isRandomizable resetLearningImpl newRule // to build an
     * object with the parameters. trainOnInstanceImpl isAnomaly getVotesForInstance
     * getModelMeasurementsImpl getModelDescription // to printout to MOA GUI debug // use
     * debug('string') to printout to console
     */
    @Override
    public abstract boolean isRandomizable();

    /**
     * Rule.Builder() to build an object with the parameters. If you have an algorithm with many
     * parameters, especially if some of them are optional, it can be beneficial to define an object
     * that represents all of the parameters.
     *
     * @return
     */
    protected abstract Rule newRule(
            int ID,
            RuleActiveLearningNode learningNode,
            double[] statistics); // Learning node and statistics can be null
        ...
        if (rulesCoveringInstance == false) {
            ...
            if (defaultRule.getInstancesSeen() % this.gracePeriodOption.getValue() == 0.0) {
                ...
                if (defaultRule.tryToExpand(
                    ...
                    Rule newDefaultRule =
                            newRule(
                                    defaultRule.getRuleNumberID(),
                                    defaultRule.getLearningNode(),
                                    defaultRule
                                            .getLearningNode()
                                            .getStatisticsOtherBranchSplit()); // other branch
                    defaultRule.split();
     ...
     * moa.classifier.java returns the prediction of the instance. Called in EvaluateModelRegression
     */
    @Override
    ...
    public abstract RuleActiveLearningNode newRuleActiveLearningNode(Builder builder);

    public abstract RuleActiveLearningNode newRuleActiveLearningNode(
     ...
     * the prediction of the instance. Called in WeightedRandomRules
     */
