...
public class MultiLabelRandomAMRules extends AbstractMultiLabelLearner
    ...
    public ClassOption baseLearnerOption =
            new ClassOption(
                    ...
                    'l',
                    "Classifier to train.",
                    AMRulesMultiLabelLearner.class,
    ...
    public MultiChoiceOption votingTypeOption =
            new MultiChoiceOption(
                    "votingTypeOption",
                    'C',
                    "Select whether the base learner error is computed as the overall error or only"
                            + " the error of the rules that cover the example.",
                    new String[] {"Overall (Static)", "Only rules covered (Dynamic)"},
                    new String[] {"Overall", "Covered"},
                    0);
    ...
    public ClassOption errorMeasurerOption =
            new ClassOption(
                    ...
                    AbstractMultiTargetErrorMeasurer.class,
    ...
    public void resetLearningImpl() {
        ...
        AMRulesMultiLabelLearner baseLearner =
                (AMRulesMultiLabelLearner) getPreparedClassOption(this.baseLearnerOption);
        MultiLabelErrorMeasurer measurer =
                (MultiLabelErrorMeasurer) getPreparedClassOption(this.errorMeasurerOption);
    ...
    public void trainOnInstanceImpl(MultiLabelInstance instance) {
        ...
        for (int i = 0; i < this.ensemble.length; i++) {
            ...
            if (k > 0) {
                ...
                if (p != null) errorMeasurer[i].addPrediction(p, inst);
                // train learner
    ...
    public Prediction getPredictionForInstance(MultiLabelInstance inst) {
        ...
        ErrorWeightedVoteMultiLabel combinedVote =
                (ErrorWeightedVoteMultiLabel)
                        ((ErrorWeightedVoteMultiLabel)
                                        getPreparedClassOption(this.votingFunctionOption))
                                .copy();
        StringBuilder sb = null;
        if (VerbosityOption.getValue() > 1) sb = new StringBuilder();

        for (int i = 0; i < this.ensemble.length; i++) {
            // transformInstance method visibility changed from private to protected in RandomRules
            ErrorWeightedVoteMultiLabel v =
                    ((AMRulesMultiLabelLearner) this.ensemble[i]).getVotes(inst);
    ...
    protected Measurement[] getModelMeasurementsImpl() {
        ...
        int numMeasurements;
        if (featureRanking instanceof NoFeatureRanking) numMeasurements = nMeasurements + 1;
        else numMeasurements = nMeasurements + nAttributes + 1;
        ...
        if (this.ensemble != null) {
            ...
            for (int i = 0; i < nMeasurements; i++) {
                ...
                m[i + 1] =
                        new Measurement(
                                "Avg " + baseLearnerMeasurements[i].getName(),
                                value / ensembleSize);
            }
        } else {
        ...
        if (!(featureRanking instanceof NoFeatureRanking)) {
            ...
            for (int i = 0; i < nAttributes; i++) {
                double importance = 0;
                if (rankings != null) importance = rankings.getValue(i);
