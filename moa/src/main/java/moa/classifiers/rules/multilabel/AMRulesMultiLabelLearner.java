 ...
 * @version $Revision: 2$*
 *     <p>This algorithm learns ordered and unordered rule set from data streams. Each rule detect
 *     anomalies and react to changes by pruning the rule set. This algorithm also does the
 *     detection of anomalies.
 */
public abstract class AMRulesMultiLabelLearner extends AbstractMultiLabelLearner
    ...
    public Prediction getPredictionForInstance(MultiLabelInstance inst) {
        ...
        if (vote != null) {
            if ((MultiLabelLearner) getPreparedClassOption(this.learnerOption)
                    instanceof
    ...
    public void trainOnInstanceImpl(MultiLabelInstance instance) {
         ...
         * AMRules Algorithm
         *
         * <p>//For each rule in the rule set //If rule covers the instance //if the instance is not
         * an anomaly //Update Change Detection Tests //Compute prediction error //Call PHTest //If
         * change is detected then //Remove rule //Else //Update sufficient statistics of rule //If
         * number of examples in rule > Nmin //Expand rule //If ordered set then //break //If none
         * of the rule covers the instance //Update sufficient statistics of default rule //If
         * number of examples in default rule is multiple of Nmin //Expand default rule and add it
         * to the set of rules //Reset the default rule
         */
            ...
            if (rule.isCovering(instance) == true) {
                ...
                if (!rule.updateAnomalyDetection(instance)) {
                    if (rule.updateChangeDetection(instance)) {
                        ...
                        if (rule.getWeightSeenSinceExpansion() % this.gracePeriodOption.getValue()
                                == 0.0) {
                            if (rule.tryToExpand(
