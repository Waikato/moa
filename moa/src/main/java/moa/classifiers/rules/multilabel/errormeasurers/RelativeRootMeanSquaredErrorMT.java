...
public class RelativeRootMeanSquaredErrorMT extends AbstractMultiTargetErrorMeasurer {
    ...
    public void addPrediction(Prediction prediction, Prediction trueClass, double weight) {
        ...
        if (!hasStarted) {
            ...
            for (int i = 0; i < numOutputs; i++)
                if (prediction.hasVotesForAttribute(i)) ++numLearnedOutputs;
            hasStarted = true;
        }
        ...
        for (int i = 0; i < numOutputs; i++) {
            if (prediction.hasVotesForAttribute(i)) {
                sumY[i] =
                        trueClass.getVote(i, 0) * weight
                                + fadingErrorFactor * sumY[i]; // sum target
                double errorOutput = prediction.getVote(i, 0) - trueClass.getVote(i, 0);
                double errorOutputTM =
                        prediction.getVote(i, 0) - sumY[i] / weightSeen; // error to target mean
                sumSquaredError[i] =
                        errorOutput * errorOutput * weight + fadingErrorFactor * sumSquaredError[i];
                sumSquaredErrorToTargetMean[i] =
                        errorOutputTM * errorOutputTM * weight
                                + fadingErrorFactor * sumSquaredErrorToTargetMean[i];
            }
        }
    }
    ...
    public double getCurrentError() {
        if (weightSeen == 0) return Double.MAX_VALUE;
        else {
            double sum = 0;
