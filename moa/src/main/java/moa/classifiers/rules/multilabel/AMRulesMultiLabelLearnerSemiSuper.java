...
/**
 * Semi-supervised method for online multi-target regression. This method uses the AMRules as
 * learner and it based on probabilistic method very similar to anomaly detection. This method
 * measures predicts the benefit of a unlabeled example to the models(using only the input
 * information) to the model. If this benefit is higher than <scoreThreshold> then the algorithm
 * predicts an output and artificially labels the example and use it for training.
 *
 * <p>The algorithm is fully explained in the following publication:
 *
 * <p>Sousa R., Gama J. (2016) Online Semi-supervised Learning for Multi-target Regression in Data
 * Streams Using AMRules. In: Boström H., Knobbe A., Soares C., Papapetrou P. (eds) Advances in
 * Intelligent Data Analysis XV. IDA 2016. Lecture Notes in Computer Science, vol 9897. Springer.
 *
 * @author RSousa
 * @version $Revision: 2 $
 */
public abstract class AMRulesMultiLabelLearnerSemiSuper extends AbstractMultiLabelLearner
        implements MultiTargetLearnerSemiSupervised {
    ...
    public FloatOption splitConfidenceOption =
            new FloatOption(
                    "splitConfidence",
                    'c',
                    "Hoeffding Bound Parameter. The allowable error in split decision, values"
                            + " closer to 0 will take longer to decide.",
                    0.0000001,
                    0.0,
                    1.0);
    public FloatOption tieThresholdOption =
            new FloatOption(
                    "tieThreshold",
                    't',
                    "Hoeffding Bound Parameter. Threshold below which a split will be forced to"
                            + " break ties.",
                    0.05,
                    0.0,
                    1.0);
    public IntOption gracePeriodOption =
            new IntOption(
                    "gracePeriod",
                    'g',
                    "Hoeffding Bound Parameter. The number of instances a leaf should observe"
    ...
    public FlagOption unorderedRulesOption =
            new FlagOption("setUnorderedRulesOn", 'U', "unorderedRules.");
    public FlagOption dropOldRuleAfterExpansionOption =
            new FlagOption(
                    "dropOldRuleAfterExpansion",
                    'D',
                    "Drop old rule if it expanded (by default the rule is kept for the set of"
                            + " outputs not selected for expansion.)");
    ...
    public FloatOption scoreThreshold =
            new FloatOption("scoreThreshold", 'h', "Initial dataset (%)", 1);
    public IntOption slidingWindowSize =
            new IntOption("slidingWindowSize", 'W', "slidingWindowSize", 1000);
    public IntOption slidingWindowStep =
            new IntOption("slidingWindowStep", 'j', "slidingWindowStep", 1);
    ...
    public Prediction getPredictionForInstance(MultiLabelInstance inst) {
        ...
        if (vote != null) return vote.getPrediction();
        else return null;
    }

    /**
     * getVotes extension of the instance method getVotesForInstance in moa.classifier.java returns
        ...
        for (MultiLabelRule rule : ruleSet) {
            ...
            if (rule.isCovering(instance) == true) {
                ...
                if (vote != null) { // should only happen for first instance
                    ...
                    if (errors == null) // if errors==null, rule has seen no predictions since
                        // expansion: return maximum error, since prediction is not
                        // reliable
                        errors = defaultRuleErrors(vote);
                    debug(
                            "Rule No"
                                    + rule.getRuleNumberID()
                                    + " Vote: "
                                    + vote.toString()
                                    + " Error: "
                                    + errors
    ...
    protected double[] defaultRuleErrors(Prediction vote) {
        ...
        for (int i = 0; i < vote.numOutputAttributes(); i++) {
            if (vote.hasVotesForAttribute(i)) errors[i] = Double.MAX_VALUE;
        }
        return errors;
    }
    ...
    /** AMRules Algorithm. Method for updating (training) the AMRules model using a new instance */
    ...
    public void trainOnInstanceImpl(MultiLabelInstance instance) {
        ...
        if (nAttributes == 0) nAttributes = instance.numInputAttributes();

        numInstances += instance.weight();
        debug("Train", 3);
        debug(
                "NÃƒÆ’Ã†â€™Ãƒâ€"
                    + " Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Âº"
                    + " instance "
                        + numInstances
                        + " - "
                        + instance.toString(),
            ...
            if (rule.isCovering(instance) == true) {
                ...
                if (!rule.updateAnomalyDetection(instance)) {
                    ...
                    if (rule.updateChangeDetection(instance)) {
                        debug(
                                "I) Drift Detected. Exa. : "
                                        + numInstances
                                        + " ("
                                        + rule.getWeightSeenSinceExpansion()
                                        + ") Remove Rule: "
                                        + rule.getRuleNumberID(),
                                1);
                        ...
                        if (rule.getWeightSeenSinceExpansion() % gracePeriodOption.getValue()
                                == 0.0) {
                            if (rule.tryToExpand(
                                    splitConfidenceOption.getValue(),
                                    tieThresholdOption.getValue())) {

                                MultiLabelRule otherMultiLabelRule =
                                        rule.getNewRuleFromOtherOutputs(); // Need to be outside
                                // to make sure other
                                // rules are cleaned

                                if (!dropOldRuleAfterExpansionOption.isSet()
                                        && rule.hasNewRuleFromOtherOutputs()) {
                                    ...
                                    if (observer != null) otherMultiLabelRule.addObserver(observer);
                                }
                                setRuleOptions(rule);
                ...
                if (!unorderedRulesOption.isSet()) break;
            }
        }
        ...
        if (rulesCoveringInstance == false) {
            ...
            if (defaultRule.getWeightSeenSinceExpansion() % this.gracePeriodOption.getValue()
                    == 0.0) {
                debug("Nr. examples " + defaultRule.getWeightSeenSinceExpansion(), 4);
                if (defaultRule.tryToExpand(
                    ...
                    if (observer != null) defaultRule.addObserver(observer);
                }
            }
        }
        // ----------------------------------------------------------------------
    ...
    protected Measurement[] getModelMeasurementsImpl() {
        ...
        if (featureRanking instanceof NoFeatureRanking) {
            ...
            for (int i = 0; i < this.nAttributes; i++)
                m[i + mNoFeatureRanking.length] =
                        new Measurement("Attribute" + i, rankings.getValue(i));
        }
        return m;
    }
    ...
    public void resetLearningImpl() {
        ...
        MultiLabelLearner l =
                (MultiLabelLearner)
                        ((MultiLabelLearner) getPreparedClassOption(learnerOption)).copy();
    ...
    protected void setRuleOptions(MultiLabelRule rule) {
        rule.setSplitCriterion(
                (MultiLabelSplitCriterion)
                        ((MultiLabelSplitCriterion) getPreparedClassOption(splitCriterionOption))
                                .copy());
        rule.setChangeDetector(
                (ChangeDetector) ((ChangeDetector) getPreparedClassOption(changeDetector)).copy());
        rule.setAnomalyDetector(
                (AnomalyDetector)
                        ((AnomalyDetector) getPreparedClassOption(anomalyDetector)).copy());
        rule.setNumericObserverOption(
                (NumericStatisticsObserver)
                        ((NumericStatisticsObserver) getPreparedClassOption(numericObserverOption))
                                .copy());
        rule.setNominalObserverOption(
                (NominalStatisticsObserver)
                        ((NominalStatisticsObserver) getPreparedClassOption(nominalObserverOption))
                                .copy());
        rule.setErrorMeasurer(
                (MultiLabelErrorMeasurer)
                        ((MultiLabelErrorMeasurer) getPreparedClassOption(errorMeasurerOption))
                                .copy());
        rule.setOutputAttributesSelector(
                (OutputAttributesSelector)
                        ((OutputAttributesSelector) getPreparedClassOption(outputSelectorOption))
                                .copy());
        ...
        rule.setInputAttributesSelector(
                (InputAttributesSelector)
                        ((InputAttributesSelector) getPreparedClassOption(inputSelectorOption))
                                .copy());
    }
