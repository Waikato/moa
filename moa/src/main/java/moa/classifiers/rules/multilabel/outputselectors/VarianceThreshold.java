...
public class VarianceThreshold extends AbstractOptionHandler implements OutputAttributesSelector {
    ...
    public FloatOption thresholdOption =
            new FloatOption(
                    "Threshold",
                    'p',
                    "Maximum allowed variance ratio (variance(new)/variance(old)).",
                    1.0,
                    0.5,
                    2.0);
        ...
        for (int i = 0; i < numCurrentOutputs; i++) {
            double stdRes =
                    Utils.computeVariance(
                            resultingStatistics[i].getValue(0),
                            resultingStatistics[i].getValue(3),
                            resultingStatistics[i].getValue(4));
            double stdCur =
                    Utils.computeVariance(
                            currentLiteralStatistics[i].getValue(0),
                            currentLiteralStatistics[i].getValue(3),
                            currentLiteralStatistics[i].getValue(4));

            if (stdRes / stdCur <= threshold || stdCur == 0) newOutputsList.add(currentIndices[i]);
        }
        // list to array
