 ...
 * @version $Revision: 1 $
 */
public abstract class AbstractErrorWeightedVoteMultiLabel extends AbstractMOAObject
    ...
    public void addVote(Prediction vote, double[] error) {
        ...
        if (outputAttributesCount == null) outputAttributesCount = new int[numOutputs];

        for (int i = 0; i < numOutputs; i++)
            if (vote.hasVotesForAttribute(i)) outputAttributesCount[i]++;
        votes.add(vote);
        errors.add(error);
    }
    ...
    public Prediction getPrediction() {
        if (this.weightedVote == null) weightedVote = computeWeightedVote();
        return weightedVote;
    }
