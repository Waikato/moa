...
/**
 * Multi-label Information Gain. Online Multi-label Classification heuristic for AMRules.
 *
 * @author RSousa
 */
public class MultilabelInformationGain extends AbstractOptionHandler
    ...
    public double[] getBranchesSplitMerits(DoubleVector[][] postSplitDists) {
        ...
        for (int j = 0; j < numOutputs; j++) {
            ...
            for (int i = 0; i < numBranches; i++) {
                if (postSplitDists[j][i].getValue(0) > 0) merits[i] -= branchMeritsOutput[i];
                else merits[i] = -Double.MAX_VALUE;
            }
        }
        return merits;
    }

    protected double getMeritOfSplitForOutput(
            DoubleVector[] preSplitDist,
            DoubleVector[][] postSplitDists,
            int outputAttributeIndex) {
        return getMeritOfSplitForOutput(
                preSplitDist[outputAttributeIndex], postSplitDists[outputAttributeIndex]);
    }
