...
public class MeritThreshold extends AbstractOptionHandler implements InputAttributesSelector {
    ...
    public FloatOption percentageThresholdOption =
            new FloatOption(
                    "percentageThreshold",
                    'p',
                    "Allowed merit decrease in percentage of best input attribute.",
                    0.1,
                    0.0,
                    1.0);
    ...
    public int[] getNextInputIndices(AttributeExpansionSuggestion[] sortedSplitSuggestions) {
        ...
        if (sortedSplitSuggestions.length > 0) {
            ...
            double threshold =
                    sortedSplitSuggestions[sortedSplitSuggestions.length - 1].merit
                            * (percentageThresholdOption.getValue());
            temp[0] =
                    sortedSplitSuggestions[sortedSplitSuggestions.length - 1].predicate
                            .getAttributeIndex();
            int c = 1;
            for (int i = sortedSplitSuggestions.length - 2;
                    i >= 0 && sortedSplitSuggestions[i].merit >= threshold;
                    i--) {
                temp[c] = sortedSplitSuggestions[i].getPredicate().getAttributeIndex();
                c++;
            }
            if (c == 1
                    && sortedSplitSuggestions.length
                            > 1) { // if only one is selected, add a second attribute for computing
                // hoeffding bound
                c = 2;
                temp[1] =
                        sortedSplitSuggestions[sortedSplitSuggestions.length - 2].predicate
                                .getAttributeIndex();
            }
            nextInput = new int[c];
            for (int i = 0; i < c; i++) {
