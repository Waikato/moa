...
/**
 * This class contains the functions for learning the literals for Multi-label classification (in
 * same way as Multi-Target regression). used by multi-label AMRules.
 *
...
public class LearningLiteralClassification extends LearningLiteral {
    ...
    protected double[] getNormalizedErrors(Prediction prediction, Instance instance) {
        ...
        for (int i = 0; i < outputsToLearn.length; i++) {
            ...
            double trueY =
                    normalizeOutputValue(i, instance.valueOutputAttribute(outputsToLearn[i]));
    ...
    public boolean tryToExpand(double splitConfidence, double tieThreshold) {
        ...
        if (sumMerit == 0)
            meritPerInput =
                    null; // this indicates that no merit should be considered (e.g. for feature
        // ranking)
        ...
        if (bestSplitSuggestions.length < 2) {
            // shouldSplit = ((bestSplitSuggestions.length > 0) && (bestSplitSuggestions[0].merit >
            // 0));
            ...
            double hoeffdingBound =
                    computeHoeffdingBound(
                            splitCriterion.getRangeOfMerit(this.literalStatistics),
                            splitConfidence,
                            weightSeen);
        ...
        if (shouldSplit) {
            ...
            // check which branch is better and update bestSuggestion (in amrules the splits are
            // binary )
            ...
            // if is AMRulesFunction and the  number of output attributes changes, start learning a
            // new predictor
                ...
                if (newOutputs.length != outputsToLearn.length) {
                    ...
                    if (otherOutputs.length > 0) {
                        otherOutputsLearningLiteral =
                                new LearningLiteralClassification(otherOutputs);
    ...
    private DoubleVector[] getBranchStatistics(
            DoubleVector[][] resultingStatistics, int indexBranch) {
                ...
                if (obs != null) {
                    AttributeExpansionSuggestion bestSuggestion =
                            obs.getBestEvaluatedSplitSuggestion(
                                    criterion, literalStatistics, inputsToLearn[i]);
                    ...
                    if (bestSuggestion == null) {
                        ...
                        bestSuggestion =
                                new AttributeExpansionSuggestion(
                                        new NumericRulePredicate(inputsToLearn[i], 0, true),
                                        null,
                                        -Double.MAX_VALUE);
                    }
    ...
    public void trainOnInstance(MultiLabelInstance instance) {
        ...
        if (!hasStarted) {
            ...
            if (inputsToLearn == null) {
                ...
                int ct = 0;
                for (int i = 0;
                        i < instance.numInputAttributes();
                        i++) { // TODO JD: check with mask?
                    if (attributesMask[i]) {
                        inputsToLearn[ct] = i;
                        ct++;
                    }
        ...
        for (int i = 0; i < outputsToLearn.length; i++) {
            ...
            double squaredSumShifted =
                    weight
                            * (target - EntropyShift[i])
                            * (target - EntropyShift[i]); // Not needed for now
            exampleStatistics[i] =
                    new DoubleVector(
        ...
        for (int i = 0; i < inputsToLearn.length; i++) {
            if (attributesMask[inputsToLearn[i]]) { // this is checked above. Remove?
                ...
                if (obs == null) {
                    ...
                    } else if (instance.attribute(inputsToLearn[i])
                            .isNominal()) { // just to make sure its nominal (in the future there
                        // may be ordinal?
                        obs = ((NominalStatisticsObserver) nominalStatisticsObserver.copy());
                    }
                    this.attributeObservers.set(inputsToLearn[i], obs);
                }
                obs.observeAttribute(
                        instance.valueInputAttribute(inputsToLearn[i]), exampleStatistics);
            }
        }
    ...
    public String getStaticOutput(InstanceInformation instanceInformation) {
        ...
        if (this.literalStatistics != null) {
            for (int i = 0; i < this.literalStatistics.length; i++) {
                sb.append(
                        instanceInformation.outputAttribute(outputsToLearn[i]).name()
                                + ": "
                                + literalStatistics[i].getValue(1)
                                        / literalStatistics[i].getValue(0)
                                + " "); //
            }
        }
