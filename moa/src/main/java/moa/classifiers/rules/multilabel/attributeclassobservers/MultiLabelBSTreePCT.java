...
public class MultiLabelBSTreePCT extends AbstractOptionHandler
        implements NumericStatisticsObserver {

    /** */
    public IntOption maxNodesOption =
            new IntOption(
                    "maxNodesOption", 'z', "Maximum number of nodes", 2000, 0, Integer.MAX_VALUE);
    ...
    public void observeAttribute(double inputAttributeValue, DoubleVector[] statistics) {
        throw new UnsupportedOperationException(
                "This is an attribute observer used for PCTs and needs both input and output"
                        + " statistics.");
    }

    public void observeAttribute(
            double inputAttributeValue,
            DoubleVector[] targetStatistics,
    ...
    public AttributeExpansionSuggestion getBestEvaluatedSplitSuggestion(
            MultiLabelSplitCriterion criterion,
            DoubleVector[] preSplitTargetStatistics,
            DoubleVector[] preSplitInputStatistics,
            int inputAttributeIndex) {
        ...
        for (int i = 0; i < numOutputs; i++) {
            leftTargetStatistics[i] =
                    new DoubleVector(new double[preSplitTargetStatistics[i].numValues()]); // sets
            // statistics
            // to
            // zeros
        ...
        for (int i = 0; i < numInputs; i++) {
            leftInputStatistics[i] =
                    new DoubleVector(new double[preSplitInputStatistics[i].numValues()]); // sets
            // statistics
            // to zeros
        ...
        return searchForBestSplitOption(
                ...
                null,
                criterion,
                preSplitTargetStatistics,
                preSplitInputStatistics,
                inputAttributeIndex);
    }

    protected AttributeExpansionSuggestion searchForBestSplitOption(
            Node currentNode,
        ...
        double merit =
                ((PCTWeightedICVarianceReduction) criterion)
                        .getMeritOfSplit(
                                preSplitTargetStatistics,
                                postSplitTargetDists,
                                preSplitInputStatistics,
                                postSplitInputDists);

        if ((!Double.isNaN(merit))
                && (currentBestOption == null || (merit > currentBestOption.merit))) {
            currentBestOption =
                    new AttributeExpansionSuggestion(
                            new NumericRulePredicate(
                                    inputAttributeIndex, currentNode.cutPoint, true),
                            Utils.copy(postSplitTargetDists),
                            merit);
        }
    ...
    protected class Node implements Serializable {
        ...
        public Node(
            ...
            for (int i = 0; i < numOutputAttributes; i++) {
                ...
                else this.targetStatistics[i] = new DoubleVector();
            }
            ...
            if (inputAttributeValue == this.cutPoint) {
                ...
                if (this.left == null) {
                    if (numNodes < maxNodes) {
                        this.left =
                                new Node(inputAttributeValue, targetStatistics, inputStatistics);
                        numNodes++;
                    }
                } else {
                    this.left.observeAttribute(
                            inputAttributeValue, targetStatistics, inputStatistics);
                }
            } else {
                if (this.right == null) {
                    if (numNodes < maxNodes) {
