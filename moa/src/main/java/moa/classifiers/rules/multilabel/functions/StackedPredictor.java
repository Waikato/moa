...
public class StackedPredictor extends AbstractMultiLabelLearner
    ...
    public FloatOption learningRatioOption =
            new FloatOption(
                    "learningRatio",
                    'l',
                    "Learning Ratio to use for training the 1st layer.",
                    0.025);

    public FloatOption learningRatio2ndLayerOption =
            new FloatOption(
                    "learningRatio2ndLayer",
                    'n',
                    "Learning Ratio to use in the second layer.",
                    0.001);
    ...
    public FloatOption learningRateDecayOption =
            new FloatOption(
                    "learningRateDecay",
                    'm',
                    " Learning Rate decay to use for training the 1st layer.",
                    0.001);
    ...
    public FlagOption skipStackingOption =
            new FlagOption(
                    "skipStackingOption",
                    's',
                    "Predicts the outputs of the first layer (no dependence among output is"
                            + " computed)");
    ...
    public FlagOption printWeightsOption =
            new FlagOption("printWeights", 'p', "Outputs the 2nd layer weights as measurements.");
    ...
    public void trainOnInstanceImpl(MultiLabelInstance instance) {
        ...
        if (!hasStarted) {
            ...
            for (int i = 0; i < instance.numInputAttributes(); i++)
                if (instance.inputAttribute(i).isNumeric()) numericIndices.add(i);
        ...
        if (!constantLearningRatioDecayOption.isSet()) {
            currentLearningRate =
                    learningRatioOption.getValue()
                            / (1 + count * this.learningRateDecayOption.getValue());
        }
        ...
        for (int j = 0; j < numOutputs; j++) {
            ...
            for (int i = 0; i < numInputs; i++) {
                layer1Weights[i][j] +=
                        currentLearningRate * delta * normInputs[i] * instance.weight();
        ...
        if (!skipStackingOption.isSet()) {
            ...
            for (int j = 0; j < numOutputs; j++) {
                ...
                for (int i = 0; i < numOutputs; i++) {
                    layer2Weights[i][j] +=
                            learningRate2ndLayer * delta * firstLayerOutput[i] * instance.weight();
    ...
    public Prediction getPredictionForInstance(MultiLabelInstance inst) {
        ...
        if (hasStarted) {
            ...
            } else denormalizedOutput = getDenormalizedOutput(firstLayerOutput);
    ...
    protected double[] getNormalizedInput(MultiLabelInstance instance) {
        ...
        while (it.hasNext()) {
            ...
            if (std > SD_THRESHOLD) normalizedInput[i] /= std;
            i++;
        }
    ...
    protected double[] getNormalizedOutput(MultiLabelInstance instance) {
        ...
        for (int i = 0; i < numOutputs; i++) {
            ...
            if (std > SD_THRESHOLD) normalizedOutput[i] /= std;
        }
    ...
    protected double[] getDenormalizedOutput(double[] normOutputs) {
        ...
        for (int i = 0; i < numOutputs; i++) {
            double mean = outAttrSum[i] / count;
            double std = Utils.computeSD(outAttrSquaredSum[i], outAttrSum[i], count);
            if (std > SD_THRESHOLD) denormalizedOutput[i] = normOutputs[i] * std + mean;
            else denormalizedOutput[i] = normOutputs[i] + mean;
        }
        ...
        if (printWeightsOption.isSet()) {
            ...
            for (int j = 0; j < numWeights - 1; j++) {
                for (int i = 0; i < numWeights - 1; i++) {
                    measurements[ct++] =
                            new Measurement(
                                    "W Out" + (i + 1) + ": Out" + (j + 1), layer2Weights[i][j]);
                }

                measurements[ct++] =
                        new Measurement("W Bias: Out" + (j + 1), layer2Weights[numWeights - 1][j]);
            }
        }
        return measurements;
    }
