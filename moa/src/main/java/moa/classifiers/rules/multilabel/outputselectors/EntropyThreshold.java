...
public class EntropyThreshold extends AbstractOptionHandler implements OutputAttributesSelector {
    ...
    public FloatOption thresholdOption =
            new FloatOption(
                    "Threshold",
                    'p',
                    "Maximum allowed Entropy (entropy(new)/entropy(old)).",
                    1.0,
                    0.5,
                    2.0);
    ...
    public int[] getNextOutputIndices(
        ...
        for (int i = 0; i < numCurrentOutputs; i++) {
            ...
            double EntRes =
                    Utils.computeEntropy(
                            resultingStatistics[i].getValue(0), resultingStatistics[i].getValue(1));
            double EntCur =
                    Utils.computeEntropy(
                            currentLiteralStatistics[i].getValue(0),
                            currentLiteralStatistics[i].getValue(1));

            if ((EntCur - EntRes) > 0 || EntCur == 0) newOutputsList.add(currentIndices[i]);
        }
        // list to array
