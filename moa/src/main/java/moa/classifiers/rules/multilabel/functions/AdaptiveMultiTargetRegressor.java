...
/**
 * Adaptive MultiTarget Regressor uses two learner The first is used in first stage when high error
 * are produced(e.g. Target mean) The second is used in a second stage when low error are
 * produced(e.g perceptron) baseLearnerOption1- Learner one ; baseLearnerOption2- learner two
 */
public class AdaptiveMultiTargetRegressor extends AbstractMultiLabelLearner
    ...
    public void trainOnInstanceImpl(MultiLabelInstance instance) {
        if (!this.hasStarted) {
            ...
            for (int i = 0; i < NUM_LEARNERS; i++) {
                if (baseLearner[i].isRandomizable()) baseLearner[i].setRandomSeed(this.randomSeed);
                baseLearner[i].resetLearning();
                errorMeasurer[i] =
                        (MultiTargetErrorMeasurer)
                                ((MultiTargetErrorMeasurer)
                                                getPreparedClassOption(this.errorMeasurerOption))
                                        .copy();
            }
    ...
    public void resetWithMemory() {
        if (errorMeasurer == null) errorMeasurer = new MultiTargetErrorMeasurer[NUM_LEARNERS];
        for (int i = 0; i < NUM_LEARNERS; i++) {
            errorMeasurer[i] =
                    (MultiTargetErrorMeasurer)
                            ((MultiTargetErrorMeasurer)
