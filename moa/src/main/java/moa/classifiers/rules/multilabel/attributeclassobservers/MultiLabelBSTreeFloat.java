...
public class MultiLabelBSTreeFloat extends AbstractOptionHandler
        implements NumericStatisticsObserver {

    /** */
    public IntOption maxNodesOption =
            new IntOption(
                    "maxNodesOption", 'z', "Maximum number of nodes", 50, 0, Integer.MAX_VALUE);
    ...
    public AttributeExpansionSuggestion getBestEvaluatedSplitSuggestion(
        ...
        for (int i = 0; i < numOutputs; i++) {
            leftStatistics[i] =
                    new DoubleVector(
                            new double
                                    [preSplitStatistics[i]
                                            .numValues()]); // sets statistics to zeros
            rightStatistics[i] = new DoubleVector(preSplitStatistics[i]);
        }
        AttributeExpansionSuggestion ret =
                searchForBestSplitOption(
                        this.root, null, criterion, preSplitStatistics, inputAttributeIndex);
        leftStatistics = null;
