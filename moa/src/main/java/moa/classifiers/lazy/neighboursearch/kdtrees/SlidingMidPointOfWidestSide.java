...
public class SlidingMidPointOfWidestSide extends KDTreeNodeSplitter {
     ...
     * non-empty side until there is at least one point on the empty side. The two nodes created
     * after the whole splitting are correctly initialised. And, node.left and node.right are set
     * appropriately.
     *
    ...
    public void splitNode(
        ...
        double maxRectWidth = Double.NEGATIVE_INFINITY,
                maxPtWidth = Double.NEGATIVE_INFINITY,
                tempval;
        ...
        if (splitVal < node.m_NodeRanges[splitDim][MIN])
            ...
            splitVal =
                    node.m_NodeRanges[splitDim][MAX] - node.m_NodeRanges[splitDim][WIDTH] * 0.001;
    ...
    /**
     * Re-arranges the indices array such that the points {@literal <=} to the splitVal are on the
     * left of the array and those {@literal >} the splitVal are on the right.
     *
