 ...
 * Matches in Logarithmic Expected Time. ACM Transactions on Mathematics Software. 3(3):209-226.<br>
 * <br>
...
public class KDTree extends NearestNeighbourSearch {
    ...
    /**
     * Array holding the distances of the nearest neighbours. It is filled up both by
     * nearestNeighbour() and kNearestNeighbours().
     */
    protected double[] m_DistanceList;

    /**
     * Indexlist of the instances of this kdtree. Instances get sorted according to the splits. the
     * nodes of the KDTree just hold their start and end indices
     */
    ...
    /**
     * Builds the KDTree on the supplied set of instances/points. It is adviseable to run the
     * replace missing attributes filter on the passed instances first. NOTE: This method should not
     * be called from outside this class. Outside classes should call setInstances(Instances)
     * instead.
     *
    ...
    /**
     * Recursively splits nodes of a tree starting from the supplied node. The splitting stops for
     * any node for which the number of instances/points falls below a given threshold (given by
     * m_MaxInstInLeaf), or if the maximum relative width/range of the instances/points (i.e.
     * max_i(max(att_i) - min(att_i)) ) falls below a given threshold (given by m_MinBoxRelWidth).
     *
    ...
    protected void splitNodes(KDTreeNode node, double[][] universe, int depth) throws Exception {
        ...
        if (node.numInstances() <= m_MaxInstInLeaf
                || getMaxRelativeNodeWidth(nodeRanges, universe) <= m_MinBoxRelWidth) return;
    ...
    /**
     * Returns (in the supplied heap object) the k nearest neighbours of the given instance starting
     * from the give tree node. &gt;k neighbours are returned if there are more than one neighbours
     * at the kth boundary. NOTE: This method should not be used from outside this class. Outside
     * classes should call kNearestNeighbours(Instance, int).
     *
     ...
     * @param heap The MyHeap object to store/update the kNNs found during the search.
     * @param distanceToParents The distance of the supplied target to the parents of the supplied
     *     tree node.
     * @throws Exception if the nearest neighbour could not be found.
     */
    protected void findNearestNeighbours(
            Instance target, KDTreeNode node, int k, MyHeap heap, double distanceToParents)
            throws Exception {
        if (node.isALeaf()) {
            double distance;
            ...
            for (int idx = node.m_Start; idx <= node.m_End; idx++) {
                ...
                if (heap.size() < k) {
                    ...
                    distance =
                            m_EuclideanDistance.distance(
                                    target, m_Instances.instance(m_InstList[idx]), temp.distance);
            ...
            boolean targetInLeft =
                    m_EuclideanDistance.valueIsSmallerEqual(
            ...
            if (heap.size() < k) { // if haven't found the first k
                double distanceToSplitPlane =
                        distanceToParents
                                + m_EuclideanDistance.sqDifference(
                                        node.m_SplitDim,
    ...
    /**
     * Returns the k nearest neighbours of the supplied instance. &gt;k neighbours are returned if
     * there are more than one neighbours at the kth boundary.
     *
     * @param target The instance to find the nearest neighbours for.
     * @param k The number of neighbours to find.
     * @return The k nearest neighbours (or &gt;k if more there are than one neighbours at the kth
     *     boundary).
     * @throws Exception if the nearest neighbour could not be found.
     */
     ...
     * @return The nearest neighbour from among the previously supplied training instances.
    ...
    /**
     * Returns the distances to the kNearest or 1 nearest neighbour currently found with either the
     * kNearestNeighbours or the nearestNeighbour method.
     *
     * @return array containing the distances of the nearestNeighbours. The length and ordering of
     *     the array is the same as that of the instances returned by nearestNeighbour functions.
     * @throws Exception if called before calling kNearestNeighbours or nearestNeighbours.
     */
    public double[] getDistances() throws Exception {
        if (m_Instances == null || m_DistanceList == null)
            throw new Exception(
                    "The tree has not been supplied with a set of "
    ...
    /**
     * Adds one instance to the KDTree. This updates the KDTree structure to take into account the
     * newly added training instance.
     *
     * @param instance the instance to be added. Usually the newly added instance in the training
     *     set.
     ...
     * should not be called by outside classes, outside classes should instead call update(Instance)
     * method.
     *
     * @param inst The instance to add to the tree
     * @param node The node to start the recursive search from, for the leaf node where the supplied
     *     instance would go.
    ...
    protected void addInstanceToTree(Instance inst, KDTreeNode node) throws Exception {
        if (node.isALeaf()) {
            ...
            try {
                System.arraycopy(
                        m_InstList, 0, instList, 0, node.m_End + 1); // m_InstList.squeezeIn(m_End,
                // index);
     ...
     * The start and end indices for the master index array (m_InstList) stored in the nodes need to
     * be updated for all nodes in the subtree on the right of a node where the instance was added.
     * NOTE: No outside class should call this method.
     *
    ...
    /**
     * Adds one instance to KDTree loosly. It only changes the ranges in EuclideanDistance, and does
     * not affect the structure of the KDTree.
     *
     * @param instance the new instance. Usually this is the test instance supplied to update the
     *     range of attributes in the distance function.
     */
    ...
    /**
     * Checks if there is any instance with missing values. Throws an exception if there is, as
     * KDTree does not handle missing values.
     *
            ...
            for (int j = 0; j < ins.numValues(); j++) {
                if (ins.index(j) != ins.classIndex())
                    if (ins.isMissingSparse(j)) {
                        throw new Exception(
                                "ERROR: KDTree can not deal with missing "
    ...
    /**
     * Returns the maximum attribute width of instances/points in a KDTreeNode relative to the whole
     * dataset.
     *
     * @param nodeRanges The attribute ranges of the KDTreeNode whose maximum relative width is to
     *     be determined.
     * @param universe The attribute ranges of the whole dataset (training instances + test
    ...
    protected double getMaxRelativeNodeWidth(double[][] nodeRanges, double[][] universe) {
        ...
        else return nodeRanges[widest][WIDTH] / universe[widest][WIDTH];
    }
     ...
     *
     * @param measurePerformance Should be true if performance statistics are to be measured.
     */
     ...
     * @throws Exception If there is some problem assigning instances to centers.
     */
    protected void determineAssignments(
            KDTreeNode node, Instances centers, int[] candidates, int[] assignments, double pc)
            throws Exception {
     ...
     *
     * @param node The current node from whose hyperrectangle the distance is to be measured.
     * @param x the point
     * @return the distance
     * @throws Exception If some problem occurs in determining the distance to the hyperrectangle.
     */
    ...
    /**
     * Finds the closest point in the hyper rectangle to a given point. Change the given point to
     * this closest point by clipping of at all the dimensions to be clipped of. If the point is
     * inside the rectangle it stays unchanged. The return value is true if the point was not
     * changed, so the the return value is true if the point was inside the rectangle.
     *
     * @param node The current KDTreeNode in whose hyperrectangle the closest point is to be found.
     * @param x a point
     ...
     *
     * <p>The candidate has been the closer point to the current rectangle or even has been a point
     * within the rectangle. The competitor is competing with the candidate for a few points out of
     * the rectangle although it is a point further away from the rectangle then the candidate. The
     * extrem point is the corner of the rectangle that is furthest away from the candidate towards
     * the direction of the competitor.
     *
     * <p>If the distance candidate to this extreme point is smaller then the distance competitor to
     * this extreme point, then it is proven that none of the points in the rectangle can be owned
     * be the competitor and the candidate is full owner of the rectangle in respect to this
     * competitor. See also D. Pelleg and A. Moore's paper 'Accelerating exact k-means Algorithms
    ...
    /**
     * Assigns instances of this node to center. Center to be assign to is decided by the distance
     * function.
     *
     ...
     */
    public void assignSubToCenters(
            KDTreeNode node, Instances centers, int[] centList, int[] assignments)
            throws Exception {
    ...
    { // to make sure we have only one object of EuclideanDistance
        ...
        else m_DistanceFunction = m_EuclideanDistance = new EuclideanDistance();
    }
    ...
    /**
     * Sets the flag for normalizing the widths of a KDTree Node by the width of the dimension in
     * the universe.
     *
