...
/**
 * Abstract class for nearest neighbour search. All algorithms (classes) that do nearest neighbour
 * search should extend this class.
 *
...
public abstract class NearestNeighbourSearch implements Serializable {
    ...
    /**
     * A class for a heap to store the nearest k neighbours to an instance. The heap also takes care
     * of cases where multiple neighbours are the same distance away. i.e. the minimum size of the
     * heap is k.
     *
     * @author Ashraf M. Kibriya (amk14[at-the-rate]cs[dot]waikato[dot]ac[dot]nz)
     * @version $Revision: 8034 $
    ...
    protected class MyHeap {
        ...
        public MyHeapElement get() throws Exception {
            if (m_heap[0].index == 0) throw new Exception("No elements present in the heap");
        ...
        public void putBySubstitute(int i, double d) throws Exception {
            ...
            if (head.distance
                    == m_heap[1].distance) { // Utils.eq(head.distance, m_heap[1].distance)) {
                putKthNearest(head.index, head.distance);
            } else if (head.distance
                    > m_heap[1].distance) { // Utils.gr(head.distance, m_heap[1].distance)) {
                m_KthNearest = null;
                m_KthNearestSize = 0;
                initSize = 10;
            } else if (head.distance < m_heap[1].distance) {
                throw new Exception(
                        "The substituted element is smaller than the "
        ...
        public MyHeapElement getKthNearest() {
            if (m_KthNearestSize == 0) return null;
            m_KthNearestSize--;
    ...
    /**
     * A class for a linked list to store the nearest k neighbours to an instance. We use a list so
     * that we can take care of cases where multiple neighbours are the same distance away. i.e. the
     * minimum length of the list is k.
     *
     * @author Ashraf M. Kibriya (amk14[at-the-rate]cs[dot]waikato[dot]ac[dot]nz)
     * @version $Revision: 8034 $
     */
    ...
    protected class NeighborList {
        ...
        public void insertSorted(double distance, Instance instance) {
            ...
            if (isEmpty()) {
                ...
                if (distance < m_First.m_Distance) { // Insert at head
                    ...
                    for (;
                            (current.m_Next != null) && (current.m_Next.m_Distance < distance);
                            current = current.m_Next)
                        ;
        ...
        /**
         * Prunes the list to contain the k nearest neighbors. If there are multiple neighbors at
         * the k'th distance, all will be kept.
         *
     ...
     * needs to be called first for this to work.
     *
    ...
    /**
     * Updates the NearNeighbourSearch algorithm for the new added instance. P.S.: The method
     * assumes the instance has already been added to the m_Instances object by the caller.
     *
     ...
     * @param linkedArray The array that should have the same ordering as arrayToSort.
     */
