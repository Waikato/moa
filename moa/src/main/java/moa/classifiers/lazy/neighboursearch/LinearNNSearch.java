...
public class LinearNNSearch extends NearestNeighbourSearch {
    ...
    /**
     * Sets the property to skip identical instances (with distance zero from the target) from the
     * set of neighbours returned.
     *
            ...
            if (firstkNN < kNN) {
                ...
                distance =
                        m_DistanceFunction.distance(
                                target, m_Instances.instance(i), Double.POSITIVE_INFINITY);
                if (distance == 0.0 && m_SkipIdentical)
                    if (i < m_Instances.numInstances() - 1) continue;
                ...
                distance =
                        m_DistanceFunction.distance(target, m_Instances.instance(i), temp.distance);
    ...
    /**
     * Returns the distances of the k nearest neighbours. The kNearestNeighbours or nearestNeighbour
     * must always be called before calling this function. If this function is called before calling
     * either the kNearestNeighbours or the nearestNeighbour, then it throws an exception. If,
     * however, if either of the nearestNeighbour functions are called at any point in the past then
     * no exception is thrown and the distances of the training set from the last supplied target
    ...
    public double[] getDistances() throws Exception {
        if (m_Distances == null)
            throw new Exception(
                    "No distances available. Please call either "
     ...
     *
     * @param insts The set of instances on which the nearest neighbour search is carried out.
     *     Usually this set is the training set.
    ...
    /**
     * Updates the LinearNNSearch to cater for the new added instance. This implementation only
     * updates the ranges of the DistanceFunction class, since our set of instances is passed by
     * reference and should already have the newly added instance.
     *
     * @param ins The instance to add. Usually this is the instance that is added to our
     *     neighbourhood i.e. the training instances.
    ...
    /**
     * Adds the given instance info. This implementation updates the range datastructures of the
     * DistanceFunction class.
     *
     * @param ins The instance to add the information of. Usually this is the test instance supplied
