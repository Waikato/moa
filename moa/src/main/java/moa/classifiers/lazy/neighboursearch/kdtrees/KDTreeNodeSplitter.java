...
public abstract class KDTreeNodeSplitter implements Serializable {
    ...
    /**
     * Checks whether an object of this class has been correctly initialized. Performs checks to see
     * if all the necessary things (master index array, training instances, distance function) have
    ...
    protected void correctlyInitialized() throws Exception {
        if (m_Instances == null) throw new Exception("No instances supplied.");
        else if (m_InstList == null) throw new Exception("No instance list supplied.");
    ...
    /**
     * Splits a node into two. After splitting two new nodes are created and correctly initialised.
     * And, node.left and node.right are set appropriately.
     *
    ...
    /**
     * Sets the master index array containing indices of the training instances. This array will be
     * rearranged as the tree is built, so that each node is assigned a portion in this array which
    ...
    /**
     * Sets whether if a nodes region is normalized or not. If set to true then, when selecting the
     * widest attribute/dimension for splitting, the width of each attribute/dimension, of the
     * points inside the node's region, is divided by the width of that attribute/dimension for the
     * whole point-space. Thus, each attribute/dimension of that node is normalized.
     *
    ...
    /**
     * Returns the widest dimension. The width of each dimension (for the points inside the node) is
     * normalized, if m_NormalizeNodeWidth is set to true.
     *
     * @param nodeRanges The attributes' range of the points inside the node that is to be split.
     * @param universe The attributes' range for the whole point-space.
     * @return The index of the attribute/dimension in which the points of the node have widest
     *     spread.
     */
