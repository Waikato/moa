...
/**
 * <!-- globalinfo-start -->
 * The class that splits a node into two such that the overall sum of squared distances of points to
 * their centres on both sides of the (axis-parallel) splitting plane is minimum.<br>
 * <br>
 * For more information see also:<br>
 * <br>
 * Ashraf Masood Kibriya (2007). Fast Algorithms for Nearest Neighbour Search. Hamilton, New
 * Zealand.
 *
 * <p>
 * <!-- globalinfo-end -->
...
public class KMeansInpiredMethod extends KDTreeNodeSplitter {
    ...
    /**
     * Splits a node into two such that the overall sum of squared distances of points to their
     * centres on both sides of the (axis-parallel) splitting plane is minimum. The two nodes
     * created after the whole splitting are correctly initialised. And, node.left and node.right
     * are set appropriately.
     *
     * @param node The node to split.
     * @param numNodesCreated The number of nodes that so far have been created for the tree, so
     *     that the newly created nodes are assigned correct/meaningful node numbers/ids.
    ...
    public void splitNode(
        ...
        for (int dim = 0; dim < m_Instances.numAttributes(); dim++) {
                ...
                for (int j = 0; j < m_Instances.numAttributes(); j++) {
                    ...
                    if (m_NormalizeNodeWidth) {
                        if (Double.isNaN(universe[j][MIN]) || universe[j][MIN] == universe[j][MAX])
                            val = 0.0;
                        else val = ((val - universe[j][MIN]) / universe[j][WIDTH]); // normalizing
                        // value
                    }
                    if (i == node.m_Start) {
            ...
            for (int i = node.m_Start; i <= node.m_End - 1; i++) {
                ...
                if (minSum > (leftSqSum + rightSqSum)) {
                    ...
                    if (i < node.m_End)
                        splitVal =
                                (m_Instances.instance(m_InstList[i]).value(dim)
                                                + m_Instances
                                                        .instance(m_InstList[i + 1])
                                                        .value(dim))
                                        / 2;
        ...
        node.m_Left =
                new KDTreeNode(
                        ...
                        node.m_Start,
                        rightStart - 1,
                        m_EuclideanDistance.initializeRanges(
                                m_InstList, node.m_Start, rightStart - 1));
        node.m_Right =
                new KDTreeNode(
                        numNodesCreated + 2,
                        rightStart,
    ...
    /**
     * Sorts the instances according to the given attribute/dimension. The sorting is done on the
     ...
     * @param left The begining index of the portion of the master index array that needs to be
     *     sorted.
     * @param right The end index of the portion of the master index array that needs to be sorted.
     */
    protected static void quickSort(
            Instances insts, int[] indices, int attidx, int left, int right) {
    ...
    /**
     * Re-arranges the indices array so that in the portion of the array belonging to the node to be
     * split, the points {@literal <=} to the splitVal are on the left of the portion and those
     * {@literal >} the splitVal are on the right.
     *
     ...
     * @return The startIdx of the points {@literal >} the splitVal (the points belonging to the
     *     right child of the node).
     */
    protected int rearrangePoints(
            int[] indices,
            final int startidx,
            final int endidx,
            final int splitDim,
