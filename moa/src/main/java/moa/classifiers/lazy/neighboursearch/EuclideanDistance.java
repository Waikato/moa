...
public class EuclideanDistance extends NormalizableDistance implements Cloneable {
     ...
     * first, Instance second, double cutOffValue). It is necessary to do so to get the correct
     * distances if distance(distance(Instance first, Instance second, double cutOffValue) is used.
     * This is because that function actually returns the squared distance to avoid inaccuracies
     * arising from floating point comparison.
     *
    ...
    /**
     * Returns the index of the closest point to the current instance. Index is index in Instances
     * object that is the second parameter.
     *
    ...
    public int closestPoint(Instance instance, Instances allPoints, int[] pointList)
        ...
        for (int i = 0; i < pointList.length; i++) {
            double dist =
                    distance(instance, allPoints.instance(pointList[i]), Double.POSITIVE_INFINITY);
            if (dist < minDist) {
                minDist = dist;
                bestPoint = i;
            }
