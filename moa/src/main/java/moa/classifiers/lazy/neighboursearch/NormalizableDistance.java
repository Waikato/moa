...
/**
 * Represents the abstract ancestor for normalizable distance functions, like Euclidean or Manhattan
 * distance.
 *
...
public abstract class NormalizableDistance implements DistanceFunction {
    ...
    /**
     * Gets whether if the attribute values are to be normazlied in distance calculation. (default
     * false i.e. attribute values are normalized.)
     *
    ...
    /**
     * Sets the range of attributes to use in the calculation of the distance. The indices start
     * from 1, 'first' and 'last' are valid as well. E.g.: first-3,5,6-last
     *
    ...
    /**
     * Calculates the distance between two instances. Offers speed up (if the distance function
     * class in use supports it) in nearest neighbour search by taking into account the cutOff or
     * maximum distance. Depending on the distance function class, post processing of the distances
     * by postProcessDistances(double []) may be required if this function is used.
     *
     * @param first the first instance
     * @param second the second instance
    ...
    /**
     * Updates the current distance calculated so far with the new difference between two
     * attributes. The difference between the attributes was calculated with the
    ...
    protected double norm(double x, int i) {
        if (Double.isNaN(m_Ranges[i][R_MIN]) || (m_Ranges[i][R_MAX] == m_Ranges[i][R_MIN]))
            return 0;
        else return (x - m_Ranges[i][R_MIN]) / (m_Ranges[i][R_WIDTH]);
    }
        ...
        if (m_Data.attribute(index).isNominal() == true) {
            ...
            if (isMissingValue(val1) || isMissingValue(val2)) {
                if (isMissingValue(val1) && isMissingValue(val2)) {
                    ...
                    return 1;
                    else return (m_Ranges[index][R_MAX] - m_Ranges[index][R_MIN]);
                } else {
                    double diff;
                    ...
                    if (!m_DontNormalize && diff < 0.5) {
                        ...
                        else return diff - m_Ranges[index][R_MIN];
                    }
                    return diff;
                }
            } else {
    ...
    /**
     * Used to initialize the ranges. For this the values of the first instance is used to save
     * time. Sets low and high to the values of the first instance and width to zero.
     *
    ...
    /**
     * Initializes the ranges of a subset of the instances of this dataset. Therefore m_Ranges is
     * not set.
     *
    ...
    /**
     * Initializes the ranges of a subset of the instances of this dataset. Therefore m_Ranges is
     * not set. The caller of this method should ensure that the supplied start and end indices are
     * valid (start &lt;= end, end&lt;instList.length etc) and correct.
     *
    ...
    public double[][] getRanges() throws Exception {
        ...
        if (m_Ranges == null) throw new Exception("Ranges not yet set.");
