 ...
 *
 * <p>Parameters: -k number of neighbours -limitW maximum number of instances inside the window
 * -limitR maximum number of instances inside the reservoir
 *
 * @author Maroua Bahri (maroua.bahri@inria.fr) Paper: "Incremental k-Nearest Neighbors Using
 *     Reservoir Sampling for Data Streams" Maroua Bahri and Albert Bifet
 *     https://link.springer.com/chapter/10.1007/978-3-030-88942-5_10 BibTex:
 *     "@inproceedings{bahri2021incremental, title={Incremental k-nearest neighbors using reservoir
 *     sampling for data streams}, author={Bahri, Maroua and Bifet, Albert}, booktitle={Discovery
 *     Science: 24th International Conference}, pages={122--137}, year={2021},
...
public class RW_kNN extends AbstractClassifier implements MultiClassClassifier {
    ...
    public IntOption limitOptionWindow =
            new IntOption(
                    "limitW",
                    'w',
                    "The maximum number of instances to store in the window",
                    500,
                    1,
                    Integer.MAX_VALUE);

    public IntOption limitOptionReservoir =
            new IntOption(
                    "limitR",
                    'r',
                    "The maximum number of instances to store in the reservoir",
                    500,
                    1,
                    Integer.MAX_VALUE);
    ...
    public MultiChoiceOption nearestNeighbourSearchOption =
            new MultiChoiceOption(
                    ...
                    new String[] {
                        "Brute force search algorithm for nearest neighbour search. ",
    ...
    public void trainOnInstanceImpl(Instance inst) {
        // Random r = new Random();
        if (inst.classValue() > (double) this.C) this.C = (int) inst.classValue();
        ...
        if (this.limitOptionReservoir.getValue() <= this.reservoir.numInstances()) {
            int replaceIndex =
                    this.classifierRandom.nextInt(this.limitOptionReservoir.getValue() - 1);
            this.reservoir.set(replaceIndex, inst);
        } else this.reservoir.add(inst);

        if (this.limitOptionWindow.getValue() <= this.window.numInstances()) this.window.delete(0);
    ...
    public double[] getVotesForInstance(Instance inst) {
        ...
        try {
            ...
            if (this.window.numInstances() > 0) {
                ...
                Instances neighbours_Wind =
                        search.kNearestNeighbours(
                                inst,
                                Math.min(this.kOption.getValue(), this.window.numInstances()));
                // kNN inside the reservoir
                Instances neighbours_Reser =
                        searchR.kNearestNeighbours(
                                inst,
                                Math.min(this.kOption.getValue(), this.reservoir.numInstances()));
