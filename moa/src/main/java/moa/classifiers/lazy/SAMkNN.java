...
public class SAMkNN extends AbstractClassifier implements CapabilitiesHandler {
	...
	private double [] getDistanceWeightedVotesCM(double distances[], int[] nnIndices, InstancesHeader stm, InstancesHeader ltm){
		double v[] = new double[this.maxClassValue +1];
		for (int nnIdx : nnIndices) {
	 ...
	 */
	private double [] getCMVotes(double distancesSTM[], InstancesHeader stm, double distancesLTM[], InstancesHeader ltm){
	 ...
	 */
	private List<Integer> getIncrementalTestTrainPredHistory(InstancesHeader instances, int startIdx, List<Integer> predictionHistory){
