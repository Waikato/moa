 ...
 *
 * @author Viktor Losing (vlosing@techfak.uni-bielefeld.de) Paper: "KNN Classifier with Self
 *     Adjusting Memory for Heterogeneous Concept Drift" Viktor Losing, Barbara Hammer and Heiko
 *     Wersing http://ieeexplore.ieee.org/document/7837853 PDF can be found at
 *     https://pub.uni-bielefeld.de/download/2907622/2907623 BibTex: "@INPROCEEDINGS{7837853,
 *     author={V. Losing and B. Hammer and H. Wersing}, booktitle={2016 IEEE 16th International
 *     Conference on Data Mining (ICDM)}, title={KNN Classifier with Self Adjusting Memory for
 *     Heterogeneous Concept Drift}, year={2016}, pages={291-300}, keywords={data
 *     mining;optimisation;pattern classification;Big Data;Internet of Things;KNN classifier;SAM-kNN
 *     robustness;data mining;k nearest neighbor algorithm;metaparameter optimization;nonstationary
 *     data streams;performance evaluation;self adjusting memory model;Adaptation models;Benchmark
 *     testing;Biological system modeling;Data mining;Heuristic algorithms;Prediction
 *     algorithms;Predictive models;Data streams;concept drift;data mining;kNN},
 *     doi={10.1109/ICDM.2016.0040}, month={Dec} }"
 */
public class SAMkNN extends AbstractClassifier
    ...
    public IntOption kOption =
            new IntOption("k", 'k', "The number of neighbors", 5, 1, Integer.MAX_VALUE);
    ...
    public IntOption minSTMSizeOption =
            new IntOption(
                    "minSTMSize",
                    'm',
                    "The minimum number of instances in the STM",
                    50,
                    1,
                    Integer.MAX_VALUE);
    ...
    public FlagOption recalculateSTMErrorOption =
            new FlagOption(
                    "recalculateError",
                    'r',
                    "Recalculates the error rate of the STM for size adaption (Costly operation)."
    ...
    protected void init() {
        ...
        // store calculated STM distances in a matrix to avoid recalculation, are reused in the STM
        // adaption phase
    ...
    public void trainOnInstanceImpl(Instance inst) {
        // this.trainStepCount++;
        if (inst.classValue() > maxClassValue) maxClassValue = (int) inst.classValue();
        this.stm.add(inst);
        memorySizeCheck();
    ...
    public double[] getVotesForInstance(Instance inst) {
        ...
        try {
            if (this.stm.numInstances() > 0) {
                ...
                int nnIndicesSTM[] =
                        nArgMin(
                                Math.min(distancesSTM.length, this.kOption.getValue()),
                                distancesSTM);
                ...
                if (this.ltm.numInstances() >= 0) {
                    int nnIndicesLTM[] =
                            nArgMin(
                                    Math.min(distancesLTM.length, this.kOption.getValue()),
                                    distancesLTM);
    ...
    private List<double[]> kMeans(List<double[]> points, int k) {
        List<double[]> centroids =
                CoresetKMeans.generatekMeansPlusPlusCentroids(k, points, this.random);
        CoresetKMeans.kMeans(centroids, points);
        return centroids;
    }

    /**
     * Performs classwise kMeans++ clustering for given samples with corresponding labels. The
     * number of samples is halved per class.
     */
    private void clusterDown() {
        ...
        for (int c = 0; c <= this.maxClassValue; c++) {
            ...
            if (classSamples.size() > 0) {
                ...
                List<double[]> centroids =
                        this.kMeans(classSamples, Math.max(classSamples.size() / 2, 1));
    ...
    private void memorySizeCheck() {
        if (this.stm.numInstances() + this.ltm.numInstances() > this.maxSTMSize + this.maxLTMSize) {
            if (this.ltm.numInstances() > this.maxLTMSize) {
                ...
                for (int i = 0; i < this.stm.numInstances(); i++) {
                    for (int j = 0; j < this.stm.numInstances(); j++) {
                        this.distanceMatrixSTM[i][j] =
                                this.distanceMatrixSTM[numShifts + i][numShifts + j];
                    }
                }
            }
        }
    ...
    private void cleanSingle(Instances cleanAgainst, int cleanAgainstindex, Instances toClean) {
        ...
        double distancesSTM[] =
                get1ToNDistances(cleanAgainst.get(cleanAgainstindex), cleanAgainstTmp);
        int nnIndicesSTM[] =
                nArgMin(Math.min(this.kOption.getValue(), distancesSTM.length), distancesSTM);

        double distancesLTM[] = get1ToNDistances(cleanAgainst.get(cleanAgainstindex), toClean);
        int nnIndicesLTM[] =
                nArgMin(Math.min(this.kOption.getValue(), distancesLTM.length), distancesLTM);
        double distThreshold = 0;
        for (int nnIdx : nnIndicesSTM) {
            if (cleanAgainstTmp.get(nnIdx).classValue()
                    == cleanAgainst.get(cleanAgainstindex).classValue()) {
        ...
        for (int nnIdx : nnIndicesLTM) {
            if (toClean.get(nnIdx).classValue()
                    != cleanAgainst.get(cleanAgainstindex).classValue()) {
    ...
    private double[] getDistanceWeightedVotes(
            double distances[], int[] nnIndices, Instances instances) {

        double v[] = new double[this.maxClassValue + 1];
        for (int nnIdx : nnIndices) {
            v[(int) instances.instance(nnIdx).classValue()] +=
                    1. / Math.max(distances[nnIdx], 0.000000001);
        }
        return v;
    }

    private double[] getDistanceWeightedVotesCM(
            double distances[], int[] nnIndices, Instances stm, Instances ltm) {
        double v[] = new double[this.maxClassValue + 1];
        for (int nnIdx : nnIndices) {
            if (nnIdx < stm.numInstances()) {
                v[(int) stm.instance(nnIdx).classValue()] +=
                        1. / Math.max(distances[nnIdx], 0.000000001);
            } else {
                v[(int) ltm.instance((nnIdx - stm.numInstances())).classValue()] +=
                        1. / Math.max(distances[nnIdx], 0.000000001);
            }
        }
        return v;
    ...
    private double[] getCMVotes(
            double distancesSTM[], Instances stm, double distancesLTM[], Instances ltm) {
        ...
        int nnIndicesCM[] =
                nArgMin(Math.min(distancesCM.length, this.kOption.getValue()), distancesCM);
    ...
    private int getLabelFct(double distances[], Instances instances, int startIdx, int endIdx) {
        int nnIndices[] =
                nArgMin(
                        Math.min(this.kOption.getValue(), distances.length),
                        distances,
                        startIdx,
                        endIdx);
    ...
    private List<Integer> getIncrementalTestTrainPredHistory(
            Instances instances, int startIdx, List<Integer> predictionHistory) {
        for (int i = startIdx + this.kOption.getValue() + predictionHistory.size();
                i < instances.numInstances();
                i++) {
            predictionHistory.add(
                    (this.getLabelFct(distanceMatrixSTM[i], instances, startIdx, i - 1)
    ...
    /**
     * Returns the window size with the minimum Interleaved test-train error, using bisection (with
     * recalculation of the STM error).
     */
    private int getMinErrorRateWindowSize() {
        ...
        if (numSamples < 2 * this.minSTMSizeOption.getValue()) {
            ...
            while (numSamplesRange.get(numSamplesRange.size() - 1)
                    >= 2 * this.minSTMSizeOption.getValue())
            ...
            for (Integer numSamplesIt : numSamplesRange) {
                ...
                if (this.predictionHistories.containsKey(idx)) {
                    predHistory =
                            this.getIncrementalTestTrainPredHistory(
                                    this.stm, idx, this.predictionHistories.get(idx));
                } else {
    ...
    /**
     * Returns the window size with the minimum Interleaved test-train error, using bisection
     * (without recalculation using an incremental approximation).
     */
        ...
        if (numSamples < 2 * this.minSTMSizeOption.getValue()) {
            ...
            for (Integer numSamplesIt : numSamplesRange) {
                ...
                if (this.predictionHistories.containsKey(idx)) {
                    ...
                    predHistory =
                            this.getIncrementalTestTrainPredHistory(this.stm, idx, predHistory);
                    this.predictionHistories.put(idx, predHistory);
                } else {
                    predHistory = this.getTestTrainPredHistory(this.stm, idx);
                    this.predictionHistories.put(idx, predHistory);
    ...
    private int getNewSTMSize(boolean recalculateErrors) {
        if (recalculateErrors) return this.getMinErrorRateWindowSize();
        else return this.getMinErrorRateWindowSizeIncremental();
    }
