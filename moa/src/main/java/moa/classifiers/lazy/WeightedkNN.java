 ...
 * -m max instances <br>
 *
 * @author Maroua Bahri (maroua.bahri@inria.fr) Paper: "Effective Weighted k-Nearest Neighbors for
 *     Dynamic Data Streams" Maroua Bahri
 *     https://ieeexplore.ieee.org/stamp/stamp.jsp?arnumber=10020652 BibTex:
 *     "@inproceedings{bahri2022effective, title={Effective Weighted k-Nearest Neighbors for Dynamic
 *     Data Streams}, author={Bahri, Maroua}, booktitle={IEEE International Conference on Big Data
 *     (Big Data)}, pages={3341--3347}, year={2022}, organization={IEEE} }"
 */
public class WeightedkNN extends AbstractClassifier implements MultiClassClassifier {
    ...
    public IntOption kOption =
            new IntOption("k", 'k', "The number of neighbors", 10, 1, Integer.MAX_VALUE);
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        inst.setWeight(2);
        if (inst.classValue() > C) C = (int) inst.classValue();
        if (this.window == null) {
    ...
    /**
     * Update the weights of the instances using with 2 - (i-1)/(w-1), where i is the index of the
     * current instance Examples: current: 2 - (1-1)/w = 2 old : 2 - (w-1)/(w-1) = 1
     */
    ...
    public double[] getVotesForInstance(Instance inst) {
        ...
        try {
            if (this.window.numInstances() > 0) {
                ...
                for (int nnIdx : knnW) {
                    v[(int) this.window.instance(nnIdx).classValue()] +=
                            this.window.instance(nnIdx).weight();
                }
            }
        ...
        for (int i = 0; i < sample.numInputAttributes(); i++) {
            if (sample.attribute(i).isNominal() == true) {
                if (isMissingValue(sample.valueInputAttribute(i))
                        ...
                        || ((int) sample.valueInputAttribute(i)
                                != (int) sample2.valueInputAttribute(i))) {
                    diff = 1.0;
                } else {
                    diff = 0.0;
                }
                ...
                } else diff = sample.valueInputAttribute(i) - sample2.valueInputAttribute(i);
            }
    ...
    /**
     * Returns the Euclidean distance between one instance and a collection of instances in an
     * 1D-array.
     */
