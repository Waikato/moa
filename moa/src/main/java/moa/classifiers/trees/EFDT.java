...
public class EFDT extends AbstractClassifier implements MultiClassClassifier {
    ...
    public IntOption reEvalPeriodOption =
            new IntOption(
                    "reevaluationPeriod",
                    'R',
                    "The number of instances an internal node should observe between re-evaluation"
                            + " attempts.",
                    2000,
                    0,
                    Integer.MAX_VALUE);
    ...
    public FlagOption stopMemManagementOption =
            new FlagOption(
                    "stopMemManagement", 'z', "Stop growing as soon as memory limit is hit.");

    public FlagOption removePoorAttsOption =
            new FlagOption("removePoorAtts", 'r', "Disable poor attributes.");
    ...
    protected Measurement[] getModelMeasurementsImpl() {
        ...
        return new Measurement[] {
            ...
            new Measurement("splits", this.splitCount)
        };
    }
    ...
    protected AttributeClassObserver newNominalClassObserver() {
        AttributeClassObserver nominalClassObserver =
    ...
    public void enforceTrackerLimit() {
        if ((this.inactiveLeafNodeCount > 0)
                || ((this.activeLeafNodeCount * this.activeLeafByteSizeEstimate
                        ...
                        > this.maxByteSizeOption.getValue())) {
            ...
            Arrays.sort(
                    learningNodes,
                    new Comparator<FoundNode>() {

                        @Override
                        public int compare(FoundNode fn1, FoundNode fn2) {
                            return Double.compare(
                                    fn1.node.calculatePromise(), fn2.node.calculatePromise());
                        }
                    });
            int maxActive = 0;
            while (maxActive < learningNodes.length) {
                ...
                if ((maxActive * this.activeLeafByteSizeEstimate
                                        + (learningNodes.length - maxActive)
                                                * this.inactiveLeafByteSizeEstimate)
                                * this.byteSizeEstimateOverheadFraction
                        > this.maxByteSizeOption.getValue()) {
                    maxActive--;
    ...
    public void estimateModelByteSizes() {
        ...
        if (totalInactiveSize > 0) {
            this.inactiveLeafByteSizeEstimate =
                    (double) totalInactiveSize / this.inactiveLeafNodeCount;
        }
        long actualModelSize = this.measureByteSize();
        double estimatedModelSize =
                (this.activeLeafNodeCount * this.activeLeafByteSizeEstimate
        ...
        for (FoundNode learningNode : learningNodes) {
            if (learningNode.node instanceof ActiveLearningNode) {
                deactivateLearningNode(
                        ...
                        learningNode.parent,
                        learningNode.parentBranch);
            }
        }
    }

    protected void deactivateLearningNode(
            ActiveLearningNode toDeactivate, SplitNode parent, int parentBranch) {
    ...
    protected void activateLearningNode(
            InactiveLearningNode toActivate, SplitNode parent, int parentBranch) {
    ...
    protected void findLearningNodes(
            Node node, SplitNode parent, int parentBranch, List<FoundNode> found) {
        if (node != null) {
    ...
    protected void attemptToSplit(ActiveLearningNode node, SplitNode parent, int parentIndex) {
        ...
        if (!node.observedClassDistributionIsPure()) {
            node.addToSplitAttempts(
                    1); // even if we don't actually attempt to split, we've computed infogains
            ...
            for (AttributeSplitSuggestion bestSplitSuggestion : bestSplitSuggestions) {
                ...
                if (bestSplitSuggestion.splitTest != null) {
                    if (!node.getInfogainSum()
                            .containsKey(
                                    (bestSplitSuggestion.splitTest.getAttsTestDependsOn()[0]))) {
                        node.getInfogainSum()
                                .put(
                                        (bestSplitSuggestion.splitTest.getAttsTestDependsOn()[0]),
                                        0.0);
                    ...
                    double currentSum =
                            node.getInfogainSum()
                                    .get((bestSplitSuggestion.splitTest.getAttsTestDependsOn()[0]));
                    node.getInfogainSum()
                            .put(
                                    (bestSplitSuggestion.splitTest.getAttsTestDependsOn()[0]),
                                    currentSum + bestSplitSuggestion.merit);
                } else { // handle the null attribute
                    double currentSum = node.getInfogainSum().get(-1); // null split
                    node.getInfogainSum()
                            .put(-1, Math.max(0.0, currentSum + bestSplitSuggestion.merit));
                    assert node.getInfogainSum().get(-1) >= 0.0
                            : "Negative infogain shouldn't be possible here.";
                }
            }
            ...
            if (bestSplitSuggestions.length < 2) {
                ...
                double currentAverageMerit =
                        node.getInfogainSum().get(-1) / node.getNumSplitAttempts();

                // because this is an unsplit leaf. current average merit should be always zero on
                // the null split.

                if (bestSuggestion.splitTest == null) { // if you have a null split
                    bestSuggestionAverageMerit =
                            node.getInfogainSum().get(-1) / node.getNumSplitAttempts();
                } else {
                    bestSuggestionAverageMerit =
                            node.getInfogainSum()
                                            .get(
                                                    (bestSuggestion
                                                            .splitTest
                                                            .getAttsTestDependsOn()[0]))
                                    / node.getNumSplitAttempts();
                }
                    ...
                    for (AttributeSplitSuggestion bestSplitSuggestion : bestSplitSuggestions) {
                        if (bestSplitSuggestion.splitTest != null) {
                            ...
                            if (splitAtts.length == 1) {
                                if (bestSuggestion.merit - bestSplitSuggestion.merit
                                        > hoeffdingBound) {
            ...
            if (shouldSplit) {
                ...
                if (splitDecision.splitTest == null) {
                    ...
                    ((EFDTSplitNode) newSplit).attributeObservers =
                            node.attributeObservers; // copy the attribute observers
                    newSplit.setInfogainSum(
                            node.getInfogainSum()); // transfer infogain history, leaf to split
                    ...
                    for (int i = 0; i < splitDecision.numSplits(); i++) {
                        ...
                        if (splitDecision.splitTest.getClass() == NominalAttributeBinaryTest.class
                                || splitDecision.splitTest.getClass()
                                        == NominalAttributeMultiwayTest.class) {
                            newChild.usedNominalAttributes =
                                    new ArrayList<>(node.usedNominalAttributes); // deep copy
                            newChild.usedNominalAttributes.add(
                                    splitDecision.splitTest.getAttsTestDependsOn()[0]);
    ...
    protected SplitNode newSplitNode(
            InstanceConditionalTest splitTest, double[] classObservations, int size) {
        return new EFDTSplitNode(splitTest, classObservations, size);
    }

    protected SplitNode newSplitNode(
            InstanceConditionalTest splitTest, double[] classObservations) {
    ...
    public static class Node extends AbstractMOAObject {
        ...
        public void describeSubtree(EFDT ht, StringBuilder out, int indent) {
            ...
            this.observedClassDistribution.getSingleLineDescription(
                    out, ht.treeRoot.observedClassDistribution.numValues());
        ...
        public double calculatePromise() {
            ...
            return totalSeen > 0.0
                    ? (totalSeen
                            - this.observedClassDistribution.getValue(
                                    this.observedClassDistribution.maxIndex()))
                    : 0.0;
        }
    ...
    public static class SplitNode extends Node {
        ...
        public long calcByteSize() {
            return super.calcByteSize()
                    + SizeOf.sizeOf(this.children)
                    + SizeOf.fullSizeOf(this.splitTest);
        }
    ...
    public class EFDTSplitNode extends SplitNode implements EFDTNode {
        ...
        // DRY Don't Repeat Yourself... code duplicated from ActiveLearningNode in VFDT.java.
        // However, this is the most practical way to share stand-alone.
                ...
                if (obs != null) {
                    AttributeSplitSuggestion bestSuggestion =
                            obs.getBestEvaluatedSplitSuggestion(
                                    criterion, preSplitDist, i, ht.binarySplitsOption.isSet());
        ...
        public void learnFromInstance(
                Instance inst, EFDT ht, EFDTSplitNode parent, int parentBranch) {

            nodeTime++;
            //// Update node statistics and class distribution

            this.observedClassDistribution.addToValue(
                    (int) inst.classValue(), inst.weight()); // update prior (predictor)
            ...
            // check if a better split is available. if so, chop the tree at this point, copying
            // likelihood. predictors for children are from parent likelihood.
        ...
        protected void reEvaluateBestSplit(
                EFDTSplitNode node, EFDTSplitNode parent, int parentIndex) {

            node.addToSplitAttempts(1);

            // EFDT must transfer over gain averages when replacing a node: leaf to split, split to
            // leaf, or split to split
            ...
            if (this.splitTest != null) {
                ...
                // given the current implementations in MOA, we're only ever expecting one int to be
                // returned
            }

            // compute Hoeffding bound
            SplitCriterion splitCriterion =
                    (SplitCriterion) getPreparedClassOption(EFDT.this.splitCriterionOption);
            double hoeffdingBound =
                    computeHoeffdingBound(
                            splitCriterion.getRangeOfMerit(
                                    node.getClassDistributionAtTimeOfCreation()),
                            EFDT.this.splitConfidenceOption.getValue(),
                            node.observedClassDistribution.sumOfValues());

            // get best split suggestions
            AttributeSplitSuggestion[] bestSplitSuggestions =
                    node.getBestSplitSuggestions(splitCriterion, EFDT.this);
            Arrays.sort(bestSplitSuggestions);
                ...
                if (bestSplitSuggestion.splitTest != null) {
                    ...
                    node.getInfogainSum()
                            .put(
                                    (bestSplitSuggestion.splitTest.getAttsTestDependsOn()[0]),
                                    currentSum + bestSplitSuggestion.merit);
                } else { // handle the null attribute. this is fine to do- it'll always average
                    // zero, and we will use this later to potentially burn bad splits.
                    double currentSum = node.getInfogainSum().get(-1); // null split
            ...
            if (node.splitTest == null) { // current is null- shouldn't happen, check for robustness
                ...
                currentAverageMerit =
                        node.getInfogainSum().get(node.splitTest.getAttsTestDependsOn()[0])
                                / node.getNumSplitAttempts();
            }
            ...
            if (deltaG > hoeffdingBound
                ...
                if (splitDecision.splitTest == null) {
                    ...
                    replacement.setInfogainSum(
                            node.getInfogainSum()); // transfer infogain history, split to
                    // replacement leaf
                    ...
                    if (node.splitTest == splitDecision.splitTest
                            ...
                            && (argmax(splitDecision.resultingClassDistributions[0])
                                            == argmax(
                                                    node.getChild(0).getObservedClassDistribution())
                                    || argmax(splitDecision.resultingClassDistributions[1])
                                            == argmax(
                                                    node.getChild(1)
                        ...
                        for (int i = 0; i < splitDecision.numSplits(); i++) {
                            ...
                            if (splitDecision.splitTest.getClass()
                                            == NominalAttributeBinaryTest.class
                                    || splitDecision.splitTest.getClass()
                                            == NominalAttributeMultiwayTest.class) {
                                newChild.usedNominalAttributes =
                                        new ArrayList<>(node.usedNominalAttributes); // deep copy
                                newChild.usedNominalAttributes.add(
                                        splitDecision.splitTest.getAttsTestDependsOn()[0]);
                                // no  nominal attribute should be split on more than once in the
                                // path
                            }
                            ((EFDTSplitNode) newSplit).setChild(i, newChild);
                        }
    ...
    public static class ActiveLearningNode extends LearningNode {
        ...
        @Override
        public long calcByteSize() {
            return super.calcByteSize() + (SizeOf.fullSizeOf(this.attributeObservers));
        }
        ...
        protected void attemptToSplit(ActiveLearningNode node, SplitNode parent, int parentIndex) {
            if (!node.observedClassDistributionIsPure()) {
                ...
                for (int i = 0; i < bestSplitSuggestions.length; i++) {
                    ...
                    node.addToSplitAttempts(
                            1); // even if we don't actually attempt to split, we've computed
                    // infogains

                    if (bestSplitSuggestions[i].splitTest != null) {
                        if (!node.getInfogainSum()
                                .containsKey(
                                        (bestSplitSuggestions[i]
                                                .splitTest
                                                .getAttsTestDependsOn()[0]))) {
                            node.getInfogainSum()
                                    .put(
                        ...
                        node.getInfogainSum()
                                .put(
                                        (bestSplitSuggestions[i]
                                                .splitTest
                                                .getAttsTestDependsOn()[0]),
                                        currentSum + bestSplitSuggestions[i].merit);
                    } else { // handle the null attribute
                        double currentSum = node.getInfogainSum().get(-1); // null split
                        node.getInfogainSum()
                                .put(-1, currentSum + Math.max(0.0, bestSplitSuggestions[i].merit));
                        assert node.getInfogainSum().get(-1) >= 0.0
                                : "Negative infogain shouldn't be possible here.";
                    }
                }
                ...
                if (bestSplitSuggestions.length < 2) {
                    ...
                    if (secondBestSuggestion.splitTest == null) { // if you have a null split
                        secondBestSuggestionAverageMerit =
                                node.getInfogainSum().get(-1) / node.getNumSplitAttempts();
                    } else {
                        secondBestSuggestionAverageMerit =
                                node.getInfogainSum()
                    ...
                    if (bestSuggestion.merit < 1e-10) { // we don't use average here
                        shouldSplit = false;
                    } else if ((bestSuggestionAverageMerit - secondBestSuggestionAverageMerit
                                    > hoeffdingBound)
                            || (hoeffdingBound < this.tieThresholdOption.getValue())) {
                        shouldSplit = true;
                    }
                            ...
                            if (splitDecision.splitTest.getClass()
                                ...
                                newChild.usedNominalAttributes =
                                        new ArrayList<Integer>(
                                                node.usedNominalAttributes); // deep copy
                                newChild.usedNominalAttributes.add(
                                        splitDecision.splitTest.getAttsTestDependsOn()[0]);
                                // no  nominal attribute should be split on more than once in the
                                // path
            ...
            if (leafNode instanceof LearningNode) {
                ...
                if (this.growthAllowed && (learningNode instanceof ActiveLearningNode)) {
                    ...
                    if (activeLearningNode.nodeTime % this.gracePeriodOption.getValue() == 0) {
                        attemptToSplit(
                                activeLearningNode, foundNode.parent, foundNode.parentBranch);
                        activeLearningNode.setWeightSeenAtLastSplitEvaluation(weightSeen);
                    }
                }
            }
