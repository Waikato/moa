...
public class EFHATNode extends CustomEFDTNode {
    ...
    private CustomADWINChangeDetector
            changeDetector; // we need to access the width of adwin to compute switch significance.
    // This is not possible with the default adwin change detector class.
    ...
    public EFHATNode(
        ...
        this.changeDetector =
                changeDetector == null ? new CustomADWINChangeDetector() : changeDetector;
    }
    ...
    public double[] predict(Instance instance) {
        ...
        if (pred.length > 0) predictions.add((double) argmax(pred));
        if (backgroundLearner != null) backgroundLearner.predict(instance);
        return pred;
    }
    ...
    public void learnInstance(Instance instance, int totalNumInstances) {
        ...
        if (backgroundLearner != null) backgroundLearner.learnInstance(instance, totalNumInstances);

        if (isLeaf() && nodeTime % gracePeriod == 0) attemptInitialSplit(instance);

        if (!isLeaf()
        //                && nodeTime % minSamplesReevaluate == 0
        ) hatGrow();

        if (!isLeaf()) // Do NOT! put this in the upper (!isleaf()) block. This is not the same
            // since we might kill the subtree during reevaluation!
    ...
    private void hatGrow() {
        if (changeDetector.getChange()) {
            backgroundLearner =
                    newNode(depth, new DoubleVector(), new LinkedList<>(usedNominalAttributes));
            backgroundLearner.isBackgroundLearner = true;
        } else if (backgroundLearner != null) {
            // adopted from HAT implementation
            if (changeDetector.getWidth() > minSamplesReevaluate
                    && backgroundLearner.changeDetector.getWidth() > minSamplesReevaluate) {
