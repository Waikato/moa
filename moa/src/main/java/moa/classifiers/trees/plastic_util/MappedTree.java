...
class MappedTree implements Iterator<PlasticBranch> {
    ...
    private void mapBranches(
        ...
        for (int i = 0; i < numBranches; i++) {
            ...
            boolean branchIsFinished =
                    getEndConditionForBranch(
                            branch, swapAttribute, swapAttributeIndex, splitValue, maxBranchLength);
            if (branchIsFinished) {
            ...
            for (PlasticTreeElement extension : branchExtensions.getBranchRef()) {
                PlasticBranch extendedBranch =
                        new PlasticBranch(
                                (LinkedList<PlasticTreeElement>) branch.getBranchRef().clone());
    ...
    private boolean getEndConditionForBranch(
            PlasticBranch branch,
            Attribute swapAttribute,
            int swapAttributeIndex,
            Double splitValue,
            int maxBranchLength) {
        ...
        if (isSwapAttribute || isLeaf) return true;
        return branch.getBranchRef().size() >= maxBranchLength;
    }

    private PlasticBranch disconnectSuccessors(
            PlasticTreeElement ancestor, Attribute swapAttribute) {
        ...
        if (endCondition) {
            ...
            for (SuccessorIdentifier successorSuccessorKey :
                    successor.getSuccessors().getKeyset()) {
                PlasticTreeElement element =
                        new PlasticTreeElement(successor, successorSuccessorKey);
                branchExtensions.getBranchRef().add(element);
            }
        }
    ...
    private void expandBranch(
        ...
        if (splitAttribute.isNominal())
            splitLastNodeIfRequiredCategorical(
                    branch, splitAttribute, splitAttributeIndex, splitValue);
        else
    ...
    private LinkedList<PlasticBranch> decoupleLastNode(PlasticBranch branch) {
        ...
        for (SuccessorIdentifier key : lastNodeSuccessors.getKeyset()) {
            ...
            PlasticTreeElement replacedEndElement =
                    new PlasticTreeElement(branchCopy.getLast().getNode(), key);
            branchCopy.getBranchRef().removeLast();
            branchCopy.getBranchRef().add(replacedEndElement);
            PlasticTreeElement finalElement =
                    new PlasticTreeElement(
                            (PlasticNode) lastNodeSuccessors.getSuccessorNode(key), null);
    ...
    private void splitLastNodeIfRequiredCategorical(
            PlasticBranch branch,
            Attribute swapAttribute,
            int swapAttributeIndex,
            Double splitValue) {
        ...
        if (lastNode.isLeaf()
                || !splitAttributesMatch) { // Option 1: the split attributes don't match
            lastNode.forceSplit(swapAttribute, swapAttributeIndex, splitValue, shouldBeBinary);
            if (lastNode.isLeaf()) {
                System.out.println("Error");
            }
            lastNode.successors
                    .getAllSuccessors()
                    .forEach(s -> ((PlasticNode) s).setIsArtificial());
            return;
        }

        boolean isBinary = lastNode.successors.isBinary();

        if (!isBinary && !shouldBeBinary) // Option 2: the split attributes match and the splits are
            // multiway (this is really the best case possible)
            // do nothing
            return;

        if (isBinary
                && shouldBeBinary) { // Option 3: the split attributes match and also both splits
            // should be binary
            ...
            lastNode.forceSplit(swapAttribute, swapAttributeIndex, splitValue, shouldBeBinary);
            PlasticNode defaultSuccessor =
                    (PlasticNode)
                            lastNode.getSuccessors()
                                    .getSuccessorNode(SuccessorIdentifier.DEFAULT_NOMINAL_VALUE);
            defaultSuccessor.transferSplit(
                    lastNodeSuccessors,
                    lastNodeSplitAttribute,
                    lastNodeSplitAttributeIndex,
                    splitTest);
            return;
        }

        if (!isBinary
                && shouldBeBinary) { // Option 4: The split attributes match and the current split
            // is multiway while the old one was binary. In this case, we
            // do something similar to the numeric splits
        ...
        if (isBinary
                && !shouldBeBinary) { // Option 5: The split is binary but should be multiway. In
            // this case, we force the split and then use the old subtree
            // of the left branch of the old subtree.
            // Corresponds to transformation 1 in the paper
            SuccessorIdentifier keyToPreviousSuccessor =
                    new SuccessorIdentifier(false, splitValue, splitValue, false);
            PlasticNode previousSuccessor =
                    (PlasticNode) lastNode.getSuccessors().getSuccessorNode(keyToPreviousSuccessor);
    ...
    private void splitLastNodeIfRequiredNumeric(
            PlasticBranch branch,
            Attribute swapAttribute,
            int swapAttributeIndex,
            Double splitValue) {
        ...
        if (forceSplit) {
            lastNode.forceSplit(swapAttribute, swapAttributeIndex, splitValue, true);
            lastNode.getSuccessors()
                    .getAllSuccessors()
                    .forEach(s -> ((PlasticNode) s).setIsArtificial());
            return;
        ...
        if (splitValue.equals(oldThreshold)) return; // do nothing
    ...
    private void updateThreshold(PlasticNode node, int splitAttributeIndex, double splitValue) {
        ...
        SuccessorIdentifier leftKey =
                new SuccessorIdentifier(true, oldThreshold, oldThreshold, true);
        SuccessorIdentifier rightKey =
                new SuccessorIdentifier(true, oldThreshold, oldThreshold, false);
        ...
        if (succ1 != null) newSuccessors.addSuccessorNumeric(splitValue, succ1, true);
        if (succ2 != null) newSuccessors.addSuccessorNumeric(splitValue, succ2, false);
    ...
    private void removeUnreachableSubtree(
            PlasticNode node, int splitAttributeIndex, double threshold, boolean isLower) {
        if (node.isLeaf()) return;

        if (node.getSplitAttributeIndex() != splitAttributeIndex) {
            for (CustomEFDTNode successor : node.getSuccessors().getAllSuccessors()) {
                removeUnreachableSubtree(
                        (PlasticNode) successor, splitAttributeIndex, threshold, isLower);
            }
            return;
        }
    ...
    private void putLastElementToFront(PlasticBranch branch, Attribute splitAttribute) {
        ...
        PlasticTreeElement newFirstBranchElement =
                branch.getBranchRef().remove(branch.getBranchRef().size() - 2);
        ...
        // TODO not sure this is actually required! I think it could be sufficient to just change
        // the successors when building the tree in a later step.
    ...
    private void resetSuccessorsInBranch(PlasticBranch branch) {
        if (branch.getBranchRef().size() == 1) return;
        int i = 0;
        for (PlasticTreeElement item : branch.getBranchRef()) {
            if (i == branch.getBranchRef().size() - 1) break;
    ...
    private void setRestructuredFlagInBranch(PlasticBranch branch) {
        if (branch.getBranchRef().size() <= 2) return;
        int i = 0;
        for (PlasticTreeElement item : branch.getBranchRef()) {
