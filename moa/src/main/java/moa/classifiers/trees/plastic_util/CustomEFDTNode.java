...
class CustomEFDTNode extends AbstractMOAObject
        implements PerformsTreeRevision, MeasuresNumberOfLeaves {
    ...
    protected final GaussianNumericAttributeClassObserver numericObserverBlueprint =
            new GaussianNumericAttributeClassObserver();
    ...
    public CustomEFDTNode(
        ...
        this.usedNominalAttributes =
                usedNominalAttributes != null ? usedNominalAttributes : new LinkedList<>();
        this.nominalObserverBlueprint = nominalObserverBlueprint;
        this.observedClassDistribution =
                observedClassDistribution != null ? observedClassDistribution : new DoubleVector();
    ...
    protected double computeHoeffdingBound() {
        ...
        double n = seenWeight;
        return Math.sqrt(((range * range) * Math.log(1.0 / currentConfidence())) / (2.0 * n));
    }

    /**
     * Gets the index of the attribute in the instance, given the index of the attribute in the
     * learner.
     *
    ...
    public boolean setSplitTest(InstanceConditionalTest newTest) {
        ...
        if (successors != null) {
            if (successors.isNominal() && successors.isBinary()) {
                if (!(newTest instanceof NominalAttributeBinaryTest)) return false;
            }
            if (successors.isNominal() && !successors.isBinary()) {
    ...
    Double currentConfidence() {
        if (!useAdaptiveConfidence) return confidence;
        ...
        for (int i = 0; i < attributeObservers.size(); i++) {
            ...
            if (obs != null) {
                AttributeSplitSuggestion bestSuggestion =
                        obs.getBestEvaluatedSplitSuggestion(criterion, preSplitDist, i, binaryOnly);
     ...
     * @param totalNumInstances the total number of instances observed so far. (not used in EFDT but
    ...
    public void learnInstance(Instance instance, int totalNumInstances) {
        ...
        if (!"MC".equals(leafPrediction)) updateNaiveBayesAdaptive(instance);
    }
     ...
     *
     * <p>Traverses the tree until reaching a leaf and then returns the class votes of that leaf.
     *
     * <p>
     *
    ...
    public double[] predict(Instance instance) {
        if (!isLeaf()) {
            ...
            if (successor == null) return getClassVotes();
            return successor.predict(instance);
        }
        if ("MC".equals(leafPrediction)) return getClassVotes();
        else if ("NB".equals(leafPrediction))
            return NaiveBayes.doNaiveBayesPrediction(
                    instance, this.observedClassDistribution, this.attributeObservers);
        else return doNBAdaptive(instance);
    }
    ...
    protected void attemptInitialSplit(Instance instance) {
        ...
        xBest =
                replaceBestSuggestionIfAttributeIsBlocked(
                        xBest, bestSuggestions, blockedAttributeIndex);
        ...
        int instanceIndex =
                modelAttIndexToInstanceAttIndex(
                        xBest.splitTest.getAttsTestDependsOn()[0], instance);
        ...
        classDistributionAtTimeOfCreation =
                new DoubleVector(observedClassDistribution.getArrayCopy());
    }
    ...
    protected void reevaluateSplit(Instance instance) {
        ...
        AttributeSplitSuggestion[] bestSuggestions = getBestSplitSuggestions(splitCriterion);
        Arrays.sort(bestSuggestions);
        if (bestSuggestions.length == 0) return;
        ...
        double bestSuggestionAverageMerit =
                bestSuggestion.splitTest == null ? 0.0 : bestSuggestion.merit;
        ...
        if (deltaG > eps || (eps < tauReevaluate && deltaG > tauReevaluate * relMinDeltaG)) {
            ...
            if (bestSuggestion.splitTest == null) {
                ...
                if (getSplitTest() instanceof NumericAttributeBinaryTest
                        && getSplitTest().getAttsTestDependsOn()[0]
                                == bestSuggestion.splitTest.getAttsTestDependsOn()[0]) {
                    Set<SuccessorIdentifier> keys = successors.getKeyset();
                    for (SuccessorIdentifier key : keys) {
                        if (key.isLower()) {
                            if (argmax(bestSuggestion.resultingClassDistributions[0])
                                    == argmax(
                                            successors
                                                    .getSuccessorNode(key)
                            ...
                            if (argmax(bestSuggestion.resultingClassDistributions[1])
                                    == argmax(
                                            successors
                                                    .getSuccessorNode(key)
                                                    .observedClassDistribution
                ...
                if (!doResplit) {
                    NumericAttributeBinaryTest test =
                            (NumericAttributeBinaryTest) bestSuggestion.splitTest;
                    ...
                    int instanceIndex =
                            modelAttIndexToInstanceAttIndex(
                                    bestSuggestion.splitTest.getAttsTestDependsOn()[0], instance);
                    Attribute newSplitAttribute = instance.attribute(instanceIndex);
     ...
     */
    protected boolean initializeSuccessors(
            AttributeSplitSuggestion xBest, Attribute splitAttribute) {
        ...
        if (splitAttribute.isNominal()) {
            if (!isBinary) {
                    ...
                    CustomEFDTNode s =
                            newNode(
                                    ...
                                    getUsedNominalAttributesForSuccessor(
                                            splitAttribute, splitAttributeIndex));
                ...
                CustomEFDTNode s1 =
                        newNode(
                                depth + 1,
                                new DoubleVector(stats1),
                                getUsedNominalAttributesForSuccessor(
                                        splitAttribute, splitAttributeIndex));
                CustomEFDTNode s2 =
                        newNode(
                                depth + 1,
                                new DoubleVector(stats2),
                                getUsedNominalAttributesForSuccessor(
                                        splitAttribute, splitAttributeIndex));
            ...
            boolean success =
                    successors.addSuccessorNumeric(
                            splitValue,
                            newNode(
                                    depth + 1,
                                    new DoubleVector(xBest.resultingClassDistributionFromSplit(0)),
                                    getUsedNominalAttributesForSuccessor(
                                            splitAttribute, splitAttributeIndex)),
                            true);
            success =
                    success
                            && successors.addSuccessorNumeric(
                                    ...
                                    newNode(
                                            depth + 1,
                                            new DoubleVector(
                                                    xBest.resultingClassDistributionFromSplit(1)),
                                            getUsedNominalAttributesForSuccessor(
                                                    splitAttribute, splitAttributeIndex)),
                                    false);
            if (!success) {
    ...
    protected void propagateToSuccessors(Instance instance, int totalNumInstances) {
        ...
        if (successor == null) successor = addSuccessor(instance);
        if (successor != null) successor.learnInstance(instance, totalNumInstances);
    }
    ...
    protected CustomEFDTNode addSuccessor(Instance instance) {
        ...
        if (splitAttribute.isNominal()) {
            if (!successors.isBinary()) {
                ...
                if (!success) // this is the case if the split is binary nominal but the "left"
                    // successor exists.
    ...
    protected CustomEFDTNode newNode(
            int depth, DoubleVector classDistribution, List<Integer> usedNominalAttributes) {
        return new CustomEFDTNode(
                ...
                binaryOnly,
                noPrePrune,
                nominalObserverBlueprint,
                classDistribution,
                usedNominalAttributes,
                -1 // we don't block attributes in EFDT
                );
    }
    ...
    protected void updateObservers(Instance instance) {
        for (int i = 0; i < instance.numAttributes() - 1; i++) { // update likelihood
            ...
            if (obs == null) {
                obs =
                        instance.attribute(instAttIndex).isNominal()
                                ? newNominalClassObserver()
                                : newNumericClassObserver();
                this.attributeObservers.set(i, obs);
            }
            obs.observeAttributeClass(
                    instance.value(instAttIndex), (int) instance.classValue(), instance.weight());
        }
    }
     ...
     */
    boolean isLeaf() {
        if (successors == null) return true;
    ...
    protected List<Integer> getUsedNominalAttributesForSuccessor(
            Attribute splitAttribute, Integer splitAttributeIndex) {
        List<Integer> usedNomAttributesCpy = new ArrayList<>(usedNominalAttributes); // deep copy
        if (splitAttribute.isNominal()) usedNomAttributesCpy.add(splitAttributeIndex);
    ...
    protected void updateInfogainSum(AttributeSplitSuggestion[] suggestions) {
        for (AttributeSplitSuggestion sugg : suggestions) {
            if (sugg.splitTest != null) {
                ...
                infogainSum.put(
                        (sugg.splitTest.getAttsTestDependsOn()[0]), currentSum + sugg.merit);
    ...
    protected boolean shouldSplitLeaf(
            AttributeSplitSuggestion[] suggestions,
            double confidence,
            DoubleVector observedClassDistribution) {
        ...
        if (suggestions.length < 2) {
            ...
            if (bestSuggestion.merit < 1e-10) shouldSplit = false; // we don't use average here
            ...
            if (splitTest instanceof NominalAttributeMultiwayTest) {
                ...
                NominalAttributeClassObserver obs =
                        (NominalAttributeClassObserver)
                                attributeObservers.get(getSplitAttributeIndex());
                AttributeSplitSuggestion xCurrent =
                        obs.forceSplit(
                                splitCriterion,
                                observedClassDistribution.getArrayCopy(),
                                getSplitAttributeIndex(),
                                true,
                                currentValue);
                merit = xCurrent == null ? 0.0 : xCurrent.merit;
                if (xCurrent != null) merit = xCurrent.splitTest == null ? 0.0 : xCurrent.merit;
     ...
     * This is the case when the parent node splits on that attribute. It prevents splitting the
     * space into thinner and thinner slices.
     *
     ...
     * @return
     */
    AttributeSplitSuggestion replaceBestSuggestionIfAttributeIsBlocked(
            AttributeSplitSuggestion bestSuggestion,
            AttributeSplitSuggestion[] suggestions,
            int blockedAttributeIndex) {
        if (suggestions.length == 0) return null;
        if (bestSuggestion.splitTest == null) return bestSuggestion;
        if (suggestions.length == 1) return bestSuggestion;
