...
public class IademCommonProcedures implements Serializable {
    ...
    public static void insertLotsHoles(
            ArrayList<Double> lots, ArrayList<Integer> holes, double inf, double sup) {
        if (inf != sup) {
            int currentPos = 1;
            if (inf == 0.0) {
    ...
    public static double computeLevel(
            ArrayList<Double> lots, ArrayList<Integer> holes, double amount) {
        double level = 0.0;
        int currentLot = 0;
    ...
    public static double computeBound(double average, double n) {
        ...
        if (n == 0.0) {
            ...
            if (average >= (1.0 / 6.0)) {
                ...
                chernoffBound =
                        ((3.0 * ln_2_div_confidence)
                                        + (Math.sqrt(
                                                (9.0 * square_ln_2_div_confianza)
                                                        + (12.0
                                                                * n
                                                                * average
                                                                * ln_2_div_confidence))))
