...
public class FIMTDD extends AbstractRegressor implements Regressor {
	...
	public static class LeafNode extends Node {
		...
		public void learnFromInstance(Instance inst, boolean growthAllowed, double prediction) {
			...
			// sum of absolute errors
			sumOfAbsErrors += inst.weight() * Math.abs(tree.normalizeTargetValue(Math.abs(inst.classValue() - prediction)));
	...
	public class FIMTDDPerceptron implements Serializable {
		...
		public double[] normalizedInstance(Instance inst) {
			...
			normalizedInstance[inst.numInputAttributes()] = tree.examplesSeen > 1 ? 1 : 0; // Value to be multiplied with the constant factor
