 ...
 *
 * <p>Local Adaptive Streaming Tree (LAST) is an incremental decision tree with adaptive splitting
 * mechanisms. LAST maintains a change detector at each leaf and splits this node if a change is
 * detected in the error or the leaf`s data distribution.
 *
 * <p>LAST is still not suitable for use as a base classifier in ensembles due to the change
 * detectors. The authors of the paper are working on a version of LAST that overcomes this
 * limitation.
 *
 * <p>D. N. Assis, J. P. Barddal, and F. Enembreck. 2024. Just Change on Change: Adaptive Splitting
 * Time for Decision Trees in Data Stream Classification. In Proceedings of the 39th ACM/SIGAPP
 * Symposium on Applied Computing (SAC '24).
 *
 * <p>Parameters:
 *
 * <ul>
 *   <li>-m : Maximum memory consumed by the tree
 *   <li>-n : Numeric estimator to use :
 *       <ul>
 *         <li>Gaussian approximation evaluating 10 splitpoints
 *         <li>Gaussian approximation evaluating 100 splitpoints
 *         <li>Greenwald-Khanna quantile summary with 10 tuples
 *         <li>Greenwald-Khanna quantile summary with 100 tuples
 *         <li>Greenwald-Khanna quantile summary with 1000 tuples
 *         <li>VFML method with 10 bins
 *         <li>VFML method with 100 bins
 *         <li>VFML method with 1000 bins
 *         <li>Exhaustive binary tree
 *       </ul>
 *   <li>-e : How many instances between memory consumption checks
 *   <li>-g : The number of instances a leaf should observe between split attempts
 *   <li>-s : Split criterion to use. Example : InfoGainSplitCriterion
 *   <li>-c : The allowable error in split decision, values closer to 0 will take longer to decide
 *   <li>-t : Threshold below which a split will be forced to break ties
 *   <li>-b : Only allow binary splits
 *   <li>-z : Stop growing as soon as memory limit is hit
 *   <li>-r : Disable poor attributes
 *   <li>-p : Disable pre-pruning
 *   <li>-x : Change detector that determines the split point
 *   <li>-D : activates distribution purity monitoring by the change detector
 *   <li>-l : Leaf prediction to use: MajorityClass (MC), Naive Bayes (NB) or NaiveBayes adaptive
 *       (NBAdaptive).
 *   <li>-q : The number of instances a leaf should observe before permitting Naive Bayes
 * </ul>
 *
 * @author Daniel Nowak Assis (daniel.nowak.assis@gmail.com)
 * @version $Revision: 1 $
 */
public class LAST extends AbstractClassifier implements MultiClassClassifier, CapabilitiesHandler {
    ...
    public ClassOption changeDetectionMethodOption =
            new ClassOption(
                    "changeDetectionMethodOption",
                    'x',
                    "Change detector for determining split points. If a change is detected in the"
                            + " leaf node, a split will happen",
                    ChangeDetector.class,
                    "ADWINChangeDetector");

    public FlagOption distributionOption =
            new FlagOption(
                    "distributionOption", 'D', "Monitor distribution purity instead of error rate");
    ...
    public static class Node extends AbstractMOAObject {
        ...
        public void describeSubtree(LAST ht, StringBuilder out, int indent) {
    ...
    protected Measurement[] getModelMeasurementsImpl() {
        return new Measurement[] {
            ...
            new Measurement(
                    "tree size (leaves)", this.activeLeafNodeCount + this.inactiveLeafNodeCount),
        ...
        if (shouldSplit) {
            ...
            if (splitDecision.splitTest == null) {
                ...
                for (int i = 0; i < splitDecision.numSplits(); i++) {
                    Node newChild =
                            newLearningNode(splitDecision.resultingClassDistributionFromSplit(i));
    ...
    public static class LearningNodeNBAdaptive extends LearningNodeNB {
        ...
        public void learnFromInstance(Instance inst, LAST ht) {
            ...
            if (!this.monitorDistributionPurity) {
                ...
                double purity = 0.0;
                if (ht.splitCriterionOption
                        .getValueAsCLIString()
                        .equals("InfoGainSplitCriterion")) {
                    purity =
                            InfoGainSplitCriterion.computeEntropy(
                                    this.observedClassDistribution.getArrayRef());
                } else {
                    purity =
                            GiniSplitCriterion.computeGini(
                                    this.observedClassDistribution.getArrayRef());
                }
    ...
    protected LearningNode newLearningNode(double[] initialClassObservations) {
        ...
        if (predictionOption == 0) { // MC
            ret =
                    new ActiveLearningNode(
                            initialClassObservations,
                            ((ChangeDetector)
                                            getPreparedClassOption(
                                                    this.changeDetectionMethodOption))
                                    .copy(),
                            this.distributionOption.isSet());
