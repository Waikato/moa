...
public class ORTO extends FIMTDD implements Regressor {
    ...
    public FloatOption optionDecayFactorOption =
            new FloatOption(
                    "optionDecayFactor",
                    'z',
                    "The option decay factor that determines how many options can be selected at a"
                            + " given level.",
                    0.9,
                    0.0,
                    1.0);
    ...
    public MultiChoiceOption optionNodeAggregationOption =
            new MultiChoiceOption(
                    ...
                    new String[] {"average", "bestTree"},
                    new String[] {"Average", "Best tree"},
                    0);
    ...
    public void processInstance(
        ...
        if (node instanceof OptionNode) {
            processInstanceOptionNode(
                    inst, (OptionNode) node, prediction, normalError, growthAllowed, inAlternate);
        } else {
                ...
                if (currentNode instanceof LeafNode) {
                    ...
                    if (!inAlternate && iNode.alternateTree != null) {
                        ...
                        double lossA =
                                Math.pow(
                                        inst.classValue()
                                                - currentNode.alternateTree.getPrediction(inst),
                                        2);
                        ...
                        if (iNode.lossExamplesSeen - iNode.previousWeight
                            ...
                            if (Qi > 0) {
                                ...
                                optionNodeCount +=
                                        currentNode.alternateTree.getNumSubtrees()
                                                - currentNode.getNumSubtrees();
                                removeExcessTrees();
                                ...
                                altTree = false;
                                            ...
                                            && iNode.lossExamplesSeen
                                                    >= (10 * alternateTreeTMinOption.getValue())
                                    || iNode.lossExamplesSeen
                                            >= alternateTreeTimeOption.getValue()) {
                        ...
                        if (altTree) {
                            ...
                            processInstance(
                                    inst,
                                    currentNode.alternateTree,
                                    prediction,
                                    normalError,
                                    true,
                                    true); // growth is allowed in the alt tree
    ...
    public void processInstanceOptionNode(
            Instance inst,
            OptionNode node,
            double prediction,
            double normalError,
        ...
        if (node.changeDetection) {
            ...
            if (((InnerNode) node)
                    .PageHinckleyTest(
                            error
                                    - node.sumOfAbsErrors / node.examplesSeen
                                    + PageHinckleyAlphaOption.getValue(),
                            PageHinckleyThresholdOption.getValue())) {
        ...
        for (Node child : node.children) {
            ...
            // Loop for compatibility with bagging methods
            for (int i = 0; i < inst.weight(); i++) {
                node.optionFFSeen[index] =
                        node.optionFFSeen[index] * optionFadingFactorOption.getValue() + 1;
                node.optionFFSSL[index] =
                        node.optionFFSSL[index] * optionFadingFactorOption.getValue()
        ...
        for (Node child : node.children) {
            processInstance(
                    ...
                    child.getPrediction(inst),
                    normalError,
                    growthAllowed && node.alternateTree == null,
                    inAlternate);
        }
    }
    ...
    protected void attemptToSplit(LeafNode node, Node parent, int parentIndex) {
        ...
        if (bestSplitSuggestions.length == 1) {
        ...
        } else if (bestSplitSuggestions.length
                > 1) { // Otherwise, consider which of the splits proposed may be worth trying
            ...
            double hoeffdingBound =
                    computeHoeffdingBound(1, splitConfidenceOption.getValue(), node.examplesSeen);
            ...
            } else if (numTrees < maxTreesOption.getValue()
                    && node.getLevel() <= maxOptionLevelOption.getValue()) {
        ...
        // If the splitting criterion was met, split the current node using the chosen attribute
        // test, and
        ...
        if (numSplits > 0) {
            double optionFactor =
                    numSplits
                            * Math.pow(
                                    optionDecayFactorOption.getValue(), (double) node.getLevel());
