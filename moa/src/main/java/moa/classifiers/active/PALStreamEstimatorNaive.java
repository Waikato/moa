 ...
 * @version $Revision: 1 $
 */
public class PALStreamEstimatorNaive{
	
	private double bandwidth;
	private RingBuffer<double[]> points;
	
	public PALStreamEstimatorNaive(double bandwidth, int bufferSize){
	...
	public double getFrequencyEstimate(double[] x, double[] std){
		...
		for(int j = 0; j < x.length; ++j){
			...
			for(int i=0;i<points.size();i++){
				help +=getNormal(x[j], points.get(i)[j], std[j]);
			}
			freq *= help;
		}
		return Math.pow(freq,1.0/x.length);
	}
