...
public class Entry implements Serializable {
    ...
    /**
     * The buffer of this entry. It can also be seen as the buffer of the child node, it is here
     * just to simplify the insertion recuersion.
     */
    private ClusKernel buffer;

    /**
     * A reference to the next node in the tree. <code>null</code> if we are at a leaf, or this is
     * an entry is part of a lying <code>Node</code>.
     */
    private Node child;
    ...
    /**
     * The timestamp to be used when no operation has yet been done on this entry.
     *
     * @see #timestamp
     */
    ...
    /**
     * Constructor for the entry. To be used when we want to create an empty entry. Notice that the
     * timestamp will be set to zero, since there is no reason to know when an empty entry was
     * generated.
     *
     * @param numberDimensions The dimensionality of the data point in tree where this entry is
     *     used.
     */
    ...
    /**
     * Constructor that creates an <code>Entry</code> that points to the given node. The values of
     * <code>data</code> will be calculated for this.
     *
     * @param numberDimensions The dimensionality of the node.
     * @param node The node to which the new <code>Entry</code> should point.
     * @param currentTime The timestamp for the moment where this Entry was was generated.
     * @see Node
     * @see #data
     */
    protected Entry(
    ...
    /**
     * Constructuctor that creates an <code>Entry</code> with an empty buffer and the <code>data
     * </code> given by the <code>Kernel</code>.
     *
     * @param numberDimensions The dimensionality of the information in the cluster.
    ...
    /**
     * Clear the Entry. All points in the buffer and in the data cluster are lost, the connection to
     * the child is lost and the timestamp is set to the default value.
     */
    ...
    /**
     * Clear the <code>data</code> and the <code>buffer Custer</code> in this entry. This function
     * does not clear the child of this <code>Entry</code>.
     *
     * @see #data
     * @see #buffer
     * @see ClusKernel
     ...
     *
     * @param cluster The Kernel cluster to which the distance is to be calculated.
     * @return The distance to the data <code>Kernel</code> in this <code>Entry</code>
     * @see ClusKernel
     * @see #data
     */
     ...
     *
     * @param other The <code>Entry</code> to which the distance is to be calculated.
     * @return The distance to the data <code>Kernel</code> in this <code>Entry</code> of the data
     *     <code>Kernel</code> in the other <code>Entry</code>.
     * @see ClusKernel
     * @see #data
     */
    ...
    /**
     * When this entry is empty, give it it's first values. It makes sense to have this operation
     * separated from the aggregation, because the aggregation first weights the values in <code>
     * data</code> and <code>Kernel</code>, which makes no sense in an empty entry.
     *
     * @param other The entry with the information to be used to initialize this entry.
    ...
    /**
     * Add the data cluster of another entry to the data cluster of this entry. By using this
     * function the timestamp does not get updated, nor does this entry get older.
     *
     * @param other The entry of which the data cluster should be added to the local data cluster.
     * @see #data
    ...
    /**
     * Aggregate the <code>data</code> in the <code>Kernel</code> of the other <code>Entry</code>.
     *
    ...
    /**
     * Aggregate the given <code>Kernel</code> to the <code>data</code> cluster of this entry.
     *
    ...
    /**
     * Aggregate the given <code>Kernel</code> to the <code>buffer</code> cluster of this entry.
     *
     * @param pointToInsert The cluster to aggregate to the buffer.
     * @param currentTime The time at which the aggregation occurs.
     * @param negLambda A parameter needed to weight the current state of the buffer.
     */
    ...
    /**
     * Merge this entry witht the given <code>Entry</code>. This adds the data cluster of the given
     * Entry to the data cluster of this entry and sets the timestamp to the newest one of the the
     * two entries.
     *
    ...
    /**
     * Return the reference to the child of this <code>Entry</code> to navigate in the tree.
     *
     ...
     *
     * @param child The <code>Node</code> that should be a child of this <code>Entry</code>
     * @see Node
     */
    ...
    /**
     * Clear the buffer in this entry and return a copy. No side effects are possible (given that
     * the copy constructor of <code>Kernel</code> makes a deep copy).
     *
    ...
    /**
     * Check if this <code>Entry</code> is empty or not. An <code>Entry</code> is empty if the
     * <code>data Kernel</code> is empty, since then the buffer has to be empty.
     *
     * @return <code>true</code> if the data cluster has no data points, <code>false</code>
     *     otherwise.
     */
    ...
    /**
     * Overwrites the LS, SS and weightedN in the data cluster of this <code>Entry</code> to the
     * values of the data cluster in the given <code>Entry</code>, but adds N and classCount of the
     * cluster in the given Entry to the data cluster in this one. This function is useful when the
    ...
    /**
     * This functions reads every entry in the child node and calculates the corresponding <code>
     * data Kernel</code>. Timestamps are not changed.
     *
     * @see #data
     * @see ClusKernel
     */
    ...
    /**
     * Returns true if this entry is irrelevant with respecto the given threshold. This is done by
     * comparing the weighted N of this Entry to the threshold, if it is smaller, than the entry is
     * deemed to be irrelevant.
     *
     * @param threshold The threshold under which entries at leafs can be erased.
