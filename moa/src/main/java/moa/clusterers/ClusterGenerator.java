...
public class ClusterGenerator extends AbstractClusterer {
    ...
    public IntOption timeWindowOption =
            new IntOption("timeWindow", 't', "Rang of the window.", 1000);
    ...
    public FloatOption clusterRemoveOption =
            new FloatOption(
                    "clusterRemove",
                    'D',
                    "Deletes complete clusters from the clustering.",
                    0,
                    0,
                    1);
    ...
    public FloatOption joinClustersOption =
            new FloatOption(
                    "joinClusters",
                    'j',
                    "Join two clusters if their hull distance is less minRadius times this factor.",
                    0,
                    0,
                    1);

    public FloatOption clusterAddOption =
            new FloatOption("clusterAdd", 'A', "Adds additional clusters.", 0, 0, 1);
    ...
    public Clustering getMicroClusteringResult() {
        ...
        if (sourceClustering == null) {
            ...
            System.out.println(
                    "You need to set a source clustering for the ClusterGenerator to work");
            return null;
        }
    ...
    public Clustering getClusteringResult() {
        ...
        //        if(sourceClustering == null){
        //            System.out.println("You need to set a source clustering for the
        // ClusterGenerator to work");
        //            return null;
        //        }
        return alterClustering(sourceClustering);
    }
    ...
    private Clustering alterClustering(Clustering scclustering) {
        ...
        for (int i = 0; i < numCluster; i++) {
            ...
            double[] center =
                    Arrays.copyOf(sourceCluster.getCenter(), sourceCluster.getCenter().length);
            ...
            if (errLevelPosition > 0) {
                ...
                for (int d = 0; d < center.length; d++) {
                    if (newCenter[d] >= 0 && newCenter[d] <= 1) {
                        ...
                        System.out.println(
                                "This shouldnt have happend, Cluster center out of bounds:"
            ...
            if (errLevelRadiusDecrease > 0 || errLevelRadiusIncrease > 0) {
                ...
                if (errLevelRadiusDecrease > 0
                        && (errLevelRadiusIncrease == 0 || random.nextBoolean())) {
                    double level =
                            (errLevelRadiusDecrease
                                    + errOffsetDirection
                                            * errOffset); // *sourceCluster.getWeight()/sumWeight;
    ...
    private Clustering joinClusters(Clustering clustering) {
        ...
        if (radiusFactor > 0) {
            for (int c1 = 0; c1 < clustering.size(); c1++) {
                ...
                int maxindex = -1;
                if (maxindexOver != -1) maxindex = maxindexOver;
                else maxindex = maxindexCon;

                if (maxindex != -1 && !merged[c1]) {
                    merged[c1] = true;
                    merged[maxindex] = true;
                    SphereCluster scnew =
                            new SphereCluster(sc1.getCenter(), sc1.getRadius(), sc1.getWeight());
        ...
        for (int i = 0; i < merged.length; i++) {
            if (!merged[i]) mclustering.add(clustering.get(i));
        }
