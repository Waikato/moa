 ...
 *
 * <p>Citation: David Arthur, Sergei Vassilvitskii: k-means++: the advantages of careful seeding.
 * SODA 2007: 1027-1035
 */
public class CoresetKMeans {
    ...
    public static List<double[]> generatekMeansPlusPlusCentroids(
            int k, List<double[]> input, Random random) {
        int n = input.size();
        assert (n > 0);
        ...
        for (int i = 1; i < k; i++) {
            ...
            for (int j = 0; j < n; j++) {
                ...
                distance[j] =
                        Math.min(
                                distance[j],
                                point[0] * Metric.distanceSquared(lastCenter, point, 1));
                sum += distance[j];
            }
            int candidate = 0;
            if (sum > 0) {
