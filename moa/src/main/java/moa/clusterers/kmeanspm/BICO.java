 ...
 *
 * <p>Citation: Hendrik Fichtenberger, Marc Gillé, Melanie Schmidt, Chris Schwiegelshohn, Christian
 * Sohler: BICO: BIRCH Meets Coresets for k-Means Clustering. ESA 2013: 481-492 (2013)
...
public class BICO extends AbstractClusterer {
    ...
    public IntOption numClustersOption =
            new IntOption("Cluster", 'k', "Number of desired centers.", 5, 1, Integer.MAX_VALUE);

    public IntOption numDimensionsOption =
            new IntOption(
                    "Dimensions",
                    'd',
                    "Number of the dimensions of the input points.",
                    10,
                    1,
                    Integer.MAX_VALUE);
    ...
    public Clustering getMicroClusteringResult() {
        return this.root.addToClustering(
                new Clustering(new AutoExpandVector<Cluster>(this.rootCount)));
    }
    ...
    public Clustering getClusteringResult() {
        ...
        if (this.bufferPhase) {
            ...
            microClustering =
                    this.root.addToClusteringCenters(new ArrayList<double[]>(this.rootCount));
        }
        ...
        for (int i = 0; i < 5; i++) {
            ...
            List<double[]> newResult =
                    CoresetKMeans.generatekMeansPlusPlusCentroids(
                            this.numClusters, microClustering, this.clustererRandom);
    ...
    public void resetLearningImpl() {
        ...
        int hashSize =
                (int)
                        Math.ceil(
                                Math.log(5 * this.maxNumClusterFeaturesOption.getValue())
                                        / Math.log(2));
        this.root =
                new ClusteringTreeHeadNode(
                        null,
                        new ClusteringFeature(new double[0], 1),
                        this.numDimensionsOption.getValue(),
                        this.numProjectionsOption.getValue(),
                        Math.min(hashSize, 30),
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        if (this.numDimensions != x.length) {
            System.out.println(
                    "Line skipped because line dimension is "
                            + x.length
                            + " instead of "
                            + this.numDimensions);
            return;
    ...
    /**
     * If the number of ClusteringTreeNodes exceeds the maximum bound, the global threshold T will
     * be doubled and the tree will be rebuild with the new threshold.
     */
    ...
    protected void bicoCFUpdate(ClusteringTreeNode x) {
        ...
        while (true) {
            ...
            if (r.hasNoChildren()
                ...
                // exceeding the global threshold
                if (y.getClusteringFeature()
                                .calcKMeansCosts(y.getCenter(), x.getClusteringFeature())
                        <= this.T) {
