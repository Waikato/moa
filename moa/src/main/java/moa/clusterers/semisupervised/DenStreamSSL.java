...
public class DenStreamSSL extends AbstractClusterer {
    ...
    public FloatOption offlineOption = new FloatOption("offline", 'o',
            "offline multiplier for epsilon.", 2, 2, 20);
        ...
        for (int p = 0; p < initBuffer.size(); p++) {
            ...
            if (!point.covered) {
                ...
                if (neighbourhood.size() > minPoints) {
                    MicroClusterSSL mc = new MicroClusterSSL(point, point.numAttributes(), timestamp, lambda, currentTimestamp);
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        if (!initialized) {
            ...
            if (p_micro_cluster.getClustering().size() != 0) {
                MicroClusterSSL x = nearestCluster(point, p_micro_cluster);
                MicroClusterSSL xCopy = x.copy();
            ...
            if (!merged && (o_micro_cluster.getClustering().size() != 0)) {
                MicroClusterSSL x = nearestCluster(point, o_micro_cluster);
                MicroClusterSSL xCopy = x.copy();
                xCopy.insert(point, timestamp);
            ...
            if (!merged) {
                o_micro_cluster.getClustering().add(
                        new MicroClusterSSL(point, point.toDoubleArray().length, timestamp, lambda, currentTimestamp));
            }
            ...
            //////////////////////////////
            if (timestamp % tp == 0) {
                ArrayList<MicroClusterSSL> removalList = new ArrayList<>();
                for (Cluster c : p_micro_cluster.getClustering()) {
                    if (c.getWeight() < beta * mu) {
                        removalList.add((MicroClusterSSL) c);
                    }
                }
    ...
    /**
     * Expands a cluster with a set of points and neighboring points
     * @param mc the cluster to be expanded
     * @param points the set of points
     * @param neighbourhood the neighboring points
     */
    private void expandCluster(MicroClusterSSL mc, ArrayList<DenPoint> points, ArrayList<Integer> neighbourhood) {
    ...
    private MicroClusterSSL nearestCluster(DenPoint p, Clustering cl) {
        MicroClusterSSL min = null;
        double minDist = 0;
        for (int c = 0; c < cl.size(); c++) {
            MicroClusterSSL x = (MicroClusterSSL) cl.get(c);
            if (min == null) {
                min = x;
            }
