/**
 * [CluStream_kMeans.java] CluStream with k-means as macroclusterer
 *
 * <p>Appeared in seminar paper "Understanding of Internal Clustering Validation Measure in
 * Streaming Environment" (Yunsu Kim) for the course "Seminar: Data Mining and Multimedia Retrival"
 * in RWTH Aachen University, WS 12/13
 *
 * @author Yunsu Kim based on the code of Timm Jansen (moa@cs.rwth-aachen.de) Data Management and
...
public class WithKmeans extends AbstractClusterer {
    ...
    public IntOption maxNumKernelsOption =
            new IntOption("maxNumKernels", 'm', "Maximum number of micro kernels to use.", 100);

    public IntOption kernelRadiFactorOption =
            new IntOption("kernelRadiFactor", 't', "Multiplier for the kernel radius", 2);

    public IntOption kOption =
            new IntOption("k", 'k', "k of macro k-means (number of clusters)", 5);
        ...
        if (!initialized) {
            if (buffer.size() < bufferSize) {
                ...
                for (int i = 0; i < buffer.size(); i++) {
                    kernels[i] =
                            new ClustreamKernel(
                                    new DenseInstance(1.0, buffer.get(i).getCenter()),
                                    dim,
                                    timestamp,
                                    t,
                                    m);
    ...
    protected static Clustering cleanUpKMeans(
        ...
        count = 0;
        for (int i = 0; i < converted.length; i++) {
            if (converted[i] != null) cleaned[count++] = converted[i];
        }
