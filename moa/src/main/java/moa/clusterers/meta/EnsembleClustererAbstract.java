...
// From the json, we create the Algorithm and Parameter classes (depending on the type of the
// parameter) which form the ensemble of clusterers
// These classes are then used to cluster and evaluate the configurations
// When a new configuration is required, a parameter configuration is copied and the parameters
// manipulated
...
public abstract class EnsembleClustererAbstract extends AbstractClusterer {
        ...
        if (this.numberOfCores == 1) {
            ...
            for (int i = 0; i < this.ensemble.size(); i++) {
                EnsembleRunnable trainer =
                        new EnsembleRunnable(this.ensemble.get(i).clusterer, inst);
            ...
            if (this.settings.useTestEnsemble && this.candidateEnsemble.size() > 0) {
                ...
                for (int i = 0; i < this.candidateEnsemble.size(); i++) {
                    EnsembleRunnable trainer =
                            new EnsembleRunnable(this.candidateEnsemble.get(i).clusterer, inst);
                    trainers.add(trainer);
                }
            }
            try {
            ...
            System.out.println(
                    "Clusterer "
                            ...
                            + this.ensemble
                                    .get(this.bestModel)
                                    .clusterer
                                    .getCLICreationString(Clusterer.class)
                            + ") is the active clusterer with performance: "
    ...
    protected double computePerformanceMeasure(Algorithm algorithm) {
        ...
        ClassOption opt =
                new ClassOption(
                        "", ' ', "", MeasureCollection.class, this.settings.performanceMeasure);
        MeasureCollection performanceMeasure =
                (MeasureCollection) opt.materializeObject(null, null);
        ...
        if (this.settings.evaluateMacro || result == null) {
            ...
            if (this.verbose >= 2)
                System.out.println(
                        "Micro-Cluster not available for "
        ...
        if (result == null) {
            throw new RuntimeException(
                    "Neither micro- nor macro clusters available for "
                            + algorithm.clusterer.getCLICreationString(Clusterer.class));
        ...
        for (int i = 0; i < this.candidateEnsemble.size(); i++) {
            ...
            if (this.verbose >= 1) {
                System.out.println(
                        "Test "
                                + i
                                + ") "
                                + newAlgorithm.clusterer.getCLICreationString(Clusterer.class)
                                + "\t => \t Performance: "
                                + performance);
            }
            ...
            if (this.ensemble.size() < this.settings.ensembleSize) {
                ...
                this.ensemble.add(newAlgorithm);
                    ...
                    > EnsembleClustererAbstract.getWorstSolution(this.performanceMeasures)) {
                ...
                int replaceIdx =
                        EnsembleClustererAbstract.sampleProportionally(
    ...
    protected Algorithm sampleNewConfiguration(ArrayList<Double> silhs, int parentIdx) {
        ...
        Algorithm newAlgorithm =
                new Algorithm(
                        ...
                        this.settings.reinitialiseWithClusters,
                        this.verbose);
    ...
    public static void main(String[] args) throws Exception {
        ...
        confStream.fileOption.setValue(
                System.getProperty("user.dir")
                        + "/moa/src/main/java/moa/clusterers/meta/settings.json");
