...
/**
 * The Characteristic Vector of a density grid is defined in Definition 3.2 of Chen and Tu 2007 as:
 * The characteristic vector of a grid g is a tuple (tg,tm,D, label,status), where tg is the last
 * time when g is updated, tm is the last time when g is removed from grid list as a sporadic grid
 * (if ever), D is the grid density at the last update, label is the class label of the grid, and
 * status = {SPORADIC, NORMAL} is a label used for removing sporadic grids.
 *
 * <p>Citation: Y. Chen and L. Tu, “Density-Based Clustering for Real-Time Stream Data,” in
 * Proceedings of the 13th ACM SIGKDD international conference on Knowledge discovery and data
 * mining, 2007, pp. 133–142.
 */
public class CharacteristicVector {
    ...
    /** t_g in the characteristic vector tuple; The last time when g is updated */
    private int updateTime;

    /**
     * t_m in the characteristic vector tuple; the last time when g is removed from grid_list as a
     * sporadic grid (if ever).
     */
    private int removeTime;

    /** D in the characteristic vector tuple; the grid density at the last update */
    private double gridDensity;

    /** label in the characteristic vector tuple; the cluster label of the grid */
    private int label;

    /** status in the characteristic vector tuple; status = {SPORADIC, NORMAL} */
    private boolean isSporadic;

    /**
     * attribute mentioned in figure 4, line 3 of Chen and Tu 2007; attribute = {SPARSE,
     * TRANSITIONAL, DENSE}
     */
    ...
    /**
     * Flag marking whether there was a change in the attribute field the last time the grid density
     * was updated.
     */
    ...
    public CharacteristicVector(
            int tg, int tm, double D, int label, boolean status, double dl, double dm) {
        ...
        if (this.isSparse(dl)) this.attribute = SPARSE;
        else if (this.isDense(dm)) this.attribute = DENSE;
        else this.attribute = TRANSITIONAL;
    ...
    /**
     * @param isSporadic TRUE if the characteristic vector is to be labelled as sporadic, FALSE
     *     otherwise
     */
    ...
    public void densityWithNew(int currTime, double decayFactor) {
        ...
        // System.out.print("["+decayFactor+"^("+currTime+" - "+this.getDensityTimeStamp()+") *
        // "+densityOfG+"] + 1.0 = ");
    ...
    /**
     * Implements the update the density of all grids step given at line 2 of both Fig 3 and Fig 4
     * of Chen and Tu 2007.
     *
     * @param currTime the data stream's current internal time
     * @param decayFactor the value of lambda
    ...
    public void updateGridDensity(int currTime, double decayFactor, double dl, double dm) {
        ...
        // Update the density grid's density
        double densityOfG =
                (Math.pow(decayFactor, (currTime - this.getDensityTimeStamp()))
                        * this.getGridDensity());
        ...
        // Evaluate whether or not the density grid attribute has changed and set the attChange flag
        // accordingly
        if (this.getAttribute() == lastAtt) this.attChange = false;
        else this.attChange = true;
    }
    ...
    public boolean isDense(double dm) {
        if (this.gridDensity >= dm) return true;
        else return false;
    }
    ...
    public boolean isSparse(double dl) {
        if (this.gridDensity <= dl) return true;
        else return false;
    }

    /**
     * Implements the test for whether a density grid is transitional given in eq 10 of Chen and Tu
     * 2007.
     *
     * @param dm the threshold for dense grids
     * @param dl the threshold for sparse grids
    ...
    public boolean isTransitional(double dm, double dl) {
        if (this.isDense(dm) || this.isSparse(dl)) return false;
        else return true;
    }
    ...
    /**
     * @return true if the characteristic vector's attribute changed during the last density update,
     *     false otherwise.
     */
    ...
    public String toString() {
        ...
        if (this.getAttribute() == DENSE) sb.append("D ");
        else if (this.getAttribute() == SPARSE) sb.append("S ");
        else sb.append("T ");
        ...
        if (this.isSporadic()) sb.append("Sporadic ");
        else sb.append("Normal ");

        if (this.isAttChanged()) sb.append("CHANGED");
