...
/**
 * Density Grids are defined in equation 3 (section 3.1) of Chen and Tu 2007 as: In D-Stream, we
 * partition the d−dimensional space S into density grids. Suppose for each dimension, its space Si,
 * i =1, ··· ,d is divided into pi partitions as Si = Si,1 U Si,2 U ··· U Si,pi (2) then the data
 * space S is partitioned into N = PRODUCT (i=1..d) pi density grids. For a density grid g that is
 * composed of S1,j1 ×S2,j2 ···×Sd,jd , ji =1, ...,pi, we denote it as g =(j1,j2, ··· ,jd). (3)
 *
...
public class DensityGrid extends CFCluster {
    ...
    /**
     * For each dimension, its space Si, i =1, ··· ,d is divided into pi partitions as Si = Si,1 U
     * Si,2 U ··· U Si,pi A density grid g that is composed of S1,j1 ×S2,j2 ···×Sd,jd , ji =1,
    ...
    /**
     * Flag denoting whether this density grid has been inspected during the adjustClustering() step
     * of D-Stream.
     */
    ...
    public boolean equals(Object o) {
        ...
        if (o.getClass() != DensityGrid.class) return false;

        DensityGrid dg = (DensityGrid) o;

        if (dg.getDimensions() != dimensions) return false;

        int[] dgCoord = dg.getCoordinates();
        for (int i = 0; i < dimensions; i++) {
            if (dgCoord[i] != coordinates[i]) return false;
        }
    ...
    /**
     * Overrides Object's method hashCode to generate a hashCode for DensityGrids based on their
     * coordinates.
     *
    ...
    /**
     * Generates an Array List of neighbours for this density grid by varying each coordinate by one
     * in either direction. Does not test whether the generated neighbours are valid as DensityGrid
     * is not aware of the number of partitions in each dimension.
     *
    ...
    public double getInclusionProbability(Instance instance) {
        for (int i = 0; i < this.dimensions; i++) {
            if ((int) instance.value(i) != this.coordinates[i]) return 0.0;
        }
