...
/**
 * Citation: Y. Chen and L. Tu, “Density-Based Clustering for Real-Time Stream Data,” in Proceedings
 * of the 13th ACM SIGKDD international conference on Knowledge discovery and data mining, 2007, pp.
 * 133–142.
 */
public class Dstream extends AbstractClusterer {
    ...
    public FloatOption decayFactorOption =
            new FloatOption(
                    "decayFactor", 'd', "The decay factor, lambda, in (0,1)", 0.998, 0.001, 0.999);

    public FloatOption cmOption =
            new FloatOption(
                    "Cm",
                    'm',
                    "Controls the " + "threshold for dense grids, > 1",
                    3.0,
                    1.001,
                    Double.MAX_VALUE);
    ...
    public FloatOption clOption =
            new FloatOption(
                    "Cl",
                    'l',
                    "Controls the " + "threshold for sparse grids, in (0,1)",
                    0.8,
                    0.001,
                    0.999);
    ...
    /**
     * User defined parameter: Adjusts the window of protection for renaming previously deleted
     * grids as being sporadic
     */
    private double beta;
    ...
    /**
     * A list of all density grids which are being monitored; given in figure 1 of Chen and Tu 2007
     */
    private HashMap<DensityGrid, CharacteristicVector> grid_list;

    /**
     * A list of all density grids which have been deleted; allows the recording of tm - the last
     * time when the grid is removed from grid list as a sporadic grid (if ever).
     */
    private HashMap<DensityGrid, Integer> deleted_grids;

    /** A list of all Grid Clusters, which are defined in Definition 3.6 of Chen and Tu 2007 */
    ...
    /**
     * @see moa.clusterers.Clusterer#isRandomizable() D-Stream is not randomizable.
     */
    @Override
    ...
    /**
     * @see
     *     moa.clusterers.AbstractClusterer#trainOnInstanceImpl(com.yahoo.labs.samoa.instances.Instance)
     *     <p>trainOnInstanceImpl implements the procedure given in Figure 1 of Chen and Tu 2007
     */
    @Override
    public void trainOnInstanceImpl(Instance inst) {
        ...
        if (recalculateN) {
            ...
            for (int i = 0; i < this.d; i++) {
                // System.out.print(" "+n);
                if (inst.attribute(i).isNominal()) n = n * inst.attribute(i).numValues();
                else n = n * (3 + maxVals[i] - minVals[i]);
            }
            // System.out.print(" "+n);
            this.N = n;
            ...
            gap =
                    (int)
                            Math.floor(
                                    Math.log(Math.max(optionA, optionB))
                                            / Math.log(this.getDecayFactor()));
            // Ensure that gap is not zero (i.e. if the procedure to calculate gap rounds down to
            // zero, then set gap to 1 and adjust clustering every instance)
            if (gap == 0) {
                gap = 1;
            }
        ...
        if (!this.grid_list.containsKey(dg)) {
            ...
            if (this.deleted_grids.containsKey(dg)) {
                ...
                cv =
                        new CharacteristicVector(
                                this.getCurrTime(),
                                this.deleted_grids.get(dg).intValue(),
                                1.0,
                                -1,
                                false,
                                this.getDL(),
                ...
                cv =
                        new CharacteristicVector(
                                this.getCurrTime(), -1, 1.0, -1, false, this.getDL(), this.getDM());
        ...
        //    b. Adjust clustering
        // System.out.println("\nCurrent Time is " + this.getCurrTime() + " and gap is " +
        // this.gap);
    ...
    private void initialClustering() {
        ...
        Iterator<Map.Entry<DensityGrid, CharacteristicVector>> glIter =
                this.grid_list.entrySet().iterator();
        HashMap<DensityGrid, CharacteristicVector> newGL =
                new HashMap<DensityGrid, CharacteristicVector>();
        ...
        while (glIter.hasNext()) {
            ...
            if (cvOfG.getAttribute() == DENSE) {
                ...
                GridCluster gc =
                        new GridCluster((CFCluster) dg, new ArrayList<CFCluster>(), gridClass);
                gc.addGrid(dg);
                this.cluster_list.add(gc);
                // System.out.print(" was dense (class "+gridClass+")");
            } else cvOfG.setLabel(NO_CLASS);
     ...
     *   <li>For each outside grid g of c
     *   <li>For each neighbouring grid h of g
     *   <li>If h belongs to c', label c and c' with the label of the largest cluster
     *   <li>Else if h is transitional, assign it to c
     * </ol>
     *
            ...
            for (Map.Entry<DensityGrid, Boolean> grid : c.getGrids().entrySet()) {
                ...
                if (!inside) {
                    ...
                    while (dgNeighbourhood.hasNext()) {
                        ...
                        // System.out.print("Inspecting neighbour, dgprime:"+dgprime.toString()+",
                        // standby...");
                        ...
                        if (this.grid_list.containsKey(dgprime)) {
                            ...
                            if (class1 != class2) {
                                ...
                                if (class2 != NO_CLASS) {
                                    ...
                                    if (this.cluster_list.get(class1).getWeight()
                                            < this.cluster_list.get(class2).getWeight())
                                        mergeClusters(class1, class2);
                                    else mergeClusters(class2, class1);
                                ...
                                // If dgprime is transitional and outside of c, assign it to c
                                    ...
                                    // System.out.println("h is transitional and is assigned to
                                    // cluster "+class1);
                                    cv2.setLabel(class1);
                                    c.addGrid(dgprime);
    ...
    /**
     * Performs the periodic adjustment of clusters every 'gap' timesteps. Implements the procedure
    ...
    /**
     * Inspects each density grid in grid_list whose attribute has changed since the last call to
     * adjustClustering. Implements lines 3/4/7/19 of the procedure given in Figure 4 of Chen and Tu
     * 2007.
     *
     * @return TRUE if any grids are updated; FALSE otherwise.
     */
    private boolean inspectChangedGrids() {
        HashMap<DensityGrid, CharacteristicVector> glNew =
                new HashMap<DensityGrid, CharacteristicVector>();
        Iterator<Map.Entry<DensityGrid, CharacteristicVector>> gridIter =
                this.grid_list.entrySet().iterator();
        ...
        while (gridIter.hasNext() && glNew.isEmpty()) {
            ...
            if (cv.isAttChanged() && !dg.isVisited()) {
                ...
                if (cv.getAttribute() == SPARSE) glNew.putAll(adjustForSparseGrid(dg, cv, dgClass));
        ...
        // If there are grids in glNew, update the corresponding grids in grid_list and clean up the
        // cluster list
        if (!glNew.isEmpty()) {
            // System.out.println("There are "+glNew.size()+" entries to update from glNew to
            // grid_list.");
     ...
     * @return a HashMap<DensityGrid, CharacteristicVector> containing density grids for update
     *     after this iteration
     */
    private HashMap<DensityGrid, CharacteristicVector> adjustForSparseGrid(
            DensityGrid dg, CharacteristicVector cv, int dgClass) {
        HashMap<DensityGrid, CharacteristicVector> glNew =
                new HashMap<DensityGrid, CharacteristicVector>();
        ...
        if (dgClass != NO_CLASS) {
            ...
            if (gc.getWeight() > 0.0 && !gc.isConnected()) glNew.putAll(recluster(gc));
        }
        // else
    ...
    /**
     * Reclusters a gridcluster into two (or more) constituent clusters when it has been identified
     * that the original cluster is no longer a grid group. It does so by echoing the initial
     * clustering procedure over only those grids in gc.
     *
    ...
    private HashMap<DensityGrid, CharacteristicVector> adjustNewLabels(
            HashMap<DensityGrid, CharacteristicVector> glNew) {
        Iterator<GridCluster> newClusIter = newClusterList.iterator();
        HashMap<DensityGrid, CharacteristicVector> glAdjusted =
                new HashMap<DensityGrid, CharacteristicVector>();
                        ...
                        if (glNew.containsKey(dgprime)) {
                            ...
                            if (class1 != class2) {
                                ...
                                if (class2 != NO_CLASS) {
                                    ...
                                    else glAdjusted.putAll(mergeNewClusters(glNew, class2, class1));
    ...
    private HashMap<DensityGrid, CharacteristicVector> mergeNewClusters(
            HashMap<DensityGrid, CharacteristicVector> glNew, int smallClus, int bigClus) {
    ...
    /**
     * Adjusts the clustering of a dense density grid. Implements lines 8 through 18 from Figure 4
     * of Chen and Tu 2007.
     *
     ...
     *     after this iteration
     */
    private HashMap<DensityGrid, CharacteristicVector> adjustForDenseGrid(
            DensityGrid dg, CharacteristicVector cv, int dgClass) {
        // System.out.print("Density grid "+dg.toString()+" is adjusted as a dense grid at time
        // "+this.getCurrTime()+". ");

        // Among all neighbours of dg, find the grid h whose cluster ch has the largest size
        GridCluster ch; // The cluster, ch, of h
        DensityGrid hChosen =
                new DensityGrid(dg); // The chosen grid h, whose cluster ch has the largest size
        ...
        // System.out.println(" Chosen neighbour is "+hChosen.toString()+" from cluster
        // "+hChosenClass+".");
        ...
        if (hChosenClass != NO_CLASS && hChosenClass != dgClass) {
            ...
            if (this.grid_list.get(hChosen).getAttribute() == DENSE) {
                ...
                // Else if dg belongs to cluster c and h belongs to c'
                    ...
                    if (gSize <= hChosenSize) mergeClusters(dgClass, hChosenClass);
                    else mergeClusters(hChosenClass, dgClass);
                }
            }
            ...
            // Else if h is a transitional grid
                ...
                // Else if dg is in cluster c and |c| >= |ch|
                    ...
                    if (gSize >= hChosenSize) {
                        ...
                        // System.out.println("dgClass is "+dgClass+", hChosenClass is
                        // "+hChosenClass+", gSize is "+gSize+" and hChosenSize is "+hChosenSize+" h
                        // is added to cluster "+dgClass+".");
                        this.cluster_list.set(hChosenClass, ch);
        ...
        // grids to add
            ...
            while (dgNeighbourhood.hasNext()) {
                ...
                if (this.grid_list.containsKey(dghprime) && !c.getGrids().containsKey(dghprime)) {
                    ...
                    if (cvhprime.getAttribute() == TRANSITIONAL) {
                        // System.out.println("Added "+dghprime.toString()+" to cluster
                        // "+newClass+".");
                        c.addGrid(dghprime);
            ...
            // System.out.println("Cluster "+newClass+":
            // "+this.cluster_list.get(newClass).toString());
        }
    ...
    /**
     * Adjusts the clustering of a transitional density grid. Implements lines 20 and 21 from Figure
     ...
     *     after this iteration
     */
    private HashMap<DensityGrid, CharacteristicVector> adjustForTransitionalGrid(
            DensityGrid dg, CharacteristicVector cv, int dgClass) {
        // System.out.print("Density grid "+dg.toString()+" is adjusted as a transitional grid at
    ...
    /**
     * Iterates through cluster_list to ensure that all empty clusters have been removed and that
     * all cluster IDs match the cluster's index in cluster_list.
     */
    private void cleanClusters() {
        ...
        while (clusIter.hasNext()) {
            ...
            Iterator<Map.Entry<DensityGrid, Boolean>> gridsOfClus =
                    c.getGrids().entrySet().iterator();
    ...
    private HashMap<DensityGrid, CharacteristicVector> cleanNewClusters(
            HashMap<DensityGrid, CharacteristicVector> glNew) {
    ...
    private void removeSporadic() {
        ...
        while (glIter.hasNext()) {
            ...
            if (cv.isSporadic()) {
                ...
                if ((this.getCurrTime() - cv.getUpdateTime()) >= gap) {
                    ...
                    if (dgClass != -1) this.cluster_list.get(dgClass).removeGrid(dg);
                ...
                // Else if (S1 && S2), mark as sporadic - Else mark as normal
                    ...
                    // System.out.println("within gap" + dg.toString() + " sporadicity assessed
                    // "+cv.isSporadic());
                    newGL.put(dg, cv);
                }
        ...
        while (remIter.hasNext()) {
            ...
            // System.out.println("Removing sporadic grid "+sporadicDG.toString()+" at time
            // "+this.getCurrTime()+".");
    ...
    /**
     * Determines whether a sparse density grid is sporadic using rules S1 and S2 of Chen and Tu
     * 2007
     *
    ...
    private boolean checkIfSporadic(CharacteristicVector cv) {
        // Check S1
        if (cv.getCurrGridDensity(this.getCurrTime(), this.getDecayFactor())
                < densityThresholdFunction(
                        cv.getDensityTimeStamp(), this.cl, this.getDecayFactor(), this.N)) {
            // Check S2
            if (cv.getRemoveTime() == -1
                    || this.getCurrTime() >= ((1 + this.beta) * cv.getRemoveTime())) return true;
        }
     ...
     * @param decayFactor - user defined parameter which is represented as lambda in Chen and Tu
     *     2007
    ...
    private double densityThresholdFunction(int tg, double cl, double decayFactor, int N) {
        return (cl * (1.0 - Math.pow(decayFactor, (this.getCurrTime() - tg + 1.0))))
                / (N * (1.0 - decayFactor));
    }

    /**
     * Reassign all grids belonging in the small cluster to the big cluster Merge the GridCluster
     * objects representing each cluster
     *
    ...
    /**
     * Iterates through grid_list and updates the density for each density grid therein. Also marks
     * each density grid as unvisited for this call to adjustClustering.
     */
    private void updateGridListDensity() {
        for (Map.Entry<DensityGrid, CharacteristicVector> grid : grid_list.entrySet()) {
            ...
            dg.setVisited(false);
            cvOfG.updateGridDensity(
                    this.getCurrTime(), this.getDecayFactor(), this.getDL(), this.getDM());
    ...
    /**
     * @return dm - the density threshold for dense grids. It is controlled by cl and given in eq 8
     *     of Chen and Tu 2007
     */
    public double getDM() {
        return this.dm;
    ...
    /**
     * @return dl - the density threshold for sparse grids. It is controlled by cl and given in eq 9
     *     of Chen and Tu 2007
     */
    public double getDL() {
        return this.dl;
    ...
    /**
     * Prints out the values of the parameters associated with this instance of the D-Stream
     * algorithm:
     *
     * <ol>
     *   <li>gap;
     *   <li>decay factor (lambda);
     *   <li>C_m and C_l;
     *   <li>D_m and D_l; and
     *   <li>beta.
     * </ol>
    ...
    public void printGridList() {
        ...
        for (Map.Entry<DensityGrid, CharacteristicVector> grid : grid_list.entrySet()) {
            ...
            if (cv.getAttribute() != SPARSE) {
                double dtf =
                        densityThresholdFunction(
                                cv.getUpdateTime(), this.cl, this.getDecayFactor(), this.N);
                System.out.println(
                        dg.toString()
                                + " "
                                + cv.toString()
    ...
    public void printGridClusters() {
        ...
        for (GridCluster gc : this.cluster_list) {
            System.out.println(
                    gc.getClusterLabel() + ": " + gc.getWeight() + " {" + gc.toString() + "}");
        }
    }
