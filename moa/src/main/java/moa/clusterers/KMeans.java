 ...
 * groundtruthclustering for implementation. There should also be an option to use random centers.
 * TODO: random centers TODO: Create a macro clustering interface to make different macro clustering
 * algorithms available to micro clustering algorithms like clustream, denstream and clustree
 */
public class KMeans {

    /**
     * This kMeans implementation clusters a big number of microclusters into a smaller amount of
     * macro clusters. To make it comparable to other algorithms it uses the real centers of the
     * ground truth macro clustering to have the best possible initialization. The quality of
     * resulting macro clustering yields an upper bound for kMeans on the underlying
     * microclustering.
     *
    ...
    public static Clustering gaussianMeans(Clustering gtClustering, Clustering clustering) {
        ...
        for (CFCluster microcluster : microclusters) {
            ...
            for (int i = 0; i < k; i++) {
                double distance =
                        distance(kMeansResult.get(i).getCenter(), microcluster.getCenter());
                if (distance < minDistance) {
                    closestCluster = i;
                    minDistance = distance;
                }
        ...
        count = 0;
        for (int i = 0; i < res.length; i++) {
            if (res[i] != null) cleaned[count++] = res[i];
        }
