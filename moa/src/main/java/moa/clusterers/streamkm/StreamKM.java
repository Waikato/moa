 ...
 *     <p>Citation: Marcel R. Ackermann, Christiane Lammersen, Marcus Märtens, Christoph Raupach,
 *     Christian Sohler, Kamil Swierkot: StreamKM++: A Clustering Algorithms for Data Streams.
 *     ALENEX 2010: 173-187
 */
public class StreamKM extends AbstractClusterer {

    public IntOption sizeCoresetOption =
            new IntOption("sizeCoreset", 's', "Size of the coreset (m).", 10000);
    ...
    public IntOption lengthOption =
            new IntOption(
                    "length", 'l', "Length of the data stream (n).", 100000, 0, Integer.MAX_VALUE);

    public FlagOption evaluateOption =
            new FlagOption(
                    "evaluateFinalOnly", 'e', "If true, only the final clustering is evaluated.");
    ...
    private static final double THRESHOLD = 1.000;
    ...
    public void trainOnInstanceImpl(Instance inst) {
        ...
        if (this.initialized == false) {
            ...
            manager =
                    new BucketManager(
                            this.length, this.dimension, this.coresetsize, this.clustererRandom);
    ...
    public CoresetCostTriple lloydPlusPlus(int k, int n, int d, Point points[]) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < points[i].dimension; j++) {
                distance =
                        Math.abs(
                                (centres[centre].coordinates[j] / centres[centre].weight)
    ...
    /**
     * computes the target function for the given pointarray points[] (of size n) with the given
     * array of centres centres[] (of size k)
     */
