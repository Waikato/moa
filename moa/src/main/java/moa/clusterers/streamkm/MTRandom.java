 ...
 *     <p>A Java implementation of the MT19937 (Mersenne Twister) pseudo random number generator
 *     algorithm based upon the original C code by Makoto Matsumoto and Takuji Nishimura (see <a
 *     href="http://www.math.sci.hiroshima-u.ac.jp/~m-mat/MT/emt.html">
 *     http://www.math.sci.hiroshima-u.ac.jp/~m-mat/MT/emt.html</a> for more information.
 *     <p>As a subclass of java.util.Random this class provides a single canonical method next() for
 *     generating bits in the pseudo random number sequence. Anyone using this class should invoke
 *     the public inherited methods (nextInt(), nextFloat etc.) to obtain values as normal. This
 *     class should provide a drop-in replacement for the standard implementation of
 *     java.util.Random with the additional advantage of having a far longer period and the ability
 *     to use a far larger seed value.
 *     <p>This is <b>not</b> a cryptographically strong source of randomness and should <b>not</b>
 *     be used for cryptographic systems or in any other situation where true random numbers are
 *     required.
 *     <p>
 *     <!-- Creative Commons License -->
 *     <a href="http://creativecommons.org/licenses/LGPL/2.1/"><img alt="CC-GNU LGPL" border="0"
...
public class MTRandom extends Random {
    ...
    /**
     * The default constructor for an instance of MTRandom. This invokes the no-argument constructor
     * for java.util.Random which will result in the class being initialised with a seed value
     * obtained by calling System.currentTimeMillis().
     */
    public MTRandom() {}

    /**
     * This version of the constructor can be used to implement identical behaviour to the original
     * C code version of this algorithm including exactly replicating the case where the seed value
     * had not been set prior to calling genrand_int32.
     *
     * <p>If the compatibility flag is set to true, then the algorithm will be seeded with the same
     * default value as was used in the original C code. Furthermore the setSeed() method, which
     * must take a 64 bit long value, will be limited to using only the lower 32 bits of the seed to
     * facilitate seamless migration of existing C code into Java where identical behaviour is
     * required.
     *
     * <p>Whilst useful for ensuring backwards compatibility, it is advised that this feature not be
    ...
    /**
     * This version of the constructor simply initialises the class with the given 64 bit seed
     * value. For a better random number sequence this seed value should contain as much entropy as
     * possible.
     *
     ...
     * will be identical.
     *
     * <p>If this instance was initialised in 'compatibility' mode then this method will only use
     * the lower 32 bits of any seed value passed in and will match the behaviour of the original C
     * code exactly with respect to state initialisation.
     *
     ...
     * Note that calling this method is equivalent to calling "setSeed(pack(buf))" and in particular
     * will result in a new integer array being generated during the call. If you wish to retain
     * this seed data to allow the pseudo random sequence to be restarted then it would be more
     * efficient to use the "pack()" method to convert it into an integer array first and then use
     * that to re-seed the instance. The behaviour of the class will be the same in both cases but
     ...
     * original C code and ensures that using the standard nextInt() function (inherited from
     ...
     * <pre>
     * mt.setSeed(12345);
     * int foo = mt.nextInt(16) + (mt.nextInt(16) {@literal <<} 16);</pre>
     *
    ...
    /**
     * This simply utility method can be used in cases where a byte array of seed data is to be used
     * to repeatedly re-seed the random number sequence. By packing the byte array into an integer
     * array first, using this method, and then invoking setSeed() with that; it removes the need to
     * re-pack the byte array each time setSeed() is called.
     *
     ...
     *
     * <p>Note that this method will not complain if the given byte array is empty and will produce
     * an empty integer array, but the setSeed() method will throw an exception if the empty integer
