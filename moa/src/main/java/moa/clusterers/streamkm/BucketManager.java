...
/**
 * @author Marcel R. Ackermann, Christiane Lammersen, Marcus Maertens, Christoph Raupach, Christian
 *     Sohler, Kamil Swierkot
 *     <p>Modified by Richard Hugh Moulton (24 Jul 2017)
 *     <p>Modified by Mustafa Çelik (13 June 2019) Contact: celikmustafa89@gmail.com
 * @see <a href="https://groups.google.com/forum/#!topic/moa-development/qe8vRKfI_K4">google group
 *     topic</a> for arrayOutOfBoundsException of bucketsize
 * @see <a href ="https://github.com/Waikato/moa/issues/97#issuecomment-497658907">github issue</a>
...
public class BucketManager {
    ...
    public BucketManager(int n, int d, int maxsize, MTRandom random) {
        ...
        this.numberOfBuckets =
                (int) Math.ceil(Math.log((double) n / (double) maxsize) / Math.log(2)) + 2;
    ...
    void insertPoint(Point p) {
        ...
        if (cursize >= this.maxBucketsize) {
            ...
            if (this.buckets[nextbucket].cursize == 0) {
                ...
                int i;
                for (i = 0; i < this.maxBucketsize; i++) {
                    this.buckets[nextbucket].spillover[i] =
                            this.buckets[curbucket].points[i].clone();
                    // copyPointWithoutInit: we should not copy coordinates?
                }
                ...
                */
                while (nextbucket < this.numberOfBuckets
                        && this.buckets[nextbucket].cursize == this.maxBucketsize) {
                    // printf("Bucket %d full \n",nextbucket);
                    this.treeCoreset.unionTreeCoreset(
                            this.maxBucketsize,
                            this.maxBucketsize,
                            this.maxBucketsize,
                            p.dimension,
                            this.buckets[curbucket].points,
                            this.buckets[curbucket].spillover,
                            this.buckets[nextbucket].spillover,
                ...
                if (nextbucket < this.numberOfBuckets) {
                    this.treeCoreset.unionTreeCoreset(
                            ...
                            this.buckets[curbucket].points,
                            this.buckets[curbucket].spillover,
                            this.buckets[nextbucket].points,
                            this.clustererRandom);
                    this.buckets[curbucket].cursize = 0;
                    this.buckets[nextbucket].cursize = this.maxBucketsize;
    ...
    /**
     * It may happen that the manager is not full (since n is not always a power of 2). In this case
     * we extract the coreset from the manager by computing a coreset of all nonempty buckets
     *
     * <p>Case 1: the last bucket is full => n is a power of 2 and we return the contents of the
     * last bucket
     *
     * <p>Case2: the last bucket is not full => we compute a coreset of all nonempty buckets
     *
     * <p>this operation should only be called after the streaming process is finished
     */
        ...
        for (j = i + 1; j < this.numberOfBuckets; j++) {
            if (this.buckets[j].cursize != 0) {
                ...
                this.treeCoreset.unionTreeCoreset(
                        ...
                        this.maxBucketsize,
                        d,
                        this.buckets[j].points,
                        coreset,
                        this.buckets[j].spillover,
                        this.clustererRandom);
