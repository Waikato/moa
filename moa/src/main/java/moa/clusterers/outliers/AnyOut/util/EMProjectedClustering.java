...
/**
 * Implements clustering via Expectation Maximization but return a clear partitioning of the data,
...
public class EMProjectedClustering {
    ...
    /**
     * Performs an EM clustering on the provided data set !! Only the variances are calculated and
     * used for point assignments ! !!! the number k' of returned clusters might be smaller than k
     * !!!
     *
    ...
    public int[][] getEMClusteringVariancesBestChoice(
            double[][] pointArray, int k, int nrOfChoices) {
    ...
    private void setInitialPartitions(double[][] pointArray, int k) {
        ...
        // TODO: choose more stable initialization (comment: this is run several times (with
        // different random seeds), i.e. the worst case should not occur)
    ...
    private void calculateMeans() {
        ...
        for (int i = 0; i < k; i++) {
            for (int d = 0; d < dim; d++) {
                ...
                if (clusterWeights[i] <= 0.0) continue;
                for (int x = 0; x < n; x++) {
    ...
    private void calculateVariances() {
        ...
        for (int i = 0; i < k; i++) {
            for (int d = 0; d < dim; d++) {
                ...
                else clusterVariances[i][d] /= (clusterWeights[i] * n);
            }
        }
    }
    ...
    private void calculatePCgivenX() {
        ...
        for (int i = 0; i < k; i++) {
            for (int x = 0; x < n; x++) {
                if (pX[x] <= 0.0) pCgivenX[i][x] = 0.0;
                else pCgivenX[i][x] = clusterWeights[i] * pXgivenC[x][i] / pX[x];
            }
        }
    }
