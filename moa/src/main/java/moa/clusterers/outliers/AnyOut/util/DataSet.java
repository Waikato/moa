 ...
 * so it is recommended to use with care. For speedy operations after the data is collected use
 * {@link #getFeaturesAsArray()}. When access speed is not a factor the iterator can be used to go
 * through the data with a <code>foreach</code> loop.
 *
...
public class DataSet implements Iterable<DataObject> {
    ...
    /**
     * Creates a Set with only the given object. The dimension for the <code>DataSet</code> is given
     * by the given <code>DataObject</code>.
     *
     ...
     * @throws praktikum.framework.data.InconsistentDimensionException This <code>Exception</code>
     *     is thrown when the dimension of the object to be added does not fit the rest.
     * @see DataObject
     * @throws praktikum.framework.data.InconsistentDimensionException
     */
    ...
    /**
     * Returns the <code>DataObject</code> at the given position. Use with care. For speedy
     * operations after the data is collected use {@link #getFeaturesAsArray()}.
     *
    ...
    /**
     * Counts the number of classes that are present in the data set. !!! It does not check whether
     * all classes are contained !!!
     *
    ...
    /**
     * Returns an array with all the features of all the objects in the set. Be aware that it does
     * not copy the features so that any changes to the values of the features in this array will
     * result in changes in the initial <code>DataObject</code>s.
     *
    ...
    /**
     * Returns an array of all the <code>DataObject</code>s in the set. Use for speedy access when
     * the labels and/or the ids are also needed.
     *
    ...
    /**
     * An iterator for the set. This allows to use a <code>foreach</code> loop over the <code>
     * DataObject</code>s. This is a nice way of going through the data, but when the access speed
     * is relevant its use is not recommended.
     *
    ...
    /**
     * Returns a <code>String</code> representation of all the <code>DataObject</code>s in the code
     * as a list of the representation implemented for these.
     *
