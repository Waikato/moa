...
public class EMTopDownTreeBuilder {
    ...
    private Node buildTreeRecursively(DataSet dataSet, int level) throws Exception {
        ...
        if (dataSet.size() <= maxFanout) {
            ...
            for (DataObject dataObject : dataSet) {
                ...
                entries[count].add(
                        new Entry(
                                dataObject.getNrOfDimensions(),
                                new ClusKernel(
                                        dataObject.getFeatures(), dataObject.getFeatures().length),
                                0));
                count++;
            }
            // else split DataSet
            ...
            // case 2: if the data set could not be split, we insert all objects into a new leaf
            // node
            if (subSets.length == 1) {
                ...
                for (DataObject dataObject : dataSet) {
                    Entry newEntry =
                            new Entry(
                                    dataObject.getNrOfDimensions(),
                                    new ClusKernel(
                                            dataObject.getFeatures(),
                                            dataObject.getFeatures().length),
                                    0);
                    if (count < maxFanout) {
                        // fill entry using the current 'dataObject'
                        entries[count] = new Entry(dataSet.getNrOfDimensions());
                    ...
                    if (i < newChildNodes.length) {
                        if (newChildNodes[i] == null) {
                            ...
                            if (subSets[i].size() == 0) {
                                ...
                                entries[i].add(
                                        new Entry(
                                                subSets[i].getObject(0).getNrOfDimensions(),
                                                new ClusKernel(
                                                        subSets[i].getObject(0).getFeatures(),
                                                        subSets[i]
                                                                .getObject(0)
                                                                .getFeatures()
                                                                .length),
    ...
    /**
     * This methods splits the given data set into partitions using the EM algorithm. The resulting
     * number of partitions is <= nrOfPartitions. If EM returns only one partition, the data set is
     * split using mean shift. The size of each resulting partition is AT LEAST 2, i.e. partitions
     * with size 1 are merged with the closest remaining partition. Hence, this method might return
    ...
    private DataSet[] splitDataSetUsingEM(DataSet dataSet, int nrOfPartitions) throws Exception {
        ...
        int[][] emMapping =
                myEM.getEMClusteringVariancesBestChoice(
                        dataSet.getFeaturesAsArray(), nrOfPartitions, nrOfIterations);
    ...
    private DataSet[] splitDataSetUsingMeanShift(DataSet dataSet) {
        ...
        try {
            ...
            for (int i = 0; i < dataObjects.length; i++) {
                ...
                if (dist1 < dist2) subDataSets[0].addObject(dataObjects[i]);
                else subDataSets[1].addObject(dataObjects[i]);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
