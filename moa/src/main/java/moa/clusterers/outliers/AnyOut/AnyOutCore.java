...
public class AnyOutCore extends ClusTree {
    ...
    public IntOption trainingSetSizeOption =
            new IntOption("TrainingSetSize", 't', "Training Set Size.", 1000, 0, 10000);
    // public FlagOption UseBulkLoadingOption = new FlagOption("UseBulkLoading", 'b', "Use
    // Bulkloading or traditional learning.");
    public IntOption oScoreKOption =
            new IntOption("OScorek", 'o', "Size of Oscore aggregate.", 2, 1, 10);
    public IntOption confKOption =
            new IntOption("Confidencek", 'c', "Size of confidence aggregate.", 2, 1, 10);
    public IntOption confidenceChoiceOption =
            new IntOption("confidence", 'd', "Confidence Measure.", 4, 1, 6);
    public FlagOption UseMeanScoreOption =
            new FlagOption("UseMeanScore", 'm', "Use Mean score or Density score.");
    public FloatOption threshholdOption =
            new FloatOption("Threshold", 'z', "Threshold", 0.07, 0, 1);
    ...
    public void train(DataSet trainingSet) {
        ...
        // ClusTree private variables are not updated but are mandatory for the algorithm to
        // function.
    ...
    public void initObject(int objectId, double[] features) {
        ...
        else lastOScoreResult.put(objectId, getDensityOutlierScore(newKernel, closestEntry.data));
    ...
    private void useAggregatedOScoreResults(int objectId) {
        if (oScoreK <= 1) {
            ...
            int count = 0;
            for (int i = Math.max(0, previousOScoreResultList.get(objectId).size() - (oScoreK - 1));
                    i < previousOScoreResultList.get(objectId).size();
                    i++) {
    ...
    public boolean moreImprovementsPossible(int objectId, double depthPercentage) {
        if (currentLevel.get(objectId) < maxHeight * depthPercentage
                && descendToNode.get(objectId) != null) {
            return true;
        }
        return false;
    }
    ...
    public void improveObjectOnce(int objectId) {
        ...
        if (closestEntry.data.getWeight() < weightThreshold) {
            ...
            else lastOScoreResult.put(objectId, getDensityOutlierScore(mKernel, closestEntry.data));
    ...
    private double calcC1(int objectId) {
        ...
        double difSum_k =
                Math.abs(
                        lastOScoreResult.get(objectId)
                                - previousOScoreResultList
                                        .get(objectId)
                                        .get(nrOfPreviousResults - 1));

        // if previousOscoreResultList contains more than two results, this loop sums up further
        // diffs
        for (int i = Math.max(0, nrOfPreviousResults - (confK - 1)) + 1;
                i < nrOfPreviousResults;
                i++) {
            difSum_k +=
                    Math.abs(
                            previousOScoreResultList.get(objectId).get(i)
                                    - previousOScoreResultList.get(objectId).get(i - 1));
            count++;
        }
        // hier msste gelten count==confK-1, d.h. wenn ich die letzten 3 Werte betrachten will,
        // bekomme ich 2 differenzen
        // XXX SW: Nicht ganz. Wenn ich die letzten 4 Werte betrachten will,  aber erst 2 Ergebnisse
        // zur Verf�gung stehen, bekomme ich an anstatt der 3 Differenzen nur 1
    ...
    private void updateConfidence(int objectId) {
        ...
        if (confChoice == 1) lastConfidenceResult.put(objectId, calcC1(objectId));
        if (confChoice == 2) lastConfidenceResult.put(objectId, calcC2(objectId));
        if (confChoice == 3) lastConfidenceResult.put(objectId, calcC3(objectId));
        ...
        if (confChoice == 7)
            lastConfidenceResult.put(
                    objectId, calcC1(objectId) * calcC2(objectId) * calcC3(objectId));
    }
