 ...
 *
 * @author Stephen Wels Data Management and Data Exploration Group, RWTH Aachen University
    ...
    protected void setClusterIDs(Clustering clustering) {
        ...
        for (Cluster c : clustering.getClustering()) {
            ...
            for (Cluster mc : ncc.getMicroClusters()) {
                if (!ids.containsKey(mc.getId())) ids.put(mc.getId(), new Integer(1));
                else {
            ...
            if (!countIDs.containsKey(maxID)) countIDs.put(maxID, new Integer(1));
            else {
