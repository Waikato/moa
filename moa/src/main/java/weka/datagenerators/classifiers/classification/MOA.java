...
public class MOA extends ClassificationGenerator {
    ...
    protected ClassOption m_Generator =
            new ClassOption(
                    "InstanceStream",
                    'B',
                    "The MOA instance stream generator to use from within WEKA.",
                    InstanceStream.class,
                    m_ActualGenerator.getClass().getName());
    ...
    public Enumeration listOptions() {
        ...
        result.add(
                new Option(
                        "\tThe MOA stream generator.\n"
                                + "\t(default: "
                                + MOAUtils.toCommandLine(new LEDGenerator())
                                + ")",
                        "B",
                        1,
                        "-B <classname + options>"));
    ...
    public void setOptions(String[] options) throws Exception {
        ...
        if (tmpStr.length() == 0) option.setCurrentObject(new LEDGenerator());
    ...
    /**
     * Return if single mode is set for the given data generator mode depends on option setting and
     * or generator type.
     *
    ...
    public Instances defineDataFormat() throws Exception {
        ...
        m_DatasetFormat =
                new Instances(instanceConverter.wekaInstances(m_ActualGenerator.getHeader()));
     ...
     * @throws Exception if the generator only works with generateExamples which means in non single
     *     mode
     */
    ...
    /**
     * Generates all examples of the dataset. Re-initializes the random number generator with the
     * given seed, before generating instances.
     *
     * @return the generated dataset
     * @throws Exception if the format of the dataset is not yet defined
     * @throws Exception if the generator only works with generateExample, which means in single
     *     mode
     * @see #getSeed()
     */
    ...
    /**
     * Generates a comment string that documentates the data generator. By default this string is
     * added at the beginning of the produced output as ARFF file type, next after the options.
     *
    ...
    /**
     * Generates a comment string that documentats the data generator. By default this string is
     * added at the end of theproduces output as ARFF file type.
     *
     * @return string contains info about the generated rules
