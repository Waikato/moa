...
public class MOAUtils {
    ...
    public static MOAObject fromCommandLine(Class requiredType, String commandline) {
        ...
        try {
            ...
            try {
                ...
                result =
                        (MOAObject)
                                Class.forName(requiredType.getPackage().getName() + "." + classname)
                                        .newInstance();
            }
            if (result instanceof AbstractOptionHandler) {
                ((AbstractOptionHandler) result)
                        .getOptions()
                        .setViaCLIString(Utils.joinOptions(tmpOptions));
    ...
    /**
     * Returs the commandline for the given object. If the object is not derived from
     * AbstractOptionHandler, then only the classname. Otherwise the classname and the options are
     * returned.
     *
