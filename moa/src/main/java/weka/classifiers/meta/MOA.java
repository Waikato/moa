 ...
 * <br>
 * Since MOA doesn't offer a mechanism to query a classifier for the types of attributes and classes
 * it can handle, the capabilities of this wrapper are hard-coded: nominal and numeric attributes
 * and only nominal class attributes are allowed.
 *
 * <p>
 * <!-- globalinfo-end -->
 * <!-- options-start -->
...
public class MOA extends weka.classifiers.AbstractClassifier implements UpdateableClassifier {
    ...
    protected ClassOption m_Classifier =
            new ClassOption(
                    "classifier",
                    'B',
                    "The MOA classifier to use from within WEKA.",
                    Classifier.class,
                    m_ActualClassifier.getClass().getName().replace("moa.classifiers.", ""),
    ...
    public Enumeration listOptions() {
        ...
        result.addElement(
                new Option(
                        "\tThe MOA classifier to use.\n"
                                + "\t(default: "
                                + MOAUtils.toCommandLine(new DecisionStump())
                                + ")",
                        "B",
                        1,
                        "-B <classname + options>"));
    ...
    public void setOptions(String[] options) throws Exception {
        ...
        if (tmpStr.length() == 0) option.setCurrentObject(new DecisionStump());
        else option.setCurrentObject(MOAUtils.fromCommandLine(m_Classifier, tmpStr));
     ...
     * default. MOA doesn't specify what
     *
