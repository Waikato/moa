...
public class PHTFalsePositiveTester implements Tester
{
    int iterations = 100;
	...
	try
	{
		...
		for (int q = 0; q < epsilonPrimes.length; q++)
		{
		    ...
		    double[] deltas = { 10, 15, 20, 25, 30, 35, 40, 45, 50 };
...
//		    BufferedWriter bWriter = new BufferedWriter(new FileWriter("src\\testers\\Volatility&ChangeDetection\\" + "Test" + "\\" + blockSize + "_" + DECAY_MODE + "_" + COMPRESSION_MODE + "_" + compTerms + "_" + epsilonPrimes[q] + "_" + alphas[w] + "_" + numInstances + "_" + iterations + ".csv"));
		    BufferedWriter bWriter = new BufferedWriter(new FileWriter("src\\testers\\Volatility&ChangeDetection\\" + "Test" + "\\FP_PHT_" + numInstances + "_" + iterations + ".csv"));

		    bWriter.write("mean\\d.threshold, 10, 15, 20, 25, 30, 35, 40, 45, 50");
		    bWriter.newLine();
			...
			for (int j = 0; j < deltas.length; j++)
			{
				...
				{
				    if (pht.setInput(Double.parseDouble(line)))
				    {
					totalDrift++;
				    }
				}
