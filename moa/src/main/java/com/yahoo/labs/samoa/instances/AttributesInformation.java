...
public class AttributesInformation implements Serializable {
	...
	/**
	 * Used for backwards compatibility, indexes are ignored
	 * 
	 * @param v
	 * @param indexValues ignored
	 */
	public void setAttributes(List<Attribute> v, int[] indexValues) {
	...
	public void deleteAttributeAt(int index) {
		if ((index < 0) || (index > this.attributes.size() - 1)) {
			throw new IllegalArgumentException("Can't delete attribute: index out " + String.valueOf(index) +  "of range");
		}
	...
	public void insertAttributeAt(Attribute attribute, int index) {
		if ((index < 0) || (index > this.attributes.size())) {
			throw new IllegalArgumentException("Can't insert attribute: index out " + String.valueOf(index) + "of range");
		}
