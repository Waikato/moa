...
import com.yahoo.labs.samoa.instances.util.AttributeDefinitionUtil;
...
public class ArffLoader {
	...
	/**
	 * The old getHeader method, included for backwards compatibility.
	 * 
	 * @return the instance header, with no differentiation between attributes
	 */
    private InstanceInformation getHeader() {
        ...
        //ArrayList<Attribute> 
        attributes = new ArrayList<Attribute>();//JD
        int numAttributes = 0;
        try {
            streamTokenizer.nextToken();
            while (streamTokenizer.ttype != StreamTokenizer.TT_EOF) {
