...
public class SamoaToWekaInstanceConverter implements Serializable {
	...
	public weka.core.Instance wekaInstance(Instance inst) {
		...
		if (((InstanceImpl) inst).instanceData instanceof SparseInstanceData) {
			...
			/*
			 * if (this.wekaInstanceInformation == null) { this.wekaInstanceInformation =
			 * this.wekaInstancesInformation(inst.dataset()); }
			 ...
			 * //wekaInstance.setValueSparse(wekaInstance.numAttributes(),
			 * inst.classValue());
			 */
		} else {
			...
			/*
			 * if (this.wekaInstanceInformation == null) { this.wekaInstanceInformation =
			 * this.wekaInstancesInformation(inst.dataset()); } //We suppose that the class
			 * is the last attibute. We should deal when this is not the case
			 * wekaInstance.insertAttributeAt(inst.classIndex());
			 * wekaInstance.setDataset(wekaInstanceInformation);
			 * wekaInstance.setClassValue(inst.classValue());
			 */
	...
	public weka.core.Instances wekaInstances(InstancesHeader instances) {
		...
		// We assume that we have only one WekaInstanceInformation for
	...
	public weka.core.Instances wekaInstancesInformation(InstancesHeader instances) {
		...
		ArrayList<weka.core.Attribute> attInfo = new ArrayList<>();
		...
		if (instances.instanceInformation.numOutputAttributes() == 1) {
			...
			wekaInstances.setClassIndex(instances.instanceInformation.numOutputAttributes() - 1); // instances.numAttributes()-1);
																									// //Last
		}
