...
\date{March 2012 }%\\ \vspace{3cm} \includegraphics[height=1cm]{cosi_logo.png} }
\begin{document}
...
training examples. 
%Some papers use more than this, up to ten million examples,
%and only very rarely is there any study like Domingos and 
%Hulten  that is in the order of tens of millions 
%of examples. 
...
%such as StreamKM++~\cite{StreamKM}, CluStream~\cite{CluStream}, ClusTree~\cite{KAB+09}, Den-Stream~\cite{DenStream}, D-Stream~\cite{DStream} and CobWeb~\cite{CobWeb}.
...
\begin{itemize}
	\item StreamKM++: It computes a small weighted sample of the data stream
	 ...
	 %Coresets are small weighted sets of points that approximates the original point set with
	 %respect to clustering. It and implements a coreset tree to speed up the time necessary for
	 %the non-uniform sampling during the coreset construction.
	...
	%\item D-Stream: This method maps each input data record into a grid and it
	% computes the grid density. The grids are clustered based on the density. 
	% This algorithm adopts a density decaying technique to capture the dynamic changes of a
	 % data stream. 
...
\begin{figure*}[t]
\begin{center} 
%\epsfig{file=SetupTab.png, scale=0.3}
\includegraphics[width=0.9\textwidth]{images/VisualTab}%, width=0.77\textwidth}%scale=0.4}
\end{center} 
...
\end{lstlisting}
%The following example is based on a Unix/Linux system with Java 5 SDK or greater installed. Other operating systems such as Microsoft Windows will be similar but may need adjustments to suit.
...
%\section{Classifiers for evolving streams}
...
\adwin Bagging is the online bagging method of Oza and Rusell % and boosting algorithms 
%implemented in MOA 
...
%Until now, MOA and WEKA have been independent systems. Now 
...
\begin{tikzpicture}[scale=.8,domain=0:18]
  %\draw[step=2,very thin,color=gray] (-0.1,-0.1) grid (8.2,2.1);
...
%  \draw[<->] (12,-1.5) -- (18,-1.5) node[below] {};
  %\draw[color=blue!50!black, domain=2:6] plot[id=x]   function{(x-2)}           node[right]{}; %{$f(x) =x$};
  %\draw[color=niceblue!50!white, domain=2:6] plot[id=x]   function{(x-2)}           node[right]{}; %{$f(x) =x$};
  %\draw[color=blue]   plot[id=sin] function{sin(x)}       node[right] {$f(x) = \sin x$};
  %\draw[color=nicered!50!white] plot[id=exp] function{4/(1+exp(4-x))} node[above] {$f(t)$};% = 1/{(1+ \mathrm e^{-s (x-x_0)}})$};
  %\draw[color=red!50!black] plot[id=exp] function{4/(1+exp(4-x))} node[above] {$f(t)$};
  %\shadedraw[left color=gray,right color=green, draw=green!50!black] (2,0) -- (2.5,0) arc (0:45:.5) -- cycle; 
  %\colorlet{anglecolor}{blue!50!black} %white} %black
  %\filldraw[fill=blue!20,draw=anglecolor] (2,0) -- (2.5,0) arc(0:45:.5);
  %\draw (2.7,.3) node[anglecolor] {$\alpha$};
  %\filldraw[fill=blue!20,draw=anglecolor] (4,2) -- (4.5,2) arc(0:45:.5);
  %\draw (4.7,2.3) node[anglecolor] {$\alpha$};
