 ...
 *
 * <p>Assumptions: - The ordering of the instances in the topic is unimportant, or if it is
 * important, it is ensured by a topic with only one partition. - The stream is considered ended
 * when a record with a null value is found. - The serialised form of the instances is using Java's
 * own serialisation tools (i.e. {@link ObjectSerializer}).
 *
 * @author Corey Sterling (csterlin at waikato dot ac dot nz)
 */
public class KafkaStream extends AbstractOptionHandler
        implements InstanceStream, CapabilitiesHandler, Closeable {
    ...
    public StringOption hostOption =
            new StringOption("host", 'h', "The Kafka broker host", "localhost");
    ...
    public long estimatedRemainingInstances() {
        ...
        // that remain
        if (m_EndOfStreamReached) return m_InstanceBuffer.size();
    ...
    /** Makes sure the Kafka consumer is available and ready to retrieve instances. */
    protected void establishConsumer() {
        // If the consumer is already resident, abort
        if (m_Consumer != null) return;
    ...
    protected List<TopicPartition> getPartitions() {
        ...
        if (m_Consumer == null) return result;

        // Iterate through the topics and partitions, adding them to the result
        for (Entry<String, List<PartitionInfo>> topicPartitions :
                m_Consumer.listTopics().entrySet()) {
    ...
    protected void fillBufferIfNecessary() {
        ...
        if (m_EndOfStreamReached) return;
        ...
        if (m_InstanceBuffer == null) m_InstanceBuffer = new LinkedList<>();

        // Get some records from Kafka
        ConsumerRecords<Long, Instance> records =
                m_Consumer.poll(KafkaUtils.WAIT_AS_LONG_AS_POSSIBLE);
    ...
    protected void cacheHeaderIfNecessary() {
        ...
        if (dataset instanceof InstancesHeader) m_Header = (InstancesHeader) dataset;
        else m_Header = new InstancesHeader(dataset);
    }
