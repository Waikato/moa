 ...
 *
 * A Hoeffding tree is an incremental, anytime decision tree induction
 * algorithm that is capable of learning from massive data streams, assuming
 ...
 * to choose an optimal splitting attribute. This idea is supported
 * mathematically by the Hoeffding bound, which quantiﬁes the number of
 * observations (in our case, examples) needed to estimate some statistics
 * within a prescribed precision (in our case, the goodness of an attribute).</p>
 ...
 * <li> -c : The allowable error in split decision, values closer to 0 will 
 * take longer to decide</li>
